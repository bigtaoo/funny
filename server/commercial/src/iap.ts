...
// ── Non-coin product resolution (subscriptions / starter packs) ───────────────────────
 ...
 */
function resolveNonCoinProduct(productId: string): IapProductKind | null {
  const raw = process.env.NW_IAP_PRODUCT_MAP;
  if (raw) {
    for (const pair of raw.split(',')) {
      const colonIdx = pair.indexOf(':');
 ...
 */
function resolveNonCoinProductFromAmount(amount: number): IapProductKind | null {
