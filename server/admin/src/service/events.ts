...
export class EventsService {
  constructor(private readonly core: AdminCore) {}
    ...
    async listEvents(): Promise<EventDoc[]> {
      if (!this.core.events.available) return [];
      return this.core.events.list();
    }
    ...
    async createEvent(actor: Actor, input: EventInput): Promise<EventDoc> {
      const ev = await this.core.events.create(input);
      await this.core.audit(actor.adminId, 'event.create', { target: ev._id, summary: ev.title });
      return ev;
    }
    ...
    async updateEvent(actor: Actor, eventId: string, input: EventInput): Promise<EventDoc> {
      const ev = await this.core.events.update(eventId, input);
      await this.core.audit(actor.adminId, 'event.update', { target: ev._id, summary: ev.title });
      return ev;
    }

    /** Delete an event definition. Audited. */
    async deleteEvent(actor: Actor, eventId: string): Promise<void> {
      await this.core.events.remove(eventId);
      await this.core.audit(actor.adminId, 'event.delete', { target: eventId });
    }
}
