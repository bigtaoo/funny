...
export interface AnalyticsHandlers {
  ...
  analyticsQuery(type: string, days: number, platform?: string, newCohort?: boolean): Promise<AnalyticsQueryResult & { available: boolean }>;
...
export class AnalyticsService {
  constructor(private readonly core: AdminCore) {}
    ...
    async analyticsQuery(type: string, days: number, platform?: string, newCohort?: boolean): Promise<AnalyticsQueryResult & { available: boolean }> {
      if (!this.core.analytics.available) return { available: false };
      // Only forwarded when actually set: an explicit trailing `undefined` argument is a different
      // call shape than omitting it (matters for the call-recording harness in
      // analyticsService.test.ts, and is the more honest call anyway — this param means nothing to
      // every query type except 'retention').
      const result = newCohort
