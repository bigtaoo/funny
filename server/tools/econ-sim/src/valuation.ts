...
// settle output, so anything that passes here is genuinely safe (§2.4 "conservative upper bound").
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Regular F2P material grind baseline (per player), for the per-capita dilution denominator (§2.3 / §3) ──
//
// §2 stamina gate: 240 natural stamina/day, normal stage costs 6 -> 40 runs/day.
// §3 "baseline ×1" normal-stage material drop has no single absolute in code; we take a
// representative MID-GAME normal stage from server/shared/src/pveRewards.ts (ch3-5 band),
// discounted by the §3 re-grind factor (re-grind ×0.7). These are the player's recurring
...
export const REGRIND_FACTOR = 0.7; // §3 re-grind material discounted to 70%
