...
describe('loadWorldsvcEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.NW_WORLD_MONGO_URI = 'mongodb://its-own-host:27017/?replicaSet=rs0';
  });
  afterEach(() => {
    for (const k of ENV_KEYS) {
  ...
  it('defaults: no NW_WORLD_*/internal-url/auto-settle env vars set -> documented fallbacks', () => {
    ...
    expect(env.worldMongoUri).toBe('mongodb://its-own-host:27017/?replicaSet=rs0'); // its OWN login, threaded through untouched
  ...
  it("with its own Mongo URI unset, falls back to the local dev Mongo — never to metaserver's login (ADR-090)", () => {
    // Until 2026-09-12 this line read `?? base.mongoUri`, so an unset NW_WORLD_MONGO_URI silently handed this
    ...
    delete process.env.NW_WORLD_MONGO_URI;
    expect(loadWorldsvcEnv().worldMongoUri).toBe(DEV_MONGO_URI);
  });
});
