...
export interface FunnelHandlers {
  queryFunnel(days: number, platform?: string): Promise<FunnelDailyDoc[]>;
  runFunnelEtl(dateStr: string): Promise<void>;
  queryTutorialFunnel(days: number): Promise<StepFunnelResult>;
  querySceneFunnel(days: number): Promise<StepFunnelResult>;
  queryLevelFunnel(days: number, platform?: string): Promise<LevelFunnelRow[]>;
  queryFeatureGuideFunnel(days: number, platform?: string): Promise<FeatureGuideFunnelRow[]>;
}

export function FunnelMixin<TBase extends AnalyticsServiceBaseCtor>(Base: TBase): TBase & Constructor<FunnelHandlers> {
