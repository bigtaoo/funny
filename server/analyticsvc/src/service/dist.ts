...
export interface DistHandlers {
  queryRegionDist(days: number): Promise<RegionRow[]>;
  queryOsDist(days: number): Promise<OsRow[]>;
  queryBrowserDist(days: number): Promise<BrowserRow[]>;
  queryDeviceTypeDist(days: number): Promise<DeviceTypeRow[]>;
  queryGeoDist(days: number): Promise<GeoRow[]>;
  queryBadgeDist(days: number): Promise<BadgeDistRow[]>;
}

export function DistMixin<TBase extends AnalyticsServiceBaseCtor>(Base: TBase): TBase & Constructor<DistHandlers> {
