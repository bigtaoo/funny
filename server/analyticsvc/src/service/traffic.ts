...
export interface TrafficHandlers {
  queryEventCounts(days: number): Promise<EventCountRow[]>;
  queryDau(days: number): Promise<DauRow[]>;
  queryLoginHour(days: number): Promise<LoginHourRow[]>;
  queryRetention(days: number): Promise<RetentionRow[]>;
  queryFirstSession(days: number): Promise<FirstSessionResult>;
}

export function TrafficMixin<TBase extends AnalyticsServiceBaseCtor>(Base: TBase): TBase & Constructor<TrafficHandlers> {
