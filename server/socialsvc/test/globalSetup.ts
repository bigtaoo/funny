...
// suite has a real Mongo — no Docker, no manual install. socialsvc performs only
...
const URI_FILE = join(tmpdir(), 'nw-socialsvc-mongo-uri');

// Pinned mongod binary version — bump deliberately, never float. Mirrors worldsvc's harness
// so the two suites share one cached binary; stays compatible with the mongodb driver (^6.10).
