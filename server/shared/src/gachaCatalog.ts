...
export const GACHA_CATEGORIES: readonly GachaCategory[] = ['skin', 'card', 'equipment', 'material'];
...
// Equipment carries its own EquipRarity (common/fine/rare/epic, no legendary). Map to the display Rarity
// the standard pool assigns each tier (GACHA_POOLS: fine→rare, rare→epic, epic→legendary) so custom-pool
// equipment tints match how the same item looks in the standard pool.
...
/** Catalogue grouped by category, for the ops picker. */
export function catalogByCategory(): Record<GachaCategory, GachaCatalogItem[]> {
  const out: Record<GachaCategory, GachaCatalogItem[]> = { skin: [], card: [], equipment: [], material: [] };
...
export function validateCustomPool(cfg: {
  ...
  for (const cat of cats) {
    if (!GACHA_CATEGORIES.includes(cat.category)) return `unknown category: ${cat.category}`;
