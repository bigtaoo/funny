...
export interface EquipmentInstance {
  ...
  /** Provenance tag (e.g. 'craft', 'gacha:<orderId>', 'checkin:<monthKey>', 'pve_drop:<levelId>').
   *  Optional — absent on pre-2026-08-04 instances. No consumer reads this yet; reserved for future
   *  support/anti-cheat tooling (see design/game/ITEM_IDENTITY_DESIGN.md). */
  sourceType?: string;
...
/**
 * Skin instance (ADR-059 follow-up / ITEM_IDENTITY_DESIGN.md task1, 2026-08-08): skins have no
 * level/affixes, so unlike EquipmentInstance/CardInstance this is just an id + provenance wrapper.
 * `SaveData.inventory.skins` stays the plain dedup "do I currently own this skinId" view (unchanged —
 * every existing consumer keeps reading it as before); `skinInstances` is the new underlying per-unit
 * ledger that makes a "duplicate" gacha pull a real, sellable/auctionable item instead of a silent
 * no-op (the bug this fixes: a dupe pull used to vanish — no item, no coin refund, see GACHA_DESIGN §4.3
 * / economy.ts's old "deferred to S5" comment). Multiple instances may share the same skinId; ownership
 * of that skinId (inventory.skins membership) is true iff at least one instance remains.
 */
export interface SkinInstance {
  id: string; // instance id (generated by the server)
  skinId: string; // skin catalogue id (GACHA_CATALOG / skinDefs.ts)
  /** Provenance tag, same convention as EquipmentInstance.sourceType (e.g. 'gacha:<orderId>', 'auction_return'). */
  sourceType?: string;
  /** Epoch ms when this instance was created. Optional, same caveat as EquipmentInstance.obtainedAt. */
  obtainedAt?: number;
}
...
export interface CardInstance {
  ...
  locked: boolean;
  /** Provenance tag (e.g. 'starter', 'checkin', 'pve_anchor:<levelId>', 'pve_drop:<levelId>', 'gacha:<orderId>').
   *  Optional — absent on pre-2026-08-04 instances. No consumer reads this yet; reserved for future
   *  support/anti-cheat tooling (see design/game/ITEM_IDENTITY_DESIGN.md). */
  sourceType?: string;
  /** Epoch ms when this instance was created. Optional, same caveat as sourceType. */
...
export interface SaveData {
  ...
  // —— Skin instance counts (ITEM_IDENTITY_DESIGN.md task1, 2026-08-08). ——
  // skinId → total owned instance count (>=2 means duplicates exist, sellable/auctionable beyond the
  // one an equip slot may be holding). Instances themselves live in the `skinInstances` collection
  // (mirrors equipmentInv/cardInv's split) — this field is the transient wire join, same "populate on
  // GET /save, null = intentionally omitted (caller already knows what changed)" convention as
  // equipmentInv/cardInv above. `inventory.skins` (the plain owned/not-owned array) is unaffected and
  // stays the source every pre-existing consumer reads; this field only adds "how many" on top.
  skinCounts?: Record<string, number> | null;
