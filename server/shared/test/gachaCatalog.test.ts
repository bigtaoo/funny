...
import { GACHA_CATEGORY_ORDER } from '../src/economy';
...
describe('GACHA_CATALOG', () => {
  it('every item has a known category and a rarity', () => {
    for (const it of GACHA_CATALOG) {
      expect(GACHA_CATEGORY_ORDER).toContain(it.category);
  ...
  it('sources equipment + cards from their registries', () => {
    expect(catalogItem('wp_pen')?.category).toBe('equip_t1'); // fine equipment → tier 1
    expect(catalogItem('wp_marker')?.category).toBe('equip_t2'); // rare equipment → tier 2
    expect(catalogItem('wp_highlighter')?.category).toBe('equip_t3'); // epic equipment → tier 3
