...
// metaserver uses multi-document transactions, so it needs a single-node rs0 replica set.
...
export const { setup, teardown } = createMongoHarness({ pkg: 'metaserver', replSet: true });
