...
export function registerInternalRoutes(app: FastifyInstance, deps: InternalDeps): void {
  ...
  // ── GET /leaderboard ─────────────────────────────────────────────────────
  // 全服 Top100 + 查询者名次（S11-SE-5，SEASON_DESIGN §5）。JWT 鉴权（openapi glue 覆盖，此处为内部
  // admin/系统调用路由；玩家侧通过 openapi glue 在 service.ts 接入 getLeaderboard）。
  // 此 /internal 版无 JWT，供 admin 查询；玩家侧见 service.ts getLeaderboard。
