...
// `this.deps` plus a handful of protected MetaServiceBase methods (rejectIfBanned/ensureCommercial) or
...
import { createRateLimiter, type RateLimiter, type Constructor, type MetaBaseCtor } from './base.js';
  ...
  return class extends Base {
    ...
    async authWx(req: FastifyRequest, reply: FastifyReply) {
      return authWxHandler(
        { deps: this.deps, rejectIfBanned: this.rejectIfBanned.bind(this), allowAuthAttempt: this.allowAuthAttempt.bind(this), gatewayField: this.gatewayField },
        req,
        reply,
      );
    }
    ...
    async authPasswordChange(req: FastifyRequest, reply: FastifyReply) {
      return authPasswordChangeHandler(this.deps, req, reply);
    }

    async authOAuth(req: FastifyRequest, reply: FastifyReply) {
      return authOAuthHandler(
        { deps: this.deps, oauth: this.oauth, rejectIfBanned: this.rejectIfBanned.bind(this), allowAuthAttempt: this.allowAuthAttempt.bind(this), gatewayField: this.gatewayField },
        req,
        reply,
      );
    }
    ...
    async deleteAccount(req: FastifyRequest) {
      return deleteAccountHandler(this.deps, req);
    }

    async cancelAccountDeletion(req: FastifyRequest, reply: FastifyReply) {
      return cancelAccountDeletionHandler(this.deps, req, reply);
    }

    async recordGdprConsent(req: FastifyRequest) {
      return recordGdprConsentHandler(this.deps, req);
    }

    async profileRename(req: FastifyRequest, reply: FastifyReply) {
      return profileRenameHandler({ deps: this.deps, ensureCommercial: this.ensureCommercial.bind(this) }, req, reply);
    }

    async submitAppeal(req: FastifyRequest, reply: FastifyReply) {
      return submitAppealHandler(this.deps, req, reply);
    }

    async submitFeedback(req: FastifyRequest, reply: FastifyReply) {
      return submitFeedbackHandler(this.deps, this.feedbackRate, req, reply);
    }
  };
}
