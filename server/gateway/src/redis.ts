// gateway Redis 订阅端（S8-4b，§8.4 横扩推送）。gateway 订阅 GW_PUSH_REDIS_CHANNEL，
// worldsvc 把「收件人列表 + 一条 push 消息」发到该 channel，本进程收到后只向本机在线的
// 收件人 socket 扇出（routeBroadcast）。多 gateway 实例各自订阅，天然实现跨实例路由（SOC9）。
//
// 缺省无 Redis URL → 返回 null，宗门频道实时推送关闭（worldsvc 侧降级为 O(n) HTTP 直推兜底，
...
export async function connectGatewaySubscriber(
  ...
  try {
    ...
    log.error('subscribe failed; sect real-time push disabled', { url, err: (e as Error).message });
    return null;
  }
}
