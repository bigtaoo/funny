...
test('m_crit scales with enhancement level (main affix): +9 = ×5.00', () => {
  ...
  assert.ok(Math.abs(critPct - 30) < 1e-9, `expected ~30, got ${critPct}`);
});
...
test('ENHANCE_LEVEL_MULTIPLIER: +6 breakpoint is 1.76× (literal anchor, ADR-063)', () => {
  ...
  assert.equal(enhanceMultiplier(6), 1.76);
});

test('ENHANCE_LEVEL_MULTIPLIER: +9 is 5.00× (literal anchor, ADR-063 2026-08-10 bump)', () => {
  assert.equal(enhanceMultiplier(9), 5.0);
  assert.equal(ENHANCE_LEVEL_MULTIPLIER[ENHANCE_LEVEL_MULTIPLIER.length - 1], 5.0);
});
...
test('m_crit at the +6 breakpoint: effective value computed from the table, not a hand-copied literal', () => {
  ...
  const bp = buildCampaignBlueprints([card(1, 'eq1')], inv);
  const expected = 6 * enhanceMultiplier(6);
  assert.ok(Math.abs((bp[UnitType.Infantry].critPct ?? 0) - expected) < 1e-9, `expected ~${expected}`);
});
...
test('instance level above +9 is clamped before scaling (accumInstance clamps inst.level to [0,9])', () => {
  ...
  const bp = buildCampaignBlueprints([card(1, 'eq1')], inv);
  const expected = 6 * enhanceMultiplier(9);
  assert.ok(Math.abs((bp[UnitType.Infantry].critPct ?? 0) - expected) < 1e-9, `expected ~${expected}, clamped to +9 scaling`);
});
