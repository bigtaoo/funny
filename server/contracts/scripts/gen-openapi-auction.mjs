// gen-openapi-auction.mjs
// Parse contracts/openapi-auction.yml → generate auctionsvc/src/generated/routes.gen.ts
//
...
//
// auctionsvc uses node:http (not fastify) so no registerRoutes() is emitted.
// The generated file provides: AuctionOperationId, AUCTION_ROUTES, and JSON schemas.
// Mirrors server/contracts/scripts/gen-openapi-world.mjs (AUCTION_DESIGN §9 task4).
//
// Usage (from server/auctionsvc/):
//   node ../contracts/scripts/gen-openapi-auction.mjs           # write
//   node ../contracts/scripts/gen-openapi-auction.mjs --check   # CI: fail if stale
...
const specPath = resolve(serverRoot, 'contracts/openapi-auction.yml');
const outPath = resolve(serverRoot, 'auctionsvc/src/generated/routes.gen.ts');
const isCheck = process.argv.includes('--check');

// Resolve js-yaml from auctionsvc's node_modules.
const _require = createRequire(resolve(serverRoot, 'auctionsvc/package.json'));
...
try {
  ...
  console.error(`[gen-openapi-auction] cannot read ${specPath}: ${e.message}`);
  process.exit(1);
}
...
try {
  ...
  console.error(`[gen-openapi-auction] YAML parse error in openapi-auction.yml:\n  ${e.message}`);
  process.exit(1);
}

// ── 2. Validate OpenAPI structure ─────────────────────────────────────────────
function fail(msg) {
  console.error(`[gen-openapi-auction] spec validation failed: ${msg}`);
  process.exit(1);
}
...
// Source: server/contracts/openapi-auction.yml  (${operations.length} operations)
...
if (isCheck) {
  ...
  if (existing === generated) {
    console.log(`[gen-openapi-auction] check passed — ${operations.length} operations, ${Object.keys(spec.components?.schemas ?? {}).length} schemas`);
  } else {
    console.error('[gen-openapi-auction] check FAILED: committed routes.gen.ts is stale — run: npm run gen:api:auction');
    process.exit(1);
  }
} else {
  mkdirSync(dirname(outPath), { recursive: true });
  writeFileSync(outPath, generated, 'utf8');
  console.log(`[gen-openapi-auction] written ${outPath}`);
