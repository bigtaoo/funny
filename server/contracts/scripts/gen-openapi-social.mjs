// gen-openapi-social.mjs
// Parse contracts/openapi-social.yml → generate socialsvc/src/generated/routes.gen.ts
//
...
//
// socialsvc uses node:http (not fastify) so no registerRoutes() is emitted.
// The generated file provides: SocialOperationId, SOCIAL_ROUTES, and JSON schemas.
// Mirrors server/contracts/scripts/gen-openapi-auction.mjs (SOCIAL_SVC_DESIGN.md §4.1 openapi backfill).
//
// Usage (from server/socialsvc/):
//   node ../contracts/scripts/gen-openapi-social.mjs           # write
//   node ../contracts/scripts/gen-openapi-social.mjs --check   # CI: fail if stale
...
const specPath = resolve(serverRoot, 'contracts/openapi-social.yml');
const outPath = resolve(serverRoot, 'socialsvc/src/generated/routes.gen.ts');
const isCheck = process.argv.includes('--check');

// Resolve js-yaml from socialsvc's node_modules.
const _require = createRequire(resolve(serverRoot, 'socialsvc/package.json'));
...
try {
  ...
  console.error(`[gen-openapi-social] cannot read ${specPath}: ${e.message}`);
  process.exit(1);
}
...
try {
  ...
  console.error(`[gen-openapi-social] YAML parse error in openapi-social.yml:\n  ${e.message}`);
  process.exit(1);
}

// ── 2. Validate OpenAPI structure ─────────────────────────────────────────────
function fail(msg) {
  console.error(`[gen-openapi-social] spec validation failed: ${msg}`);
  process.exit(1);
}
...
// Source: server/contracts/openapi-social.yml  (${operations.length} operations)
...
if (isCheck) {
  ...
  if (existing === generated) {
    console.log(`[gen-openapi-social] check passed — ${operations.length} operations, ${Object.keys(spec.components?.schemas ?? {}).length} schemas`);
  } else {
    console.error('[gen-openapi-social] check FAILED: committed routes.gen.ts is stale — run: npm run gen:api:social');
    process.exit(1);
  }
} else {
  mkdirSync(dirname(outPath), { recursive: true });
  writeFileSync(outPath, generated, 'utf8');
  console.log(`[gen-openapi-social] written ${outPath}`);
