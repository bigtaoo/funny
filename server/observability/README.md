...
- **⚠ 网络坑（部署必处理）**：obs 栈（`docker-compose.obs.yml`）是**独立 compose / 独立网络**，主栈 metaserver 默认**解析不到 `loki`**。要启用转发需让 metaserver 到达 Loki——二选一：① 把 metaserver 接入 obs 网络（compose `networks` 互通）；② `NW_LOKI_PUSH_URL` 用宿主可达地址。两 compose（cloud/prod）的 metaserver 段已加 `NW_LOKI_PUSH_URL`（默认空 = 接受但静默丢弃）+ `NW_ADMIN_INTERNAL_URL`（**缺它则 flag 轮询禁用 → bootstrap 恒空 map → 采集永不生效**）。Loki 不可达时静默丢弃，不影响玩家。
