// Ranked 匹配队列单测（S1-R）：窗口内立即配、超窗口等待加宽、出队、重复入队覆盖。
// 注入 now() + autoTick:false，手动 tick，不依赖真实定时器。
import { describe, it, expect } from 'vitest';
import { Matchmaking } from '../src/Matchmaking';
import type { Connection } from '../src/Connection';

const conn = (id: string): Connection =>
  ({ accountId: id, roomId: null }) as unknown as Connection;
...
describe('Matchmaking', () => {
  it('分差在窗口内立即配对', () => {
    ...
    mm.enqueue(conn('a'), 1000);
    mm.enqueue(conn('b'), 1050); // diff 50 ≤ baseWindow 100
  ...
  it('分差超窗口先不配，等待加宽后配对', () => {
    ...
    mm.enqueue(conn('a'), 1000);
    mm.enqueue(conn('b'), 1300); // diff 300 > 100
    expect(pairs).toHaveLength(0);
    t = 5000; // 等 5s → 窗口 100 + 5×50 = 350 ≥ 300
    mm.tick();
    expect(pairs).toEqual([['a', 'b']]);
  });
  ...
  it('三人 → 最近分差两人先配，落单者留队', () => {
    ...
    mm.enqueue(conn('a'), 1000);
    mm.enqueue(conn('b'), 1500); // 远
    mm.enqueue(conn('c'), 1050); // 靠近 a
