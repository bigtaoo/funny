// proto → TypeScript codegen (ts-proto via buf, ADR-023 P3 gameserver side).
...
console.log(`[gen-proto:gameserver] buf=${buf}`);
console.log(`[gen-proto:gameserver] in=${contractsDir}`);
console.log(`[gen-proto:gameserver] out=${outDir}`);

const res = spawnSync(buf, args, { stdio: 'inherit', cwd: root, shell: isWin });
if (res.status !== 0) {
  console.error(`[gen-proto:gameserver] failed (exit ${res.status})`);
  process.exit(res.status ?? 1);
}
console.log('[gen-proto:gameserver] done: src/generated/transport.ts');
