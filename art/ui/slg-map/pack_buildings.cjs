...
const LONG_EDGE = 256;  // buildings render ~tile-sized (not clustered like res motifs) → higher res
...
async function main() {
  ...
  const atlasPng = path.join(OUT_DIR, 'building_atlas.png');
  await canvas.composite(composites).png({ palette: true, compressionLevel: 9, effort: 10 }).toFile(atlasPng);

  // Export JSON (TexturePacker JSON-Hash) — frame names have no extension, e.g. textures['building_keep']
  const frames = {};
  ...
  const json = {
    frames,
    meta: { app: 'pack_buildings.cjs', image: 'building_atlas.png', format: 'RGBA8888', size: { w: ATLAS_W, h: ATLAS_H }, scale: '1' },
  };
  fs.writeFileSync(path.join(OUT_DIR, 'building_atlas.json'), JSON.stringify(json, null, 2));

  const kb = (fs.statSync(atlasPng).size / 1024).toFixed(1);
  console.log(`✅ Packed ${sprites.length} frames → client/src/assets/slg/building_atlas.png (${ATLAS_W}×${ATLAS_H}, ${kb} KB) + building_atlas.json`);
