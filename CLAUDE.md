...
### 形态：四进程（meta/gateway/matchsvc/game）+ 共享包（npm workspaces）
...
- **模块系统**：`metaserver` 是 ESM（NodeNext，因 `fastify-openapi-glue` 为 ESM-only）；`shared`/`gateway`/`matchsvc`/`gameserver` 是 CJS。五包 `npx tsc -b shared metaserver gateway matchsvc gameserver` 验证。
