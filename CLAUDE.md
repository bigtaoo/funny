...
| `game/net/InputSource.ts` | 统一输入管线（M13）：`InputSource` 接口 + `LocalInputSource`（单机 DELAY 0 自转发）；`NetInputSource`(S1-7)/`ReplayInputSource`(S1-RP) 同接口待落地 |
...
| `net/proto/{transport,game}.ts` | ts-proto 生成（C-2，勿手改；`npm run proto:gen` 重生） |
