...
- **验证**：`tsc -b shared metaserver gameserver` 全绿 + 67 测试（+4 `ladder` / +5 `matchmaking` / +4 `ranked` 端到端：匹配→开局→一致结果±16 写 saves、hash 不一致作废、认输判胜+ELO、无 Mongo 拒）。**待办**：客户端 ranked 入队/搜索 UI + 段位徽章 + `match_result` 带 `winner_side`（proto 客户端 codegen 重生）+ 双真机联调；分段差异化胜利金币（ECONOMY §2.3b）依赖经济服务 S2。
...
| S1-R ranked 队列+ELO | [~] | **服务端已落地**：`ladder.ts`（9 段+ELO 纯函数）+ `Matchmaking.ts`（ELO 配对+等待放宽）+ `RoomManager` 入队/建房/`settleRanked` 写 saves.pvp（乐观锁）+ `Room.beginRanked`/async `endMatch`（双方 hash+winner 一致才认，否则作废；掉线/认输服务器权威）；`transport.proto` `MatchResult+winner_side`。67 测试绿。**待办**：客户端 ranked UI/段位徽章/`winner_side` 上报 + 双真机联调 |
| S1-J | 待办 | 服务器裁判复算 |
