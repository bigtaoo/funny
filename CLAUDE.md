...
- **补偿一律走邮件（OPS5）**：admin **从不直接写钱包**；执行器 `HttpMailDispatcher` 调 meta 系统邮件端点（`POST /internal/mail/system/{send,preview}`，带 `dispatchKey` 幂等），钱在玩家领邮件时才经 commercial/inventory 入账。**邮件后端 = SOCIAL_DESIGN S6-3（并行做）**——未就绪时执行器命中 404/501 → 工单 `failed` 可重试（`retryTicket`，dispatchKey 不变）。
...
- **验证**：七包 `tsc -b shared metaserver gateway matchsvc gameserver commercial admin` 全绿 + admin 11 e2e（登录/RBAC/发起≠审批/超额+全服走超管/dry-run/幂等执行+重试/审计可见性/player.lookup/采样 trend/账号管理）+ gateway 10/matchsvc 17/meta 74 不破 + `tools/ops` tsc + webpack 构建绿。**待办**：S6-3 邮件后端就绪后联调执行器；TOTP 二次审批后置。
