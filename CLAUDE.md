...
### 形态：六进程（meta/gateway/matchsvc/game/commercial/admin）+ 共享包（npm workspaces）

> admin（运维后台后端，S7）是第六个 workspace：玩家不可达、独立库 `notebook_wars_admin`，配套纯前端 `tools/ops`。见下方「admin 运维后台」节。
