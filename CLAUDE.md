...
- **待办**：client REST DTO codegen（world 端点进 `openapi.yml` + `rest:gen`，现 worldsvc 用自有 REST 未进 meta openapi）；worldsvc→gateway `/gw/push` 实际推送（行军/被攻预警事件，S8-2 才需）；owner publicId 解析（需 meta `/internal/profile`）。**下一步 S8-2**：行军（Redis ZSET 调度 + 旅行耗时 + 到点占领/增援）；S8-3 围攻复算（复用 judgeRunner）。
