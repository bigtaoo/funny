...
- **待办**：client REST DTO codegen（world 端点进 `openapi.yml` + `rest:gen`，现 worldsvc 用自有 REST 未进 meta openapi）；owner publicId 解析（需 meta `/internal/profile`）；行军列表 GET 端点 + UI。**下一步 S8-3**：围攻（attack/sweep 行军 → 确定性引擎复算防守 config + 录像，复用 judgeRunner）+ `under_attack` 预警推送。
