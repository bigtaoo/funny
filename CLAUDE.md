...
| `net/config.ts` | `getApiBaseUrl`（REST 基址）+ `getGameWsUrl`（WS 端点：`__NW_GAME_WS__` > localStorage `nw_game_ws` > 由 API 基址推导 `https→wss`、`/api→/ws`；null=无 PvP 联机） |
| `net/NetClient.ts` | gameserver WS 客户端（S1-6）：连接/重连（退避 + 代次作废滞后回调）/ ts-proto 编解码；typed send（createRoom/joinRoom/setReady/startMatch/submitCmd/reportResult/resume/ping）+ `onServerMsg`/`onReconnect`/`onStateChange`；只管消息管道，锁步播放在 NetInputSource(S1-7) |
...
| `net/NetSession.ts` | 联机会话编排（S1-8）：绑 `NetClient`（传输）+ `NetInputSource`（锁步输入，sink=client.submitCmd）；`route(ServerMsg)` 全量喂 input + 把 room_state/room_error/peer_dc/match_over 抛给 UI（`handlers` 单订阅，RoomScene→app 接力）；`onReconnect`→`resume(roomId, input.resumeFrame())`（S1-4）；room 动作转发；`onMatchStart` 抛给 app 建引擎。跨 RoomScene→GameScene 存活（input 须持续吃 frame_batch） |
| `net/proto/{transport,game}.ts` | ts-proto 生成（C-2，勿手改；`npm run proto:gen` 重生）。`game.PlayCard` 含 `row`（陨石目标行，S1-7 加） |
...
├── gameserver/  WS（有状态，CJS）  ws + JWT 握手 + 心跳 + 房间/锁步节拍器中继/重连（S1-1~5）
│                Connection（连接封装）/ Room（节拍器+日志+重连+结算）/ RoomManager（建房/加入/路由）/ proto（transport.proto 运行期 protobufjs 编解码）
...
> 设计基准：`design/game/SERVER_API.md` §3（WS 协议）/§4（节拍器）、`META_TASKS.md` S1。`ranked` 队列 + ELO（S1-R）、服务器裁判复算（S1-J）暂未做。录像（S1-RP）客户端录制/回放已落地（`game/net/ReplayInputSource.ts`），服务端 `replay.proto` + PvP 输入日志持久化待做。
...
- **房间（`RoomManager`）**：建房生成 6 位无歧义房间码（去 `0O1IL`）走 `RoomRegistry`（内存，留 Redis 口子）；输码加入 / `ROOM_NOT_FOUND` / `ROOM_FULL` / `RANKED_UNAVAILABLE`（S1-R 前拒 ranked）/ `ALREADY_IN_ROOM`；同账号新连接顶替旧连接（双开/残连）；`room_state` 广播变更。
...
- **结算（S1-5）**：双方 `match_result{state_hash}` 齐 → 比对，写 `matches` 归档（friendly 仅记结果，`hashOk`/`mismatch`、`seed`、`players`）；下发 `match_over{reason: base|mismatch, mismatch}`。**friendly 正常结束的 `winner_side` 由客户端模拟权威决定**（服务器不复算，归档 `winner=-1`）；`winner_side` 仅在 disconnect/认输（=对手胜）与未来 ranked 下服务器权威。
...
| S1-RP 录像录制 + 回放 | [~] | **客户端就位**：`code/src/game/net/ReplayInputSource.ts`（`RecordingInputSource` 录 + `ReplayInputSource` 放，同 `InputSource` 接口）；`Replay` 类型镜像 `replay.proto` + `ENGINE_VERSION`；10 新测试（PvP-vs-AI/PvE 录→放指纹全等、JSON round-trip、版本拒绝），共 110 绿。**待办**：`replay.proto` + gameserver PvP 输入日志持久化 + 回放播放器 UI |
| S1-R / S1-J | 待办 | ranked 队列+ELO / 服务器裁判复算 |
