...
- **S8-0 收尾待办**：部署接线（`dev-up.ps1`/`docker-compose.*.yml`/`Dockerfile` 八包/反代 `/world,/family,/auction`/`.env.example` `NW_WORLD_*`）；client REST DTO codegen（world 端点进 `openapi.yml` + `rest:gen`，现 worldsvc 用自有 REST 未进 meta openapi）；worldsvc→gateway `/gw/push` 实际推送接线（S8-1 才需）。**下一步 S8-1**：占领写 TileDoc + 更新 yieldRate + 保护罩/驻军/资源产出真接入。
...
| S8-0 SLG 契约 + shared + worldsvc 骨架 | ✅（骨架落地） | 第七进程 `server/worldsvc`（独立库 `notebook_wars_world`，公网 REST 第四面）+ `shared/slg.ts`（`proceduralTile()` + 枚举/ID/常量，U2/U4/U6 拍板）+ `transport.proto` 7 个 SLG push（重生）+ `api.ts` 18 错误码。地图/玩家状态做实（程序化 + 稀疏合并 + 资源惰性结算），行军/防守/兵力/家族/拍卖 `NOT_IMPLEMENTED`。验证：八包 `tsc -b` + client tsc + proto-wire-compat 18 全绿。详见上「worldsvc SLG 大世界」节 + `SLG_DESIGN §14`。**待办**：部署接线 / client REST codegen / S8-1 占领+资源真接入 |
