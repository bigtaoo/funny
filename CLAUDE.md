...
| `design/game/PVE_INTEGRITY_PLAN.md`（新增，设计稿待拍板）| PvE 通关/材料/养成全是客户端同步段、无服务器校验（改 save 即可伪造）| 起草分层校验提议：L0 廉价不变量（服务器重算奖励/前置/限速 + 开局战力一致性 + **材料守恒账本**）+ L1 录像抽检复算（复用 S1-RP 录像 + S1-J 无头复算）。**根问题待拍板**：`pveUpgrades`/`materials` 保留客户端应用+守恒校验（A）还是升级权威迁服务器（B）；离线合并校验窗口同议。仅文档，未实现 |
