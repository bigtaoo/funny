...
| `src/render/UnitView.ts` | 弓箭手镜像后弓/箭跑到身体错误的一侧：镜像是干净的 `scale.x*=-1` 翻转，但弓被非对称烘焙进弓箭手的前臂（左臂）精灵，水平翻转把弓带到身体另一侧（其他兵种是单手小道具，翻转读着正常） | 新增 `shouldMirror(unit)`：弓箭手豁免 Top 侧水平翻转，两个阵营都保持作者朝向（敌我已靠蓝/红 faction ink 区分，不靠朝向）；其余兵种照旧镜像。`buildStickmanContainer` 的新建与池复用两路均走此判定 |
