...
| `game/net/InputSource.ts` | 统一输入管线（M13）：`InputSource` 接口 + `LocalInputSource`（单机 DELAY 0 自转发）；`ReplayInputSource`(S1-RP) 同接口待落地 |
...
> 设计基准：`design/game/SERVER_API.md` §3（WS 协议）/§4（节拍器）、`META_TASKS.md` S1。`ranked` 队列 + ELO（S1-R）、服务器裁判复算（S1-J）、录像（S1-RP）暂未做。
...
| S1-9 | 待办 | 双真机联调（逐 tick 一致 + 重连）；含 joiner 换边视角（GameRenderer 现固定 owner-0 视角） |
| S1-R / S1-J / S1-RP | 待办 | ranked 队列+ELO / 服务器裁判复算 / 录像录制回放 |
