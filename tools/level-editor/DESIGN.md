...
| **P-D** ✅ | 波次时间线（主菜）：时间轴 + 车道行 + 出兵块增删改 + 单位调色板 | 已完成：Canvas 视口时间线（横=秒、纵=10 攻击车道），出兵块拖拽改 atTick(吸附 0.1s)/col、右键删除、滚轮平移、Ctrl+滚轮缩放；DOM Inspector 编辑 unitType/col/atTick/count/spacing/isBoss + 新增/删除；布局升级为 棋盘 + 时间线 + Inspector + 实时 JSON 四区。浏览器内验证拖拽/新增/删除/字段编辑全部正确反映到 JSON。**纵轴=车道（待你看实际效果后定 §9 的车道 vs 波次组）** |
| **P-E** ✅ | 表单字段 + 打磨（objective/经济/rewards/story/loadout/bannedCards） | 已完成：右栏改为「关卡 / 波次」双 Tab；LevelForm 编辑 id/chapter/seed、objective（survive↔timed_defense 秒输入）、startCoins/coinRegenMult、loadout/bannedCards（从 `CARD_DEFINITIONS` 多选）、rewards（coins/星级阈值/皮肤/故事键）、story 键；选中波次自动跳「波次」Tab；可选字段留空即从 JSON 删除。浏览器内验证全字段写回 + 整关经共享校验器往返通过 |
