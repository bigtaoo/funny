...
const en: Dict = {
  ...
  'tool.river.title': 'Drag to paint terrain live (click for a single dot), Esc to cancel the stroke',
  'tool.mountain.label': 'Mountain',
  'tool.mountain.title': 'Drag to paint terrain live (click for a single dot), Esc to cancel the stroke',
  ...
  'toolbar.undoPoint.title': 'Remove the last point from the stroke being painted (Backspace)',
  ...
  'hint.pathsAndPan': 'Paint rivers/mountains: pick the River/Mountain tool, set Brush Size (tiles), then drag on the map to paint terrain live — same as painting a tilemap (a plain click paints a single dot). Esc cancels the stroke being painted. In the Select tool, drag an endpoint or click a path to select it, then delete it. The map renders a real isometric camera view with the game\'s art (same as in-game): use the Pan tool, or middle-mouse-drag in any tool, to pan; wheel or the Zoom slider to zoom.',
...
const zh: Dict = {
  ...
  'tool.river.label': '河流',
  'tool.river.title': '按住拖动实时刷地形（单击画一个点），Esc 取消当前笔画',
  'tool.mountain.label': '山脉',
  'tool.mountain.title': '按住拖动实时刷地形（单击画一个点），Esc 取消当前笔画',
  'tool.city.label': '城池',
  ...
  'toolbar.undoPoint.title': '删除当前笔画中最后一个点（Backspace）',
  ...
  'hint.pathsAndPan': '刷河流/山脉：选中 River/Mountain 工具，设置笔刷大小（格数），然后在地图上按住拖动即可实时刷地形——跟刷 tilemap 一样（单击画一个点）。Esc 取消正在刷的笔画。Select 工具下拖动端点、点击路径选中后可删除。地图是真实贴图的等距相机视角（跟游戏内一样）：Pan 工具或任意工具下按住中键拖动可平移，滚轮/Zoom 滑块缩放。',
