 ...
 * Stored in the new BoneKeyframe format (rotation-only, matching old behaviour).
 */
import type { AnimationClip, BoneKeyframe, Keyframe } from '../core/types';

/** Convert a plain { [boneId]: rotationDelta } record to a Keyframe's bones Map. */
function bmap(record: Record<string, number>): Map<string, BoneKeyframe> {
  const m = new Map<string, BoneKeyframe>();
  for (const [id, rotation] of Object.entries(record)) m.set(id, { rotation });
  return m;
}

/** Wrap raw keyframe data into a proper Keyframe[]. */
function kfs(raw: Array<{ time: number; bones: Record<string, number> }>): Keyframe[] {
  return raw.map(r => ({ time: r.time, bones: bmap(r.bones) }));
}
