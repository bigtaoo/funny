...
const GACHA_CATEGORY_ORDER: GachaCategory[] = ['skin', 'card', 'equipment', 'material'];
...
function emptyDraft(): Draft {
  return {
    skin: { enabled: false, weight: 1, items: [] },
    card: { enabled: false, weight: 1, items: [] },
    equipment: { enabled: false, weight: 1, items: [] },
    material: { enabled: false, weight: 1, items: [] },
  };
}
...
export async function pageGachaPools(ctx: Ctx): Promise<void> {
  ...
  let catalog: Record<GachaCategory, GachaCatalogItem[]> = { skin: [], card: [], equipment: [], material: [] };
  try {
