...
describe('ResourceSystem', () => {
  it('accumulates coins at the documented normal rate (2 coin/s)', () => {
    const state = new GameState(1);
    const sys = new ResourceSystem();
    // normal phase: rate = COIN_REGEN_BASE(2) * 33 fp/tick = 66 fp/tick
    // after 16 ticks: trunc(16*66 / 1000) = trunc(1056/1000) = 1 coin
    state.elapsedTicks = 0;
    pump(state, sys, 16);
    expect(state.bottomPlayer.coins).toBe(1);
    expect(COIN_REGEN_BASE).toBe(2);
  });

  it('never exceeds COIN_CAP', () => {
    const state = new GameState(2);
    const sys = new ResourceSystem();
    pump(state, sys, 5000); // way more than enough to overflow
    expect(state.bottomPlayer.coins).toBe(COIN_CAP);
    expect(state.topPlayer.coins).toBe(COIN_CAP);
  });

  it('base upgrade increases regen rate', () => {
    const state = new GameState(3);
    const sys = new ResourceSystem();
    state.bottomPlayer.upgradeLevel = 1; // rate 3 coin/s vs top's 2
    pump(state, sys, 200);
    expect(state.bottomPlayer.coins).toBeGreaterThan(state.topPlayer.coins);
  });
  ...
  it('acceleration phase regenerates faster than normal phase', () => {
    ...
    expect(accel.bottomPlayer.coins).toBeGreaterThan(normal.bottomPlayer.coins);
  });

  it('emits resource_changed only when the integer coin count changes', () => {
    const state = new GameState(5);
    const sys = new ResourceSystem();
    // First tick adds 66 fp → still 0 coins → no event.
    state.clearEvents();
    sys.tick(state);
    expect(state.events.filter((e) => e.type === 'resource_changed')).toHaveLength(0);

    // Pump until a coin ticks over, then the next crossing emits an event.
    let sawEvent = false;
    for (let i = 0; i < 40; i++) {
      state.clearEvents();
      sys.tick(state);
