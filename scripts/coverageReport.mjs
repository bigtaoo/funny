...
function heading(ev) {
  ...
  return `## Coverage — ✅ ${ev.measured}/${ev.rows.length} packages ≥ ${bar} lines · overall ${overall}`;
}
...
export function renderSection(ev, baseline = null) {
  ...
  const drops = ev.results
    .filter(
      (r) =>
        r.pct !== undefined &&
        base[r.pkg] !== undefined &&
        base[r.pkg].lines - r.pct > DELTA_EPSILON * 2,
    )
    .map((r) => ({ pkg: r.pkg, d: r.pct - base[r.pkg].lines }))
    .sort((a, b) => a.d - b.d);
  if (drops.length > 0) {
    out.push(
  ...
  out.push('| Package | Lines | Δ | Headroom | Branches | Functions | Scope (measured/src) |');
  out.push('|---|--:|--:|--:|--:|--:|---|');
  ...
  const sorted = ev.results
    ...
    .sort(
      (a, b) =>
        (a.headroom ?? -Infinity) - (b.headroom ?? -Infinity) || a.pkg.localeCompare(b.pkg),
    );
  for (const r of sorted) {
    ...
    out.push(
      `| ${r.pkg} | ${fmtPct(row.lines.pct)} | ${delta} | ${fmtSigned(r.headroom, 0)} | ${fmtPct(row.branches.pct)} | ${fmtPct(row.functions.pct)} | ${fmtScope(row)} |`,
    );
  }
  ...
  out.push(
    `| **Overall (gated)** | **${fmtPct(ev.overall.lines)}** | **${overallDelta}** |  | **${fmtPct(ev.overall.branches)}** | **${fmtPct(ev.overall.functions)}** |  |`,
  );
  out.push('', '</details>', '');
