...
上游那段「待补图」还点了一项**盾**（`world.protected`——注意它其实不在弹窗里，是 [`WorldMapPanels/hud.ts`](../../client/src/scenes/worldmap/WorldMapPanels/hud.ts) 的保护倒计时 buff 行，目前无图标）。本批不出：`armor`/`armorHeavy`（正面圆盘小盾）在 26px 上读不出来是这个项目已经交过学费的结论（见 [[pick-icon-glyphs-by-eye-not-name]] 与 batch7-log），而"保护中"到底该画盾、画伞、还是干脆用 `lock`，是一次独立的语义判断，不该顺手塞进一批以「地块结构物」为主题的出图里。**留在 backlog，下一批连同判据一起做。** → 2026-09-03 已做：判据、prompt、接线清单在 [`batch10`](tab-icon-art-prompts-batch10.md)（结论是**画伞**，`umbrella`；顺带纠了「无图标」这个说法——它其实挂着 `armorHeavy`，而同一屏上那块四分圆有三个含义）。
...
- 这不是本批引入的，是 2026-09-02 图标槽那轮就记在案的既有排版问题（当时三个建造按钮共用 `hammer`，同样画不出来），**根治办法是缩短标签 + 把门禁改成实测**（有图标之后 `移动到此（停留）`/`移动并驻扎` 可以只留 `停留`/`驻扎`）；标签是产品决定，门禁是代码改动——见本节末尾 2026-09-03 的订正。
- **门禁的闭式阈值**（给以后缩标签的人）：`mw = PANEL_W.md = 900`、`modalMargin = 12`、`cols = min(n, 5)`，于是 `btnW = min(300, (900 - 12·(cols+1)) / cols)` —— n≤2 → 300、n=3 → 284、**n=4 → 210**、n≥5 → 166。也就是（在 2026-09-03 换掉门禁之前）**菜单 ≤4 个按钮才画图标**。己方地块菜单最少也有 8 个（增援 + 停留/驻扎 + 设防 + 瞭望塔 + 箭塔 + 阻挡 + 放弃 + 关闭），所以它永远画不出；**盟友地块菜单只有 2 个**（驻扎/召回 + 关闭）、**中立地块菜单最多 4 个**（占领 + 扫荡 + 停留 + 关闭），这两处**今天就在画**。
- **本批真正兑现的是那三条状态行**：`已建瞭望塔`/`箭塔`/`阻挡` 从 emoji 换成了手绘墨线，这是三个 P0 的主要目的；坐标行的 `mapPin` 和险地标题的 `stronghold` 也都在真实弹窗里就位。
- 所以逐个字形的实际可见性是：`mapPin`（10 处坐标行）、`stronghold`（险地标题）、`watchtower`/`arrowTower`/`blocker`（三条状态行）**全部可见**；`camp` 在**盟友地块**按钮上可见、`footsteps` 在**中立地块**按钮上可见；只有三个**建造按钮**上的图标要等标签缩短。**7 张没有一张是完全看不到的。**
- 按钮那条路没有白接：验收时用同一条 `showModal` 路径造了一个宽列弹窗（`btnW = 284`/`210`），确认五个字形在**深色按钮填充**上都渲染正确、也都分得开。

> **2026-09-03 订正（这一节上面两条的结论错了一半）**：「缩短标签就会自动显示、不需要改代码」**不成立**。`btnW` 只由**按钮个数**决定（`cols = min(n, 5)`），跟标签一个字都无关，所以己方地块菜单缩到多短都还是 166px、还是一个图标都不画。真正的错在门禁本身：`btnW >= 200` 是在替一个它没问的问题作答（「图标占掉 34px 之后标签还放得下吗」）。已改成**实测**——把标签按缩窄后的宽度量一遍，只有当图标不让它多占一行、且仍装得进按钮高度时才画。缩标签这件事照做（`移动到此（停留）`→`停留` 等六个键 × 三语），但它是让实测通过的**前提**，不是替代品。落地与三语实测数字见 [`UI_DESIGN_LOG_2026-08.md` §45](../game/UI_DESIGN_LOG_2026-08.md)：中文现在 **9/9 个按钮都画图标**，本批那五个按钮字形全部在屏上。
