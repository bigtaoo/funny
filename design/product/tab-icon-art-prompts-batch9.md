...
- **门禁的闭式阈值**（给以后缩标签的人）：`mw = PANEL_W.md = 900`、`modalMargin = 12`、`cols = min(n, 5)`，于是 `btnW = min(300, (900 - 12·(cols+1)) / cols)` —— n≤2 → 300、n=3 → 284、**n=4 → 210**、n≥5 → 166。也就是**菜单 ≤4 个按钮就画图标**。己方地块菜单最少也有 8 个（增援 + 停留/驻扎 + 设防 + 瞭望塔 + 箭塔 + 阻挡 + 放弃 + 关闭），所以它永远画不出；**盟友地块菜单只有 2 个**（驻扎/召回 + 关闭）、**中立地块菜单最多 4 个**（占领 + 扫荡 + 停留 + 关闭），这两处**今天就在画**。
