...
## 卡片布局（2026-07-17 改为图标卡）

商店/充值两个 Tab 的商品统一为**大图为主的竖版图标卡**（`ShopSceneBase.drawCard`）：顶部一张大方图（美术图或矢量图标），其下依次居中排 **标题 → 价格（金币或 ¥，可带划线原价 + 右上角省钱角标）→ 状态行 → 底部整宽动作按钮（多个则纵向堆叠）**。栅格每行约 4 列（横屏 `targetW=w*0.16`）/ 3 列（竖屏 `w*0.30`），`cellH=cellW*1.5`，见 `gridMetrics()`。替代了此前「左小图 + 右文字」的横长行卡。
