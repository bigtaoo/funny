...
3. 产物输出到 `client/src/assets/slg/world_atlas.png`（1024×1024）+ `terrain_atlas.json`（TexturePacker JSON-Hash，帧名同资产名，如 `terrain_grass`）。
...
- **生物群系地表染色 + 资源母题分级不透明度（2026-07-11）**：反馈整张 SLG 地图（对比三战）读起来像「墙纸」——`resourceDensity=1.0`（[ADR-032](../DECISIONS.md)）下每格都是资源地、每格都盖一张母题，且地表统一走 `terrain_grass` 的单一 sage tint，全图一个颜色、无疏密节奏。两处修法（客户端 + map-editor 同步，保 render parity）：
  - **地表按 biome 染色**：此前一版刻意把资源格的地表 wash 关掉（「biome wash + 母题双重编码 resType，图显得乱」），`RES_TEX_TINT` 沦为 UNUSED 常量。本次**重新启用**：`resource` 格的地表 tint 从 `terrain_grass` 改取 `RES_TEX_TINT[resType]`（paper 暖草黄 / ink 冷蓝 / graphite 中性灰 / metal 薄荷 / sticker 柔玫瑰），使同资源大区呈现淡色分块（三战式地形辨识）。色值本就高明度、贴近纸色，不抢母题、也不撞阵营水洗色。客户端 [`tileGraphics.ts`](../../client/src/scenes/worldmap/tileGraphics.ts) `drawTileL1` 把 `motifResType` 上提、地表与母题共用；map-editor [`tileGraphics.ts`](../../tools/map-editor/src/render/tileGraphics.ts) `drawEditorTile` 镜像。两处 `tileStyle.ts` 的 `RES_TEX_TINT` 注释同步去掉「UNUSED」。
  - **母题按等级分级不透明度**：`drawResMotif` 原来每格母题全强度绘制 → confetti。改为 `sp.alpha = 0.4 + 0.6×((lv-1)/9)`（lv1≈0.4 淡、lv10=1.0 实），低级资源退成背景、高级资源跳出来，用透明度做主次层级（用户拍板：不减母题数量、也不改「每资源 10 张分级图」的既定方案，只调 alpha）。客户端与 map-editor 的 `drawResMotif` 两处同改。

- **障碍地形透明度再提（2026-07-12）**：反馈「地图上完全看不出河流/山脉有专门的手绘贴图，就是纯色色块」。排查确认贴图确实在加载、`beginTextureFill` 也确实按 tile 铺了纹理（非渲染缺陷）——根因是常规游玩缩放下单格只有 30–80px（贴图原始 256px），细排线经降采样已大幅糊平，`TERRAIN_TEX_ALPHA` 0.8 又在此基础上再叠 20% 纸底色，把本就所剩不多的对比度进一步冲淡到肉眼几乎读不出"这是岩石/水纹"，只剩一块纯色。`terrain_mountain`/`terrain_river` 的 alpha `0.8→0.92`（更贴近 land 的 `0.95`，仍比 land 稍软、保留"退底"的设计意图），同时把两张贴图的 tint 略微加深/加饱和（river `#a9cbe0→#8fbadb`，mountain `#b3b7bd→#a2a7b0`）配合提升的 alpha 维持与相邻地块的区分度。客户端 [`tileStyle.ts`](../../client/src/scenes/worldmap/logic/tileStyle.ts) 与 map-editor [`tileStyle.ts`](../../tools/map-editor/src/tiles/tileStyle.ts) 两处同改，已用 map-editor 实际渲染截图核对前后对比。注：细排线在中低倍缩放下仍会糊成较均匀的色块（降采样的物理限制，非本次改动能解决），此次只解决"色块本身够不够深/够不够跳"，不是让排线在小尺寸下变清晰。
