...
### 0.6 iPad 上是信箱式留白（待你拍板，2026-08-18）

拍 2048×2732 时看清的：iPad（4:3）下游戏**上下左右都有米色留白**。不是 bug——`PortraitLayout` 的设计高下限是 1920（即 9:16），4:3 屏幕按 Contain 缩放后必然留边（见 [[portrait-safe-area-layout]] 的动态设计尺寸方案，它解决的是"比 9:16 更修长"那一侧，没解决"比 9:16 更方"这一侧）。

三个选项，**没定之前 iPad 截图就照现状交**（这是 App 在 iPad 上的真实表现，交一个假的更糟）：① 接受留白；② iOS 上架时**不勾选 iPad 支持**（则 §1.1 的 iPad 截图整行作废，但 App Store 会把它当 iPhone-only App）；③ 给 4:3~3:4 区间做真正的宽版适配（独立的活，不小）。
