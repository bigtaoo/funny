...
- **裁切参数**（2026-08-15 视觉核对后定档，见 §四-8）：胸像走 `buildPortraitIcon` 的 `'bust'` 取景——按圆直径铺满宽度后再 **zoom 1.10、上移 4%**，把脸放到圆心而不是让它沉在下半圆；全身立绘（`skin` 分类）仍走 `'full'`——**不缩放、顶部对齐 +3% headroom**（头贴着画面顶边，缩放会切进躯干）。胸像盘面填充 = 圆盘直径的 92%，即分类色只剩一圈细边，不再是厚圆环。
