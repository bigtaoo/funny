...
> ⚠️ **过时提示（请勿照此目录结构实现）**：本文 §二 描述的 `packages/game-logic` + `game-client`（React/Canvas）单仓布局**已废弃**。实际代码为 `client/src/game/`（纯 TS）+ `client/src/render/`（**PixiJS，非 React**）；**数值权威已迁至 `server/engine/src/config.ts`（`@nw/engine`，ADR-001）**，不在 `@nw/engine/config.ts`。另：手牌自动刷新为 **30 秒**（`CARD_REFRESH_TICKS`），文中「2 分钟」为旧值。坐标系、系统职责划分、录像/事件契约等概念仍可参考。
...
本文 2026-08-17 按 500 行约定拆分。**小节编号一律未变**，源码/文档里既有的 `logic-architecture.md §N` 引用照旧有效——按下表找所在分册。

| 内容 | 文件 |
|---|---|
| 开头 ~ 逻辑层架构文档 | **本文** |
| 七 事件系统、八 指令系统、九 录像格式、十–十四 AI/联机/计时器/抽牌/结算 | [`logic-architecture-events.md`](logic-architecture-events.md) |
...
**接下页** → [`logic-architecture-events.md`](logic-architecture-events.md)：七 事件系统、八 指令系统、九 录像格式、十–十四 AI/联机/计时器/抽牌/结算。
