...
六章战役之间，真实层同步推进一条涛与 Anna 的关系线（见 `world.md` 表格）。目前这条线只有文字设定，没有插画。本文档给每个章末节点各配一张场景插画的 AI 出图 prompt，风格延续开场插画（`intro_notebook.png`）的手绘本子插画路线——铅笔线稿 + 局部水彩、四色克制配色、横版构图、上三分之一留白给文字——但每张换一套贴合当章情绪的配色。

角色随时间线长大：Ch1 约 13～14 岁（对应德国文理中学 8 年级），Ch2 约 15 岁，Ch3 约 16 岁，Ch4 约 17 岁，Ch5 约 18 岁，终章（Ch6 后）是多年后的成年涛。涛始终是东亚男孩／男人，家在中欧（与开场插画一致，非东亚室内/器物）；Anna 是当地德国女孩，衣着随年龄自然变化，不设夸张发色或标志性配饰——她是"普通但外向"的人，不是奇幻角色卡里的 Hartmann 三人。深蓝色笔记本封面（`#2E4055`）作为贯穿六张图的视觉母题，每张都要出现至少一本笔记本。
...
| Ch5 后·涛动摇 | `client/src/assets/story/interlude_ch5_falter.png` | 同上 |
...
呼应：从各打各的到看见对方（涛有爱无路，Anna 有路不愿走）。
...
呼应：涛把故事做成了游戏，弱化父亲视角（见 `world.md`「尾声」修订）。
...
文案定稿（2026-08-05）：六段真实层正文见 `world.md`「章末真实层：涛与 Anna」（Ch1～Ch5 各 4 行 + 尾声改稿 8 行，与开场 `story.line.1`~`.7` 首尾呼应）。落地方式拍板：新建可复用插画过场组件（抽取 `IntroScene` 的插画淡入 + 逐行文字 + 点按推进机制），而非在 `ResultScene` 里加插画。
...
- [x] 文字展示方式改回堆叠（2026-08-08）：上一条「文字逐条替换显示」改成和 `IntroScene` 一致的堆叠展示——每一拍独立淡入、不清空前一拍，看完最后一拍时整段文字都还在屏幕上，可以从头读到尾。详见 `CAMPAIGN_STORY.md`「章末真实层插画：文字改回堆叠展示」
