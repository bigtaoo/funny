...
**⚠️ 2026-08-20 实机复核（§6.12）：`res_sticker_l9` / `res_sticker_l10` 曾判为 §6.2 #1 剪影违规（卷状物），当日两帧全部重画落地（§6.12.5）——`l10` density 0.259（穹顶堆体）、`l9` density 0.098（三叠星 + 散星的矮丘，第二版命中）。46 帧现在全部同时满足门禁与剪影铁律，无美术欠项。**另查出并已处理一处**：sticker 的 tan→gold 色带在屏上等于不存在（结构性），用户拍板**取消色带**——五族现在统一纯黑墨，`BAND`/`applyBand`/`tintLevelFrame` 已删，见 **§6.12.6**。 同日还修掉**同一对 sharp 陷阱在这条管线里的三处实例**（`palette: true` 静默量化 + `composite` 的 alpha 预乘漂移）：合并页就地回贴（§6.12.7）、`pack_resources.cjs` 写源图集本身（§6.12.8，最上游、损伤 alpha 精度 240→143 种），并给整条管线补上了 4 条对成品文件的断言（§6.12.9，此前零覆盖）。 同节还记了两条不要重复踩的结论：剪影铁律**做不成构建期门禁**（形状签名实测否证，§6.12.2）；`Lv.N` 标签的 l6+ 阈值**不限制屏上标签数**，可读性靠字号上限撑住（§6.12.3）。
...
> **⚠️ 决策变更 III（2026-08-19，用户拍板）**：分级读数整套重构，见 **§6 → 已拆出为 [`slg-resource-art-levels.md`](slg-resource-art-levels.md)**（权威，覆盖 §5.3 #2 与 §5.4 的形态跃迁条款）。起因是「按宽归一」这条旧契约会**惩罚横向生长**——高等级的丰度是横着铺开画的，归一化反而把它压小，实测四类资源全部在 l5→l6 墨量回落，ink l4 成了全十级里视觉最重的一帧。
...
> **本节已拆出为独立分册** → [`slg-resource-art-levels.md`](slg-resource-art-levels.md)（ADR-067 第七轮，2026-08-24）。
>
> 它是分级读数的**权威**（病根、等效面积归一、`LEVEL_SCALE`、构建期单调门禁、剪影铁律与 20 张违规帧清单、精确等级通道）。小节编号未变，仍是 §6.1–§6.12。

---

## 7. 通用母题帧的 UI 墨量地板（2026-08-27 定 · 权威）

> 起因：用户报「主城里的图标，看起来非常淡」，截图上圈了资源条的**石墨**、建筑卡的**纸盘**和**石墨坊**三处。

### 7.1 病根：不是笔太淡，是缩放把覆盖率吃掉了

5 张无分级的 `res_<type>` 母题帧**不是地图素材，是游戏的资源图标**——CityScene 资源条（33px）、五张产出建筑卡（60px）、WorldMapScene 顶栏 HUD、领土面板都画它；地图瓦片那条回退路径实际上永不触发（能刷出来的每一级都有专属图）。这两份工作的要求完全不同，而它在后一份上是不合格的。

实测（2026-08-27，感知墨量 = `Σ alpha·(1−luma)` / 帧面积，按入库像素测）：

| frame | 感知墨量 | 2% 分位笔芯亮度 | 判定 |
|---|---|---|---|
| `res_ink` | 0.165 | 31 | 够读 |
| `res_metal` | 0.093 | 33 | 够读 |
| `res_sticker` | 0.038 | 14 | 偏薄 |
| `res_graphite` | 0.040 | 21 | 偏薄 |
| `res_paper` | **0.019** | 49 | **不可读** |

`res_paper` 的墨量只有旁边墨水瓶的 **1/8**——正是用户第一眼挑出来的两张。

**笔并不淡**：石墨的笔芯亮度 21、纸的 49，都够黑。淡的是**覆盖率**。这些是发丝级的轮廓画，而 §5.8 步骤 3 缩到长边 128px 是面积正确的——一条 1px 的笔画落到 1/4 像素上，颜色不变、alpha 掉到 1/4。屏上看到的就是一条淡灰线，而不是一条细黑线。客户端再从 128px 缩到 33px，同样的损失叠第二次。画满块面的墨水瓶感觉不到；一张空白纸除了发丝什么也没有。

> 顺带否证一个看起来很像的猜想：§5.8 步骤 1 的去白底（`alpha = 255 − luma` 且 RGB 保留原色）确实会让**灰色笔**被衰减两次（当色又当覆盖率）。实测这条在这 5 张上不是主因——笔芯本来就够黑。别照着"提笔色"去修。

### 7.2 修法：打包期量到地板为止

`pack_resources.cjs` 在缩放之后、测墨量之前，对**仅 5 张通用帧**做一次墨量恢复（`restoreUiInk()`），两种只补覆盖率、不动笔色的操作：

1. **抬 alpha**（`liftAlpha`，gamma）——二分求指数，正好落在 `UI_INK_FLOOR = 0.070`。
2. **加粗一像素**（`dilateStroke`，3×3 取最不透明邻居的 RGBA）——只在单靠 (1) 到不了地板时用；加粗会**冲过**地板，所以之后允许 gamma 回到 1 以上把墨量收回到地板上，而不是一路冲过去。

均由测量驱动、不手调，重画一张图下次打包会自己重新找平。落地结果：

| frame | 墨量 | → | alphaGamma | dilate |
|---|---|---|---|---|
| `res_ink` | 0.165 | 0.165 | 1.00 | 0 |
| `res_metal` | 0.093 | 0.093 | 1.00 | 0 |
| `res_paper` | 0.019 | 0.070 | 0.89 | 2 |
| `res_graphite` | 0.040 | 0.070 | 1.21 | 1 |
| `res_sticker` | 0.038 | 0.070 | 1.77 | 1 |

`UI_INK_FLOOR = 0.070` 卡在 metal 的 0.093 之下，所以本来就够读的两张一个像素都不动。

**alpha 的趾部（`UI_ALPHA_TOE = ALPHA_TRIM = 16`）不是细节，是第一版的返工点。** 去白底之后，轮廓内部并不是空的，而是源稿灰白纸留下的 alpha 2–20 的雾。裸 `a^0.5` 会把 alpha 10 抬到 42，第一版就这么干的：纸和贴纸出来后形状里填了一块可见的灰盒子，比它要修的"淡"更糟。先把这层雾归零零成本（笔画外缘 6% 的 alpha 低于可察觉），而且用的就是裁剪已经在用的那个阈值。

**故意不施加于 `_lN` 分级帧**：它们的墨量**就是**分级读数（§6.3），地板会把门禁存在的意义——各级之间的差——抹平；而在瓦片上（小、成簇、一眼四十个）偏轻的分量是对的。

### 7.3 顺手修掉的一个潜伏 bug：打包器会把自己的产物打进图集

`pack_resources.cjs` 的源文件筛选是 `/^res_.*\.(webp|png)$/i`，只排除了 `res_contact_sheet.png`——但 `OUT_DIRS[0]` 就是它自己所在的目录，**`res_atlas.png` 从第二次运行起就一直躺在源文件旁边**。2026-08-27 第一次重跑打包器就撞上了：它把 `res_atlas` 当成第 51 帧打了进去（一张 512×2048 的、所有 50 张母题的合影），画面边缘还触发了 `stripBorderRing` 的告警。已入库的图集是 50 帧，所以**没有任何一个发布版本带过它**；`worldMapResMotifLevelRead.ui.ts` 的「恰好 50 帧」用例会在下一次有人重跑打包器并提交时挂掉——也就是说它不会被悄悄发布，但会在一个跟改动本身毫无关系的地方炸给你看。改成按名字排除 `res_contact_sheet.png` / `res_atlas.png` / `res_atlas.json` 三者（`OWN_OUTPUT`）。

### 7.4 落地与验证

- 重跑 `node art/slg/slg-map/pack_resources.cjs` → 50 帧、分级门禁全绿；再 `node art/scripts/patchMergedAtlas.js art/slg/slg-map/res_atlas.json client/src/assets/slg/world_atlas.json` 就地回贴（帧尺寸未变 → 走 in-place，不 reflow）。
- 逐帧像素比对合并页：**89 帧里恰好 3 帧变了**（`res_graphite` / `res_paper` / `res_sticker`），零帧移位，页尺寸不变。
- `test:ui` 231 文件 / 2183 例全绿，含 `worldMapResMotifLevelRead.ui.ts` 的「成品图集每一个可见像素都是中性墨」——加粗取的是邻居的完整 RGBA，灰度不破。
