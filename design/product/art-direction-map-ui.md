...
**批次 10：「保护中」的伞（1 张，2026-09-03 判断+prompt，出图待办）**。批 9 留下的最后一个空位，也是唯一一次纯语义判断先于造型判断的：这一行不是没有图标，它挂着 `armorHeavy`——批 7 那面纹章式四分圆，验收时就写明"更像纹章/饼图、语义靠文字承担"，而这一行的文字恰好是"保护中（剩 X）"，图标要说的正是保护本身。三个候选的取舍：**再画一张盾**否决，因为 `equipIcon` 的鸢形盾已经是全库"装备"的意思（所有装备类奖励都长这样），再画一张等于跟最广的一条语义撞车；**复用 `lock`** 否决，因为挂锁在本库是"未解锁/不可用"的负向标记，而且紧跟倒计时会被读成"还有 X 才解锁"，正好反了；**画伞**采用——全库唯一没有第二个"穹顶 + 竖杆 + 弯钩"的低频剪影，中文「保护伞」、德语 `Schutzschirm`、英语 "under an umbrella of protection" 三语都通，且与倒计时同向。判断依据、prompt、接线与验收口径见 [`tab-icon-art-prompts-batch10.md`](tab-icon-art-prompts-batch10.md)。同日顺带把队伍状态行的 `armor`/`armorHeavy` 换成批 9 的 `footsteps`/`camp`（零新美术），因为实拍发现那块四分圆在世界地图 HUD 一屏上有三个不同含义。
