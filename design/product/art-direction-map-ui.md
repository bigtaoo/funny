...
> - **图集加载**：`client/src/render/atlas/decorAtlas.ts` 用 `PIXI.Spritesheet` 加载 `client/src/assets/decor/decor_merged_atlas.{png,json}`（帧名不带扩展名，如 `decor_sun`）。App 启动时 `loadDecorAtlas()` 后台预解码（fire-and-forget，纯装饰，失败不阻塞启动）；图集很小，进战斗前一般已就绪，未就绪则该局无装饰（可接受）。线条为原墨色，**不 `tint`**。
...
**批次 10：「保护中」的伞（1 张，2026-09-03 判断+prompt+出图+验收，全部同日完成）**。批 9 留下的最后一个空位，也是唯一一次纯语义判断先于造型判断的：这一行不是没有图标，它挂着 `armorHeavy`——批 7 那面纹章式四分圆，验收时就写明"更像纹章/饼图、语义靠文字承担"，而这一行的文字恰好是"保护中（剩 X）"，图标要说的正是保护本身。三个候选的取舍：**再画一张盾**否决，因为 `equipIcon` 的鸢形盾已经是全库"装备"的意思（所有装备类奖励都长这样），再画一张等于跟最广的一条语义撞车；**复用 `lock`** 否决，因为挂锁在本库是"未解锁/不可用"的负向标记，而且紧跟倒计时会被读成"还有 X 才解锁"，正好反了；**画伞**采用——全库唯一没有第二个"穹顶 + 竖杆 + 弯钩"的低频剪影，中文「保护伞」、德语 `Schutzschirm`、英语 "under an umbrella of protection" 三语都通，且与倒计时同向。判断依据、prompt、接线与验收口径见 [`tab-icon-art-prompts-batch10.md`](tab-icon-art-prompts-batch10.md)。同日顺带把队伍状态行的 `armor`/`armorHeavy` 换成批 9 的 `footsteps`/`camp`（零新美术），因为实拍发现那块四分圆在世界地图 HUD 一屏上有三个不同含义——伞落地后这一屏的四分圆**清零**。出图一版过：裁边 128×128 = **1.00:1**，是全批次最方的一张；实心穹顶 + 细杆弯钩在 26px 上跟 `defense` 的空心头盔穹顶、`camp` 的折线帐篷（**同屏**）都分得开。

**批次 9：世界地图弹窗图标槽剩下的 7 个空位**（2026-09-02 判断+prompt，出图待办）。`showModal` 长出图标槽（同日，见 [`SLG_LOG_2026-08.md`](../game/SLG_LOG_2026-08.md)）之后，24 个位点接上了现成美术，剩下两类填不了：三条结构状态行（瞭望塔/箭塔/阻挡）在墨色表里压根没有对应美术，当时**刻意保留了字符串里的 emoji 🗼🏹🚧**——删掉等于让这三行彻底失去标记，比留一个字体不受控的字形更糟；另外四处（坐标行借 `globe`、驻扎借 `unit`、停留借 `spd`、险地标题借 `siege`）是能用但语义借来的。判断表和 prompt 见 [`tab-icon-art-prompts-batch9.md`](tab-icon-art-prompts-batch9.md)。**这批有三条跟以往不同的地方**：①**不复用地图上那三张同名建筑图**（`icon_watchtower`/`icon_blocker`/`icon_arrowTower` 都已存在且语义一致）——它们是写实钢笔排线的另一套语言，而且 [`slg-building-art.md`](slg-building-art.md) §6 已经在它们各自真实的渲染高度（≈30px/≈17px）上量过，结论是「本来就是一团模糊的排线纹理」，当地景可以，当 26px 的信息标记不行；②**三个建造图标是一个集合而不是三张图**（同一个建造菜单里同屏），按沙漏三档的老规矩一次请求出三张；③`blocker` 这次明确要求**塞进正方形**而不是照地图那张的 2.91:1 画矮长条——`iconArtAspect.test.ts` 的 2.2 上限会直接拦，而正确解法是重出，不是往 `ELONGATED_ON_PURPOSE` 加豁免。「保护中」的盾**没有**进这批：`armor`/`armorHeavy` 那面正面圆盘在 26px 上读不出来是已经交过学费的结论，而"保护中"该画盾、画伞还是干脆用 `lock` 是一次独立的语义判断，不该顺手塞进一批以地块结构物为主题的出图里。 **同日出图 + 接线 + 实拍完成**：6 张一版过，`camp` 记为可用但最弱并登记 v2——它没跟 `lead`/`play`/`home` 混（"入口 + 绷绳 + 地面线"确实把它从三角剪影里拽了出来），但**代价付错了维度**：绷绳把外框拉到 2.00:1、contain-fit 后只占方框一半高度，在同一行里比邻居轻一档，而绷绳本身在 26px 上完全看不见。这条值得单独记住——**破除撞车的那几笔，如果自己在目标尺寸上会消失，就只剩下"把外框撑宽"这一个副作用**，等于用可读性换了可读性。v2 改成用脊杆端头破三角、整体收成方形，同日一版过（2.00 → 1.44:1，26px 上填满方框、与 `home` 同弹窗并排不混）。另外实拍量到 `btnW = 166 < 200`（图标门禁），所以五个按钮图标在最常见的己方地块菜单里当天一个都不画——既有排版问题，本批兑现的是三条状态行 + 坐标行 + 险地标题。**2026-09-03 已补上**：当时写的"根治靠缩短标签"只对了一半（`btnW` 只跟按钮个数有关），门禁改成实测标签能否放下之后，中文菜单 9/9 个按钮都画图标，五个字形全部在屏上，见 [`UI_DESIGN_LOG_2026-08.md` §45](../game/UI_DESIGN_LOG_2026-08.md)。
