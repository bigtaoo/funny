...
7. **圆章模式复议：图标依旧看不清（2026-08-29，用户截图报告"选了背景之后，图标就几乎看不到了"）**：上一条的"纯色圆章"把线稿垫在实心色盘上、四边各内缩约 18% 留边给描边圈，数学上就决定了圆章分支里知出的图标永远不超过 `size * 0.64`——而 `BADGE_MEDALLION_MAX = 44` 这个阈值本身记录的就是"连不打折的整版染色线稿都会糊"的下限，圆章分支里被再打 0.64 折之后的图标必然更小，且这个分支覆盖的现有 5 个消费方（header/render/lists/orgBrowse/ProfilePopup/tokens）实际尺寸落在约 10–38px design-px，从未接近 44px 上限——换句话说圆章上的"知出图标"从未真正读得清过，只是把"看不见的线稿"从纸底移到了色盘上，用户报告的正是这个。**改法**：圆章分支不再尝试知出图标，只画一枚纯色实心圆（描边同前）——阈值以下就是一个干净的强调色圆点，图案细节继续依赖选色弹窗（24 图标网格）和 ProfilePopup 详情预览两个≥44px、走"整版染色线稿"分支的场景。副作用：圆章不再有子节点，`emblemIcon.ts` 内那个强制 `children:true` 的 `destroy()` 补丁随之删除（Graphics 本身没有子节点可泄漏）。`emblemIconMedallion.ui.ts` 同步改写断言（圆点是无子节点的 Graphics，不再是 Container[disc,sprite]）。
