...
> **战场箭塔已按本节规格重画（2026-08-19，已上线）**：此前实机用的不是本节规格——`game_archer_barracks.png` 原本是画给「弓箭手兵营」的茅草屋，箭塔临时借用多时（灰绿墨、1.57:1、41% 的墨在两棵树上）。新图 `game_arrow_tower.png` 照本节原有规格「梯形塔身+三角顶+一支弓」具体化，蓝墨与兵营同族；**不采用垛口平顶方案**（垛口是兵营的辨识特征，56px 下两者剪影会撞）。设计/prompt/打包管线两个坑（`.tint()` 对黑线无效、膨胀必须在打包尺寸做）/验收阈值见 [`battle-arrow-tower-art.md`](battle-arrow-tower-art.md)。
