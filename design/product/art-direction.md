...
- **全彩 AI 美术资产**（`.tao` 骨骼动画单位）：身体是有质感的全彩图，**不能整体染色**（会毁掉美术）。改用**阵营描边轮廓**——沿单位轮廓画一条细的「脱开身体一道纸色缝隙」的描边线，蓝=我/红=敌。技术见 `StickmanRuntime` 的 outline 层：加载 `.tao` 时按每骨 alpha 二值膨胀两次相减，生成纯白「细空心线」纹理（缓存在共享 `TaoAsset`），运行时每骨多挂 1 个静态 sprite、`tint` 成 faction ink。**纯白 + tint 是唯一可行机制**（PIXI `tint` 是乘法，纯黑乘任何色仍是黑，故标记色必须白；轮廓须独立成层才能只染轮廓不染本体）。**刻意不用实时描边 filter**：单位铺满时每单位一个 filter 在低端机/微信上跑不动，受众面优先。轮廓是**细的、离身体有缝隙的线**，不是紧贴的实心剪影（后者读成「穿了件衣服」）。粗细/缝隙在 `StickmanRuntime` 的 `OUTLINE_WIDTH_PX` / `OUTLINE_GAP_PX` 单点可调（屏幕像素，按每骨缩放反算保证统一）。
- **包体权衡**：轮廓在**加载期生成**（0 包体，开局前几毫秒一次性 CPU，缓存），**不烘进 `.tao`**（烘进会让每文件涨 ~15–30% 且随皮肤数线性膨胀，微信小游戏 4MB 主包尤其吃不消）。风险点：微信 canvas 若不支持 `getImageData` 回读会降级无描边（已 try/catch 兜底），届时再**只为微信**烘一份。
