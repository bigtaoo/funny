...
#### 画布点选规则（Skin + Sprite 预览，2026-08-30）

zOrder 只决定**画**的顺序，不能直接当**点选**的顺序用：贴图是矩形，spine.png 的矩形连两侧肩膀一起罩住，而 spine 的 zOrder 又高于两条手臂——只按"矩形 + zOrder 取最前"来判定的话，手臂的每一个可见像素都点不到（实测 runner 工程：r_upper_arm 采样到的 1162 个有墨点，**全部**落在 spine 的贴图矩形内）。而且从左侧骨骼列表里选中手臂也救不了，因为命中测试根本不看当前选中项。

因此点选按下面两条规则判定，优先级从高到低：

1. **按像素判定**：贴图矩形只是粗筛，之后要求该点在贴图的 **alpha ≥ 8**（`MIN_HIT_ALPHA`）处才算命中——透明区一律穿透到下一层。alpha 掩码由 `ImageController` 在载图时解码，一像素一字节、长边上限 512（`ALPHA_MASK_MAX`）。掩码建不出来时（解码失败等）该精灵退回纯矩形判定，不会变成点不到。
2. **选中优先**：若点击点同时命中了**当前已选骨骼**，则保持选中它，不让 zOrder 更靠前的抢走。这只在"确实命中"时生效，不是锁——点到别的部位照常切换。

两条都不满足时，取命中中 zOrder 最大的那个（原行为）。
