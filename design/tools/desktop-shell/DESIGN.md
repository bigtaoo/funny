...
- **生产地址接线 + 品牌化（2026-07-28）**：`tools.ts` 加 `prodUrl` + `resolveToolUrl()`（见 §4.2），装好的正式包默认连生产、开发跑源码默认连本地，验证过两种模式各自正确。品牌名从占位的"Notebook Wars 工具箱"改为 **NW Tool**（`package.json` 根 `productName` + `build.productName` + 窗口标题 + 侧边栏 `<title>`，一并改）；图标用仓库既有游戏品牌 logo（`art/logo/derived/logo-1024.png` → `tools/desktop-shell/build/icon.png`，electron-builder 按约定路径自动转 ico），验证：提取打包出的 exe 图标核对过确实是这个盾形 logo。
  ...
  - **再修正（2026-09-21）——任务栏仍然糊，根因不是选错源图，是 `.ico` 里只有一档**：上一条"electron-builder 从单张源图生成的 `.ico` 包含 16/32/48 等小尺寸"这个前提是错的。直接跑它内部的 `app-builder icon --format ico` 核对：从 1024² PNG 生成的 `.ico` **只有一个 256×256 条目**。于是任务栏（24px）、alt-tab（32px）全靠 Windows 在绘制时把 256 缩一次——它用的是廉价滤波、又是一步缩 10 倍，盾牌描边直接糊成灰块。换源图（master → simple）当时确实有改善，但治的是"太花"，没治"糊"。
    落地：新增 `art/scripts/make-desktop-icon.mjs`，用 sharp 从 `art/logo/logo-simple.png` **逐档**重采样（lanczos3，≤48px 再补一点点锐化把描边找回来）打出 16/24/32/48/64/128/256 七档，自己拼 `.ico` 容器（PNG 压缩条目，Vista+ 全尺寸支持）写到 `tools/desktop-shell/build/icon.ico`；`package.json` 加 `build.win.icon = "build/icon.ico"`（根 `icon` 指向 PNG 会盖掉 electron-builder 的 `build/icon.ico` 约定，必须显式写）。`build/icon.png` 保留，供 mac/linux 与留档。
    验证：① 打包后逐档在 `NW Tool.exe` 里按字节搜到，七档全部嵌入；② 起了一次打包出的 exe，截屏抓真实任务栏——24px 下三笔交叉、盾形描边都清晰可辨；③ 对比图（老路径 256→24 绘制时缩放 vs 新的 24px 条目）逐像素放大看过。
    **注意**：改完只有重新打包安装才生效，且 Windows 会缓存 shell 图标——已固定在任务栏的旧快捷方式可能仍显示旧图。
  - **装机验证（2026-09-21）**：`npx electron-builder --win --publish never` 出 `release/NW Tool Setup 0.1.0.exe`（78 MB，oneClick + perMachine=false，装到 `%LOCALAPPDATA%/Programs/nw-desktop-shell`），跑了一遍真装。两条只有装机才暴露的事：
    - **2026-07-28 那次换源图从来没进过装机版**——机器上装的 exe 时间戳是 07/28 10:30，换源图的提交是当天 19:06，也就是说用户这两个月看的一直是 **master 版缩到 24px**。改 `build/` 不等于用户看得到，中间隔着打包 + 安装。
    - **Windows 图标缓存会一直画老图**：装完新包、`ie4uinit.exe -show` 刷过、杀掉重开应用，任务栏仍是老 master 图；但同一个 exe 用 `Icon.ExtractAssociatedIcon` 提出来已经是对的扁平版。删 `%LOCALAPPDATA%/Microsoft/Windows/Explorer/iconcache*.db` + `%LOCALAPPDATA%/IconCache.db` 并重启 `explorer.exe` 之后，任务栏才真的换成新图（已截屏确认）。想绕开缓存做纯净验证，就把 exe 放到一个没出现过的新路径跑（`release/win-unpacked/` 就是）。
