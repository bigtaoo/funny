...
- **资源母题渲染下沉到 `@nw/shared`，两端不再各抄一份（2026-08-19）**：`drawResMotif` 的分级读数契约整个重构（起因与裁决见 [`design/product/slg-resource-art.md` §6](../../product/slg-resource-art.md)：旧的「按宽归一」惩罚横向生长，高等级画得越多屏上越小）。新契约把等级→尺寸/透明度整条**在打包期解完**，烘进 atlas 每个 frame 条目的 `nw` 字段，渲染层零等级逻辑。
