...
- **栅格化 + 发布到服务端模板**（2026-07-05，§8）：路径/城池两层编辑一直是"矢量覆盖层"（DESIGN.md §6.2），发布时才一次性"烘焙"成 tile 级 diff，不做双向同步——服务端模板不会反向生成回矢量图层，继续用已有的 Export/Import JSON 保存可编辑源数据（§6.2 的数据形态本身没变，只是新增了"怎么落地"这一步）。
  ...
  - **已知限制，非本次范围**：发布不做"从服务端拉取已有编辑回显"（§24 §5 提到的"编辑器需要模板列表接口"里的"列表"部分——`listMapTemplates` 已接进 `api.ts` 但 UI 还没有模板选择器/详情展示，当前只有手填 templateId 一条路）；没有 activate/delete 的 UI 入口（`api.ts` 已有对应方法，接线留后续，優先级低于发布本身）；单个 templateId 只能对应固定 `SLG_MAP_W`×`SLG_MAP_H`（同 §24 已知限制 1，`proceduralTile()` 尚未参数化尺寸）。
