# 决策日志（ADR）— ADR-001 ~ ADR-040（2026-06-21 ~ 2026-07-26）
...
- **决策**（用户拍板）：
  ...
  - **钱包/充值币按支付渠道隔离**：`SaveData.wallet.coins` 当前为全局单一钱包。上线微信/苹果/谷歌前必须改造——**站外渠道（如 Stripe）购买的虚拟货币不得在微信/苹果内消费**（违反各平台"不得消费站外购买虚拟货币"条款）。落地方式：充值币标记来源渠道，或钱包按平台隔离。**现在就要记入数据结构设计**，避免后期迁移。
    > **2026-07-27 落地**：选了"充值币标记来源渠道"这条（否决"钱包按平台隔离"=iOS/Android 各自独立部署——会牺牲本条已支持的跨端同账号游玩能力，且 `IOS_RELEASE.md §4.2` 既定计划本就是原生 IAP 复用同一套西方大区部署）。`wallets.coins` 保留为免费池（广告/胜场/兑换码等非充值来源，处处可花），新增 `wallets.recharged:{web?,apple?,google?}` 按渠道标记的充值池，花费时以请求平台（客户端新增 `X-NW-Platform` 头）门控只可花「免费池 + 匹配渠道桶」。详细机制见 [`game/COMMERCIAL_DESIGN.md §11`](game/COMMERCIAL_DESIGN_IAP.md#11-钱包按支付渠道隔离adr-0202026-07-27)。微信仍是完全独立部署/独立库，天然合规、不受影响。
  - **CrazyGames**：门户限制主要在前端行为（禁站外支付/外链跳转），账号层可与 web 共享，无需隔离。
- **未决/待查**：微信小游戏、CrazyGames、苹果/谷歌的开发者协议中"虚拟货币跨渠道流通"的具体条款原文（上线对应平台前逐条核实）。
- **影响**：[`product/deploy-cloudflare.md`](product/deploy-cloudflare.md) 新增「平台隔离边界」节为现行口径；钱包渠道隔离机制见 [`game/COMMERCIAL_DESIGN.md §11`](game/COMMERCIAL_DESIGN_IAP.md#11-钱包按支付渠道隔离adr-0202026-07-27)（已实现，不再是缺口）。
...
- **补丁（2026-08-27，客户端补上同一道闸）**：上一条补丁挂 `pre<script>` 钩子的是**六个服务端包**，`client/` 不在其中；CI 里 `server-checks` 有六道 codegen `:check`，`client-test` 里一道都没有。于是客户端 `rest:gen` 的产物只能靠人自觉重生成——这天发现 `client/src/net/openapi.ts` 比 `server/contracts/openapi.yml` 落后 18 行（异常上报端点的 `device`/`dpr`/`mem` + 逐事件 `orient`/`vp`/`sinceRot`；fragment 和 bundle 后的 `openapi.yml` 都有，就是没人重跑 codegen）。`gen-openapi.mjs` 头注释声称「漂移在 `tsc` 期就炸」，但那只对**被 import 的**端点成立：客户端没有任何地方引用 `operations['reportAnomaly']`（`net/anomaly/reporter.ts` 自己写了一套本地类型），所以 `tsc` 前后都是绿的，全仓库没有一道闸能发现。**已补**：`ci.yml` 的 `client-test` job 在 `client install` 之后加一步 `client codegen check`，`rest:gen` + `proto:gen` 后 `git diff --exit-code` 四个 openapi 产物 + `src/net/proto`（proto 这次是同步的，一并纳入防复发）。**仍未做**：`client/package.json` 的 `pre<script>` 自动前置钩子——服务端六个包有、客户端没有，这才是漂移的根因；补 CI 闸只是把发现时机挡在合并前，不是消除窗口。
...
> **部分被取代**：地图尺寸已由 [ADR-049](DECISIONS_ADR-041-069.md) 改为 1500×1500；单大区容量 500 已由 [ADR-092](DECISIONS_ADR-086-onward.md) 改为目标 3000（代码常量待改）。
