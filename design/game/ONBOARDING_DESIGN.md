...
- 高亮 `meteor_1` + 那团敌人（脉冲圆环锚定在关卡固定出生行 `TOP_SPAWN_ROW`，不是猜的棋盘百分比行——2026-08-09 修过一次环画在敌群上方 3 格空地上、陨石往那扔会打空的对齐 bug，`TutorialDirector.renderBeatPrompt`）："敌人挤成一团时，用**法术**一次清掉。法术不留实体、**很费墨**，是你的大招。"
