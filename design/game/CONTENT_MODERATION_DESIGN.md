...
- **O-CM6**（已知缺口，audit-followup-fixes-0730 复查发现，2026-07-30）：`ReportsMixin.resolveReport`（`admin/src/service/reports.ts`）的注释声称"处罚调用失败可重试"，但 `resolveReport()` 把"报告本身 resolve"和"施加处罚"绑在同一次调用里顺序执行——报告一旦被 socialsvc 的 CAS（`status:'open'`守卫）标记为 `upheld`，字面意义上的"重试 `resolveReport()`"会在从未到达 `applyPenalty` 之前就先撞上"报告已解决"的 404。回归测试 `admin/test/contentModerationBridge.e2e.test.ts` 的 `it.fails('resolveReport(upheld) can be retried after a penalty-call failure...')` 已捕获此缺口（标记为预期失败，等真正的"仅重试处罚"恢复路径落地后应转为通过）。修复方向：拆出一个独立的"仅重试处罚"端点/方法，不依赖报告状态仍为 `open`。
- **O-CM7**（已知缺口，audit-followup-fixes-0730 复查发现，2026-07-30）：`resolveReport()` 的 `accountId` 是调用方（ops UI）显式传入的参数，服务端从未回查报告自身的 `targetId` 做交叉校验——当前唯一调用方（`tools/ops/src/pages/reports.ts`）确实传的是 `r.targetId`，实际生产路径没有问题，但服务端本身对不匹配的 `accountId`没有任何防护，未来任何新调用点的疏漏都可能"处理 A 报告却处罚了 B"且不会报错。回归测试同文件的 `it.fails('resolveReport(upheld) rejects a caller-supplied accountId that does not match...')` 已捕获（预期失败，等 `resolveReport()` 改为从报告自身取 `targetId` 而非信任调用方参数后应转为通过）。
