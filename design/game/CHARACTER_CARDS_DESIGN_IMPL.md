...
- **皮肤页首屏空白修复 + 页签各自记滚动位置（2026-08-27）**：玩家反馈"皮肤页签刚打开时是空白的，也没有加载提示，切到别的页签再回来才显示"。根因是两层：①`core.scrollY` 由卡背包和衣柜**共用**，切页签时原样带过去（`showTab` 的注释还把"不动 scrollY"当有意为之），卡背包滚到下面时那个偏移远超衣柜 6 张卡的滚动范围；②`renderSkinsTab()` 是"先画再夹"——clamp 写在绘制循环**之后**，于是这一帧每张卡都被 `renderSkinCard` 的 draw-cull 判定为滚出屏幕、整页不画，被夹回合法值的 `scrollY` 只在**下一次** render 才生效（这就是"切走再切回就好了"的由来）。修复：① `CardSceneCore.setTab()` 取代所有裸 `core.tab = ...` 赋值（侧栏 rail 与 `CardScene.showTab` 共用），离开的页签把偏移寄存在 `scrollByTab`、进入的页签取回自己的——衣柜每次从顶部打开，回到卡背包还在原处；② `renderSkinsTab()` 改成"先量后画"（新 `measureSkinCard`/`cardMetrics` 只算高度不落笔），masonry 排完、clamp 完才开始绘制，这样任何来路不明的大 `scrollY`（含缩放窗口导致网格变矮）都不会再画出空页。回归见 `test/ui/cardSceneSkins.ui.ts`「wardrobe first paint after a scrolled roster」3 例（修复前空白/偏移丢失，修复后通过）。顺带把标题栏那两个方法（`renderHeader`/`headerCurrencySpec`）从 `core.ts` 拆到新的 `CardScene/header.ts` 纯函数模块，让 core 回到 500 行约定内（同 `EquipmentScene/headerRow.ts`、`FamilyScene/header.ts` 的既有拆法）。
