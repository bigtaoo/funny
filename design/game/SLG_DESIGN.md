...
- **基础五种（文具主题，对齐三战 粮/木/石/铁/铜）**：读时惰性产出 + 仓储上限，被攻破时按比例掠夺。
  ...
  - **贴纸/铜钱产出 = 地图铜矿（2026-07-07 拍板反转）**：回到三战规则——铜矿**上地图**、只在**等级 ≥6 的格子**生成（[`SGZ_LAND_REFERENCE.md`](SGZ_LAND_REFERENCE.md) §49「6 级地及以上特例」），占领后产铜钱，铜钱用于野外征兵等软操作。**推翻**旧口径「贴纸=非地块、家城自产、主动避开三战铜矿只在高级地」。家城 `stickerShop` 是否仍并存产出=经济侧待定；地图美术侧铜矿只有 l6–10 五级（无 l1–5），见 [`design/product/slg-resource-art.md`](../product/slg-resource-art.md) §5.7-sticker。
