...
- **等级分布曲线 ✅ 已实现（ADR-032 定型，ADR-033 重新校准，2026-07-05）**：`level = round((1-dr)^levelFalloffExp × (MAX-1) + 1 + noise)`。`dr` 的定义在 ADR-033 里发生了变化——**不再是**"距地图几何中心的距离比例"，而是"距该地块**自己最近首府**的距离 ÷ `GEN_MAX_CAP_DIST`"（后者=采样整张地图得到的"离最近首府最远的一点"到其首府的距离；见下方"10 首府"条目）。10 首府比 1 个几何中心覆盖面更大，原 ADR-032 的指数 1.1（针对旧的"距中心"口径，52.8% 达 5 级+）套到新口径实测暴涨到 ~81%，因此指数**重新校准到 1.9**，实测（500×500 网格采样 62,500 点）**51.2% 的格子达到 5 级以上**，与 500 人 × 人均上限 200 块 5 级+地的目标重新吻合（推导过程见 ADR-032/ADR-033）。worldsvc 27 个 e2e 测试文件（210 例）+ `@nw/shared` 单测（463 例）全绿——但注意：本轮为了验证这条改动，顺带发现并修复了一个此前从未被真正跑过的 worktree 依赖解析回归（ADR-033 附记），10 个测试文件的坐标假设也一并修了（`resourceDensity=1.0` 后 `'neutral'` 地块已绝迹、`(250,250)` 在 500×500 下变成地图正中心等）。
- **稀疏存储**：DB 只落被占领/被改动的格子；阻挡格、险地等静态地形由 `proceduralTile()` 程序化生成，不落库。
- **程序化分布**（`SLG_GEN` 旋钮）：阻挡地形（山脉/河流，`obstacleThreshold=0.88`）实测约 **2.7-2.9%**（不是曾经注释声称的 10-15%——`valueNoise` 的双线性插值+平滑步进压缩了噪声分布，这个校准误差在 ADR-032 之前就存在，本轮只改了准确的注释，没有重新调阻挡密度本身，那是独立的数值平衡任务）；关隘/桥数量 DRAFT 嵌于阻挡带之间；险地稀疏分布于高战略价值位置。ADR-033 后阻挡地形改为按"离最近首府的距离"（`obstacleMinDistRatio=0.15`）生成，天然集中在每个国家的边境，而不是围绕地图单一几何中心。
- **10 首府三层同心环布局 ✅ 已实现（ADR-033，2026-07-05）**：`CAPITAL_FRACTIONS`（`shared/slg.ts`）——外环 **6 外围国**（正六边形，半径 0.40，idx 0-5）+ 中环 **3 资源国**（正三角形，半径 0.20，与外环错开 30° 交错，idx 6-8）+ 中心 **1 霸业国**（地图正中心，idx 9 = `CENTER_CAPITAL_IDX`，行为不变）。共 10 国（不是三战常见的 9 国——用户明确要保留 10 这个数字对称性）。`NATION_KIND_BY_IDX` 常量标注每个 idx 的国家类型，供后续 UI/econ-sim 使用。Voronoi 分区（`nearestCapitalIdx`）逻辑不变，由新坐标派生。原"⚠️ 待定"项已拍板落地，详见 [DECISIONS.md ADR-033](../DECISIONS.md)。
