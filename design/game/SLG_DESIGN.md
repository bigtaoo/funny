...
- **大区**：技术上 = 一个赛季服实例 + 一张独立地图。单大区容量目标 **3000 活跃玩家**（地图 1500×1500，ADR-049 起从 500×500 放大；容量目标由 ADR-092 从 500 上调，**代码 `SLG_WORLD_CAPACITY_MAX` 仍为 500，待服务端扩容验证后再改**）；超出则开新大区；大区间完全隔离（经济/地图/战斗互不影响）。
...
- 单大区容量目标 **3000 活跃玩家**（ADR-092，2026-09-26；代码常量仍为 500，待服务端扩容验证后再改）；超出则并行开新大区。
...
- **客户端 UI 已接（2026-07-16）**：占地操作不再直接弹 `showDeployDialog(tx,ty,'occupy')`，而是走统一的选队流程。原 `showAttackTeamPicker` 泛化为 `showTeamPicker(tx,ty,kind)`（`kind:'attack'|'occupy'`），`doMarchTeam(tx,ty,teamId,kind)` 相应带上 `kind`，占地时 `startMarch(...,'occupy',1,teamId)`。选队占地下投入的兵力归属卡牌（`cardState.currentTroops`，战后按实际存活写回、可继续出战），不再永久变成地块驻军而从兵力池划走。**兼容早期玩家**：占地选队弹窗内保留一颗「散兵占领（兵力池）」按钮，回退到旧的 `showDeployDialog` flat 派兵路径（无卡牌队伍时仍可占地）。受影响文件：`WorldMapNet.ts`（`showTeamPicker`/`doMarchTeam` 泛化）、`WorldMapInput.ts`（占地操作改调选队）、i18n（`world.team.pickTitleOccupy`/`noTeamsOccupy`/`flatOccupy`）。
  ...
  - **选队行改成「名字 + 两枚字形数字」（2026-09-17，用户拍板）**：行文案原为 `Team 1 · Troops 2525 · Stamina 100`，在 210px 列宽、`FS.title` 下英文折成**三行**而按钮（84px）只装得下两行，第三行被切掉；而五行里 `Troops`/`Stamina` 两个词重复了五遍，有差别的恰恰是被切掉的数字。现在 label 只剩队伍名，兵力/体力走新的 `ModalButton.stats`（`[字形][数字]` chip 行，画在标签下方）：`unit` = 兵力（地块驻军行、派兵弹窗已经是这个含义）、`flame` = 体力。**体力这枚是当日为这一行新出的图**（批 13）：它先借了半天 `hourglassMd`，而沙漏在这一屏上已经是三层「时间」（加速 buff 剩余、占领/围攻倒计时、商城加速道具三档），词一删就让 `100` 读成时长——见 [`tab-icon-art-prompts-batch13.md`](../product/tab-icon-art-prompts-batch13.md)。有 stats 的弹窗按钮**加高**一个 chip 行，而不是把字缩回去。受影响文件：`WorldMapPanels/modalLine.ts`、`WorldMapPanels/core.ts`、`worldmap/net/march.ts`；细节见 [`UI_DESIGN_LOG_2026-09.md` §58](UI_DESIGN_LOG_2026-09.md)。
...
- **客户端预过滤（`WorldMapInput`，2026-07-14；2026-08-02 改为整按钮隐藏）**：中立格弹出菜单里的 **占领** 按钮在不连地时**直接不出现**（不支持的操作整个隐藏而非置灰显示，避免"点了才被服务端 400 拒"）——等距投影下一块隔行的空地视觉上就贴着主城，玩家极易误判。**仅对单人玩家（无 `familyId`）启用**：服务端连地算"自己家族 ∪ 同宗门兄弟家族"，但客户端只给自己家族的格子打 `mine`/`ally` 标志，兄弟家族领地无客户端标志，无法可靠判"不连地"——故有家族的玩家一律不预禁用（保留按钮、交服务端校验），杜绝误禁用合法扩张。占领/围攻仍以服务端校验为准，此处纯 UX。**扫荡不隐藏**（本就不须连地）。
  - **围攻/攻城的等效前置校验（`WorldMapNet.showTeamPicker` → `net/march.ts` + `logic/attackConnectivity.ts`，2026-08-29）**：敌方领地/州府/险地/野城的 Attack 按钮本身不隐藏（其目标锚点——敌方主城 3×3、城池整块地盘——客户端缓存里未必能可靠推出，见 `attackFootprintCells`：主城靠扫描点击格 3×3 邻域找锚点，野城靠 `cityNodes` 拿真实 footprint，两者都推不出时退化为只查点击的单格）；但打开选队面板前会先跑同一套 `isConnectedToSectTerritory` 判定，不连地时直接弹 `world.err.notConnected` 并跳过整个选队流程。**根治的具体 bug**：此前不连地时若恰好名下可用队伍为 0（比如 5 个队伍全部在外执行任务），会落到选队面板"可用队伍为空"的兜底文案——该文案不区分"确实没建队伍"与"队伍都在外，暂时没有可用的"，统一显示"尚无队伍，先去编辑布阵"，把真正的拦截原因（连地不满足）盖掉了（2026-08-29 用户报告）。占领/围攻仍以服务端校验为准，此处依旧纯 UX。
...
- **5.4.6 主城/城池围攻的 5 分钟也要看得见：不弹窗、队伍留在原地打（2026-09-12，用户反馈）**：用户报告打赢别的玩家（主城围攻）后弹了一个 "Siege won!" 的确认框，要求"不要弹确认框，一条 toast 提示即可。然后和攻打土地一样进入五分钟的计时，队伍播放攻击动画，计时结束后扣除目标基地的耐久"。核查后结论是：**5.4.5 第 3 点"主城无需改动"只在服务端成立，客户端那半从来没接上**——`SLG_SIEGE_DAMAGE_DELAY_MS` 的延迟扣血一直都在跑，但客户端对这 5 分钟一无所知，于是同一件事既弹了一个"已经赢了"的终局弹窗（announce 了一个还没发生的结果），又让队伍在这 5 分钟里凭空消失。拍板 + 落地（`围攻驻留`）：
  1. **不再弹 modal**：`applySiegeResult` 攻方胜利分支新增"这次胜利是不是开了一个待结算的围攻"的判断，为真则轻量 toast（`world.siegeWinBaseHold` / `world.siegeWinCityHold`，带倒计时与将扣的耐久值），与 5.4.4 的 `world.siegeWinHold` 同一种处理。**判据是服务器权威信号，不靠客户端猜**：`SiegeDamageDoc._id` 就是这场战报的 `siegeId`，客户端拿刚收到的 `SiegeResult.siegeId` 去新的 `siegeHolds` 列表里找同 id 的一条即可——和 5.4.4 用 `contestedByMe` 的形状一致。仍然弹 modal 的只剩真正的终局：结构磨血、PvE 据点/关口攻占，以及**所有失败**（回放校验正是失败时最有用）。
  2. **队伍留在目标上打**：`SiegeDamageDoc` 新增 `teamId`/`leaderUnitType`/`army` 三个快照字段（胜利那一刻 march 文档就被删了，不存就永久丢失），于是这 5 分钟里队伍有了确定的归属：
     - **地图上**：新增 `syncSiegeTokens`，和占领倒计时的 `syncOccupyTokens` 共用一份 `syncAttackingHoldTokens`（本次把后者抽出来复用，不是复制一份），整个倒计时期间循环播 `attacking` 动作。
     - **队伍面板/主城队伍行**：新增 `besieging` 状态（`world.team.besieging`，"围攻中 剩{time}"），与 `occupying` 同级（march > 占领/围攻 > 驻扎）。
     - **TEAM_BUSY 门禁**：`startMarchTeam` 原来只查 marches/occupations/stationed 三处，围攻驻留是**第四处**——此前这 5 分钟里队伍读作"在家闲置"，可以从它正在围攻的城下被重新派出去。补上这一查。
     - **结算时走回家**：`settleSiegeDamage`/`settleCityDamage` 的几条结束分支（城破、围攻作废；「伤害落地但墙还在」那条见 5.4.7）统一改走新的 `startSiegeReturnMarch`——带上队伍走，而不是只把 `attackerSurvivors` 当一笔无名兵力塞回池子。**顺带修掉一个既有的兵力重复计数**：卡牌队伍的幸存兵力战斗结束时已经写进 `cardState.currentTroops`，旧代码在结算时又把同一批人当 flat 兵力退回池子（`applyBaseSiege`/`applyCitySiege` 的"被击退"分支一直用 `hasCardArmy ? 0 : survivors` 规避，结算分支却漏了）。
  3. **契约**：新增 `GET /world/siegeholds` → `SiegeHoldView[]`（**仅自己的**；别人打第三方的待结算不是这条路该下发的情报，防守方需要的"有人在打我"另有 `under_attack` 推送），并随 `POST /world/enter` 一并下发（与 marches/occupations/stationed 同一批，首帧就得知道）。客户端在 `refreshMarches` 里把它作为第四条 order 分片一起刷。
  - **结束时要告知客户端**：客户端的 order 分片只靠 `march_update` 推送刷新（2026-09-02 那个坑），而 `siegeDamage` 现在也是一种会被删掉的 order 文档。结算绝大多数情况会开一条回家的 march（它自带推送），唯一例外是「没人活着可以走回去」那条分支——那里改成直接 `pushOrderEnded(kind: 'attack')`。`order-end-push-audit.test.ts` 的扫描正则同步扩到 `siegeDamage`，新增条目记录这个决定。
  - **没做的（明确记录，不是遗漏）**：围攻驻留**没有撤回入口**——占领倒计时有 `cancelOccupation`，围攻没有对应的。改动前队伍在这 5 分钟里是"自由"的（那是 bug：它其实正在城下），现在如实锁住，代价是这 5 分钟内无法撤回。时长有界且与占领倒计时相同，先这样；要不要补一个"撤围"需要单独拍板。
  - **影响**：`server/contracts/openapi-world.yml`（`SiegeHoldView` + `/world/siegeholds` + `enter.siegeHolds`）；`server/worldsvc/src/{worldTypes.ts,combat.ts,combatSiege.ts,service.ts,combatShared.ts}`、`db/combatDocs.ts`（`SiegeDamageDoc` 三个字段 + `{worldId,attackerId,teamId}` 索引）、`combatSiege/{damage.ts,cityDamage.ts,arrival/baseSiege.ts,arrival/citySiege.ts}`、`combatMarch/startMarchTeam.ts`、`httpApi/mapRoutes.ts`、`core.ts`/`core/push.ts`（`pushOrderEnded` 多一个 kind: 'attack'）；`client/src/scenes/worldmap/{WorldMapContext.ts,net/{loaders.ts,push.ts,march.ts},logic/teamStatus.ts,WorldMapRenderer/{tokens.ts,fog.ts,lifecycle.ts}}`、`client/src/scenes/CityScene/{core.ts,data.ts,helpers.ts,teamRow.ts}`、`client/src/net/WorldApiClient{.ts,/types.ts,/world.ts}`、`client/src/i18n/locales/{zh,en,de}.ts`；`client/test/ui/{cityTeamOrder,cityScene,worldMapSiegeResultToast,worldMapOccupyTeamPicker}.ui.ts`、`client/test/{worldMapPush,worldMapTeamStatus,cityQueuePoll}.test.ts`（新增围攻分支用例；另有 ~20 个测试桩只是跟着多了一条分片）、`server/worldsvc/test/{base-siege.e2e,order-end-push-audit,combatMarch-command-branch-gaps}.test.ts`。

- **5.4.7 围攻不再自动收兵：一轮打完自动打下一轮（2026-09-12，用户追加）**：5.4.6 落地后用户追一句「攻打完之后，不要自动回城了，自动开始打下一轮」。拍板：**耐久扣完、目标还没破时，部队留在原地开下一轮**，而不是走回家。
  - **实现上不是「再挂一个 5 分钟定时器」，而是重跑整个攻击流程**：`startNextSiegeRound` 往 `marches` 里插一条零距离、`arriveAt = now` 的 `kind: 'attack'` 行军，下一个 scheduler tick 的 `processDueArrivalSettlements` 把它当成普通到站处理，`applySiege` 从头跑一遍：守军波次战、受伤锁、战报、下一份 `SiegeDamageDoc`。**这才是「轮」的意义**：守方的队伍在两轮之间会愈（`SLG_TEAM_INJURY_MS` = 10 分钟，比一轮长）或被重新编队，因此**守方真的有机会打退某一轮**；攻方的幸存兵力也通过 `cardState.currentTroops` 自然带入下一轮。如果只是重新挂一份伤害，围攻就变成一个攻方零风险、守方插不上手的纯定时器。
  - **因此不需要新的停止条件**：被打退 / 攻方被打光 → `applyBaseSiege` 自己开回家的腿；城破 / 目标没了 / 进保护期 → 结算分支走 `startSiegeReturnMarch`；中途连地断了 → `applySiege` 把队伍就地停留。唯一不终止的情况是「回血比掉血快」——那是这个对局的数值事实（回城也一样打不下），不是死循环 bug。
  - **没兵了就不开下一轮**：`startNextSiegeRound` 先查还有没有部队（卡牌军团，或散兵幸存 > 0）；都没了就回到回家那条路（它自己的空分支会 `pushOrderEnded` 告知客户端），而不是派一支零兵力的合成军团去撞墙。
  - **插入的那条行军不推送**：它只活 ~2 秒（下个 tick 就被 claim+delete），推了只会让客户端每轮闪一下「行军中」。这一轮真打完之后，`applyBaseSiege` 自带的 `pushMarch`/`pushSiege` 会把新的 hold 推出去。
  - **副作用（已知并接受）**：每轮都是一场真战斗，所以攻方每 5 分钟吃一条 toast、守方每 5 分钟一条战报，`sieges` 也每轮多一条（有 TTL）。这是「持续围攻」应有的反馈密度，先不降频。
  - **❗ 未核算的数值影响（本轮只记录，没调）**：改动前，打下一份耐久必须**再派一次兵**——一次命令的体力（`SLG_TEAM_STAMINA_COST`）+ 一趟行军时间。现在一次派兵可以一直磨到底：插入的那条行军绕开了 `startMarch`（因此**不扣体力**），也没有路上的时间成本。于是「拿下一个主城」的真实代价从「⌈耐久/攻城值⌉ 次派兵」降成「1 次派兵 + 等」，**攻城吞吐量是数量级上的变化**，ECONOMY_VERIFICATION_LOG / BALANCE 里跟攻城节奏相关的结论没重算过。要不要给每轮收体力（体力见底就收兵）或制定其它上限，需要单独拍板。
  - **跟 5.4.6 那条「没有撤回入口」叠在一起要注意**：以前最多卡 5 分钟，现在是「打到分出结果为止」。守方打退就能结束它，但**攻方自己仍然中途叫不停**——要不要补一个「撤围」需要单独拍板。
  - **影响**：`server/worldsvc/src/combatShared.ts`（`startNextSiegeRound` + `SiegeHoldForce`）、`combatSiege/{damage.ts,cityDamage.ts}`；`server/worldsvc/test/base-siege.e2e.test.ts`（下一轮调度 + 守方打退结束循环两个用例）。

- **5.4.8 用户当场给 5.4.6 / 5.4.7 两个开放项的答案（2026-09-12）**：上面两节各留了一个「需要单独拍板」（每轮要不要收代价、围攻能不能叫停），用户一次性定了：
  1. **「每次开始战斗，都需要扣除体力的。确实不花时间因为队伍已经在城边了，主要还是靠体力控制平衡」** —— 于是 5.4.7 那条「未核算的数值影响」当场关闭：每开一轮扣 `SLG_TEAM_STAMINA_COST`（从**回复后**的数值上扣，跟 `startMarch` 同一套写法，同一种点号路径写入）；**体力不够开下一轮 = 围攻结束，队伍回家**。现行数值（MAX 100 / 每令 15 / 每分钟回 1）下，一轮 5 分钟净耗 10，满体力一次派兵大约磨 10 轮——要调快调慢改这三个常量即可，**不用再动围攻逻辑**。散兵（无队伍）没有体力预算，因此**不循环**，维持改动前的一击即返。
  2. **「站立中立地或者攻城，都是可以随时从右边的队伍信息面板里停止的。点击队伍所在的地块的弹窗里也可以停止」** —— 于是 5.4.6 那条「没有撤回入口」也关闭了：
     - **服务端**：新增 `POST /world/team/{teamId}/cancel-siege` → `cancelSiegeHold`（作废待结算的那一击，队伍走 `startSiegeReturnMarch` 回家）。**伤害是作废而不是提前结算也不按比例结算**：一轮是原子的（ADR-026 §4 的耐久要么在 `dueAt` 整笔落地、要么不落），所以中途叫停的代价就是这一轮已经付过的体力。**与 `cancelOccupation` 刻意不同**：后者会弃掋驻军（契约里就写着 garrison forfeited），因为那是真投入到占领上的兵力；围攻没有这种投入，队伍只是站在那里，卡牌兵力在 `cardState.currentTroops` 里原样带回。
     - **客户端两个入口**（用户点名的那两个）：右侧队伍面板的行内动作键（`world.stopHold`，围攻/占领两种状态共用一个按钮位，跟召回/加速返回/召回驻军同一个位置）；以及地块弹窗：敌方地块/主城菜单、野外城池面板、以及「我自己的占领倒计时」弹窗，各多一个 `world.actStopSiege`/`world.actStopOccupy`。
     - **地块→围攻的匹配不是纯坐标相等**（`logic/siegeHold.ts`）：主城的 hold 在**锚点格**上，而主城是不可分的 3×3（ADR-025）、九格开同一个菜单，只比对精确格的话九格里有八格看不到停止按钮；城池则是行军落到的那一格，精确匹配。
  - **影响**：`server/shared/src/api.ts`（`SIEGE_HOLD_NOT_FOUND` → 404）、`server/contracts/openapi-world.yml`（`/world/team/{teamId}/cancel-siege`）、`server/worldsvc/src/{combatShared.ts,combat.ts,combatSiege.ts,service.ts,combatSiege/damage.ts,httpApi/actionRoutes.ts}`；`client/src/scenes/worldmap/{logic/siegeHold.ts,logic/teamStatus.ts,WorldMapContext.ts,WorldMapInput.ts,WorldMapInput/cityPanel.ts,WorldMapNet.ts,net/march.ts,WorldMapPanels/hud.ts}`、`client/src/net/WorldApiClient{.ts,/world.ts}`、`client/src/i18n/locales/{zh,en,de}.ts`；`server/worldsvc/test/{base-siege.e2e,order-end-push-audit}.test.ts`、`client/test/worldMapTeamStatus.test.ts`。
