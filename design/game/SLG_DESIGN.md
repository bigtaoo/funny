...
- **S8-0 契约 + shared + worldsvc 骨架**：地图/格子/行军/家族 schema；`worldsvc` 第七 workspace；Redis 接入（gateway 横扩 + 调度）。
- **S8-1 地图与领地**：格子状态机、占领、资源惰性产出、驻军、保护罩。
- **S8-2 兵力与行军**：兵力上限/训练队列、行军调度、增援/协防。
- **S8-3 围攻战**：`'siege'` 模式 + 防守 config + 真人手操 / 自动扫荡 + 关键战斗复算 + 录像。
...
- **U1 拍卖行计价币种**（原 P8）：赛季资源 / 受限金币 / 专属拍卖币——必须防与天梯/付费体系串味，挂 `ECONOMY_BALANCE`。倾向「赛季资源 + 材料计价，禁用付费币/金币」。
- **U2 地图尺寸 + 程序化分布**（原 P3）：`mapW×mapH` 量级 + "什么位置产什么资源/什么等级"的规则，决定开荒节奏与交易生态。
- **U3 赛季周期 + 宗门间大比形态**：赛季天数；大比是榜单结算还是跨服锦标赛。
- **U4 单服健康容量 + 选人规则**：单宗门世界容纳量；"系统按规则选够人"的具体规则（按段位/活跃/进度/注册批次）。
- **U5 家族容量 + 角色权限粒度**：家族人数上限；leader/elder/member 各能做什么。
