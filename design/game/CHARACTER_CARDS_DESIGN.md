...
目标卡居中，5 个材料槽环绕排布（六芒星式布局）；下方候选列表按角色（`defId`）分组、展示与目标同阵营同等级的可用卡（数量徽标），点选填入下一个空槽，点已填槽位可撤回。5 槽全部填满后「融合」按钮才亮起，点击后播放融合动画（当前是程序内占位特效，后续替换为 `vfx-editor` 专门制作的资源：5 张材料卡自身头像错峰沿弧线汇聚到目标、每次到达触发一次涟漪，最后目标卡挤压回弹+金色爆闪+放射线，见下方 2026-08-01 条目），动画结束目标卡升级、材料卡销毁。

- **横屏布局（2026-07-20）**：竖屏维持单列（标题/提示 → 环形槽位 → 候选列表 → 按钮）不变；横屏改为左右分栏——左列放标题/提示/环形槽位，右列放候选列表 + 融合/取消按钮，充分利用横向空间。
- **自动重选目标（2026-07-20）**：若玩家点开的目标卡当前没有 5 张可用材料，面板会静默换成"材料充足、等级最高"的另一张可融合卡作为目标，并 toast 提示"当前没有足量的卡来升级该角色，已为您自动选择更合适的角色"。
- **低等级连续融合（2026-07-20，2026-07-22 修订，toast 精简 2026-07-25）**：目标卡为 1 级或 2 级时，融合成功后不关闭面板继续融合；3 级及以上仍在融合后关闭面板，需玩家手动再次点开。**优先保留刚升级的那张卡（2026-07-22）**：融合成功后卡实例 id 不变、等级 +1，面板**优先把这张刚升级的卡留作目标**继续升级——只要它仍在 1/2 级窗口内且手上还有 5 张可用材料（例如 Lv.1→Lv.2 后停在 Lv.2 上准备下一次融合）。只有当这张卡当前无法继续升级时，才退回到另一张材料充足的低等级卡（先 2 级后 1 级、优先同名副本）——**不再 toast 提示（2026-07-25，移除 `roster.fuseCantContinue`）**：这条分支在连续融合中几乎每次都会命中（低等级材料很快耗尽），原提示文案又长，导致几乎每次融合都弹一条长横幅；目标切换本身已经通过环形槽位中心头像变化直观呈现，无需额外文字重复。上面"自动重选目标"（`roster.fuseAutoRetarget`）保留 toast，因为它只在打开面板瞬间触发一次，不属于高频场景。
- **自动换卡优先同名（2026-07-20 追加）**：无论是上面两种自动换目标，都优先在"玩家刚才在合成的那张卡"的同一 `defId` 里挑下一张可融合的（例如连续合成时优先继续升级同名卡的其他副本），只有该 `defId` 已经没有可融合副本时才退回到任选一张材料充足的卡——避免连续合成时莫名其妙跳去升级另一个不相关的角色。
- **自动换目标排序扩展（2026-08-02，顺序调整 2026-08-10）**：`findAutoTarget`（`feed.ts`）挑选下一个目标时按以下顺序逐级比较，只有上一级打平才看下一级——① **已上阵优先**：候选卡当前在 SLG 队伍中出战的话优先作为下一个融合目标（帮玩家优先养强在场上的阵容），但已上阵的副本依旧**不能被选为融合材料**——这条互不冲突：目标可以是上阵卡，材料必须是未上阵卡；② 同 `defId`：与玩家刚才在合成的那张卡同角色的候选优先，让自动重选/连续融合在"已上阵"打平后仍尽量续着同一条角色线走；③ **同阵营**：与玩家刚才在合成的那张卡同阵营的候选优先，避免退回"任选材料充足、等级最高的卡"时跳到不相关阵营（例如合成盾兵时突然切到 Mara，玩家反馈体验很突兀）；④ 等级最高。这条排序同时适用于"自动重选目标"（面板打开瞬间材料不足时）和"低等级连续融合"退回下一张卡时的兜底选择。
  - **顺序调整原因（2026-08-10）**：旧顺序（同 `defId` 排在已上阵之前）有个反直觉后果——材料池是按阵营共享的（`fusionMaterialCandidates` 只认阵营+等级，不认角色），只要玩家从角色 A 起手连续融合，且 A 还有没升满、材料够的副本，①同 `defId` 就会一直赢过已上阵这一级，导致 auto-continue 把整个共享材料池持续喂给角色 A 的其他副本，已上阵的角色 B（哪怕材料充足）永远排不上号，直到 A 这条线耗尽——玩家反馈"喂了 50 多张 1 级卡，上阵的角色却还是 1 星"正是这个顺序导致的。现在把"已上阵"提到最高优先级，保证只要有已上阵、材料充足的候选，auto-continue 就优先喂它，不再被"当前正在融合哪个角色"这件事挡住。
...
- **阵营改用图腾（2026-07-18）**：角色名旁不再显示阵营**文字**（`涛方`/`Anna方`）——阵营以主角命名，文字紧挨角色名会被误读成"第二个名字"。改为**图腾图标**：涛方＝东方盘龙、Anna 方＝西方纹章鹰（呼应两方中/西名字的花名册）。图腾来自 `assets/factions/` 双帧图集（白线透明，运行时按 `FACTION_COLOR` tint；打包脚本 `art/ui/camps/pack_faction_atlas.js`），开机 L0 预载，程序化 glyph 为解码前兜底。因是精细线稿（≥48px 清晰、≤20px 发糊），**只有卡详情 modal 展示完整图腾**（`buildFactionIcon`，28px）；花名册网格 / 衣柜 / 喂卡等密集小行仍保留纯色**阵营点**（`FACTION_COLOR`，小尺寸靠颜色区分即可）。色值只在 `FACTION_COLOR` 一处定义，任何调用点不会漂移。
