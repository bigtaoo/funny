...
- **低等级连续融合（2026-07-20，2026-07-22 修订）**：目标卡为 1 级或 2 级时，融合成功后不关闭面板继续融合；3 级及以上仍在融合后关闭面板，需玩家手动再次点开。**优先保留刚升级的那张卡（2026-07-22）**：融合成功后卡实例 id 不变、等级 +1，面板**优先把这张刚升级的卡留作目标**继续升级——只要它仍在 1/2 级窗口内且手上还有 5 张可用材料（例如 Lv.1→Lv.2 后停在 Lv.2 上准备下一次融合）。只有当这张卡当前无法继续升级时，才退回到另一张材料充足的低等级卡（先 2 级后 1 级、优先同名副本），并 toast 提示"该角色暂时无法继续升级，已切换到低等级角色继续融合"（`roster.fuseCantContinue`）。
