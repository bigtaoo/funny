...
> 配套阅读：`EQUIPMENT_DESIGN.md`（装备词条/强化）、`SLG_DESIGN.md §16`（布阵/队伍）、`ECONOMY_NUMBERS.md §6`（数值权威）、`GACHA_DESIGN.md`（卡池）。
> 拍板日期：2026-07-01。
...
> 数值权威在 `ECONOMY_NUMBERS.md §6`；本文的数字仅为设计意图占位。
...
- 权重 `w_hp` / `w_atk` 按兵种配置（`CardDef.powerWeights`），见 `ECONOMY_NUMBERS §6`
...
> 公式：`cost(n→n+1) = 5^n`。9 级满级累计约 48.8 万 XP。数值最终以 `ECONOMY_NUMBERS §6` 为准。
...
- 章节通关奖励：同上，转材料
- 补偿价值按稀有度/等级计算，见 `ECONOMY_NUMBERS §6`
...
```
cityTroopCap = f(主城等级, 兵营科技等级)   // 基地可容纳的总兵力上限，见 ECONOMY_NUMBERS §6
...
> `baseSurvival`（HP 归零时的最低存活率）= DRAFT，见 `ECONOMY_NUMBERS §6`。
...
- 卡面显示红色受伤遮罩 + 倒计时
- 花 coin 立即恢复按钮（价格见 `ECONOMY_NUMBERS §6`）
- 受伤期间不可被拖入布阵编辑器
...
- [ ] 各兵种 `troopCapBase` / `troopCapGrowth` 数值（→ `ECONOMY_NUMBERS §6`）
- [ ] `baseSurvival` 存活率基准值（→ `ECONOMY_NUMBERS §6`）
- [ ] 技能成长表各卡具体数值（→ `CHARACTER_DESIGN` + `ECONOMY_NUMBERS §6`）
- [ ] 受伤立即恢复的 coin 价格（→ `ECONOMY_NUMBERS §6`）
- [ ] 卡进抽卡池的各稀有度/兵种权重（→ `GACHA_DESIGN`）
- [ ] 背包满时卡的补偿价值表（→ `ECONOMY_NUMBERS §6`）
- [ ] 第三阵营设计（→ 未来独立文档）
...
| **CC-2 metaserver CRUD** | ⬜ 待做 | 喂卡/穿卸/锁定端点 + v3→v4 迁移 |
