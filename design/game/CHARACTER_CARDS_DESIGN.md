...
- **自动换目标排序扩展（2026-08-02）**：`findAutoTarget`（`feed.ts`）挑选下一个目标时按以下顺序逐级比较，只有上一级打平才看下一级——① 同 `defId`（见上一条）；② **同阵营**：与玩家刚才在合成的那张卡同阵营的候选优先，避免退回"任选材料充足、等级最高的卡"时跳到不相关阵营（例如合成盾兵时突然切到 Mara，玩家反馈体验很突兀）；③ **已上阵优先**：候选卡当前在 SLG 队伍中出战的话优先作为下一个融合目标（帮玩家优先养强在场上的阵容），但已上阵的副本依旧**不能被选为融合材料**——这条互不冲突：目标可以是上阵卡，材料必须是未上阵卡；④ 等级最高。这条排序同时适用于"自动重选目标"（面板打开瞬间材料不足时）和"低等级连续融合"退回下一张卡时的兜底选择。
