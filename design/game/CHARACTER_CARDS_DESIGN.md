...
- **衣柜卡片网格改版（2026-07-15）**：原先每个角色一整行纵向堆叠、色卡固定 96px，整屏可用宽度基本没用上（右侧大片空白，且无滚动裁剪，皮肤多时会直接溢出屏幕）。改为每个角色一张卡片——左侧全高立绘（沿用 roster 网格的 0.72 立绘比例）、右侧姓名+阵营点 + 横向铺开的皮肤色卡（超出卡片宽度自动换行）；卡片本身按 2 列（自适应 `CARD_W_TARGET=620`）masonry 网格排列，每列独立追踪当前高度，卡片自身高度随皮肤数量变化。同时补上 `drawScrollIndicator`（此前完全没有滚动裁剪）。
