...
| **章节通关（专属奖励）** | 对应角色的 **2 级卡** × 1 | 陶侧奇数章、Anna 侧偶数章（见 `CHARACTER_DESIGN §5.1`）。⚠️ 这是**首通某章的专属奖励**，与「每关掉落」（1 级，见下）不同 |
