...
- **等级靠喂卡**：喂同阵营卡涨 XP，1 级卡固定 1 点/2 级+ 打 8 折，×5 指数曲线，9 级需 ~48 万 XP。
...
| CC3 | 喂卡升级，**同阵营**任意卡可喂；1 级卡固定 1 点不打折，2 级+ 打 8 折 | 阵营内流通；1 级作为基础货币保底不归零，高级卡回收仍有损耗防套娃 |
...
- **效率**：**1 级卡固定 1 点 XP，不打折**（1 级是喂卡经济的基础货币单位）；**2 级及以上卡打 8 折**（喂 2 级卡 = 5 × 0.8 = 4 XP；喂 3 级卡 = 30 × 0.8 = 24 XP）。旧版"统一 70%"存在缺陷：1 级卡 1×0.7=0.7 XP 逐材料取整后归零，导致喂新抽的 1 级卡完全不涨经验，故改为现规则。
- **限制**：
  - `locked = true` 的卡不可被喂出
  - 当前在队伍中上阵的卡不可被喂出
...
一张卡的"喂出价值"：1 级卡固定为 1（+ 当前 xp 字段值，不打折）；2 级及以上卡 = 该卡升级到当前等级的**累计 XP 成本 + 当前 xp 字段值**，再打 8 折。

```
feedXp(card) = card.level <= 1
  ? 1 + card.xp
  : floor((levelCumulativeXp[card.level] + card.xp) × 0.8)
receiverXp += feedXp(card)
```
...
- 抽卡时若 `cardInvCount >= 150`：**抽卡照常**，不阻塞本次抽卡流程
- **前 10 张溢出卡走邮件**（2026-07-18）：自英雄名录上次有空位起，累计溢出的前 `INV_FULL_MAIL_COUNT`（=10）张卡作为真实卡实例通过系统邮件发放（`kind:'card'` 附件，与拍卖行邮件认领同一机制，见 §13 装备篇同款设计），领取前需先腾出背包空间；持久计数器 `save.cardMailOverflowCount` 记录已用额度，一旦背包再次出现空位即重置为 0
...
- **喂卡选材料改为拖动条（2026-07-18）**：材料行原为 `[−] n/total [+]` 点按步进器——若玩家拥有几十张重复卡，逐张点加号太慢。改为水平拖动条：`n / total` 数字显示在条前，拖动手柄或直接点条上任意位置跳转到对应数量；行左侧点按仍保留原「循环 +1，超上限归 0」的快捷方式。实现上新增 `CardSceneBase.modalSliders`（独立于 `modalHits` 的实时拖拽轨迹，按下即生效，不像普通 modal 命中要等松手）。
