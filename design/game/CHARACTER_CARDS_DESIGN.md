...
**落地**：`client/src/scenes/CardScene/logic/feedPlan.ts`（纯规划/排序，取代 `feedAutoTarget.ts`；2026-08-27 随 ADR-071 4b 从 `CardScene/feedPlan.ts` 搬进受门禁的 `logic/` 纯目录）+ `feedGap.ts`（面包屑/缺口通知/批量按钮/推荐条/页脚）+ `feedRing.ts`（环形，从 `feed.ts` 拆出以守 500 行约定）+ `feed.ts`（状态机重写）+ `actions.ts`（`doPrepBatch`）+ `core.ts`（`doPrepBatch` 懒钩子；`sortCards`/`injuryCountdown` 外移到 `cardSort.ts`（同上，现为 `logic/cardSort.ts`）腾出行数）。测试：`test/feedPlan.test.ts`（23 例，取代 `feedAutoTarget.test.ts`）+ `test/ui/cardFusePanel.ui.ts`（37 例，两个旧 describe 整体替换）。08-18 当时**服务端零改动**——融合规则、`5^(L-1)` 曲线、`FUSION_MATERIAL_COUNT=5`、`MAX_CARD_LEVEL=9` 全部未动，改的只是交互契约。
