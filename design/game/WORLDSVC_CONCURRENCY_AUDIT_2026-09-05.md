...
## 九、p99 700–800ms 的真凶是 Atlas M0 的每秒限流，不是 worldsvc（2026-09-15，Grafana 巡检）

### 9.1 现象

worldsvc 心跳的路由表长期是这个形状（uptime 57 小时的一次采样，`count` 累计、分位数是最近一窗）：

| label | count | p50 | p90 | p99 | max |
|---|---|---|---|---|---|
| `GET /world/me` | 275,800 | 56.4 | 700.6 | **796.3** | 835.7 |
| `GET /world/map/sparse` | 275,800 | 19.3 | 576.8 | **751.5** | 773.4 |
| `POST /world/build/upgrade` | 1,379,153 | 14.1 | 34.0 | **725.2** | 752.3 |
| `sched:arrivalSettle` | 408,583 | 6.6 | 14.6 | **511.7** | 533.2 |
| `sched:siegeDamage` | 103,479 | 6.6 | 12.6 | 388.6 | 412.2 |
| `sched:occupations` | 103,479 | 6.6 | 10.6 | 285.8 | 325.8 |
| `sched:training` | 103,479 | 12.9 | 26.5 | 246.9 | 250.1 |
| `sched:arrivals` | 103,480 | 6.5 | 13.3 | 65.1 | 87.6 |
| `sched:builds` | 103,479 | 6.4 | 15.4 | 30.4 | 35.2 |

外加 `[world-scheduler] sched:arrivalSettle took 5xx ms, longer than its 500ms interval` 一天几百条。

### 9.2 三个「不是它」

1. **不是事件循环。** 同期 `loopLagMs` 24 小时 `max = 201.9`、p99 ≈ 30。§一～§七那条路已经走完了。
2. **不是缺索引、不是查询形状。** 2026-07-26 那批 `nextTrainingCompleteAt` / `nextBuildCompleteAt` 部分索引、
   `marches` 的 `arriveAt_1`/`nextStepAt_1` 全部在位。更狠的反证：**`marches`、`occupations`、`siegeDamage`
   这三个集合当时都是 0 文档**，而 `sched:occupations` / `sched:siegeDamage` 照样 p99 285 / 388ms。
3. **不是某一类操作。** 从容器内直接量，`db.command({ping:1})` —— 没有集合、没有索引、没有文档 ——
   **p50 6.2 / p90 10.9 / p99 589.9 / max 782.5**。裸 ping 的尾巴比任何一条业务路由都长。

### 9.3 决定性的那一次测量：停顿精确地每 1000ms 一次

3000 次连续 ping（79.3 次/秒），把超过 100ms 的样本按时间轴打出来：

```
1311@315  2311@346  3312@333  4316@417  5312@721  6313@350  7313@374  8313@407
9314@492  10316@578  11314@442  12313@393  13314@447  14317@438  15315@723 …
```

**相邻停顿间隔 999 / 1001 / 1000 / 1001 / 999 …** —— 不是抖动、不是邻居争抢，是一个**每秒补一次的令牌桶**。
停顿时长 315–811ms = 「等到下一秒边界」。

把探针速率降下来，同一现象按**操作数**而不是墙钟稀释：

| 探针速率 | >100ms 的比例 | 停顿间隔 |
|---|---|---|
| 79.3/s | 1.26% | 999–1006ms（每秒一次） |
| 16.7/s | 1.00% | 5088 / 5159 / 10233 / 5046ms |
| 4.8/s | 0.70% | 30 秒内只有 1 次 |

即：**大约每 80 次操作撞一次墙**。这正是 Atlas 共享层（M0/M2/M5）**100 ops/sec** 配额被打满后的延迟式限流。

### 9.4 所以那张表其实在量什么

每条 label 的 p99 ≈ **它串行发出的 Mongo 往返次数 × 同一条尾巴**。
`sched:builds` 一次查询 → p99 30ms；`getMe` 三次**串行**往返（`playerWorld.findOne` → 锚点 `tiles.findOne`
→ `tiles.countDocuments`）→ p99 555ms（容器内实测，与心跳里的 796 同量级，差额是 HTTP/鉴权/压缩）。
...
- **七个服务共用同一个 Atlas 集群**（`cluster0.rpr2tnw`：meta / commercial / world / social / auction /
  analytics / admin 七个 `*_MONGO_URI` 全指同一 host），所以 100 ops/sec 是**整个后端**共享的一个桶。
  升到专用层（M10）直接拿掉这个盖子，是唯一能一次性消掉整条尾巴的动作。
- **降总 ops/sec。** 当前最大的单一贡献者是 botsvc：`POST /world/build/upgrade` **6.7 次/秒**跑了 2.4 天，
  其中约 1/5 纯属 `BOTSVC_DESIGN.md` §8 那个攻城半径 bug 的 fall-through（已于 2026-09-15 修）。
  bot 的 tick 节奏本身也是个旋钮。

### 9.6 教训

**「p99 高」不等于「有慢查询」。** 这一节的全部诊断力来自一次 `ping`：一个不碰任何集合、不走任何索引、
...
## 十二、「第五支队伍要等好几秒」这一次不在服务端：客户端自己的限流阀（2026-09-26）

**用户报告**：五支队伍都在外面，只是攻占各自旁边的地块，第三支开始明显延迟，第五支要等好几秒才出发。
用户的直觉是「这不是升级数据库能解决的」。前半句对，但原因不是规模。

### 12.1 先排除服务端

线上 worldsvc 心跳，用户操作的那段时间（UTC 09:28–09:43）：`POST /world/march` p50 ≈ 35ms、p99 60–300ms；
选队伍弹窗要的列表接口最慢 ~100–200ms；`loopLagMs` p99 22ms。**服务端合计 < 0.3 秒**，解释不了「好几秒」。
所以这一次和 §4（同步 A\*）、§11（Atlas 往返乘法）都不是同一类问题。

### 12.2 真凶：ADR-058 的全局令牌桶 × 一次派兵的请求扇出

`client/src/net/rateGate.ts`：全客户端共用，突发 5、稳态 5 个/秒、**单一 FIFO**。一次派兵实际发出：

| 环节 | 请求 |
|---|---|
| 打开选队伍弹窗（`getTeams` + `refreshMarches` 四条分片） | 5 |
| `POST /world/march` | 1 |
| 服务端给自己推 `march_update` → 客户端无条件 `refreshMarches`（响应里其实已带回这条行军） | 4 |
| 打旁边格子，几秒就到达：`march_update` + `tile_update` | 5 |

每队 10–15 个请求 ≈ 2–3 秒的额度，排在同一个 FIFO 里，**到第五队出征请求要排在前四队引起的刷新后面**。
这是「一个人就能触发、跟在线人数无关」的形状——服务端再快、数据库再贵都消不掉。

### 12.3 三刀（全在客户端 + 一个只读聚合接口）

1. **限流阀分两条道**（`RateGate.acquire(lane)`）：`interactive` 永远先于 `background` 出队，同道内仍 FIFO，
   两道共用一个桶——**总速率上限不变**，ADR-058 防连点/防重试风暴的初衷不受影响。
   `WorldApiCore.req` 按方法选道：worldsvc 的写操作都是玩家自己下的命令，走 `interactive`；读都走 `background`。
2. **`refreshMarches` 合并**：每个场景同一时间最多一次在途读取；在途期间的调用只置脏，
   读完后**再跑一次**。每个调用方拿到的 promise 一定在「一次在它调用**之后才开始**的读取」完成后才 resolve
   ——选队伍弹窗 `await` 完就据 `ctx` 判定谁忙，靠的就是这条（有专门的测试钉住）。
3. **`GET /world/orders`**：marches + occupations + stationed + siegeHolds 一次返回。
   四条分片永远一起读（它们共同决定一支队伍忙不忙），拆成四个请求纯属浪费额度。
   选队伍弹窗、`march_update`、召回、主城界面都改走它。**四条单分片路由保留**，给已发出去的微信包用。
4. **自己派兵的回声不再刷新**：`marching` 状态的推送等 `OWN_ORDER_GRACE_MS`（300ms）再判——
   若 `ctx.marches` 里已有同 id、同 kind、同 status、同终点、同到达时间的记录（`doMarchTeam` 从响应里追加的），
   就是自己命令的回声，丢掉；否则（比如敌军进视野）照常刷新。等一下是因为推送比 HTTP 响应先到是常态之一。

**结果**：一次派兵 10–15 个请求 → 3–4 个。用真实 `RateGate` 类 + 两版请求模式跑假时钟模拟
（RTT 80ms、玩家每步 0.5s、邻格 6s 到达）：改前第五队等 **~2.6s**（弹窗 1.9s + 出征 0.7s），
改后五队都只等一个 RTT。真实客户端（本地栈）核过：弹窗现在只发 `/world/teams` + `/world/orders` 两个请求。

### 12.4 门禁与一个被变异抓出来的冗余

- 四处都做了变异验证：去掉分道 / 去掉合并 / 回声判定恒假 / `WorldApiCore` 不选道，各自当场变红。
...
- **端到端回归测试** `client/test/slgDispatchBurst.test.ts`（同日补）：以上单测各钉一处，这个钉「玩家的感受」。
  ...
  得有人主动改这张账，而不是悄悄变慢；
  ② 12 个后台读把令牌桶打空、读请求排队时点出征：出征必须在一个补给周期（200ms）内发出、插到积压前面。
  （最初用 12 个 `tile_update` 制造积压；§12.6 合并之后它们只产生 2 个读，所以改成直接发 12 个 `getMap`。）
  变异验证：去掉分道 → ② 变红（出征等 1600ms）；回声恒刷新 → ① 账不对；弹窗多一个读 → ① 账不对。
  写这个测试时顺带暴露了 `tile_update` 不合并的规模隐患，当天已处理，见 §12.6。

### 12.5 这件事和「几千人同图」的关系

**没有关系，也不该混为一谈。** 限流阀是每个客户端各自的，在线人数翻多少倍它都不变。
几千人同图的真问题仍然是服务端：单 Node 进程、到达结算必须串行（§6 那道跨玩家并发墙）、
9-05 压测 ~75 指令/秒。方向是**按地图区域分片**（不相邻区域可并行结算），专用数据库层只是配套。
这是上线前要单独立项的工程，等有真实负载数据再定。

> **2026-09-26 订正**：「几千人同图」是真需求——ADR-092 把单大区容量目标从 500 改为 **3000**（代码常量尚未改）。
> 按 3000 人/世界重新校验后的服务端计划见 §12.7。

### 12.6 `tile_update` 整视口重读合并（同日补）

**问题**：`applyTileUpdate`（`client/src/scenes/worldmap/net/push.ts`）每收到一个推送，就各自发一次整视口 `GET /world/map`。
视口里别人打仗时，每次结算命中、每次占领计时、每个邻居的变化都推一次 `tile_update`，
30 个推送就是 30 个读，按 5 次/秒的令牌桶要排 6 秒——和 §12 派兵变慢是同一种「把桶打空」。
人少时看不出来，几千人同图时会成为常态。

**修法**：和 `refreshMarches` 同一思路，按场景单飞 + 一次尾随：
- 同一时间最多一个视口重读在路上；期间到达的推送只打一个「还欠一次」标记。
- 在路上的那次落地后，若欠着，就再读一次——这次读一定发生在所有推送之后，所以最终画面不会漏。
- 任意多个推送 → 最多「在路上那一次 + 尾随一次」；推送持续不断时，每个往返一次读。
- 状态存在以 ctx 为键的 `WeakMap` 里：场景销毁即随之消失，重建的场景不会被旧场景的在途读挡住；销毁后不再补读。

**一个要小心的地方：D-CITY-8 基地受击红屏**。推送本身不带 hp，靠「推送前的缓存 hp」对比「重读后的 hp」判断基地掉血。
合并后，「推送前」必须在**推送到达时**记下，不能等尾随那次读开始时再取——
在路上那次读可能已经把这一击的新 hp 写进缓存，这时再取「之前」就等于把这一击对比没了。代码按推送时刻记录，只记第一次未消化的基地推送。

**门禁**：
- `client/test/worldMapPush.test.ts` 新增「applyTileUpdate coalescing」6 条：连发 20 个 → 恰好 2 次读；单个推送不多读；
  落地后新推送开新读；两个场景互不阻塞；在途读已带上这一击时红屏仍触发；销毁后不补读。
- `client/test/slgDispatchBurst.test.ts` 新增用例 ③：300ms 内 30 个 `tile_update` → 读发生在 t=0/80/160/240/320，
  共 5 次而非 30 次，其后点出征在点击当刻发出（桶没被打空）。
- 变异验证：换回旧实现 → 合并单测 2 条 + 端到端 ③ 变红；「之前 hp」改成尾随读开始时取 → 红屏那条变红；
  去掉尾随读（`while` 改 `if`）→ 合并单测 2 条 + ③ 变红。

### 12.7 按 3000 人/世界重新校验服务端计划（2026-09-26，接 ADR-092）

先前的判断默认「一个世界上限 500 人，几千人 = 几个世界」，于是「按世界切进程」就能横向扩。
容量目标改为 3000 之后，**这条推理只对「多个世界」成立，对「一个 3000 人的世界」不成立**：
同一世界里跨玩家的到达结算必须串行（§6.1），按世界切进程改变不了单个世界只有一条结算线。

**3000 人世界会先撞哪几面墙（按预计先后，全部未经实测）：**

| 墙 | 现状 | 为什么 3000 人会撞 |
|---|---|---|
| Atlas 共享层 100 ops/秒（§9） | 线上尾巴 09-22 起消失，靠的是削往返，不是扩容 | 200 bot ≈ 75 指令/秒，每条指令多次往返；3000 人必然超出配额。**专用层是硬前置，花钱，等用户拍板** |
| 计算池只有 1 个 worker | 线上 2 vCPU，`min(cpus-1, 8)` = 1 | 每条到达结算跑一场真实攻城战（§6.6），**全部世界共用这一个核**；单场战斗的耗时**从没量过** |
| 同一世界结算串行（§6.1 / §7） | 时间片摊平，`arrivals.deferred` 为积压探针 | 3000 人的到达密度下，结算吞吐 = 单线程 × 串行 I/O 往返；摊平不增加吞吐（§7.1） |
| 推送扇出（U11） | 客户端已合并 `tile_update`（§12.6） | 每个推送都要算一次 `visionObservers`（一次 Mongo 查询）；密集区观察者成倍增长 |
| 单进程 HTTP | p99 33–69ms | 相对前三者最后才会撞。**⚠️ 实测推翻（§12.8）：1000 人时主线程利用率已达 84%，和计算 worker、结算串行差不多同时撞** |

**修订后的阶段（顺序即优先级）：**

0. **先量**（不量则后面全是猜）：负载测试改为每轮开一个专用新世界（顺带关掉 §6.6b 的跟进项）；
   bot 数 500 → 1000 → 2000 → 3000 阶梯加压，记录每一档的 Mongo ops/秒（本地 Mongo 没有配额，要**数**出来再对照 100）、
   单场攻城战耗时与计算池排队、`arrivals.deferred` 是否持续增长、`loopLagMs`、每秒推送条数。
   另需一个**真实玩家节奏**的负载模型——现有 bot 是风暴式的，指令率远高于真人，只能当上界。
1. **多进程的正确性前提**（小改动，无论结果都要做）（已做，见 §12.10）：行军 ID 不再用进程内计数器；
   `advanceMarch` 每步加比较并交换认领（游标更新带 `stepIndex`）；socialsvc 缓存跨进程失效；指标带实例标签。
2. **同一世界内的并发结算（3000 人真正的主刀，原计划里没有）**（已做，见 §12.11；改为每趟静态分组，不是运行时加锁）：结算的瓶颈是串行的 I/O 往返，不是 CPU。
   若每条结算能在开始前**算出它要写的玩家/格子集合**（攻方、目标格归属方、格上驻军/行军的所有者、城防协防者……），
   就可以用**按键加锁**（按玩家 id / 格子 id 排序后获取，避免死锁）让写集合互不相交的结算并发执行，
   写集合算不准的（被动迁城、宗门连坐这类扇出）退回独占。这是在**一个进程内**解开 §6.1 那道墙，比跨进程的区域分片便宜一个量级。
   跨玩家写点清单已有（本次勘察：`encounter.ts`、`baseSiege.ts`、`cityDefenders.ts`、`landSiege.ts`、`damage.ts`、`occupation.ts`、`helpers.ts`）。
3. **按世界的租约 + HTTP 多副本**（已做，见 §12.12；默认关）：多个世界 / 高可用时需要；对单个 3000 人世界不解决吞吐，优先级排在 2 之后。
4. **跨进程的区域分片**：只有当 2 做完后单进程的 CPU 成为瓶颈时才立项。

**需要用户拍板的（都不是写代码能解决的）：** Atlas 升专用层；线上机器扩核（1 个计算 worker 撑不起 3000 人的攻城量）。
两者都等第 0 步的数字出来再定。

### 12.8 阶段 0 实测：3000 人阶梯（2026-09-26，接 §12.7）

**先说结论：墙的顺序和 §12.7 的预估不一样。** 数据库配额仍然是第一面墙；但「单进程 HTTP 最后才撞」是错的——
按真人节奏，**1000 人时 worldsvc 主线程已经接近打满**，和「只有 1 个计算 worker」「结算串行」差不多同时撞上。
3000 人在现有代码和线上配置下**跑不起来**，差的不是某一个参数，而是四面墙叠在一起。

#### 这次搭了什么（量具）

- **负载测试每轮开一个专用新世界**（`test/load/loadWorld.ts`）：用一个没人用的赛季号（≥900）开一个世界，于是所有 bot 都落进它、只有 bot 数一个变量；跑完关掉（只改 `status`，不删数据）。
  没用「当前赛季新开一个分片」，因为 `resolveShardForJoin` 按人口最少分配，新分片一旦比旧分片人多，bot 就开始分流。§6.6b 的跟进项就此关闭。
- **跑前等残留安静下来**：调度器扫描不按 worldId 过滤，上一轮留下的在途行军和 5 分钟占领会和这一轮抢同一个线程。「安静」= 10 秒内 Mongo ops/秒 ≤ 30；等到的那个数顺带就是**空闲基线**（本地约 5 ops/秒）。
- **两种负载模型**：`storm`（每 bot 每 2.5 秒一条，上界探针）和 `paced`（每 bot 平均 30 秒一条指令 + 随后一次 `GET /world/orders`、平均 20 秒一次 r=20 的 `GET /world/map`，指数分布间隔）。**`paced` 的节奏是假设，不是实测的玩家行为**，是旋钮不是结论。
- **worldsvc 新指标**（全部走 `/admin/world/metrics`，也进心跳）：
  - `mongo.ops` / `mongo.op.<命令>`：驱动 `monitorCommands` 数每条命令。本地 mongod 没有配额，必须**数**出来再和 Atlas M0 的 100 比。线上心跳里也有了，可以直接看线上真实 ops/秒。
  - `compute:<kind>:wait` / `:run`：计算池排队时间与在 worker 上的时间；`compute.timeout.<kind>` / `compute.workerDown`：挂起保护超时与 worker 重启——**这两件事以前完全静默**。
  - `push.sent` / `push.<kind>`、`vision:observers`（耗时 + 次数）。
  - `elu`：主线程事件循环利用率。**`loopLagMs` 看不见一个「满载但不卡」的线程**：线程 100% 忙于大量短回调时，每个定时器仍几乎准点（延迟 20–40ms），但每个请求的十几次串行 await 每一跳都在排队。1000 人那轮 lag max 38ms、派兵 p50 3 秒，就是这个样子。
  - 各延迟标签旁边的 `.n` 计数器：心跳 drain 之后空闲的标签会从快照里消失，第一轮阶梯因此报出「-398 场攻城」。
- **顺手修的真问题：新开的世界没有预热寻路索引**。第一条行军要在 worker 上现建约 3 秒，线上只有 1 个 worker，所有世界的攻城和寻路都排在它后面。50 bot 冒烟派兵 p99 2874ms → 修后 135ms。
  修法：`WorldServiceDeps.onWorldOpened`，`openSeason` 之后由 bootstrap 调 `warmWorld`。**线上满员自动开溢出分片（`resolveShardForJoin`）正是这条路径**，而启动时的预热只覆盖已存在的 `active` 世界。
- **单场攻城战耗时**（`test/load/siegeCost.bench.load.ts`，本机单核）：势均力敌的重型攻城 46–80ms，即每个 worker 每秒 14–20 场；空地占领的 NPC 战约 2–3ms。线上 vCPU 更慢，要再打折。

#### 读数（本机，22 核开发机；除注明外计算池 = 1 个 worker 以模拟线上 2 vCPU）

| 档位 | 指令/秒 | Mongo ops/秒 | 派兵延迟 | 结算 | 备注 |
|---|---|---|---|---|---|
| storm 50 | 17.7 | 272（2.7× M0） | p50 35ms / p99 135ms | 跟得上 | 冒烟，修完预热后 |
| paced 500 | 5.5 | 251（2.5×） | 客户端 p50 115ms / p99 38s；服务端 p99 ≈ 1s | 跟得上 | 客户端尾巴与服务端对不上，见下方注意事项 |
| paced 1000，1 worker | 9.8 | 465（4.7×） | 服务端 p50 13.8s，其中寻路排队 p50 12.9s | **8~10 条/秒封顶，积压增长** | 无超时、无 worker 重启 |
| paced 1000，8 worker | 12.8 | 560（5.6×） | 服务端 p50 3.1s，寻路排队 0 | 同上 | 主线程成了瓶颈 |
| paced 1000，8 worker + profile | 9.0 | 400（4.0×） | p50 3.2s | 同上 | **主线程利用率 84%** |
| paced 2000 / 3000 | 10.9 / 22.6 | 416 / 411 | 分钟级；大量 `fetch failed` | 同上 | 混进了上一轮的残留（等待超过 10 分钟仍未安静），只能当定性 |
| storm 500 / 1000 | 160 / 146 | 1746 / 1692（17×） | p99 7.7s / 30s | 每 35 秒只结算 16~37 条 | 上界探针 |

**四面墙，按这次实测的先后：**

1. **Atlas M0 配额**：真人节奏下，**500 人就是配额的 2.5 倍，1000 人 4~6 倍**；一套「指令 + 读订单 + 看地图」约 45 次 Mongo 操作，其中 `find` 占 85%+。
   削往返（§11）已经做过一轮，再削也削不出一个数量级。**专用层是硬前置**，不是优化能替代的。
2. **主线程 CPU**（原计划认为最后才撞）：1000 人时利用率 84%，即使计算池有 8 个 worker，派兵 p50 也有 3 秒。profile 中主线程忙碌时间的分布：
   `getMap` 约 26%（同步部分），其中 **`terrainView` 约 20%——每次读地图都对视口里每一格重新跑程序化地形生成**（r=20 就是 1681 格；worker 里早就有每世界一份的地形 `Uint8Array` 索引，主线程却没用）；
   BSON 反序列化与 Mongo 驱动约 15%；JWT 校验约 5%（`jsonwebtoken` 每次都重建 key 对象）；HTTP 读写约 10%。
3. **只有 1 个计算 worker**：1 worker 时寻路排队 p50 12.9 秒，8 worker 时为 0。寻路的执行时间是双峰的（p50 0.3ms，p90 85ms，p99 155ms），正是长尾那部分把唯一的 worker 占满。
   线上 2 vCPU → 1 个 worker，**1000 人左右就撞**。重型攻城战没进这次负载（bot 只占空地），它每场 46–80ms，会让这面墙来得更早。
4. **结算串行**：吞吐**封顶在 8~10 条/秒**，1000/2000/3000 人都一样，积压一路增长（上一轮的在途行军要十几分钟才结算完）。
   单条结算约 35ms 的串行 I/O，而 150ms/500ms 的时间片只给它三成的时间。阶段 2（同世界内按键加锁的并发结算）要解的就是这面墙。
   粗算：3000 人如果每人每分钟派一次兵，就是 50 条/秒，是现在的 5 倍。
5. **推送扇出**：目前不是墙。20~45 条/秒，`vision:observers` p99 < 15ms。

**注意事项（写在这里，不挑好看的一轮报）：**

- **2000/3000 两档混进了残留**：前一轮的在途行军过了 10 分钟仍未结算完，所以这两档只能说明「结算和主线程都饱和了」，绝对值不能用。要拿干净的 3000 人数字，得先解决结算吞吐，或者每轮之间重建本地库。
- **窗口会被拖长**：设定 180 秒，实际 400~460 秒——每个 bot 要等手上的请求返回才停。ops/秒按实际窗口算，所以偏低。
- **500 人档客户端 p99 38 秒，但服务端心跳里派兵 max 约 1.2 秒**：差值在处理器之外（HTTP 排队或压测进程自己）。延迟蓄水池只保留最近 512 个样本，也可能漏掉尖峰。以服务端数字为准。
- 本机单核比线上 vCPU 快，线上的每一面墙只会来得更早。

#### 据此调整的下一步（等用户拍板）

- **不花钱、改动小、收益最大的两刀**：① 主线程 `getMap` 改读每世界缓存的地形表（和 worker 用同一份 `buildMapTerrainIndex`，每世界约 2.25MB）；② JWT 校验复用 key 对象。做完重测 1000 人，看主线程利用率降多少。**（两刀已做，见 §12.9；实现与原提议不同）**
- 阶段 1（多进程正确性前提）不变；**阶段 2（同世界并发结算）的依据现在是实测数字**：8~10 条/秒的上限。
- 需要花钱的两件事，数字已经有了：**Atlas 专用层**（500 人就超配额 2.5 倍）；**线上加核**（1 个 worker 在 1000 人左右就排队到秒级；主线程也需要一整个核）。

### 12.9 两刀主线程优化（2026-09-26，接 §12.8「下一步」）

用户定调：**不追求现在就能扛 3000 人，线上资源等真不够了再加；先把代码层面准备好。**

1. **`getMap` 地形缓存**：新增 `shared/src/slg/proceduralTileCache.ts` 的 `cachedProceduralTile`，`MapService.terrainView` 的回退路径改走它。
   - 没有复用 worker 的 `buildMapTerrainIndex`（§12.8 原提议）：那份索引只存三种寻路分类，没有 level / resType / obstacleKind；
     而且整图构建约 3 秒，不能在请求线程上跑。
   - 改为**按 32×32 分块懒填充**：首次读到某块时才生成这 1024 格，之后就是数组下标读。
     每格存一个 Uint16，指向「不同地块」的驻留表（几百种组合），整张 1500×1500 全访问过也只有 4.5MB/世界；世界数按 LRU 限 4 个。
   - 返回的对象是共享且冻结的，调用方只能拷贝（`terrainView` 本来就是拷贝字段）。
   - 微基准（本机，r=20 视口 1681 格）：不缓存每次 1.82ms，缓存命中 0.12ms（约 15×）；首次读一片新区域约 5ms（最多 4 块）。
   - 有地图模板基线（§24）的格子本来就不走 `proceduralTile`，这条路径没改。
2. **JWT 复用密钥对象**：`shared/src/jwt.ts` 按 secret 缓存 `createSecretKey` 的结果。
   `jsonwebtoken` 拿到字符串 secret 时，每次校验都先 `createPublicKey`（对 HMAC 密钥必然抛异常）再回退 `createSecretKey`，
   profile 里这就是那约 5%。传 KeyObject 直接跳过转换；算法仍是 HS256，和字符串 secret 签的令牌双向互通（有测试钉住）。空 secret 仍按原样报错。

**验证**：shared 全量 1241 通过，worldsvc 全量 123 个文件 / 1468 条通过；`proceduralTileCache.test.ts` 在块边界、世界中心、城池占地、地图边缘逐格对照未缓存版本；
...
**阶段 1 不包含的**：按世界的调度租约（§12.7 阶段 3）。现在调度器仍然没有 `worldId` 过滤，两个进程会扫同一批到期行军；有了上面的认领，它们不会重复结算，但会互相抢。真起多副本前还要做阶段 3。（2026-09-26 同日补：阶段 3 已做，见 §12.12。）

### 12.11 阶段 2：同一世界内的并发结算（2026-09-26，接 §12.7 第 2 条）

**做法与 §12.7 的原提议不同**：原计划是运行时按键加锁（按玩家 id / 格子 id 排序后获取）。实际做的是**每趟静态分组**，不需要锁管理器：

1. 结算这一趟拿到到期列表后，一次性读快照：目标格 TileDoc（投影 `ownerId / contestedBy / baseAnchor`）、行军方的 `familyId`，
   以及步进行军将要进入的格子的占位 / 覆盖（各一次 HMGET）。**每趟固定多 2 次 Mongo + 2 次 Redis**，与到期条数无关；到期不足 2 条时整段跳过。
2. 为每条结算预测写集合（`combatMarch/settlePlan.ts` 的 `settlementKeys`）：
   - 账号：行军方、目标格归属方、目标格的待定占领者（`contestedBy`）——非遭遇战的结算只会写这几个人；
   - 格子：终点的 3×3（未命中时就地驻扎、驻军加覆盖、拆箭塔去覆盖，都落在这里）、步进行军走过的每一格、
     `move` 的出发格 3×3（回退驻扎在那里）、基地环格的锚点。
3. **和本趟其他任何一条都不共享键**的结算才并发（`concurrentEligible`），同时在飞上限 `NW_SLG_SETTLE_CONCURRENCY`（默认 8）。
   其余的按 `arriveAt` 原顺序**作为屏障**：先等在飞的全部结束，再单独跑，跑完才放后面的——对它们来说和以前的串行完全一样。

**必须独占的**（勘察结论，见本次勘察的写集合表）：
- 步进行军进入的格子上有**敌方**占位或**敌方**覆盖——这就是野外遭遇战，防守方（占位者、覆盖源所在的路外格）恰恰是事先锁不住的那部分。
  友方 / 已过 `leaveAt` 的占位不算，判定规则与 `walkClaimed` 开打前的敌友判断相同；
- 落在野城地面上的 `attack`——城防方是「城池 9×9 范围内任何驻军的主人」，在攻城内部才解析；
- 本文件不认识的行军种类——新种类要显式加进来，而不是默认并发。

**为什么预测可以信**：快照只可能被**同一趟里写了同一个键的另一条结算**弄旧，而那样的两条要么互相冲突（都走串行），
要么其中一条是屏障（它结束之前后面的都没开始）。屏障里不可预测的部分（遭遇战的防守方）只会**减少**单位和兵力，
不会给别人被判定为「可以通过」的格子新添占位或覆盖——驻扎和覆盖的写入都落在屏障自己被预测的格子上。

**时间片**：仍在两条结算之间检查；在飞的结算不会被放弃，这一趟会等它们结束。某条结算抛错时，等同批的都结束后再把错误抛出——与以前「一条抛错整趟失败」的约定一致。

**没有解决的（与以前相同，不是本阶段引入的）**：步进扫描、攻城伤害 tick、占领 tick 与 HTTP 指令本来就各自按定时器和结算这一趟并发运行，
以前串行时也是如此。本阶段只保证**结算与结算之间**安全。连通性检查（`isConnectedToSectTerritory`）读的是整个宗门的领地，
不在写集合里：同一趟里两个宗门成员的占领互相看不到对方的结果——这与一趟之内先后顺序本就任意（同一个 2 秒窗口）是同一程度的不确定，接受。

**可观测性**：新增计数器 `arrivals.concurrent`（本趟被放行并发的条数），与 `arrivals.settled` 相比即为并发比例。

**验证**：`settlePlan.test.ts`（10 条：键的组成、各种独占规则、敌友判定与遭遇战一致、冲突判定）；
`settle-concurrency.e2e.test.ts`（真实 Mongo 3 条：互不相交的 6 条回城确实同时在飞（最大重叠 > 1）且各结算一次；同一玩家的 3 条走串行且全部入账——两个带 `rev` 重试的退兵互相争抢时会用完重试次数而丢账，这正是要避免的；相邻格落点共享 3×3 走串行）。
`arrival-batch-roundtrips`、`arrival-settle-slice` 不变通过。
worldsvc 全量：126 个文件 / 1489 条全部通过（阶段 1 时挂过的两条 `city-siege.e2e` 这次也过了）。

**还没量的**：并发后结算吞吐实际提高多少、`arrivals.concurrent / arrivals.settled` 在真实负载下是多少、计算池（线上只有 1 个 worker）是否成为新的排队点——留给下一轮负载测试。

### 12.12 阶段 3：按世界的调度租约（2026-09-26，接 §12.7 第 3 条）

**解决什么**：阶段 1 之后，起第二个 worldsvc 进程已经**不会出错**（每条结算都是原子认领），但两个进程的调度器会扫同一批到期文档、互相抢认领——
读和计算都做两遍。本阶段把世界分给各个进程，每个进程只扫自己持有的世界。**它不增加单个世界的吞吐**（单个 3000 人世界仍然只在一个进程里调度），
解决的是「多个世界分摊到多个进程」和「一个进程挂了，别的进程接手它的世界」。

**做法**（`worldsvc/src/worldLease.ts`）：

| 项 | 做法 |
|---|---|
| 存放 | Mongo 集合 `schedulerLeases`（不走 Redis：Mongo 本来就是权威，且代码要求没有 Redis 也能跑）。每个世界一条 `{_id: 'w:<worldId>', holder, until}`，每个进程一条存活记录 `{_id: 'i:<instanceId>', until}` |
| 谁该持有 | **会合哈希**：对每个世界算 `sha1(instance \| world)`，存活进程里权重最高的那个持有。各进程看到同一份存活列表就算出同一个答案，不需要任何协调；进程加入 / 离开时只移动它赢得或持有的那些世界 |
| 节奏 | 每 5 秒一轮（`LEASE_RENEW_MS`），租约与存活记录 TTL 都是 20 秒（`LEASE_TTL_MS`，容忍三次续约失败）。一轮：续存活 → 读存活列表 / 非 closed 世界 / 全部租约 → 放掉不该持有的、续该持有的、抢该持有且已过期的 → **以集合里的实际结果为准**重建本地持有集 |
| 抢占 | `updateOne({_id, until ≤ now}, …, {upsert: true})`：别人的租约还有效时条件不匹配，upsert 插入同一 `_id` 撞唯一索引 → 视为被占，不重试 |
| 本地判断 | 本地持有集按「记录的到期时间 − 3 秒」过期，别的进程只能在到期时间过后才接手，中间留出间隙（容忍一点时钟偏差） |
| 崩溃接手 | 存活记录和租约同一个 TTL：崩溃进程的租约过期的同时它也从存活列表消失，下一个权重最高的进程接手。最坏延迟 ≈ TTL + 一轮 = 25 秒 |
| 正常退出 | `stop()` 删掉自己的全部租约和存活记录，接手方下一轮（≤ 5 秒）就拿到 |
| 过期存活记录 | 每次重启是新的实例 ID，崩溃进程的存活记录会越积越多；每轮顺手删掉过期超过一个 TTL 的 |
| 调度器 | `startScheduler({ worlds })`：每个 tick 读一次持有集，传给任务；持有集为空时直接跳过这个 tick（不是退回全量扫描）。七个定时任务都加了可选的 `worldIds` 参数，扫描里用 `worldScope(worldIds)` 限定；不传即不限定（管理工具、测试照旧） |

**租约不承诺独占**：刚放手的进程可能还在跑那个世界的上一趟，停顿过久的进程可能在过期后继续扫一会儿。这两种都是安全的，
因为租约只是**叠在已经正确的认领之上的效率手段**——所以没有、也不需要 fencing token。

**默认关**（`NW_SLG_WORLD_LEASE=1` 打开，所有副本都要开）：单进程时租约毫无用处，反而会让崩溃前留下的租约在重启后拖住那个世界最多一个 TTL。
部署门禁（`matchsvc/test/deploy-config.test.ts`）里已登记为「加第二个副本那天提成 compose 行」，见 `DEPLOY_TOPOLOGY.md`。
同一次补上了阶段 2 漏登记的 `NW_SLG_SETTLE_CONCURRENCY`（阶段 2 合并时只跑了 worldsvc 自己的测试，这条门禁在 matchsvc 里，当时漏跑）。

**HTTP 多副本**：本次顺带核对了 worldsvc 进程内的可变状态——socialsvc 成员缓存已在阶段 1 做了跨进程失效；
聊天消息 ID 早已带随机后缀；省会坐标缓存由世界种子决定、不会变；商店价格 / 词表缓存是从 admin 轮询的只读副本。
**没有发现其他会在多副本下出错的进程内状态**，HTTP 可以直接多副本，不需要粘性会话。

**已知的不均衡**：会合哈希是统计意义上的均衡，世界少时可能偏（例如 8 个世界、2 个进程，测试里实际分到 5/3，换一组 ID 甚至可能 8/0）。
各世界的负载本来就差别巨大（3000 人的世界和空世界），按个数均分也不等于按负载均分，所以不做精确均分。真到需要时再按负载加权。

**扫描索引**：`worldId $in` 在现有到期索引（`status+arriveAt`、`nextStepAt`、`dueAt` 等）上是残余过滤，
只会多读到别的世界的**到期**文档，数量有界。多副本真正上线、且世界数多时再考虑加 `worldId` 前缀的复合索引。

**验证**：`world-lease.e2e.test.ts`（真实 Mongo 6 条：单进程持有全部非 closed 世界；两个进程收敛到会合哈希的划分、途中从不同时持有同一世界；
正常退出后存活方下一轮全部接手；崩溃后到 TTL 之前不接手、之后全部接手，且过期存活记录被清理；停顿过期的持有者醒来后不会把已被接手的世界算作自己的；
结算扫描只处理给定的世界）；`scheduler.test.ts` 新增 2 条（有持有集时七个任务都带上它、为空时跳过；没有持有集时不限定）；`config.test.ts` 新增 1 条。
