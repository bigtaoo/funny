...
- **`ConsentDialog` 标题在德语下越过卡片两边（2026-09-21 修复）**：跟上一条同族、但在另一个节点上——标题用固定的 `snapFont(unit * 0.07)` 画，从来没跟卡片宽度核过。德语 `Datenschutz & Datennutzung` 在桌面横屏（1920×911 → 2276×1080 设计矩形）量到 858 设计 px，卡片 821，左右各溢出约 18px；中英文同字号放得下，`'accept-only'` 与 `'choice'` 两种模式表现一致（字号不看模式）。修法：`measure()` 里把标题收进正文一直用的那条文字栏（`cardW * 0.84`），超栏就用 `fitFont` 沿共享字号表降一档重铸——降字号而不是 `scale.set()`（同 `widgets/buttonLabel.ts`），也不用 `txtFit`（它兜底是省略号，同意门的标题不能被截）。宽度取 `Math.max(title.width, monospaceWidth(...))`，否则这个分支在 `test/ui` 的桩 `measureText` 下根本走不到。门禁：`consentDialogWrap.ui.ts` 新增 24 例（3 语言 × 4 视口 × 2 模式），两次变异验证各红 8 例。详见 [`UI_DESIGN_LOG_2026-09.md`](UI_DESIGN_LOG_2026-09.md) §59。
...
- **提交成功改为「关面板 + toast」（2026-09-17）**：用户看着截图问「发完直接关掉、再弹个窗说收到、点确定关掉，是不是更好？」。前半截照做了，后半截没做。原行为（2026-08-04 落地时的决定）是留在原地、清空输入框、在按钮上方写一行绿色「已收到，谢谢」，理由是「反馈没有一张工单的模型，可以连着发第二条」——但连发是极少数情况（重开面板只要一次点击），换来的却是常见情况下的困惑：面板原样杵着，唯一变化是一行小字，而玩家的视线在按钮上，很容易没看见就再点一次 Send，而且要离开还得再点一次 Close。改为走 `AppealDialog` 已有的那套：`submit()` 成功后 `closeInput()` + `cb.onClose()`，确认由 `app.ts` 的 `onSubmit` 包装在 `await core.submitFeedback()` 之后 `showToastMessage(t('feedback.sent'), 'success')` 发出（`appeal.submitted` 一模一样的写法）。**没做「感谢弹窗 + 确定按钮」**：它说的话和这行 toast 一字不差，却要多一层遮罩和一次点击；确认按钮的意义是「需要你做个决定」或「这条信息重要到必须挡住你」，「我们收到了」两样都不占。**失败路径故意不对称**：仍然不关面板、红字提示、保留已输入文字——正是在这种时候玩家最不能丢那段话；`submitting` 只在 catch 里复位（成功后留 true，挡住关闭前的第二次点击），与 `AppealDialog` 一致。文案顺手加重（原 `feedback.sent` = 「已收到，谢谢！」）改成「感谢反馈，我们已经收到，会尽量处理。」/ EN、DE 同步；长度和既有 toast（`reconnect.gone` 53 字符、德语 `common.syncFailed` 66 字符）同一量级，`GlobalToast` 本来就带 `wordWrap + breakWords` 宽度帽。测试：新增 `client/test/ui/feedbackSubmitOutcome.ui.ts`（12 例：横竖屏各测「提交 trim 后的文字 + onClose 恰好一次」「不留内联确认」「顺手关掉平台输入框」「失败留在原地且红字」「失败后重试不从空白开始」，另加在途二次点击只发一次、纯空白不发网络请求）——临时回退改动验证过其中 9 例先红；`client/test/feedbackSuccessToast.test.ts`（3 例，源码扫描 `app.ts`：toast 必须在 `await` 之后、键是 `feedback.sent`、与 appeal 那条保持同一写法），因为弹窗自己不知道 toast 的存在、而 `app.ts` 没有端到端单测入口（同 `appTickerDialogWiring.test.ts` 的理由）；`test/ui/dialogModalInputGate.ui.ts` 里两例「成功后面板仍开着」的断言按新行为改写（2026-08-10 那条「关闭键把大厅导航走了」的回归意义不变——现在成功路径根本没有那次 Close 点击）。验证：`npm run typecheck`、`npx vitest run`（3778 例）、`npm run test:ui`（2705 例）、`npm run build:web` 全过；真 Chrome（9190 → 本地 8088）实跑一遍：输入 → Send → 面板消失 → 大厅上浮出绿色 toast。
...
│   └────────┘    └────────┘    │
├──────────────────────────────┤
│  房间码:  [ 4 7 1 9 0 3 ]  📋复制 │  建房后显示（6 位纯数字）
│                                │
│  房主: 你          ✓ ready     │  双方槽位
│  对手: (等待加入…) ○            │
│                                │
...
└──────────────────────────────┘
加入态：输入 6 位房间码（画布数字键盘：横屏 2 行 × 5 列，竖屏 3 列拨号盘）→ 连接中 spinner → 进房
        几何见 [`MATCHSVC_DESIGN.md`](MATCHSVC_DESIGN.md) §「输码键盘的几何」
```
...
- i18n：`gacha.*`。
- **单抽/十连按钮的图标由扭蛋球改金币（2026-09-10）**：两个按钮此前都传 `'capsule'`（`INK_ICON_ART` 里是 `gacha_active.png` 的别名，一个圆 + 一条正中水平线的空扭蛋球）。问题有二：① 这一屏「这是扭蛋」已经说了三遍（顶栏标题、侧栏条目、banner 左上徽标），按钮上第四个扭蛋球零信息量，而且**两个按钮用同一张图**，对「单抽 vs 十连」也零信息量；② 按钮标签里的数字是**价格**，但按钮从不写货币单位，缺的恰好是这条信息。改传 `'coin'` 后与顶栏余额图标、以及商店/拍卖行「购买」按钮、体力补充按钮（这些早就用 `'coin'`）对齐——盲盒按钮是最后的例外。`coin` 是满色美术、三个 `RasterIconVariant` 槽指向同一文件，深色底不需要额外传 `variant`。真机核对（本地 8088 栈 + 真 Chrome 1568×773）：启用态深底上金币清晰，禁用态灰底同样可读。
  空扭蛋球本身在 28–32px 下读成「⊖」而不是扭蛋，是跟 2026-08-15 出图评审打回 `socialTabIcon` v1（「圆+十字」读成准星）同一类的语义误读；那轮只验了轮廓糊不糊，没验读不读得对，所以这张过了关。母版已于同日重画（v2：接缝上移 + 上盖影线 + 唇边），见 `design/product/tab-icon-art-prompts.md` 末节。
