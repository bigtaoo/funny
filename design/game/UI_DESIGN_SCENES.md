...
- **单抽/十连按钮的图标由扭蛋球改金币（2026-09-10）**：两个按钮此前都传 `'capsule'`（`INK_ICON_ART` 里是 `gacha_active.png` 的别名，一个圆 + 一条正中水平线的空扭蛋球）。问题有二：① 这一屏「这是扭蛋」已经说了三遍（顶栏标题、侧栏条目、banner 左上徽标），按钮上第四个扭蛋球零信息量，而且**两个按钮用同一张图**，对「单抽 vs 十连」也零信息量；② 按钮标签里的数字是**价格**，但按钮从不写货币单位，缺的恰好是这条信息。改传 `'coin'` 后与顶栏余额图标、以及商店/拍卖行「购买」按钮、体力补充按钮（这些早就用 `'coin'`）对齐——盲盒按钮是最后的例外。`coin` 是满色美术、三个 `RasterIconVariant` 槽指向同一文件，深色底不需要额外传 `variant`。真机核对（本地 8088 栈 + 真 Chrome 1568×773）：启用态深底上金币清晰，禁用态灰底同样可读。
  空扭蛋球本身在 28–32px 下读成「⊖」而不是扭蛋，是跟 2026-08-15 出图评审打回 `socialTabIcon` v1（「圆+十字」读成准星）同一类的语义误读；那轮只验了轮廓糊不糊，没验读不读得对，所以这张过了关。母版重画另计，见 `design/product/tab-icon-art-prompts.md`。
