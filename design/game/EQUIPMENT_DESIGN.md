...
- **稀有度映射媒材**（与皮肤共用同一套文具稀有度语言）：
  ...
  > 稀有度色改为灰→绿→蓝→紫的递进序（2026-07-06，见下方"稀有度配色+图标卡走查修复"）：此前 rare 用橙、epic 用紫，橙比紫更抓眼，读起来 rare 反而比 epic 更"高级"，与稀有度顺序相悖。具体取值见 `client/src/scenes/EquipmentScene/core.ts` 的 `RARITY_COLOR`，本文不写死。
