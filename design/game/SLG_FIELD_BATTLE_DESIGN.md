...
> 状态：**DRAFT / 待审** — 2026-07-24 起草（worktree `feat/field-battle`）。
...
- **P1 — 实时行军基础**：MarchDoc 持久化路径 + 逐格步进事件 + Redis `occ` 占格索引；行军行为对齐现状（无遭遇时结果不变）。含 e2e：路径/步进/到达一致性。
...
- **O1（需拍板）**：`§3.4` 步骤 3——行军**路过敌方领地**（带 `garrison` 数值的已占格，非驻扎队伍）是否触发 pass-through 战斗？
  - **提议默认：否**。tile `garrison` 是「该格被攻打时」的静态防守，若路过即战会让敌方领地不可穿行、行军寸步难行。拦截交给**驻扎队伍**（玩家主动布防）与**箭塔**。请确认。
- **O2（提议默认）**：箭塔/必拆除阻挡只能建在**己方/家族领地**格。是否允许建在中立地/邻敌卡点？默认否（防恶意卡点）。
- **O3（数值 DRAFT）**：箭塔伤害公式/耐久/成本、驻扎是否有额外维护成本、步进时长是否随疲劳变化——均进 `config.ts` 标 DRAFT，待经济核验。
- **O4（性能）**：逐格步进事件量（§3.3 注记）——上线前压测，必要时降级为粗粒度步进。
- **O5**：两队对穿（§3.4 边缘）不触发遭遇——接受为常态，还是需补「同边对穿也算相遇」？默认接受。
