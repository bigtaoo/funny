...
- [~] **S1-J 对等裁判反作弊**（2026-06-14 实现落地）：ranked 双方 hash 不一致时，gateway 挑一个**在线 `canJudge` 非参赛玩家**把 replay 发过去**无头复算**，按其结果裁决（不在 gameserver 自己复算）。**已拍板**：①单裁判（与裁判 hash 一致方可信）；②作弊方判负 + `matches.cheat` 标记，正常结算 ELO。**已落地**：
  ...
  - **客户端** [judgeRunner.ts](../../code/src/net/judgeRunner.ts)（`runJudge`：proto 帧→Replay→netplay 引擎跑到 GameOver→FNV-1a hash；`matchStateHash` 收此处单一来源，[app.ts](../../code/src/app.ts) 改 import）；[NetClient.ts](../../code/src/net/NetClient.ts) `sendClientCaps`/`sendJudgeVerdict`；[NetSession.ts](../../code/src/net/NetSession.ts) 连上报 caps（`hardwareConcurrency≥4`）+ 路由 `judge_request` 跑 runner 回 verdict。
