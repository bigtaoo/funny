...
- [ ] **S1-J 对等裁判反作弊**（2026-06-14 改方案，待实现）：ranked 双方 hash 不一致时，随机选一个**在线高配玩家**把 replay 发过去**无头复算**，按其结果裁决（不在 gameserver 自己复算）。**已拍板**：①单裁判（与裁判 hash 一致方可信）；②作弊方判负 + `matches` 打 cheat 标记，正常结算 ELO。**架构**：复用 gameserver ranked 已有的 `await report()` → meta `/internal/match/report` 内同步编排：mismatch 时调 gateway 新 `/gw/judge`（挑非参赛最高分在线 socket、push `judge_request`、等 `judge_verdict`，~8s 超时退回作废）→ 命中哪方 claim 哪方权威。proto 加 `ClientCaps`/`JudgeRequest`/`JudgeVerdict`（客户端 `proto:gen` 重生）；客户端连 gateway 发 `client_caps`（hardwareConcurrency/deviceMemory→score）+ 收 `judge_request` 跑无头引擎算同款 FNV-1a hash 回 `judge_verdict`；meta 加 `NW_GATEWAY_INTERNAL_URL`，`MatchDoc` 加 `cheat`。**依赖**：S1-3、C-1。**验收**：篡改输入被裁判检出判负。
