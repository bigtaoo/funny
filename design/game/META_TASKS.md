...
- [~] **S8-2 兵力与行军**（行军/调度/到点/撤军/推送 ✅ 2026-06-16；训练队列待）：worldsvc `startMarch`(occupy/reinforce，出征扣兵 + `marchDurationSec` 旅行耗时)/`recallMarch`(返程腿退兵)/`processDueArrivals`(Mongo `arriveAt` 扫描权威 + `findOneAndDelete` 原子认领 → 占领写 territory / 增援加 garrison / 退兵 / 到达被占退兵)；`scheduler.ts` setInterval(2s)+unref+重入守卫；**Redis ZSET 仅精确唤醒提示、处理不依赖**（缺 `NW_WORLD_REDIS_URL` 降级，需要时再起）；实时推送 `march_update`/`tile_update` 经 worldsvc gatewayClient → gateway `/gw/push` owner 定向（gateway proto/Gateway/matchsvcClient 加两分支编码）；`POST /world/march`+`/world/march/{id}/recall` 做实。worldsvc 22 e2e（+7 march）+ gateway 10 全绿。**待办**：兵力上限 / **训练队列**（sink + 加速变现，`/world/troops/*` 仍 stub）、attack/sweep 围攻 → S8-3、`under_attack` 预警、协防（家族 S8-4）、client REST codegen + UI。
