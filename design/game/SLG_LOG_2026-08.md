...
**遗留（本刀刻意不做，另开任务）**：地图编辑器发布（`rasterizeMapEdits`）会把城池 N×N 占地整块刷成 `familyKeep`，而客户端城池精灵层（`WorldMapRenderer/city.ts` 的 `cityNodes()`）只读程序化的 `allCityNodes()`——设计师新增/挪动的城不会跟着挪精灵，其占地格反而会再次逐格盖门楼、复现同样的砖墙。正确修法是让精灵层读发布后的城池节点，属另一件事。
