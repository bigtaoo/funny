...
- **U13 多步原子性 — 前半已查清并修复（2026-08-24），后半仍开放**：
  ...
  - **仍开放**：拍卖成交（扣卖方挂存 + 给买方 + 抽税）的跨文档幂等与回滚——这是唯一还剩的**跨集合**多步原子性问题，`playerWorld` 的单文档 CAS 手法帮不上忙。另有一处已定性但刻意未动：`distributeTroops` 给"出征中的卡"分兵会被战斗结算覆盖，关掉它需要先拍板玩法规则（是否允许给出征中的卡分兵），细节见 [`claudedocs/server.md`](../../claudedocs/server.md)。
