...
- **两条入口都是 overlay（2026-08-25，ADR-072）**：从卡背包进来的这两条（按卡编辑 / 独立背包）走 `showEquipment(cb, { overlay: true })` → `SceneManager.pushOverlay`，`CardScene` 留在 `current` 上活着，`back` 是 `views.hideOverlay()` 的一次 pop——卡背包的滚动位置与打开着的卡详情弹窗因此跨岔路留存。**第三条入口（战役地图顶栏「装备」按钮）仍是整场景 `goto`**：它底下没有卡背包可保，`back` 也确实要现建一个。侧栏「皮肤」页签同理由 pop + `CardRosterView.showTab('skins')` 顶替原先的整场景重建。
...
1. **客户端目录镜像（不 import `@nw/shared`）**〔**2026-09-09 起作废，见下方第三条**〕：客户端 webpack 只 alias `@nw/engine`（零依赖），`@nw/shared` 带 mongodb/jsonwebtoken 无法打包。故 UI 展示所需的「目录 EQUIPMENT_DEFS / 合成成本 craftCost / 强化成功率·成本 enhanceSuccessRate·enhanceCost / 分解返还 salvageRefund / 上限常量」在 `equipmentDefs.ts` 镜像一份（与 `SaveData.ts` 同纪律：**改字段三处同步** 本文件 ↔ `server/shared/src/equipment.ts`）。主词条放大系数 `ENHANCE_COEFF_PER_LEVEL` 直接从 `@nw/engine` 取，不重复。**服务器仍是唯一权威**：UI 据镜像**预览**成本/成功率，真实扣费/掷骰以回推 SaveData 为准。
    - **⚠️ 2026-09-09 补上漂移门禁**：`client/test/equipmentFormulaParity.test.ts`（10 例）逐值比对镜像与 `server/shared/src/equipment.ts`——目录（12 个 def 的 slot/rarity/media/craftCost 逐字段）、七个常量、以及 `enhanceSuccessRate`/`enhanceDemoteChance`/`enhanceCost`（等级 −2..11，含两侧越界钳位）/`salvageRefund`（逐 defId）/`isSalvageable`（4 稀有度 × 10 等级）/`reforgeCoinCost`/`getEquipDef`。此前**这一侧一个测试都没有**：实测镜像 63.1%，九个函数里**七个从没被任何套件调用过**，而服务端那侧测得很全（`server/shared/test/equipment.test.ts` + metaserver 两份 unit + e2e）——于是「改字段三处同步」这条纪律全靠人记。8 个变异逐个验红（含把 binding 起征点改回 +6、把 epic 不可分解规则删掉、只改服务端那侧）。**这条门禁不钉数值**：两侧一起改是设计师的正常操作，照旧绿灯，数值本身归服务端套件与 ECONOMY_NUMBERS 管。
      - **这份门禁同日晚些时候就被删掉了，因为它守的那份重复不在了**（下一条）——两侧现在是同一个模块，逐值比对变成自己跟自己比，留着只会让人以为还有两份东西要同步。
    - **✅ 2026-09-09（同日）把这份重复删掉了，见 ADR-087**：上面「`@nw/shared` 带 mongodb/jsonwebtoken 无法打包」这条理由对 **barrel** 成立，但对 `equipment.ts` **这一个文件不成立**——它 import 数为**零**（连 `import type` 都没有，`seededRng`/`hashSeed` 都是文件内私有函数），跟当年被深别名化的 `cards.ts` 情况一样。于是新增 `@nw/shared/equipment` 深别名（`webpack.config.js` + `tsconfig.json` + `tsconfig.fulllink.json` + 五份 vitest config，共 8 处），`client/src/game/meta/equipmentDefs.ts` 从 149 行手抄副本变成 **具名 re-export 门面**（12 个符号）+ 两个客户端独有函数（`craftableDefs` 排序、`affixKind` 前缀分桶）。**先量再动**（当时定的前提）：`build:web` 主 bundle **2 237 372 → 2 237 389 字节（+17 B），gzip 633 377 → 633 368（−9 B）**——webpack 的 `usedExports` 把服务端专用的那半（`rollEnhanceSuccess`/`rollCraftedAffixes`/`makeDropInstance`/`SUB_AFFIX_POOL`/`EQUIP_AUCTION_REF_PRICE_BY_RARITY` 等）整段摇掉了，所以门面**只按名字挑该给 UI 的东西**（掷骰函数故意不 re-export），包体代价实测为零。
