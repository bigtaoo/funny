...
interface LevelRewards {
  ...
  starThresholds: [number, number, number]; // 基地剩余 HP% → 2★/3★ 升级门槛；通关（基地未被打爆）保底 1★
}
```
...
| 系统 | 说明 | 数据落点 |
|---|---|---|
| 星级评分 | 按基地剩余 HP%（`starThresholds`）；**通关保底 1★**（基地未被打爆即 ≥1★，门槛只升级到 2★/3★，`starThresholds[0]` 实质成为「胜利」隐含线）。`computeStars` 单一来源，客户端 + 裁判复算同口径 | `campaignProgress.stars[levelId]` |
