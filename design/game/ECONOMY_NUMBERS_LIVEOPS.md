...
- **奖励表重规划（`BATTLEPASS_DEFS`，2026-07-03）**：两轨均**递进**——材料档随等级爬升 scrap→lead→binding 且数量渐增；整 5 级里程碑必发金币且逐段增大，收束于 Lv30 的赛季大奖。仅用 `coins` / `material`（claim 路径实发的两类）；不发卡/皮肤（卡会进 PvP 战力、皮肤属变现脊梁，均排除）。数字 `[可调]`：改 `@nw/shared/battlepass.ts` 的 `REWARD_ROWS` → 回写本节。（**2026-09-10 起不再需要同步客户端**：`client/src/game/balance/battlepassDefs.ts` 已从手抄副本改成 `@nw/shared/battlepass` 的具名 re-export 门面，ADR-087 那条路；两份表删除前逐字节比对过，产物零字节变化。）
