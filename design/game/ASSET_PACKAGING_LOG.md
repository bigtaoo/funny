...
- ~~使用标记只有「用过 / 没用过」，没有时效。一个玩了一次 SLG 就再不碰的账号会一直预热那 2.0 MB。加个「最近 N 天」窗口是显然的下一步，但在有数据说明这值得之前不做——现在这版已经把最坏情况（从没进过的人）解决了。~~ → **2026-09-10 补上，见 §14.6。**

### 14.6 使用标记加时效（2026-09-10）

上面那条遗留。原来存的是裸 `'1'`＝「这辈子开过」，而这个命题**永远不会再变回假**：教程里点过一次世界地图的账号，此后每个会话都在预热那 2.0 MB / ~13.7 MB 解码内存，一直到卸载为止。标记要回答的是「这人**现在**在玩什么」，而太旧的证据根本不是证据。

**做法**：`markFeatureUsed` 改写时间戳，`hasUsedFeature` 改读一个**滑动窗口**（`FEATURE_MARK_TTL_MS`，14 天），每次进场景都续期。

- **窗口从「最后一次」而不是「第一次」算**，所以周末才玩一次 SLG 的人永远不会掉出窗口——会掉出去的只有真的不玩了的人，而那正是这条要治的。
- **14 天是判断，不是测出来的数**：短了，隔周才玩一次 SLG 的人要在交替的会话里付冷闸门；长了，本节要治的「弃坑」情形要几个月才停止占内存。半个月比任何正常的间隔都长，又短到「教程里点过一次」在当月内就会失效。
- **过期不删键**：`hasUsedFeature` 是个读函数，下次进场景反正会覆写；为了二十来字节让读操作带上副作用不划算。这条**写进测试**了，免得哪天被顺手「优化」掉。

**三个不是时间戳的存储值，全部当「用过，但不知道何时」处理**——答 yes 并**重新盖上今天的戳**：

| 值 | 来源 | 不这么处理会怎样 |
|---|---|---|
| `'1'` | 2026-08-25→09-10 之间发出去的所有客户端 | `Number('1')`＝1＝1970，纯按年龄判会在升级当天把**整个存量玩家**的标记一次性作废 |
| 未来的时间戳 | 设备时钟曾经跑快、后来被纠正 | 那个标记会在错误日期之后**再新鲜半个月** |
| 解析不出来的字符串 | 别的写入方 / 存储损坏 | 同上，且这条本来就该走宽容那侧 |

宽容是这里的免费方向：标记从来只是提示，猜错的代价固定是**一道冷场景闸门**——跟从没开过那个界面的玩家一直在付的价钱一模一样。最坏情况就是多预热一个窗口。

**门禁**（两处都补，各守一层）：

- `test/idlePrefetch.test.ts`（链路层，+4 例）：过窗口就不再预热、**贴着窗口边界仍然预热**（少了这条，把窗口缩成 0 也能过第一条，而窗口为 0 就是回到 §14 之前的永远冷闸门）、连续六周每周一访不掉出窗口、`'1'` 迁移**只给一个窗口**而不是重造「永远」。
- `test/prefetchPolicy.test.ts`（存储值层，+7 例）：写的是时间戳不是旗标、窗口两侧、过期不删键、上表三种非时间戳值、以及**读路径上的重新盖戳撞上会抛的 storage 也不能把调用方带下水**（`markFeatureUsed` 那条 catch 的读侧版本）。链路层只可能产出本版自己写的值，上表那三种只有单元层够得着。

四处变异全部反向验证过：写回 `'1'` → 5 红；删掉迁移分支 → 5 红；删掉未来时间戳的钳制 → 1 红；`<=` 改 `<` → 边界那条红。

**两条边界一个字没动**（这次改动特意不碰）：`cellular` 仍然**不**跳过预取（§14.2 的 ⚠），编码仍然封死（§13.5）。

**遗留**：14 天没有数据支撑。真要调，得先有「玩家两次进 SLG 的间隔分布」这类埋点，而不是再拍一次脑袋。
