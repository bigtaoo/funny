...
| 叠上两条通道（装备 +60% / 宗门 +32%，**均已实装**，见 §13-SLG-CITYSIEGE.6） | 631 | 45.4 | 634 | 8,680 | **回复 1.56×** ✅ | 20,093 | **耐久 1.43×** ✅ |

两条判据都要过：持续率 < 回复（否则一个人磨几天也能拿下）、倾池爆发 < 耐久（否则站着的兵池一坐就能打下来，回复来不及作用）。**后者更紧**，也是设计文档从未写过的那条。

> **2026-09-02 复核（ADR-079 训练队列改并行）**：`trainPerHour` 从第一天起就是 `perSlot × trainQueueMaxFor(b)`——**这张表当年就是拿并行口径标定的**，而 worldsvc 那侧一直是串行链式，即服务端比门禁假设的更慢。ADR-079 让服务端追上模型，不是放宽平衡。重跑 `npm run city-siege`：上表六列**逐字未变**（631 / 45.4 / 634 / 8,680 / 1.56× / 20,093 / 1.43×），五个门禁全 PASS；`npm run stronghold-combat` 两门同样 PASS。金币加速的定价刻意未动（一枚币仍只买一个槽的 60 秒），所以本节记的那条「金币换兵原则上无上限」残余风险的量级也未变。
...
### 13-SLG-CITYSIEGE.6 两条代码事实（设计文档假设相反，本轮实测发现）——**两条现均已解决**

1. ~~**`teamSiegeValue()` 只读卡的 `defId` + `level`，装备完全不进**~~ —— **已接线（2026-08-29，SLG_CITY_SIEGE_DESIGN §12.7）**：`teamSiegeValue`/`cardSiegeValue`（`server/shared/src/cards.ts`）新增可选 `equipmentInv` 参数，读每张卡装备的 `m_siege`/`s_siege` 攻城值词条，求和后夹到 `SIEGE_GEAR_PCT_CAP`（镜像引擎的 `EFFECT_CAPS.siegePct_fp`=+60%）——与 `applyEquipment` 作用在引擎蓝图 `siegeValue_fp` 上的算法同款系数与上限，但走的是独立的、不依赖引擎的实现（`@nw/shared` 不能 import `@nw/engine`）。worldsvc 两处调用点已接上攻击方真实的 `equipmentInv`。不是 bug 修：门禁 ③ 本来就是**带着**这条假想倍率测的（余量 1.56×/1.43×），接线后重跑仍 PASS。实测代价：把 §6.3「需要几十人」那张裸装表下移 **~37%**（`npm run --workspace @nw/econ-sim city-siege` 门禁 ⑤ 的 informational 行）。
2. ~~**§8.3 的宗门城池加成（满图 ×1.32）尚未实现**~~ —— **已于 ADR-074 P3 接线（2026-08-27）**：`applyCitySiege` 把 `teamSiegeValue` 乘上 `(1 + sectPayoff.siegeBonus)`，独立通道，不并入被 cap 的装备累加器。当初就已计入门禁 ③ 的最坏情况，所以接线后重跑仍 PASS，无需重新标定。
...
**曾经开放、现已解决（与本条方向相反，独立拍板）**：§13-SLG-CITYSIEGE.6 第 1 条（装备不进耐久伤害）已于 2026-08-29 接线。接上它**没有**破门禁 ③——那条门禁本来就是带着装备通道量测的——实测代价是把每人的耐久伤害抬最多 1.6×，也就是把 §6.2/§6.3 的「需要几十人」整张裸装表往下拉 **~37%**，而那张表是 §8.2「城池收益是宗门招人筹码」赖以成立的数字。
