...
**rockslide 的一处既有分歧（本期未拍板）**：`castRockslide` 实际打**列里所有单位、不分敌我**，而 §4.9.2 的正文写的是「对目标列所有**敌方**单位」。`spell-system.test.ts` 有断言把「不分敌我」钉死了，所以本期没动玩家那张卡的语义，只给方法加了 `enemyOnly` 参数、由 lanePunish 传 `true`（敌方施法时若砸死自己的波次兵，惩罚就不成立了）。玩家侧到底按哪份为准，留给后续拍板。
...
**章节页自下而上（2026-09-14 拍板）**：`y` 仍是屏幕向下的归一化坐标（`0` = 内容区顶边），但**数据把 1 号关摆在底部、10 号关摆在顶部**（`lv1.y ≈ 0.85`，`lv10.y ≈ 0.16`），START 在页脚、BOSS 在页顶。理由是竖屏＝手机＝拇指，前几关是新手点得最多的，不该落在离手最远的页顶；顺带对上「往上爬」的通用心智。**横竖屏同一套数据，不做分向翻转**——两套镜像布局会让玩家转一次屏地图就上下颠倒，认知成本比省下的手感贵。

自下而上带来两条**排版硬约束**（改坐标时必须守住，两条都是 2026-09-14 实测撞出来的）：

1. **节点的星级永远画在节点下方**，所以 START 旗必须放在 1 号关**上方**（`start.y = lv1.y - 0.04`，与翻转前的相对关系一致）。摆在下方会被星星压住标签——竖屏 1080×1920 下星条宽 130px、旗子含标签约 150px，1 号关在 `x≈0.16` 时左边根本排不开。
2. **页顶现在是「章节终点」那一带**，通关印章（`drawClearStamp`）与 BOSS 旗争位。印章因此改为**躲开 BOSS 那一侧的上角**：`boss.x ≥ 0.5` 走左上（`0.30w`），否则走右上（`w - 0.30w`）。旗面朝右，所以正中间的 BOSS（ch2/ch6，`x = 0.50`）也算「右侧」，把印章推去左上。
...
9. ✅ **章节页翻成自下而上**（2026-09-14）：六张 `maps/chN.json` 的所有 `y` 取 `1 - y`（节点 + decor 一起翻），1 号关落到页脚、10 号关与 BOSS 升到页顶，START 旗随之落到页脚；横竖屏共用这一套数据。翻完实测撞出两处压字，一并收口（约束正文见 §12.3 末尾）：START 旗改摆 1 号关**上方**（避开节点星条），通关印章改**躲开 BOSS 的那个上角**（`CampaignMapScene.buildChapter` 按 `boss.x` 选 `0.30w` / `w - 0.30w`）。验证：真 Chrome 竖屏把六章全部按「全 60 关三星」种到 localStorage 逐章看过（印章 + 星条 + 旗子同时在场的最坏情况），横屏抽查 ch5/ch6；`tsc --noEmit` + 生产 webpack + 全量单测/UI 测试全绿。

   **配套门禁**（两处压字都是肉眼发现的，事后补的测试）：`test/campaignMapBottomUp.test.ts` 用**纯数据**钉作者约定——自下而上（每隔 5 关至少升 0.10，容得下 ch3/ch4 那个 0.12 的回折）、1 号关在下三分之一 / 10 号关在上三分之一、START 必须在 1 号关**上方**、BOSS 在 10 号关上方，外加 `clearStampX` 对六张图在横竖屏两种设计尺寸下都与 BOSS 旗分处两半且留够 `0.20w`（印章半宽实测约 `0.15w`，德语最长约 `0.17w`）。`test/ui/campaignMapClearStamp.ui.ts` 只补一条接线：场景确实按 `clearStampX` 落章，而不是又写死一个角。四条变异（起点旗放回下方 / 印章写死右上 / 整章翻回上下颠倒 / 场景绕过 helper）逐一验证过会红。
   **这两层测不了什么**：headless 那层 `measureText` 是「字符数 × 7px、与字号无关」的桩，所以**任何依赖文字宽度的压字判断在 UI 测试里都测不出来**——那是 `npm run test:portrait` 的活，而它的 campaignMap 站至今只见过新号（没星条、没印章），最坏情况从未进过它的视野。
