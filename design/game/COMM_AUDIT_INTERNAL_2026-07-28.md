...
| 7 | worldsvc getProfile N+1 | 新增 `WorldMetaClient.batchProfiles`（复用 meta 已有的 `/internal/account/batch-profiles`）；`core/map.ts` 视口 owner 解析改一次批量；`pushTileToObservers` 提前解析一次 owner profile，`pushTile` 加可选第三参跳过重复 fetch | 其余 16 处单目标 `pushTile` 调用点不受影响（不传第三参，行为不变） |
