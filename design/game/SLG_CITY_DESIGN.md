...
**② 队伍带头部**：竖屏下头部行给 `TEAM_ROW_LABEL_H + 18`，按钮高 34、宽 268（横屏仍是 26/200）。宽度不是审美——「Fill All Teams」14 个字符在 200 设计 px 里要被 `drawButtonLabel` 整组缩到 0.94，正好把字号缩回可读下限之下（见 `UI_DESIGN_LOG_2026-09.md` §49.1）。
