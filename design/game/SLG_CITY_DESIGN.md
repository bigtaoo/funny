...
验证：`tsc --noEmit` 全绿；`test:ui` 231 文件 / 2183 例全绿；`build:web` 构建通过；**真机截图核对**——用 `web-e2e` 靶子（`window.__nwE2E.views.showCity()` + 桩 `WorldApiClient`，无需后端栈）驱动真 PixiJS 渲染，按用户截图里的等级/资源复现同一座城，改前/改后逐格对比确认：纸盘与石墨坊从「几乎看不见」变成一眼可辨，第二排七张无 chip 的卡逐像素未变。
