...
### 8.15 竖屏版面：网格填满中段 + 队伍行头部留白（2026-09-11）

竖屏几何巡检（`npm run test:portrait`）**不报**这屏，但它长得不对——报告截图上，内容全挤在上半屏，中段一大片空白，「填满所有队伍」按钮的下边缘正好贴在 Team 5 卡片的上边缘上。两个都不是重叠，所以自动巡检看不见；先定「竖屏主城该长什么样」，再改。

**竖屏版面（1080 × designHeight，390×844 手机实测 designHeight = 2337）**：

| 带 | 高度来源 | 说明 |
|---|---|---|
| 标题栏 | `sceneHeaderHeight(h)` | 返回 + 标题 + 右侧耐久读数（§8.5），不变 |
| 资源条 | 固定 108 | 5 格，不变 |
| 建造队列 | 51 / 72 | 不变 |
| **建筑网格** | **中段剩余全部** | 见下 |
| **队伍带头部** | **26 + 18** | 分区标签 + 「填满所有队伍」按钮，**下方留 18 设计 px 的空隙** |
| 队伍卡行 | 128 | 5 张并排，贴底，不变 |

**① 网格填满中段**（`client/src/scenes/CityScene/gridMetrics.ts`，新文件）。旧算法是横屏那一套：「能塞几列 222 宽的卡就几列，卡高固定 192」。竖屏下 12 格排成 4 列 ×3 行 = 600 设计 px，而中段有 **1699** —— 1099 px（47% 的屏高）是空白，且没有任何东西可滚。改成**竖屏时卡高由带高反推、列数取「卡最接近正方形」的那个**：

```
3 列 → 4 行 314×415（长宽比 1.32）  ← 选中
2 列 → 6 行 477×273（0.57）
4 列 → 3 行 232×558（2.40）
```

选「最方」而不是「列最多」或「剩得最少」：高度既然是反推的，每个候选都恰好填满，能比的只剩卡自己的形状。768×1024 平板同一条规则落到 3 列 ×326 高（1.04，几乎正方形）——这正是设计里那张 222×192 卡想要的形状。两条护栏：卡**不会比 `CARD_H` 更矮**（带装不下就退回横屏那套 + 滚动），也**不会超过宽的 1.45 倍**（超了就居中、把富余分到上下两头）。横屏走的仍是原路径，一个像素没动。

卡内每个偏移量改成卡高的比例（`CARD_LAYOUT`，按 192 那张卡标定，乘回去逐个等于原来的字面量），字号 `snapFont(cardH × 18/192)` 上限 `FS.label`——415 高的卡配 18px 标签会读成一张空框。

**② 队伍带头部**：竖屏下头部行给 `TEAM_ROW_LABEL_H + 18`，按钮高 34、宽 268（横屏仍是 26/200）。宽度不是审美——「Fill All Teams」14 个字符在 200 设计 px 里要被 `drawButtonLabel` 整组缩到 0.94，正好把字号缩回可读下限之下（见 `UI_DESIGN_LOG_2026-08.md` §49.1）。

**③ 同屏还修的两处**（下限带出来的返工，不属于本节版面）：建筑详情弹窗的行距从字面量 16 改为 `max(16, FS.tiny×1.25)`（下限把 `FS.tiny` 抬到 20 后，「→ Lv.2 / 消耗 / 时长」三行互相压穿）；训练弹窗的队列状态行改为折行 + 先量后定面板高（20px 下该行 456 设计 px，宽过 340 的面板，直接跑出屏幕）。

- 覆盖：`client/test/cityGridMetrics.test.ts`（6 例，钉住三个真实带高上的列数/卡高/居中，以及两条护栏）；`cityScene.ui.ts` 55 例不变全绿。
- 验证：`npm run test:portrait` 六个尺寸全绿（本屏 3 站：`city` / `city+buildDetail` / `city+trainModal`）。
