...
| **网络失败时玩家读到什么：两个场景的 errorMsg 映射表 + 每个 catch 的接线（2026-08-25 同日续）** | ✅ 完成（纯客户端，只加测试）。**起因**：弹窗滚动条那条合并后第四次被问「还有测试可以加吗」。前三轮都在钉「渲染/交互少做事之后还有什么能悄悄错掉」，这一轮换一条链：**请求失败 → 玩家看到的那句话**。**空洞**：`SectSceneCore.errorMsg()` 的 8 条服务端码映射、`FamilySceneCore.errorMsg()` 的 7 条，加上两处 fallback（未映射的 `WorldApiError` → `e.message`；非 `WorldApiError` → `String(e)`），全部零覆盖（`grep -rn "sect.err\.\|family.err\." client/test` 0 命中）。既有测试都擦边而过：两份 busy-lock 只钉了 `TimeoutError` → `common.networkTimeout`，而 `sectActions.test.ts` 的 fake core 干脆把 `errorMsg` 换成了 `String(e)`——**映射本身从来没进过测试**。**这轮没抓到 bug**（前三轮各抓一个，这次是真的干净）：15 个键在 zh/en/de 三份 locale 里都在（逐键 diff，不是数个数），每个 catch 也都确实走了 `errorMsg`。**补的测试**：`test/ui/sectErrorPaths.ui.ts`（16 例）+ `test/ui/familyErrorPaths.ui.ts`（15 例）——映射表**一码一例**（漏一条时红的就是那一条，而不是一个 forEach 里的匿名失败），另各一条聚合例断言「每个码都解析出真正的文案，既不是原始服务端文本、也不是漏译时透出的 key 本身」；两处 fallback 各一例；接线用例走**真实场景 + 真实 errorMsg**（宗门：doJoin / doCreate / doAlly / doLeave / openBrowseList；家族：doCreate / confirmKick→doKick / submitMessage / doJoin / openJoinList），断言 toast 收到**映射后的**文案 + `bt.busy` 回 false + 重绘发生 + 失败不改状态（创建失败仍停在表单、退出失败不掉出 mySect、踢人失败成员仍在名册、发消息失败乐观行回滚）。**外加一条结构性守卫** `test/socialErrorWiring.test.ts`：扫两个场景的 5 个域文件，凡是 toast 的 catch arm 都必须 `errorMsg(<该 catch 的绑定名>)`——因为真正会发生的不是「现有映射写错」，而是**以后新加的动作**自己 toast 了 `String(e)`，行为测试看不见（那个动作在写测试时还不存在）；带一条 canary（扫到的 catch 数 ≥15）防正则被重构成空转，形状同 2026-08-25 那条 `tabIconWarmupCallSites.test.ts`。**mutation 验证三轮**：删掉 `SECT_FULL` / `INVALID_TAG` / `ALREADY_REQUESTED` 三行 → 恰好红 6 例（3 条对应用例 + 2 条聚合 + 1 条正好用了 `ALREADY_REQUESTED` 的接线例）；把宗门 `doJoin` 的 toast 整句删掉、把家族 `doKick` 的 `errorMsg(e)` 换成 `String(e)` → 恰好红对应 2 例；结构守卫用后者单独验过，会点名 `FamilyScene/actions.ts:185`。1919 单测 + 2080 UI 全绿，`tsc --noEmit` / `check:filelength` 绿。 | 用户「还有测试可以加吗」 |

| **家族↔宗门页签互跳仍会闪一下：两次「已修好几次」的收尾都没碰这条边（2026-08-30）** | ✅ 完成（纯客户端）。**问题**：用户反馈「切换家族和宗门时还是感觉整个页面被刷新了，切换好友和邮件时页签没有闪动，家族和宗门时页签能明显看到被重画，这个已经修复好几次了」。**根因**：2026-08-20/08-25 两轮收尾（见上两条）分别治好了「FriendsScene 进 Family/Sect 入口」的一次性交接和「SectScene/FamilyScene 各自内部的增量重绘」，但都没碰**这两个场景互相跳转**这条边——`FamilyScene`/`SectScene` 各自的 rail 是**两个独立 Scene 实例**（不像 Friends/Mail 共享同一个 `FriendsSceneCore`，切 tab 只是 `switchTab()` 翻内部状态），点宗门/家族页签时 `onNavTab` 直接调 `nav/world.ts` 的 `goFamilyHub`/`goSectHub`——**互跳分支从没传过 `preloadedFamily`/`preloadedSect`**（只有 `nav/social.ts` 的 FriendsScene 入口那条一次性交接有），于是每次互跳都从 `mode:'loading'` 重新起步、重新打 `getMyFamily()`（+ `getSect()`），在网络往返期间画一帧 loading 骨架——这正是用户看到的「页签被重画」，Friends/Mail 因为从没离开过同一个 Scene 实例，永远不会有这一帧。**修复**：①`SectSceneCore` 新增 `family: FamilyDetailView | null` 字段（`DataPanel.loadData()` 里连同派生字段一起存，不再只留局部变量），`SectSceneCallbacks` 新增可选 `onSectLoaded(sect)`（`DataPanel.applySect()` 命中时触发）；`FamilySceneCallbacks`/`FamilySceneView`、`SectSceneCallbacks`/`SectSceneView` 都加 `getFamily()`（+ Sect 侧再加 `getSect()`），委托读 `core.family`/`core.sect`。②`nav/world.ts` 的 `goFamilyHub`/`goSectHub` 把 `const view = views.showXxx(...)` 改成 `let view: XxxSceneView` 先声明后赋值（同 `goWorldMap` 已有的 `view` 手法，避免自引用闭包的循环类型推断），`onNavTab`/`onOpenSect` 里读**当前挂载场景的实时 `view.getFamily()`**（而不是另开一个易失效的缓存）作为对方的 `preloadedFamily`——Sect→Family 这个方向因此**零网络请求**（SectScene 本来就存了完整 `FamilyDetailView`）；Family→Sect 方向 FamilyScene 从不拉宗门详情，仍需一次 `getSect()`，但用闭包变量 `lastSect`（`createWorldNav` 作用域，`onSectLoaded` 写入）做**同 session 内的机会性缓存**——同一会话里第二次起互跳即可免掉这次请求，信任边界与既有 `preloadedSect` 的 `sectId` 校验一致（一旦对不上就当作没有，退回正常拉取，不会展示脏数据）。**验证**：`tsc --noEmit`（主 + `tsconfig.test.json`）绿；`test/world-family-sect-nav-tabs.test.ts` 新增 5 例（互跳/`onOpenSect` 都带上 `preloadedFamily`、`lastSect` 首次为空次轮命中）+ 修掉两处因返回类型收紧而需要补 `getFamily`/`getSect` 桩的既有 mock（`test/harness/HeadlessAppViews.ts`、本文件自身另外两个既有用例）；`npm test` 207 文件 2211 例、`npm run test:ui` 236 文件 2231 例全绿，无回归。**未截图**：改动只影响“预取到多少数据、要不要等一次网络往返”，不改任何渲染/布局代码，截图证明不了时序类修复；沿用本文件历次同类条目的做法（`test/world-family-sect-nav-tabs.test.ts` 等 nav-factory 边界测试）钉住交接行为。 | 用户「已经修复好几次了」 |

| **家族↔宗门互跳修复的补测（2026-08-30 同日续）** | ✅ 完成（纯客户端，只加测试）。**起因**：上一条合并后被问「有测试可以加吗」。**空洞（两处，都是这次改动本身新增的字段/方法，没人钉过）**：①`SectSceneCore.family` 和 `SectSceneCallbacks.onSectLoaded`——`data.ts` 确实写了 `core.family = fam ?? null` 和 `core.cb.onSectLoaded?.(sect)`，但 `sectLoadDecouple.test.ts` 原有用例只断言了 `core.mode`/`core.sect`/派生字段，从没读过这两个新东西；②`SectScene.getFamily()`/`getSect()`、`FamilyScene.getFamily()` 这三个新委托方法本身——上一条加的 5 个 nav-factory 边界用例全部 mock 掉了 `views.showFamily`/`showSect` 的返回值，从没让**真实的** `SectScene`/`FamilyScene` 类执行过这三行 `return this.core.xxx`，写反（比如 `getFamily()` 手滑返回 `this.core.sect`）不会被任何测试发现。**补的测试**：`sectLoadDecouple.test.ts` +6 例（`core.family` 在 preload/fresh-fetch/无家族三种路径下都对；`onSectLoaded` 在 fresh-fetch/preload 路径下都触发、无宗门时不触发）；新增 `test/ui/orgHubViewAccessors.ui.ts`（4 例，走 `composition-wiring.ui.ts` 同款真实场景构造）——`SectScene.getFamily()`/`getSect()` 返回真实加载到的数据且**互不相同**（防止两个访问器被误接反）、`FamilyScene.getFamily()` 同理，另各一例覆盖「网络还没回来时（`getMyFamily` 永不 resolve）访问器返回 `null` 而不是抛错」——nav/world.ts 的互跳分支正是靠这个 `null` 优雅回退到不带 preload 的普通请求，而不是崩溃。**mutation 验证**：把 `SectScene.getFamily()` 改成返回 `this.core.sect`、把 `applySect()` 里的 `onSectLoaded` 调用整行删掉，各自能让新增用例中对应的那些红（前者精确命中「返回真实加载数据」这条，两条 onSectLoaded 用例各红各的），验完照原样改回。`tsc --noEmit`（主 + test）绿；`npm test` 207 文件 2216 例、`npm run test:ui` 237 文件 2235 例全绿。 | 用户「有测试可以加吗」 |
