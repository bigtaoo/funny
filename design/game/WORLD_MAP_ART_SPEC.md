...
每格在三档缩放下均会显示底色。L1 最精细，L2 次之，L3 仅色块。
美术资产优先提供 **96×96 px PNG**（L1 用，L2/L3 缩放取样即可）。

> **格边长不是固定值，且两个朝向各有一套梯子**（权威：`client/src/scenes/worldmap/logic/zoom.ts` 的 `makeZoomCfgs`；本节原先写的 76/38/20 是早期数值，久已失效）。
> · 横屏（设计宽 1920–2592）：`w/11` / `w/31` / 27px → 1920 下 174 / 61 / 27。
> · 竖屏（设计宽固定 1080）：L1 按「自己基地地皮占屏宽 5/6」反推 = `w×(5/6)/BASE_FOOTPRINT` = **300**，L2 = `w/11` = 98，L3 = 27（2026-09-15 起，见 [`SLG_LOG_2026-08.md`](SLG_LOG_2026-08.md) 当日条）。
> 也就是说 L1 的格子最大能到 300px：出图时别按 96px 的上限去想细节密度，L1 会把它放大到 3 倍。
...
| 资产名 | 描述 | 状态 |
|---|---|---|
| `city_l1..l10` | 我/敌/盟主城 + NPC 可攻占城池（每级一张，10 级，归属靠程序上色） | ✅ 已接入 `city_atlas`（3×3 base 锚点，深度排序图层；2026-08-14 起统一命名，无档位回退） | ✅ **高度修复已落地（2026-09-21）**：`l3/l5/l7/l8/l9/l10` 重出图，内容外接框宽高比 1.0 → 1.67–2.08（世界中心绘制高度 1.05× 地块宽 → 0.58×）。精灵是等比方形缩放，**高度只能靠重出图修，没有代码旋钮**；`cityAtlasContentTop.ui.ts` 现已钉住高度上限。**全线收尾**：`l3/l4/l5/l7/l8/l9/l10` 共 7 张重出，10 帧的地台角度（前半深/地台宽）全部落在 0.223–0.258（理论值 0.25）。剩 `city_l2`（1.40，3×3 起手营地，唯一超 1.2 预算）**不排队**，数据 + prompt 见 [`city-image-prompts.md`](../product/city-image-prompts.md) 「高度审计（2026-09-21）」
...
> **纵向锚点的定尺规则（2026-08-17 补，用户反馈"拒马稍微往上放点，使其看起来在格子中间"）**：
> `placeBuildingSprite()` 把精灵**底边**放在格心下方 `hh * baseF` 处（`hh` = 菱形半高 = `tp*ISO_RATIO/2`）。
> **高**建筑（`icon_watchtower` / `icon_arrowTower`）用默认 `baseF = 0.72`：从靠近下顶点处往上长，重心
> 自然落在格心之上，读作"站在地上"。**扁平**障碍（`icon_blocker`）不能套这个值——精灵高度只有 `tp*0.22`
> 而半高是 `tp*0.25`，整个剪影会全落在菱形下半区，看着掉出格子。扁平物件的居中公式是
> `baseF = targetH / (2·hh) = <H 常数> / ISO_RATIO`（拒马 = `0.22/0.5 = 0.44`，精灵视觉中心正好压在格心）；
> 实际取 `BLOCKER_BASE_F = 0.50`，留一丝"略微靠前坐着"的接地感。**几何回退分支要用同一个 baseY**，
> 否则图集解码完成的那一刻路障会在格子里往下跳。新增扁平类建筑时按这条公式定，不要直接抄 0.72。
>
