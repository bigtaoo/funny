...
| 资产名 | 描述 | 状态 |
|---|---|---|
| `city_l1..l10` | 我/敌/盟主城 + NPC 可攻占城池（每级一张，10 级，归属靠程序上色） | ✅ 已接入 `city_atlas`（3×3 base 锚点，深度排序图层；2026-08-14 起统一命名，无档位回退） | ⚠ **6 帧画得太高（2026-09-21）**：`l3/l5/l7/l8/l9/l10` 内容外接框宽高比≈1.0，绘制高度超过自己地块的宽度（玩家主城 `playerbase_*` 的标准是 1.7–1.9）。精灵是等比方形缩放，**高度只能靠重出图修**；审计数据 + 6 条重出 prompt 见 [`city-image-prompts.md`](../product/city-image-prompts.md) 「高度审计（2026-09-21）」
