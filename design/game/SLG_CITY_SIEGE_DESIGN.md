...
5. ✅ **耐久结算 + 易主**（`combatSiege/cityDamage.ts`，由 `settleSiegeDamage` 按 `SiegeDamageDoc.cityId` 分流）：惰性回复 → 扣伤害 → 累加 `siegeLog[sectId]`；归零则**最后一击那名玩家所属宗门**得城（ADR-074 决策 2），写 `capturedAt`/`protectedUntil`，耐久**重置为满**（否则刚付了整场代价的宗门拿到一座任何人下一击就能翻走的空城），清 `siegeLog`。
   ...
   - **⚠️ 频道那几条的长度上限不是 `maxBodyChars`，是列宽（截图才看出来的）**：`drawChatLine` 单行不换行，`maxBodyChars=60` 只是**字数**上限，真正先裁掉文字的是所在列的宽度。实测（landscape 设计宽 1500）**宗门频道**那一列（`SectScene/lists.ts` 是左右分栏）到 ~34 字（中文）/ ~39-41 字（拉丁）就被切断，而**世界频道**（`FriendsScene/worldChat.ts`）是整宽行，60 字都放得下。第一版德文写到 55 字，60 字断言照样绿，但在宗门频道里是**半个词被切掉**的样子——所以 `client/test/i18n-system-text.test.ts` 的上限断言按**频道**分开写：走 `postSect` 的 `captured`/`lost` 按窄列算，`worldCenterCaptured` 走全服频道按 60 算。上限都按最宽的合法宗门名（`ORG_NAME_WIDTH_MAX = 12`）代入后再量。
     - 顺带一个**遗留问题**（不在本次范围）：`drawChatLine` 按**字数**截断而不是按**可用宽度**截断，所以任何玩家自己打的长消息在宗门频道那一列同样会被硬切；仓库里已经有 `truncateToWidth`（`SectScene/lists.ts` / `FamilyScene/lists.ts` 在用），把它接进 `drawChatLine` 才是根治。
