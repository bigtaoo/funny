...
> ## ⚠️ 上表的输入已被 ADR-075 作废（2026-08-25 同日，合并时发现）
>
> 本节的 `p` 推导写于 `TROOP_CAP_BASE=10000` / `TROOP_TRAIN_QUEUE_MAX=2` 的年代。**同一天另一条线落地的 [ADR-075](../DECISIONS.md) 把兵力曲线整个换了**：`TROOP_CAP_BASE` 10000→**5000**、`DRILL_TROOPCAP_STEP` 1000→**1500**（练兵场整条弧从 2× 变 4×）、`TROOP_TRAIN_QUEUE_MAX` 2→**1**（槽位改为 L0-3:1 / L4-9:2 / L10:3）。按新常数重算：
>
> | 档位 | 兵池 | 训练/时 | 一小时兵力预算 | ≈攻城次数 | **`p`（伤害/时）** |
> |---|---|---|---|---|---|
> | 新手（练兵场 L0） | 5,000 | 720 | 5,720 | ~2.9 | **~380** |
> | 中期（L4） | 11,000 | 1,440 | 12,440 | ~6.2 | **~1,150** |
> | 满配（L10，训练速度 ×2） | 20,000 | 4,320 | 24,320 | ~12.2 | **~3,400** |
>
> **两个后果，第二个是硬伤：**
> 1. 档位间的差距从约 2× 拉到约 **9×**（ADR-075 刻意把练兵场做成 4× 成长弧的直接结果），所以「N 人 1 小时」这一列不再是一个数，而要按参战者的养成档位分布来算。
> 2. **单人封死的余量没了**：满配单人 `p_max ≈ 3,400/时` **已经超过**最弱野城（Lv.3）的回复 `R = 1,000 × 3 = 3,000/时`。§6.2 那句「2.1 倍余量，数学上封死单人」是按旧常数算的，**在当前代码下不成立**——一个练满兵的大号可以独自磨下一座 Lv.3 城。
>
> **这条是 P2 的头号输入，不是可以顺手拍的数**。可选杠杆（留给 econ-sim 定）：①抬 `CITY_REGEN_PER_LEVEL`（1000→1500 可让 Lv.3 回到 4,500/时，对 `p_max` 有 1.3× 余量，但同时把所有档位的所需人数一起抬高）；②把最弱野城的等级抬到 Lv.5 以上；③回复改为非线性/带下限，让低级城的回复不随等级线性缩水。**在 P2 跑出结论前，§6.2 的表只应被当作形状参考，不能当数值依据。**
...
| 项 | 状态 |
|---|---|
| **单人封死的余量（最重要）** | **当前代码下已失效** —— ADR-075 把兵力曲线换掉后满配单人 `p_max≈3,400/时` > 最弱野城回复 3,000/时。P2 的头号待解项，见 §6.1 的警告框 |
| §6 全部数值 | **DRAFT 推导值**，且输入常数已被 ADR-075 换过一轮，P2 econ-sim 前不得上线 |
| `p` 里「每次攻城兵耗 ~2,000」 | 估计值，由守军设计反推，未实测；ADR-075 之后更需要实测（兵池腰斩、档位差拉到 9×） |
