...
- ✅ `contracts/openapi-world.yml` 删除 `/family/*` 路径块 + 仅被其引用的孤儿 schema（`FamilyMemberView`/`FamilyView`/`FamilyMessageView`）；`SectMemberFamilyView` 保留（宗门路径仍用）。**注：socialsvc 暂无 OpenAPI 契约**（`contracts/` 只覆盖 metaserver 与 worldsvc），迁出的 `/social/family/*` 公网路由当前无契约声明——为后续待办，本次未补（已于 2026-07-27 补齐，见 §9）。
  ...
  > **评估（design-doc-audit-2026-07，2026-07-27）：建议补一版契约，本次未做，已 spawn_task 登记为独立后续项。** 理由：① `server/socialsvc/src/httpApi.ts` 目前有约 30 个公网 `/social/*` 端点（家族 11 个 + 好友 9 个 + 聊天 4 个 + 邮件 4 个 + 资料卡等），规模不小；② **已有真实字段错位事故记录**（就是本条上方紧接的 2026-07-01 记录）——`FamilyMemberView`/`FamilyMessageView` 曾因契约缺失被迫手写类型，字段名对不上真实返回形状（`senderId/senderName` vs 手写猜测的 `from/fromName`），直接导致 `FamilyScene.ts` 消息列表显示 `undefined`，不是假设性风险；③ **已有现成的分域契约模式可直接照搬**：`server/auctionsvc`（同量级中型服务）已按 `openapi-<domain>.yml` + `gen-openapi-<domain>.mjs` + `package.json` 里 `gen:api:<domain>`/`gen:api:<domain>:check` 两个 script 的模式接入（`contracts/openapi-auction.yml` 仅 254 行，规模可控，不是要重新发明一套流程）；worldsvc 同理（`openapi-world.yml`）。**结论**：不是"要不要"的问题，是"什么时候排期"的问题——三个理由都指向应该做，只是本次审计范围是"评估"不是"实现"，工作量（新契约文件 + codegen 脚本 + client `WorldApiClient.ts` 家族/好友/聊天/邮件四块手写类型切回生成类型 + 全量回归）超出本次 cleanup 的合理范围，故拆成独立任务。**实现已于 2026-07-27 当天完成，见 §9。**
...
- **`sectName`（2026-07-18 补）**：家族预览弹窗（`FriendsScene.drawFamilyDetail`）此前只在 `fam.sectId` 存在时显示一个裸标签"宗门"，没带名字——因为镜像只存了 `sectId`，没存名字。同上一并镜像 `sectName`：`setSect` 新增第三参数，`POST /internal/family/:familyId/sect` body 加 `sectName`；`sectService.ts` 的 4 处调用点里 create/join 传真实名字（建宗时用局部变量 `name`；入宗时用 `findOneAndUpdate` 返回的宗门 doc `res.name`），leave/dissolve 传 `null` 一并清空。
