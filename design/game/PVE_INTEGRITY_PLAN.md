...
3. **L1 录像抽检复算（可疑触发）** ✅（2026-06-15，复用 S1-J 第三方客户端无头复算）：
   ...
   - ✅（2026-07-18）**处置策略：取消自动封号，改人工审核**——排查一次真实误判后确定的政策（见下方"公平性
     注记"）：`pveRewards.PVE_REJECT_BAN_THRESHOLD`（=3）不再触发自动封号。`pveVerify` 每次 `rejected` 都会
     ①发 Fair Play Warning 系统邮件（不再限首次）②在 `antiCheatReviews` 集合写一条 `kind:'pve_reject'` 审核记录
     （`_id: pve:${verifyId}`，复用 S9-7 PvP 超报审核队列的表/接口），reject 次数达到旧阈值时标 `severity:'high'`
     仅作运营分流信号，不触发任何自动动作。运营在 `tools/ops` Anti-Cheat Review 页面对每条记录人工裁定
     dismiss/ban（`POST /admin/anticheat/reviews/:id/resolve`，`anticheat.action`）；ban 会走跟 Player Lookup
     "Ban" 按钮完全相同的 `/internal/accounts/:id/ban` 路径，全库只有这一条封号执行路径。
     **公平性注记**：复算流程只对比"重演算星数 vs 声称星数"，无法区分"提交了一段伪造的空回放"和"练度已远超
     该关卡门槛、基地/英雄被动攻击就能免操作通关"——后者对高练度老号是完全合法的结果。旧的三振自动封号会把
     这种合法情况和真作弊一视同仁地封掉，复现见 2026-07-18 debug 记录（`zihao` 测试号连续三关 `frames:[]`，
     核实为该号强度碾压 ch1 早期关卡，而非录制链路故障）。
