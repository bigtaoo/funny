...
## 29. 收集册卡片其余两行也带上图标（类型 / 费用 / 未解锁，2026-08-27）

**起点**：§28 把属性行修成「图标 + 名字 + 数值」之后，用户指出同一张卡片上还剩两行是纯文字——副标题 `士兵 · 费用 4` 和锁定态的 `未解锁`。判据就是 §28 那条通用判据反过来用一次：**同一块信息面板里既然有一行是「图标 + 名字」，其它行就不能只有名字。**

**做法**：新增 `drawIconTextRow()`（单行 `[图标] 文字` 若干段，装不下整行等比缩小）——副标题此前**根本没有宽度守卫**（卡名有、属性行有），加图标之后竖屏必须有。两行分别落地：

- **未解锁**：复用批次 7 已有的 `lock`。插画上本来就压着一个大锁，但那是左边另一块面板；信息面板这一行得有自己的图标。这一行不等美术、当天先落地。
- **副标题**：`士兵 · 费用 4` → `[头盔] 士兵　[墨水瓶] 费用 4`，**中间的 `·` 去掉**（两个图标已经把两段分开，跟属性行本来就没有分隔符对齐）。y 从 `0.34h` 调到 `0.33h`：行高从「一行文字」变成「一个图标高」，减掉半个差值才保得住原来的视觉中心。

**四种标签只需两张新图**：`建筑` 复用 `castle`、`费用` 复用 `ink`（战斗里费用付的就是墨水，战斗 HUD 画的就是这只瓶子），只有 `士兵`（头盔）和 `法术`（卷轴）出新图——`士兵` 的三个复用候选全撞了（`rosterIcon` 跟本页页头的 `collectionTabIcon` 同一路造型会重影、`swords` 的既有语义是「对战/PVP」、`atk` 是同一面板下一行的匕首）。出图两轮：卷轴一版过，头盔 v1 出成了毛线帽（横箍带 = 罗口 + 封死的底边 + 把下半部切成两格的护鼻），v2 改侧面科林斯盔一版过——**正面头盔的辨识全靠细缝，而细缝在 28px 上必然消失，侧面的眼窝是个大缺口**。判断表与 prompt 见 [`tab-icon-art-prompts-batch8.md`](../product/tab-icon-art-prompts-batch8.md) §8b。

**回归测试**（红绿对照都做过）：`cardCodexScene.ui.ts` 加 2 例——三种类型标签与费用标签必须有缩进（无头 PIXI 里 ink 图标贴图永不 decode、`buildIcon` 返回空容器，所以断言的是图标对布局的位移而不是图标节点本身）、且**没有任何标签还带 ` · `**（分隔符必须在每种卡上都消失）；锁定行同款一例。竖屏几何另有一例（见 §28 收尾提到的 `cardCodexPortraitWidthAndText.ui.ts`）断言「名字 → 副标题 → 属性/未解锁」的上下顺序且每段都在自己卡片矩形内。
