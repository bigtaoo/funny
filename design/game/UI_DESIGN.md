...
`worldmap/WorldMapPanels.ts` 的 toast **未改动**——该处此前已被专门放大到 26px 并刻意定位在 `h/3`（上三分之一），是为了避开世界地图底部的操作栏（见 [[worldmap-toast-size-position-2026-07-15]] 记忆），移到下三分之一会重新引入被底部 UI 遮挡的问题。
