...
- **点击穿透（真正的根因，2026-08-10 修复；同一现象的第三次报修）**：用户反馈"打开并发送一次反馈，再次打开后直接点关闭，没有退回大厅，而是退回到更早时的界面"。前两轮修复（2026-08-08 的 `DialogGate`、2026-08-09 的 `dim.eventMode`）都没修到点子上：
  ...
  - **测试**：① `client/test/ui/dialogModalInputGate.ui.ts`（新增，11 例）——把生产环境的组合真实搭出来（真实 `LobbyScene` + 真实 `InputManager` 订阅 + 真实 `FeedbackDialog`），横竖屏各一组：先钉死"没有闸门时三个控件分别撞到 `onStartRanked`/`onOpenCampaign`/`onOpenWorld`"（bug 本身，顺带保证日后 Lobby 布局挪动导致重叠消失时这个测试会红、逼人重读而不是悄悄空转），再断言抬起闸门后三次点击一个都不落到 Lobby、放下闸门后又恢复；另加 `InputManager` 闸门语义 6 例（move/up/wheel 一并拦、计数、不会被多余的 `false` 拉成负数、与 `suppress` 双向互不影响、模态不触发 `suppressedDownHook` 淡入淡出中断钩子）。② `client/test/appDialogInputGate.test.ts`（新增，3 例，纯源码扫描，理由同 `appTickerDialogWiring.test.ts`：`startApp()` 没有可行的端到端单测入口）——断言 `app.ts` 里两处 `holdForModal(true)`、两个 teardown helper 里各有 `holdForModal(false)` 且都带早退、两个弹窗的 `onClose` 确实走 helper。截图验证仍不可行（Browser 面板对该 WebGL 页面持续 "not displayed" 不合成帧，同 2026-08-08 条目记录）。`tsc --noEmit`/`npm test`（1274 例）/`npm run test:ui`（1392 例）全绿。
