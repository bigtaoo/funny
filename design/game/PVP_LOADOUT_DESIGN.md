...
> **解锁判定 = 当前分 `pvp.elo`**（不是赛季峰值）。**2026-07-02 拍板推翻原「峰值判定」方案**：峰值判定会让掉分玩家在低分匹配区（对手是新手/未冲分玩家）继续带着高分段才该有的单位，直接把新手打蒙——公平性优先于「掉分卡被锁回去」的体验割裂感。
...
| 构筑界面弹出条件 | 解锁卡数 ≤ 卡组大小（10 张）时**跳过构筑界面**，直接用全部解锁卡开始排位；解锁卡数 > 10（即**当前** ELO ≥ 1500，已过首个解锁段位）才弹出可配置的构筑界面 | 无选择余地时强弹一个全蓝格子、无事可做的界面对新手体验无意义；见 `client/src/app/createAppCore.ts` `onStartRanked()`。判定用当前 elo，掉分后会重新回落到跳过态 |
...
| 服务端校验 | gateway/matchsvc 取玩家**当前** `elo` → 算「已解锁卡集」→ **校验提交的 deck 全在已解锁集 + 基础库内**，非法则拒绝/回退默认卡组 | `Gateway.ts:411` 附近、`internal.ts` ELO 端点 |
...
- [x] **解锁判定 = 当前 `elo`**（掉分即掉锁；2026-07-02 由「赛季峰值不降级」推翻，见 §3）
