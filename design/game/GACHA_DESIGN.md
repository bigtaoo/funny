...
**阶段 1 — roll 类别**（`economy.ts CATEGORY_WEIGHTS`，参考值，`[adjustable]`，和为 100）：

| 类别 | 权重 | 说明 |
|---|---|---|
| `material` | 70 | 文具材料（scrap/lead/binding） |
| `card` | 15 | 角色卡（花名册，[CHARACTER_CARDS_DESIGN](CHARACTER_CARDS_DESIGN.md)） |
| `equip_t1` | 10 | 一级装备 = fine 品（pen/cardstock/bookmark） |
| `equip_t2` | 3 | 二级装备 = rare 品（marker/leather/sticker） |
| `equip_t3` | 1 | 三级装备 = epic 品（highlighter/foil/seal） |
| `skin` | 1 | 外观皮肤 |
...
- 其余类别用全局 `RARITY_WEIGHTS`（common700/rare230/epic60/legendary10）：材料偏 scrap、卡偏 Tao 三张(史诗)、传说 Anna 卡稀少。
...
- ⚠️ **副作用**：参考权重下有效 legendary 率约 **3%**（高于旧扁平 1%），pity 更少触达 70——如需回落调 `CATEGORY_WEIGHTS`。
...
- **数据（`@nw/shared/economy.ts`）**：新增 `GachaCategory` / `GACHA_CATEGORY_ORDER` / `CATEGORY_WEIGHTS` / `SKIN_TIER_WEIGHTS` / `withinCategoryWeight()` / `itemRarityMap()`；`GachaPoolDef` 加可选 `categories` 字段（仅常驻池填），`itemsByRarity` **保留**为展示稀有度/退币/保底发货的权威。
...
- **测试全绿**：commercial `gacha.test.ts`（15，含两阶段/皮肤四档/装备三档/legendary 重置 pity/poolEntries 归一化）；shared `economy.test.ts`（33）；commercial 全量 93；shared 全量 439；metaserver `economy.e2e`（11，含 gacha 抽取+发货）。
- **待办**：`CATEGORY_WEIGHTS` 为参考值待调（有效 legendary 率 ~3%）；普通/稀有皮肤档待补物品；限定池/节日池两阶段化留后续。
