...
> **图标已落地（2026-07-05，程序复用，非新美术）**：当前单网格弹层（`GachaScene.drawOdds`，尚未拆两 tab）里每格已换成实际图像而非通用星标——材料→`icons.ts` 专属图标；装备（`wp_`/`ar_`/`tk_`）→`equipmentGlyph.ts` 按 slot+rarity 程序绘制；角色卡（`lichuang`/`chenshou`/`suyuan`/`max`/`lena`/`mara`）→复用 `cardArt.ts` 的兵种立绘 PNG；皮肤→collection 页同款画笔图标。详见 §9.5 说明。两 tab 拆分仍未做。
>
> **名称 + 滚动已落地（2026-07-15）**：单网格弹层继续保留"不分组、逐物品展示"的形式（用户明确要求，暂不做 §8.2 目标里的按稀有度分组 Tab B），但修了两处可用性问题——① 每格原来直接显示原始 `itemId`（如 `mat_scrap`），现改为真实翻译名（材料/装备/角色卡各走自己的 i18n key，皮肤显示"角色名·皮肤"）；② 原来把所有物品硬塞进一屏、按行数压缩卡片高度，物品一多（当前池 22 项）小概率物品的字/图挤得看不清——现改为固定卡片比例 + 网格区域加 mask + 拖动滚动（复用 `ScrollIndicator` 组件，滚动节流写法照抄 `CardScene` 的 drag-scroll 模式），超出面板高度的物品滚动查看，不再挤压。
...
| G7 | 概率详情弹层改造（两 tab：概率 + 物品列表）| 美术 §9.2 / §9.5 | ⏳ 两 tab 拆分未做；物品图标已用程序复用方案顶上（2026-07-05），不再阻塞 |
