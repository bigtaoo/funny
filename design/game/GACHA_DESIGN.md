...
> 状态：实现中（G1/G2/G4/G5/G6 服务端 + 最小客户端已落地 2026-07-02；G7–G10 美术展示层待美术） · 权威：本文 · 更新：2026-07-02
>
...
- **id**: `standard`
- 永不下线，保底计数随账号存续
- 内容：common/rare = 文具材料 + 精良装备（上线起**无皮肤**）；epic/legendary = 3 款 Anna 外观皮肤 + 装备成品 + 角色卡（见 `economy.ts GACHA_POOLS[0]` 与 §9.5 皮肤目录）
...
- **软保底（G1）**：`server/commercial/src/gacha.ts` `softPityLegendaryProb` + `rollRarityBoosted`；标准/限定/单位卡池 `softPityStart=70 / softPityStep=0.05`（`economy.ts SOFT_PITY_*`）。硬崖 90 仍作兜底。软保底起点以下走原扁平权重表（旧行为不变，回归单测锁定）。
...
- **月卡（G5）**：`wallet.subscription{expiry,lastClaimDayKey}`。`POST /monthly-card/buy` 幂等叠购续期（`max(now,expiry)+30d`）+ 即赠 600 coins（聚合管道原子更新）；`POST /monthly-card/claim` 每 UTC 日一次 +120 coins（`lastClaimDayKey` 守卫）。真实 IAP 验单不在本期范围（当作已授权购买；接真实 SDK 时前置验单即可）。
...
- **类别（category）= 物品大类**：`skin | card | equipment | material`（owner 拍板：种类指皮肤/角色卡/物品/材料，非稀有度）。
- **物品目录 `GACHA_CATALOG`**：可被放入自定义池的全部物品，从既有注册表聚合——`EQUIPMENT_DEFS`（装备）、`CARD_DEFS`（角色卡）+ 皮肤/材料内联表。每项带 `{itemId, category, rarity, name}`；`rarity` 为**展示稀有度**（卡背色/概率分组），装备按标准池的放置映射（fine→rare, rare→epic, epic→legendary），角色卡按阵营（Anna→legendary, Tao→epic）。
- **`CustomPoolConfig`**：`{ id, name, costSingle, costTen?, startAt, endAt, categories: [{category, weight, items:[{itemId, weight}]}] }`。权重为**相对值**（不要求和为某数），归一化在抽卡/展示时进行。
