...
§7 **七步全部完成**：平台接缝 + cue 目录 + 程序化合成音 + 样本加载/解码/并发上限/混音器 + 战斗触发点 + UI 触发点 + 设置页音量 + 微信后端 + 素材第一批 + **BGM**，五轮实测（§0.1 战斗 / §0.2 UI / §0.3 微信 / §0.4 素材 / §0.5 BGM）都已做完，**§0.6 是第一次真正的听感反馈**（2026-09-05，大厅床太急促 → 0.8x 重切）。开放项四个：`bgm.battle` 缺 master（§2.3）、微信真机（§0.3 末尾）、**18 个 cue 仍然没有人听过**（§0.5 末尾；大厅床是唯一的例外）、ducking 未在真实 stinger 下听过。
...
> 2026-09-05 被 §0.6 取代了**——第一次听感反馈说这条床太急促，于是它按 0.8x 重切（61.0 s /
> 544368 字节 / 接缝 0.55 dB / 区段 45.0 s）。留着不改，因为这一节记的是**当时量到的东西**，
...
- 微信主包 2197453 → **2206391（+8938 字节）**，涨的是整个 BGM 运行时（播放器 + 两个 deck + 目录 +
  接线）。距 4 MB 红线仍有大量余量。
- **区段长度直接就是下载量**：74 秒 653016 字节（§0.6 重切后是 61 秒 544368 字节，−17%）。当初担心的 `dist.total` 余量（带占位估到约 95.9%）
...
### 0.6 第一次听感反馈：大厅床太急促（2026-09-05，**已 0.8x 重切**）

§0.5 末尾那条「还没有人听过」第一次被兑现了一小块：项目所有者坐下来听了大厅，反馈只有一句——
**太急促**。这是本文档所有数字里第一个不是量出来的、而是听出来的判断，所以它优先于其余一切。

**改法：0.8x，保音高，烧进资产。** 三件事各有各的理由：

| 决定 | 为什么不是另一种 |
|---|---|
| **0.8x** 而不是「速度降低一倍」（0.5x） | 0.5 要 2 倍拉伸，相位声码器在持续弦音上的涂抹在那个倍率上是**明摆着听得见**的；而且 74 s 的区段 ×2 = 148 s，直接撞穿 `duration_ms` 门禁的 90 s 上限。1.25 倍拉伸的伪影落在一条比 cue 集低 6 dB 的床的噪声底下面 |
| **保音高**（相位声码器）而不是重采样 | 重采样把 0.8x 连音高一起拉低一个小三度——那是**另一件乐器**，不是同一条床变慢 |
| **烧进资产**而不是运行时 `playbackRate` | 两个平台都有那个旋钮（`HTMLMediaElement.playbackRate` / `InnerAudioContext.playbackRate`），一行就能改完。但微信没有文档保证它背后是变调还是保音高，于是**同一个构建在两个平台上会是两个转调**；而 `XFADE_S` 在播放器里是 2 秒**墙钟**，只有「存储时间轴 = 播放时间轴」的文件才能让它和门禁量过的那 2 秒是同一段（rate ≠ 1 时，2 秒墙钟只覆盖 1.6 秒素材，接缝就落在一个没人测过的窗口上） |

**区段必须重搜，这不是顺手打扫。** 0.8x 之后，2 秒的交叉淡入窗口跨过的素材变了，原来那个
12.5 s/74.0 s 是在一条**不再存在**的时间轴上挑的（而且 74 s×1.25 = 92.5 s 已经越过 90 s 门禁）。
所以 `time_stretch` 跑在**选区段之前**（`load_master`），下游每一个测量——`--search` 的排名、shelf、
`xfade_band_diff`、`duration_ms`、播放器回绕用的 `lengthS`——都自动落在播放的那条时间轴上，
这一步之后管线其余部分一行没改。放慢后的母带 266.43 s，重搜结果：

| 桶 | cost | start | 长度 | 接缝 band-diff | 电平差 |
|---|---|---|---|---|---|
| 20–30 s | 0.90 | 19.0 s | 26.5 s | 0.85 dB | 0.10 dB |
| 30–45 s | 1.17 | 18.5 s | 30.5 s | 0.85 dB | 0.46 dB |
| 45–60 s | 1.87 | 148.5 s | 59.5 s | 1.30 dB | 1.15 dB |
| **60–75 s** | **0.69** | **45.0 s** | **61.0 s** | **0.55 dB** | **0.26 dB** |
| 75–90 s | 1.81 | 18.5 s | 88.5 s | 1.64 dB | 0.34 dB |

这一次不需要在「接缝」和「重复间距」之间取舍：60–75 桶同时是**全局最优**。72 s 以上的候选一个都活
不下来（最好的一条 2.50 dB，正好是门禁本身）。

**发货了什么**：`bgm-lobby.mp3`，**544368 字节**（−17%）、24 kHz 立体声、**61.0 s 循环**、
71.4 kbps VBR、频带电平 **−29.00 dBFS**、接缝 **0.55 dB**，`audit.py --class music` 过。回绕点从
72.0 s 移到 **59.0 s**（`lengthS − XFADE_S`），`musicCatalogue.ts` 的 `lengthS` 跟着改成 61.0——
`musicAssets.test.ts` 的 `checkLength` 把它钉在文件真实时长上，所以这两个数漂开是红的。

**管线这边新增的**（`tools/audio-pipeline/process_music.py`）：
...
- `speed` 进 `TRACKS`、进 `credits.json`（母带**加上对它做了什么**才是可复现记录：同一个区段、
  错的速度，切出来是另一条歌，而其余每一条规则都照样绿）。

**门禁**：`selftest.py` 新增 7 例（时长 ×1.25 / 音高不动 / 电平不动 / 双音都活着 / `speed=1.0` 是真
no-op / `peak_regions` 的两个已知答案），共 112 例全过；`musicAssets.test.ts` 新增 `speed` 字段 +
2 个变异（记录里没有速度、`length_s` 停在上一次切的 74），共 43 例全过。

**听不了就得量**：在同一段素材（母带 60–120 s）上拉伸前后对比，三个数字回答三个不同的问题——

| 问的是 | 量到的 |
|---|---|
| 真的变慢了吗 | 把 y 的包络按 0.8 压回去再与 x 相关：**+0.744**；不压回去只有 +0.093；假设 1.10/1.40 分别只有 +0.049/+0.167。**相关性只在 1.25 上尖起来**，所以时间轴是逐事件映射的，不是"听起来慢一点" |
| 音高动了吗 | 谱质心 1038.2 → **1009.6 Hz**（0.8x 重采样会读到 830.6）；100–200 Hz 最强的两个分音在两边都是 175.0 / 197.3 Hz，一个 bin 都没动 |
| 代价是什么 | 整体 RMS −16.90 → −16.99 dB（等于没变，之后还要归一到频带目标）。但最强低音分音（175 Hz）周围有可测的涂抹：133–162 Hz **+4.8 dB**、109–133 Hz **−2.9 dB**，即能量朝边带铺开。这就是相位声码器的代价本身，也正是为什么这里是 0.8 而不是 0.5 |

**这一轮仍然没有验证到什么**：这一版**同样还没有人听过**（重切完成于这次会话，用户还没听）。
...
| 轨 id | 场景 | 备注 |
|---|---|---|
| `bgm.lobby` | **除对局之外的一切**（大厅/菜单/商店/世界地图/结算/首启故事） | 🎵 **已发货 2026-09-01**（§0.5），**2026-09-05 按 0.8x 重切**（§0.6）：61 s 循环、544368 字节、频带 −29 dBFS |
...
| BGM 素材 + 授权 | ✅ **2026-09-01**，**2026-09-05 按 0.8x 重切**（§0.6）。`client/src/assets/audio/music/bgm-lobby.mp3`（544368 字节、61.0 s、频带 −29 dBFS），母带 `art/audio/sources/first-party/doodle-bed.flac`（无损，项目自有、无需署名）。区段由 `--search` 在**放慢后的**母带上按门禁同一个度量重挑（0.55 dB / 61 s，这一次它同时是全局最优）。记录走 `credits.json` 的 `music` / `music_terms`（含 `speed` 字段），**不进 `packs.json`** |
