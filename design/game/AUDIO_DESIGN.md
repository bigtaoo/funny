...
> **⚠️ 这一节里所有关于发货文件的数字（74.0 s / 653016 字节 / 接缝 0.57 dB / 区段 12.5 s）都在
> 2026-09-05 被 §0.6 取代了**——听感反馈说这条床太急促、而且太响，于是它按 0.7x 重切（73.0 s /
> 637848 字节 / 接缝 0.69 dB / 区段 91.5 s），BGM 总线默认值也从 0.5 降到 0.2。留着不改，因为
> 这一节记的是**当时量到的东西**，
> 而它的方法学（回绕表、ducking 包络、打包账、那条后台标签页的环境坑）一条都没有过期。
...
- 微信主包 2197453 → **2206391（+8938 字节）**，涨的是整个 BGM 运行时（播放器 + 两个 deck + 目录 +
  接线）。距 4 MB 红线仍有大量余量。
- **区段长度直接就是下载量**：74 秒 653016 字节（§0.6 两轮重切之后是 73 秒 637848 字节）。当初担心的 `dist.total` 余量（带占位估到约 95.9%）
...
### 0.6 第一次听感反馈：太急促、而且太响（2026-09-05，**两轮重切 + 总线降档**）

§0.5 末尾那条「还没有人听过」第一次被兑现了一小块：项目所有者坐下来听了大厅。反馈两句——
**太急促**，以及**音量太大**，并给了一条标尺：「背景音乐的正常音量应该是按钮音效的一半」。
这是本文档所有数字里第一批不是量出来、而是听出来的判断，所以它们优先于其余一切。

两句话落在**两个完全不同的旋钮**上，而且第二句在量过之后比它听起来更严重。

#### 一、速度：1.0x → 0.8x → **0.7x**

**改法：保音高的时间拉伸，烧进资产。** 三件事各有各的理由：

| 决定 | 为什么不是另一种 |
|---|---|
| **0.7x**（先试了 0.8x，反馈「还是太快」，这是第二档） | 用户最初问的是「速度降低一倍」（0.5x）。0.5 要 2 倍拉伸，相位声码器在持续弦音上的涂抹在那个倍率上**明摆着听得见**；而且 74 s 区段 ×2 直接撞穿 `duration_ms` 门禁的 90 s 上限。1.43 倍的伪影还落在噪声底下面——**而床变安静本身买到了这多出来的一档拉伸** |
| **保音高**（相位声码器）而不是重采样 | 重采样把 0.7x 连音高一起拉低超过一个纯四度——那是**另一件乐器**，不是同一条床变慢 |
| **烧进资产**而不是运行时 `playbackRate` | 两个平台都有那个旋钮（`HTMLMediaElement.playbackRate` / `InnerAudioContext.playbackRate`），一行就能改完。但微信没有文档保证它背后是变调还是保音高，于是**同一个构建在两个平台上会是两个转调**；而 `XFADE_S` 在播放器里是 2 秒**墙钟**，只有「存储时间轴 = 播放时间轴」的文件才能让它和门禁量过的那 2 秒是同一段（rate ≠ 1 时，2 秒墙钟只覆盖 1.4 秒素材，接缝就落在一个没人测过的窗口上） |

**每换一次速度，区段都必须重搜——这不是顺手打扫。** 放慢之后 2 秒交叉淡入窗口跨过的素材变了，
旧区段是在一条**不再存在**的时间轴上挑的（而且 74 s×1.25 = 92.5 s 已经越过 90 s 门禁）。所以
`time_stretch` 跑在**选区段之前**（`load_master`），下游每一个测量——`--search` 的排名、shelf、
`xfade_band_diff`、`duration_ms`、播放器回绕用的 `lengthS`——都自动落在播放的那条时间轴上，
这一步之后管线其余部分一行没改。同一个区段决定因此被重做了**三次**，一次一个速度：

| 速度 | 母带长度 | 选中区段 | 接缝 | 为什么 |
|---|---|---|---|---|
| 1.0x | 213.16 s | 12.5 s / 74.0 s | 0.58 dB | 60–75 桶的赢家，**不是**全局最优（20–30 桶 0.27 dB）——23.5 秒的循环重复得太勤 |
| 0.8x | 266.43 s | 45.0 s / 61.0 s | 0.55 dB | 这一次桶内最优同时是全局最优；72 s 以上全部 ≥ 2.50 dB，也就是门禁本身 |
| **0.7x** | **304.31 s** | **91.5 s / 73.0 s** | **0.69 dB** | 60–75 桶赢家。30–45 桶更好（0.56）但又被同一条理由挡回去：33 秒的循环在玩家一坐几分钟的界面上每 33 秒翻一次面 |

**发货了什么**：`bgm-lobby.mp3`，**637848 字节**、24 kHz 立体声、**73.0 s 循环**、69.9 kbps VBR、
频带电平 **−29.00 dBFS**、接缝 **0.69 dB**，`audit.py --class music` 过。回绕点落在 **71.0 s**
（`lengthS − XFADE_S`），`musicCatalogue.ts` 的 `lengthS` = 73.0——`musicAssets.test.ts` 的
`checkLength` 把它钉在文件真实时长上，所以这两个数漂开是红的。

#### 二、音量：BGM 总线默认 0.5 → **0.2**

「背景音乐应该是按钮音效的一半」这句话，量下去比听起来更严重——**床当时比按钮音还响**：

| | 交付峰值 | 交付中频 RMS |
|---|---|---|
| `sfx.ui.tap`（文件 −18.85 dBFS × 目录 gain 1.0 × sfx 总线 0.8） | −20.79 dBFS | −37.17 dBFS |
| 大厅床（文件 −29.00 dBFS × 轨 gain 1.0 × bgm 总线 **0.5**） | −16.25 dBFS | **−35.02 dBFS** |

即 RMS 上高 **2.2 dB**、峰值上高 **4.5 dB**。滑杆写着 0.5 vs 0.8 看起来已经"低一截"，那是个错觉：
两根滑杆管的是量级完全不同的信号（一条 73 秒的床 vs 一记 43 毫秒的点击），**滑杆数字之间的比值
不是响度之间的比值**。这也是 §0.5 那张头顶余量梯子看不见的东西——它比的是「床的 RMS」和
「cue 的**峰值**」，那个比较回答的是**能不能听清 cue**，不是**床吵不吵**。

**0.2**：床交付 −42.98 dBFS，落在 tap 下面 **5.8 dB**，就是要求的一半。

**改的是总线，不是资产**，两条理由：文件电平 −29 dBFS 是 `audit.py` 的 `music` 门禁钉住的参照
（窗口 [−30, −28]，也是两条轨之间唯一可比的东西），把混音决定挪进资产会同时废掉门禁和它的推导；
而总线正好还是**玩家自己能拖的那一根**，默认值本来就该是「默认的混音」而不是「唯一的混音」。
副作用是头顶余量整体 +7.96 dB，最紧的 `sfx.unit.attack` 从 +10.3 走到 **+18.2 dB**——§0.5 那条
「最轻但必须听清的 cue 站在床上方 10 dB」于是从**绑定条件**变成一条**下限**（不许再掉回去）。

> 这个数住在**两个**文件里：`audioSettings.ts` 的 `DEFAULT_AUDIO_SETTINGS.bgm`（玩家看见的滑杆）
> 和 `ContextAudioBus.DEFAULT_MUSIC_VOLUME`（设置装上之前就要有一个值）。这一次改动要动两处，
> 于是顺手在 `musicWiring.test.ts` 里断言两者相等——一对只会同时改的常数，没有断言就是等着漂。

#### 三、管线这边新增的（`tools/audio-pipeline/process_music.py`）

- `time_stretch()`——相位声码器，2048 窗 / 512 跳，带 **identity phase locking**（Laroche & Dolson）：
  只有谱峰推进自己的相位，其余 bin 钉在所属峰的旋转上。没有它，一条被拉伸的木吉他床会染上
  这个算法标志性的"空、带合唱味"的相位感——每个 bin 各漂各的，一根被拨响的弦听起来会像好几根。
- `load_master()`——上面那条「先放慢，再选区段」的顺序住在这里。
- `speed` 进 `TRACKS`、进 `credits.json`（母带**加上对它做了什么**才是可复现记录：同一个区段、
  错的速度，切出来是另一条歌，而其余每一条规则都照样绿）。`MUSIC_BUS_GAIN` 跟着总线改成 0.2。

**门禁**：`selftest.py` 新增 7 例（时长比 / 音高不动 / 电平不动 / 双音都活着 / `speed=1.0` 是真
no-op / `peak_regions` 的两个已知答案），共 112 例全过；`musicAssets.test.ts` 新增 `speed` 字段 +
2 个变异（记录里没有速度、`length_s` 停在上一次切的长度），共 43 例全过；`musicWiring.test.ts`
新增两个默认值必须相等。

#### 四、听不了就得量

在同一段素材（母带 60–120 s）上拉伸前后对比，三个数字回答三个不同的问题——

| 问的是 | 0.8x 那轮 | 0.7x 这轮 |
|---|---|---|
| 真的变慢了吗 | 包络压回去再相关：**+0.744 @1.25**，不压回去 +0.093 | **+0.650 @1.4286**，1.0 只有 +0.133、1.2 只有 +0.161、1.6 只有 +0.137 |
| 音高动了吗 | 质心 1038.2 → **1009.6 Hz**（重采样会读到 830.6） | 质心 1038.2 → **1020.4 Hz**（重采样会读到 **726.8**） |
| 代价是什么 | 整体 RMS −16.90 → −16.99 dB；最强低音分音周围可测涂抹（133–162 Hz +4.8 dB / 109–133 Hz −2.9 dB） | −16.90 → −17.04 dB；同一处涂抹 133–162 Hz **+4.0 dB**、40–49 Hz −4.1 dB |

**相关性只在正确的比率上尖起来**（其余比率都掉到 +0.16 以下），所以时间轴是逐事件映射的，
不是"听起来慢一点"。涂抹的位置两轮一样、幅度没有随拉伸变大——那是这条素材最强低音分音的边带，
也是这个方法的固有代价。

**这一轮仍然没有验证到什么**：0.7x + 总线 0.2 这一版**还没有人听过**（改完于本次会话）。
微信侧不需要任何改动——文件就是文件，两个 deck 的实现一行没动。
...
- **默认**：`master 1` / `bgm 0.2` / `sfx 0.8`，不静音（`DEFAULT_AUDIO_SETTINGS`）。**那个 `bgm` 是发货资产电平的另一半**：轨被归一到 250–2000 Hz RMS = −29 dBFS，交付出去的是 `−29 dBFS × 它`。改这个默认值而不重切资产，就是把整条床相对于 cue 集平移——**2026-09-05 就是这么从 0.5 挪到 0.2 的**（§0.6：实测那时床比 `sfx.ui.tap` 还响 2.2 dB；混音决定放在总线上而不是资产里，因为文件电平是门禁钉住的参照，而总线还是玩家自己能拖的那一根）。`ContextAudioBus.DEFAULT_MUSIC_VOLUME` 是同一个数的第二份（设置装上之前要用），`musicWiring.test.ts` 断言两者相等。
...
- **宿主强制静音（`setAudioSuspended`，2026-09-04 新增）**：一个**不持久化、不覆盖玩家自己那个 `muted`** 的第二个静音位，实际增益 = `(muted || suspended) ? 0 : master × 通道`。唯一使用者是 `CrazyGamesPlatform`：门户 QA 明确要求广告播放期间游戏静音（我们的床压在广告音上，正是它要抓的），所以 `requestAd` 的 `adStarted` 打开它，四条退出路径（finish / error / throw / 超时）都关上它。**为什么不复用 `setAudioMuted(true)`**：那一条会写盘，一次崩在回调里的广告就会让 `nw_audio.muted` 留在磁盘上，玩家下次启动一片安静且不知道为什么；而且它会抹掉玩家自己设的静音——广告结束后"帮"他把声音打开。这两个坑都不是理论上的，是同一个函数在两个语义上被复用的常见结果，所以它是独立的一位。**这也是激励视频补超时兜底的原因**：在有静音之前，卡死的 SDK 只是让 DailyScene 一直转圈；有了静音之后，它会让**整个会话**哑掉。回归测试 `client/test/crazyGamesPortalIsolation.test.ts`。
