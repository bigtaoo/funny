...
> 全服一个 SLG 赛季（60 天 ≈ 2 个月），`SETTLE_REWARDS` 一共**发出多少持久材料/金币**？折算成**月度金币当量**后，落在 `ECONOMY_NUMBERS §6.1` 预算的什么位置？会不会显著抬高 F2P 养成速度、稀释养成深度？

### 2.2 聚合公式

settle 奖励按**排名主体**（sect / family / solo，见 SLG_DESIGN §8.3）发放。每条线先算「单赛季单 shard 发放总量」，再乘 shard 数、除以赛季月数：
...
recipients(tier):
  ...
  top3        = 2          (rank 2,3)
  top10       = 7          (rank 4..10)
  participant = R - 10     (R = 该 shard 参与结算的主体数)

capitalMult(tier) = 占该档的主体里持有中原首府(CENTER_CAPITAL_IDX=9)的比例加权 ×CENTER_CAPITAL_MULT
                    （保守上界：按 champion 必占首府算，即 champion 档 ×2）

全服月度材料当量
  = (Σ_shard M_mat) / SEASON_MONTHS        // SEASON_MONTHS = 60d/30 ≈ 2
月度金币当量
  = Σ_mat ( 全服月度材料当量(mat) × 材料→金币估值(mat) ) + settle 月度 coin 当量
```

> ⚠️ **建模第一关键变量 = `recipients` 的「主体 vs 人头」口径**。`participant` 档若**按宗门主体**发（一个宗门发一份），总量是百级；若**按每个成员人头**发，总量乘以宗门人数（数十倍）。**核验前必须先在 SLG_DESIGN §8.3 把这一口径钉死并在本文记录**，否则结论差一个数量级。当前 `settleSeason` 实现按主体 grant（§17.5 发奖循环对 ranking 条目逐条发），建模按此口径，但需复核 participant 是否真的逐人发邮件。

### 2.3 验收判据（A 轨）

| 判据 | 阈值（提案，待批准） | 说明 |
|---|---|---|
| **材料月度当量占比** | settle 材料折金币当量 ≤ F2P 现有月度材料产出（§3 关卡掉落折算）的 **15%** | settle 是「赛季高光的一次性大奖」，不能成为材料主源、架空体力闸门 |
| **coin 子项** | 默认 **= 0**；若 >0，须 ≤ §6.1 月度预算剩余 headroom，且挤占既有格、不新增龙头 | 任何 coin 走 §2.4 人工签字 |
| **头部倾斜** | champion+top3 拿到的材料 ≤ 全档发放的 **40%**；participant 档撑住大盘 | 防「赢者通吃」让中游零获得感；也防头部 ×首府 ×2 叠出超配 |
| **稀释检验** | settle 大奖把「整套装备 +9 / 单位 T9」的鲸鱼级目标拉近不超过 **5%** | settle 不能变相缩短最深氪点（ECONOMY_NUMBERS §4/§5 的长期目标） |

> 阈值 15%/40%/5% 是**提案**，跑出真实聚合数后由经济负责人拍板调整并回填本表。
...
```
{
  ...
  subjectsPerShard: 800,      // 每 shard 参与结算的主体数 R（宗门+家族+solo）
  recipientGranularity: 'subject' | 'member',  // §2.2 关键口径
  membersPerSubject: 25,      // member 口径时用
...
**跑法**：至少 3 个场景——`保守`（低活跃/主体口径）、`基准`（预期值）、`激进`（高活跃/人头口径/首府全占），三档都 PASS 才算过；激进档 FAIL 但基准 PASS 则记为「上线后盯紧」（见 §10）。

> 在脚本落地前，**可先用电子表格按 §2.2 公式手算基准档**作为临时门槛——但调参阶段务必补脚本，避免每次改 `SETTLE_REWARDS` 都重描表格。

---

## 4. B 轨——赛季资源产消（国民产出加成 0.10）

`NATION_BONUS_PRODUCTION=0.10` 只加 **本国 Voronoi 区赛季资源产出**（季末清零，不入持久经济）。核验的是**赛季内节奏**，不进 §6.1 预算。
...
- [ ] **A 轨**：econ-sim（或临时表格）三场景跑完，§2.3 四判据基准档全 PASS；材料→金币估值基准已确立并记录。
