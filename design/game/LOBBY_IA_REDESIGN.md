...
本文 2026-08-17 按 500 行约定拆分。**小节编号一律未变**，源码/文档里既有的 `LOBBY_IA_REDESIGN.md §N` 引用照旧有效——按下表找所在分册。

| 内容 | 文件 |
|---|---|
| 开头 ~ 大厅信息架构重规划（讨论稿） | **本文** |
| 重排落地后的逐条改动：竖排导航、竖屏适配、红点、页签宽高统一等 | [`LOBBY_IA_REDESIGN_LOG.md`](LOBBY_IA_REDESIGN_LOG.md) |
...
**接下页** → [`LOBBY_IA_REDESIGN_LOG.md`](LOBBY_IA_REDESIGN_LOG.md)：重排落地后的逐条改动：竖排导航、竖屏适配、红点、页签宽高统一等。
