...
> 状态：已上线（F1/F2/F4 + 首条 flag `match_bot_fallback` 端到端，见 §8）· **F3（公开 `GET /bootstrap` + 客户端 `FeatureFlags`）+「客户端日志定向采集」(§9) 核心闭环（步骤 1–6）已实现**（见 §8「2026-06-24 · 客户端日志定向采集」）· 权威：本文（feature flag 机制）· 接口契约 → [SERVER_API.md](SERVER_API.md) + `server/contracts/openapi.yml` · **§8「实现记录」已按 ADR-067 拆出到 [`FEATURE_FLAGS_DESIGN_LOG.md`](FEATURE_FLAGS_DESIGN_LOG.md)（本文所有「见 §8」都去那里查）** · 更新：2026-08-24
...
- **入 Loki 约定**：单 stream，**label 仅 `{source="client", kind="anomaly"}`**（低基数），`type/publicId/platform/device/dpr/mem/buildVersion/orient/vp/sinceRot/detail/msg` 一律放**行内**（logfmt）。Grafana：`{source="client",kind="anomaly"} | logfmt | type="webgl_lost"`。
- **设备与朝向归因**（2026-08-24 加，见 §8）：`platform` 是**构建目标**（web/wechat/crazygames），不是设备——手机浏览器与桌面浏览器同报 `platform=web`，因此「只在手机上崩」这类反馈在 Loki 里**无法证实、无法计数**；朝向则从未采集，「转屏时崩」更是完全不可见。新增两组字段：会话级 `device`（`phone|tablet|desktop|unknown`，客户端粗分桶，**不上报 UA 原文**——行内字段既怕高基数也怕隐私）/`dpr`/`mem`（`navigator.deviceMemory`，仅 Chromium，用于区分「只在低内存机型」与「到处都崩」）；事件级 `orient`/`vp`/`sinceRot`。`device`/`orient` 服务端按值白名单（同 `ALLOWED_ANOMALY_TYPES` 的理由：本端点豁免 `allowPublicIds`，任意客户端都能写，不设限即可灌高基数并用近似拼写污染 `| logfmt | device="phone"` 查询）。
...
→ 已按 ADR-067 拆出到 [`FEATURE_FLAGS_DESIGN_LOG.md`](FEATURE_FLAGS_DESIGN_LOG.md)（2026-08-24 拆分；2026-06-24 ~ 2026-08-24 的逐条实现流水，按日期追加）。
本文（§0–§7 + §9）是 feature flag 机制与客户端日志采集的**规格权威**；上文所有「见 §8」均指该分册对应日期的条目。**新的实现记录追加到那边末尾，不要堆回本文**。
