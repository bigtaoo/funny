...
| 项 | 值 |
|---|---|
| 基础回墨 | 2 ink/s（随时间加速倍率缩放） |
| 墨上限 | **100** |
| 基地升级费用 | **30 / 70**（共 2 级升级，基地出生 1 级 → 最高 3 级；2026-07-15 二级费用 50→70 上调） |
| 每级回墨加成 | +1 ink/s（→ 3 / 4 ink/s）；**固定值，不随加速倍率缩放**（2026-07-15 起，见下方修复记录） |
| 基地 HP | **100** |

**2026-07-15 平衡修复**：升级加成此前和基础回墨一起吃时间加速倍率，导致 ×4 阶段（10-13min）差距被放大到 8 ink/s（3级16 vs 1级8），一次性投入80墨换来的收益远超常见卡费（3-14），造成"墨水用不完"的滚雪球体感。现改为只让基础回墨(2 ink/s)吃加速倍率，升级加成始终固定 +1 ink/s/级，任何阶段的等级差距都封顶在 2 ink/s；同时把二级升级费用从50上调到70（累计80→100），拉长回本时间。权威实现见 `server/engine/src/systems/ResourceSystem.ts`。
...
> **与 SLG「队伍体力」是两套东西，不要混**（2026-09-04 新增，见 [`SLG_DESIGN.md` §4.6](SLG_DESIGN.md)）：本节这套是**账号级**、闸 PvE 关卡刷取、可付费补充；队伍体力是**队伍级**（`t1..t5` 各自独立）、闸世界地图出征指令、**不可付费补充**。两者不共享数值、不互相消耗、错误码也不同（`INSUFFICIENT_STAMINA` vs `TEAM_EXHAUSTED`）。

---

## 10b. 队伍体力（SLG 出征频率闸门，2026-09-04）

设计与判据见 [`SLG_DESIGN.md` §4.6](SLG_DESIGN.md)；此处只列数值。

| 项 | 值 | 常量 |
|---|---|---|
| 每队上限 | **100** | `SLG_TEAM_STAMINA_MAX` |
| 每次指令消耗 | **15** | `SLG_TEAM_STAMINA_COST` |
| 自然恢复速率 | **1 点 / 分钟** | `SLG_TEAM_STAMINA_REGEN_PER_MIN` |
| 满额连续出征次数 | **6**（余 10） | 推导 |
| 稳态节奏 | **每队 15 分钟一次**；5 队合计 ≈ **20 次/小时**（满额起步 ≈ 33 次） | 推导 |
| 计费范围 | 带队伍的指令：`attack` / `occupy` / `move`（含停留队伍就地再出征） | — |
| 不计费 | 散兵行军 `reinforce` / `sweep`（不挂队伍，受兵力池+训练时间约束） | — |
| 错误码 | `TEAM_EXHAUSTED`（HTTP 409） | — |
| 付费补充 | **无**（刻意不做——它是频率闸门，可付费绕过就等于没有） | — |

**实现说明**：`playerWorld.teamState[teamId].{stamina, staminaAt}` 存检查点对，`regenTeamStamina` 懒折算（无定时任务）；两字段皆缺 = 从未出征 = 满额（也是历史存档迁移路径）。均为 **[DRAFT]**，待经济核验后调参。

---

## 10c. 领地驻军恢复（2026-09-04）

设计与判据见 [`SLG_DESIGN.md` §5.6](SLG_DESIGN.md)；此处只列数值。

| 项 | 值 | 常量 |
|---|---|---|
| 固有守军基线 | **120 × 等级**（1 级 120 → 10 级 1200） | `tileGarrisonBaseline` = `npcGarrison` |
| 满额恢复时长 | **5 分钟**（线性；残血按比例更快） | `TILE_GARRISON_REGEN_MS` |
| 恢复上限 | 基线。增援超出基线的部分**不恢复也不衰减** | — |
| 可退回兵力池 | 仅 **stored**（占领付的 + 增援），恢复出的民兵不可提现 | — |
| 不恢复的格 | 未占领格（走 `npcGarrison`，不落库）、主城锚点、主城 ring 格 | — |

**为何 5 分钟**：与 `CARD_INJURY_DURATION_MS`（卡牌受伤）刻意同值——攻方卡牌被打光的锁定时长恰等于目标格站起来的时长，所以连续猛攻同一块地是被**攻方自己的恢复**限速，而不是靠目标一直残废。均为 **[DRAFT]**。
