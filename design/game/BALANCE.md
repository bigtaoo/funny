...
> 状态：实现中 · **权威：`server/engine/src/config.ts`（`@nw/engine`；本文是它的快照，非权威）** · 更新：2026-06-22
>
...
| 弓箭兵 Archer | 35 | 22 | 1.4s | 1.1 | 2（远程，投射物 14 格/s） | 1 | 350 |

### 5.2 PvE 专属单位（无卡牌 → 永不进 PvP 池）
| 单位 | HP | 攻 | 攻击间隔 | 移速 | 射程 | 半径 | 特性 |
|---|---|---|---|---|---|---|---|
| 重甲 Ironclad | 290 | 10 | 1.5s | 0.5 | 1 | 520 | 抗箭肉盾 |
| 疾行 Runner | 30 | 9 | 0.7s | 1.9 | 1 | 250 | 快脆，密集冲锋 |
| 哈耳庇厄 Harpy | 26 | 8 | 0.9s | 2.2 | 1 | 210 | flying（仅弓兵/箭塔可打），无视阻挡 |
| 医护 Medic | 90 | 0 | — | 0.55 | 0 | 440 | aura_heal 半径 2、8 HP/s，无攻击 |
| 狂战 Berserker | 110 | 18 | 1.1s | 1.1 | 1 | 420 | HP<40% 攻速 ×1.5 |
| 分裂 Splitter | 65 | 7 | 1.0s | 0.8 | 1 | 470 | 死亡生成 Runner ×2 |
...
| 箭塔 ArrowTower | 120 | 15 | 1.5s | **2 格** | —（可打飞行，投射物 14 格/s） |
