...
| 11 | socialsvc `leaveFamily`/`kickMember` 并发双发会重复 `$inc memberCount:-1`（同一成员被删两次判定，实际只删一次）；worldsvc `resetSeason` 未清理 ADR-051 的 `occ`/`cover` Redis 索引；auctionsvc `scanAnomalies` 的 5000 条上限无排序无告警，可静默漏检最近成交 | `socialsvc/src/familyService.ts`、`worldsvc/src/{core/push,season}.ts`、`auctionsvc/src/auctionService.ts` | deleteOne 的 `deletedCount` 门控 decrement；`WorldCorePush.clearSpatialIndexes`；`scanAnomalies` 按 soldAt desc 排序（新索引）+ 命中上限时告警 | 中 |
...
| 14 | worldsvc `getMarches`/`getStationed` 每次调用都拉全服在途行军/驻防再在 JS 里按视野过滤；`computeMarchPath` 每次行军做 3 次缺索引支撑的近全表 `tiles` 扫描 | `worldsvc/src/{combatMarch,db,core/helpers}.ts` | 两步：①`tiles` 补 `{worldId,type}` + 稀疏 `{worldId,'structure.kind'}` 索引，覆盖 computeMarchPath 的关口/被占主城/阻挡建筑三次扫描；②`MarchDoc` 补 `minX/maxX/minY/maxY`（leg 两端点 bbox，一次性写入，swap 不变，故 recall 不用重算）+ 复合索引，`getMarches`/`getStationed` 敌方分支改用调用方"领地/视野包围盒"（由已算好的 `computeVisionSources` 结果推出，非重新查询）下推进 Mongo 查询，敌方 `isInVision` 精确过滤只在 bbox 缩小后的结果集上跑；`getStationed` 去掉低效的 `{$ne:accountId}`，改包围盒范围 + 内存排除自身。历史 march 文档缺 bbox 字段的懒迁移见 server.md 对应条目 | 中 |
...
| 16 | matchsvc M5（audit-followup-fixes-0730 复查发现）：`NetSession.ts` 收到 `prematch_lost{context:'queue'}` 时无条件 `createRanked(this.lastRankedDeck)` 自愈重新排队，没检查 `this.game`（是否已连上数据面、正在打真实对局）——延迟到达的过期推送能在对局进行中把玩家悄悄拽回排队 | `client/src/net/NetSession.ts` | routeControl 的 `preMatchLost` 分支拆出 `context==='queue' && this.game` 专用 no-op 分支（优先于原自愈分支判断），`this.game` 已连接时既不重新排队也不弹房间错误（避免落入原本给 room/无 deck 兜底用的 else 分支，误弹"房间已丢失"覆盖正在进行的对局） | 中 |
...
- ~~**M5（audit-followup-fixes-0730 复查发现）**：`NetSession.ts` 收到 `prematch_lost{context:'queue'}` 时无条件 `createRanked(this.lastRankedDeck)` 自愈重新排队，没有检查 `this.game`~~。**已于 2026-07-30 以独立分支 `feat/known-gap-fixes-0730` 解决**（见上方已修复表第 16 条），回归测试 `client/test/net-session-prematch-lost-in-match.test.ts` 已由 `it.fails` 转为常规 `it` 并转绿。
...
## 三次复核：matchsvc-prematch-persist 的崩溃窗口（2026-07-30 追加，audit-followup-fixes-0730）

> 上面"二次复核"已经对 `matchsvc-prematch-persist`/`worldsvc-march-query-opt`/`siegeengine-worker-threads`/`gateway-conn-ratelimit` 四个同日跟进分支做过一轮复查，`matchsvc-prematch-persist` 当时判定"文档描述与代码一致，未发现问题"。这轮是**第三次**独立复查，专门针对它——这是四个分支里架构最复杂的一个（新增 Redis 写透传 + 重启 rehydrate + 两种新推送类型），值得比其余三个多一轮审视。

- **问题**：`Matchmaking.tick()` 的配对路径、`roomReady`/`roomStart` 的双人就绪自动开局、`duelRespond` 的接受分支，都是"内存配对 → fire-and-forget 删 Redis 镜像（`void deleteQueueEntry/deleteRoom/deleteDuelInvite(...)`）→ 立即 `onPair`/`startMatch` 推送 `match_found`"。这三步之间没有任何顺序保证——Redis 删除是异步 I/O，`push()`（对 gateway 的 HTTP 调用）也是异步 I/O，两者各自何时真正落地互不相干。如果 matchsvc 恰好在"已经把 match_found 推给玩家（甚至玩家已经连上 gameserver）"之后、但"Redis 里的排队/房间/邀请记录还没删掉"之前崩溃重启，`rehydrate()` 无法区分"这条记录是真的还在等"还是"配对已经发生只是清理还没落地"——会把已经进入对局的玩家重新塞回排队/房间，可能被二次配对（同一个对手，或撮合到完全不相关的第三方），或对一个已经在打的玩家再推一次 `match_found`。
- **修复**：`Matchmaking.remove()`/`enqueue()`/`tick()` 改为 `async`，配对分支在调用 `onPair` 之前用 `await Promise.all(...)` 等齐每个出队账号的 `onDequeued` 完成（bot-fallback 超时分支的 `onTimeout` 同理）；`Matchsvc` 侧 `destroyRoom()`/`duelRespond()` 接受分支相应 await 各自的 Redis 删除，再调用 `startMatch()`。效果：只要 `match_found`/`match_bot`/duel 的推送已经发出，Redis 侧的镜像**必定**已经删除干净——matchsvc 任何时刻崩溃重启，`rehydrate()` 看到的状态都如实反映"是否真的通知过客户端"，不会出现"通知过了但 Redis 还认为在排队"的中间态。`onDequeued`/`onTimeout` 的回调类型相应放宽为可返回 `Promise<void>`；不在这条关键路径上的调用点（`roomLeave`、`onDisconnected`、reap 定时器、鞋 duel 的 decline/超时分支）保持 fire-and-forget 不变，只有真正"删除后紧跟着一次不可撤回推送"的路径改成等待。`server/matchsvc/src/internalHttp.ts` 的四个路由（`/mm/room/ready`、`/mm/room/start`、`/mm/queue/enqueue`、`/mm/duel/respond`）相应加 `await`（外层本就是 async IIFE，无需改动结构）。
- **回归测试**：`matchsvc/test/matchmaking.test.ts` 新增"onPair never fires until onDequeued has actually resolved for both paired accounts"——用一个手动控制的 pending Promise 卡住 `onDequeued`，验证配对回调在它 resolve 前绝不触发；对回退版本验证过确实会失败（`pairs` 在 Redis 删除"还没完成"时就已经是 `[['a','b']]`，而不是预期的 `[]`）。其余 `matchmaking.test.ts`/`matchsvc.test.ts`/`rehydrate.test.ts` 既有用例因 `enqueue`/`roomReady`/`roomStart`/`duelRespond` 变成 async 而补 `await`（bot-fallback 的假时钟用例额外把 `vi.advanceTimersByTime` 换成 `vi.advanceTimersByTimeAsync`，因为定时器回调本身现在是异步链，前者不会等它跑完）；`rehydrate.test.ts` 里已有的 `flush()`（macrotask 边界）helper 恰好也能正确等到新的 async 配对链完成，无需额外改动。全量 `matchsvc` 108 例回归全绿。
