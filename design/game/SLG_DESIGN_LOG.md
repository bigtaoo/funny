...
**追加（用户拍板，同日）**：用户反馈"为了省一点算力却导致无法排查问题本身就得不偿失，希望这类问题永远可以追溯"——把 ①里 `shouldUseCheapSiege` 短路分支的 `replay = null` 全部改为**保留**已经算好的 `{ seed, attackerArmy, defenderConfig, tileLevel }`，四处调用点全改（`arrival.ts` 的 `applySiege`/`applyStrongholdSiege`/`applyCrossingSiege` + `encounter.ts` 的字段遭遇战）；`recordSiege` 落库时这些字段本来就已经在 if/else 分支之前算好，只是原来在 cheap 分支里被覆盖成 `null`，改动本身零性能成本。**真正的引擎崩溃分支（`catch`）保持 `replay = null` 不变**——引擎在这组输入上真的抛过异常，客户端重放大概率原样崩溃，且崩溃原因本来就已经进了 `console.error`，不存在"查不出来"的问题，两者风险性质不同，未一并改动。

副作用（已知且接受）：`db.ts` `SiegeDoc.seed` 注释、`combatDefense.ts` `listSieges` 的 `hasReplay` 注释都已更新说明——客户端重放是拿 seed+army 从零重新跑一遍引擎（"pure presentation, not authoritative"，[`db.ts:409`](../../server/worldsvc/src/db.ts)已有此说明，此前仅用于 mid-season engineVersion drift 场景），cheap 路径的战报现在也会存 replay，重放出来的胜负可能跟当时落地结算用的线性公式结果不一致——判定为可接受的既有风险类别的自然扩展，不是新引入的 bug。`applySweep`（无 owner 中立地块清剿）未改动：它从不构造 army 编队（纯兵力数字对线性公式），没有可存的 replay 输入，跟这次的场景不是同一回事。另发现 `applyBaseSiege`（主基地按波次结算，[`arrival.ts`](../../server/worldsvc/src/combatSiege/arrival.ts) 229 行起）原本就无条件存最后一波的 replay（连引擎崩溃那一波也存），跟这次统一的"崩溃不存"策略不一致——是本轮改动之前就存在的既有行为，未改动，留作后续如需统一时再处理。

**验证（追加）**：`server/worldsvc` `tsc --noEmit` 全绿；更新 4 处受影响的既有 e2e 断言（`siege.e2e.test.ts` ×2、`stronghold.e2e.test.ts`、`passage.e2e.test.ts`——原先断言 cheap 路径"不存 seed/attackerArmy"，现改为断言"存"）；受影响的 6 个测试文件（`siege`/`stronghold`/`passage`/`base-siege`/`field-encounter`/`siege-cheap-fallback`）44 例、`server/worldsvc` 全量 47 文件/373 例均全绿。
