...
| 日期式 | 2026-08-08 起，放弃顺序编号改用日期标题（**册名是起点，不是覆盖范围**，见下） | [`SLG_LOG_2026-08.md`](SLG_LOG_2026-08.md) |
...
>
> **`SLG_LOG_2026-08.md` 这个名字里的日期是这一册开始的时间，不是它覆盖的范围**——它今天收着 2026-09 的条目，那是对的。滚新册的触发条件是行数（ADR-067 的 500 行），切口挑在语义接缝上；滚的时候记得把别处正文里「`SLG_LOG_2026-08.md` §X」这类引用一起挪，**那是纯文字，`checkDocLinks` 的链接检查看不见它**。
