...
## 13-SLG-STRONGHOLD. 险地生成 / 掠夺数值核验（§21.4 STRONGHOLD_*）`[全图口径已修复·CLOSED 2026-07-02，已用 500×500+等级10 重跑 2026-07-05，战力模拟补测 2026-07-16；分环带公平性 2026-07-22 发现 NEEDS ATTENTION，见 .6；⚠️ 1500×1500+TROOP_CAP_BASE=10000 重跑 2026-07-27 起 ③持久稀释判据、.5 新手开放门槛均转 NEEDS ATTENTION/FAIL，见 .2b/.5]`
...
>
> **⚠️→✅已重跑（2026-07-27，ADR-049 1500×1500 + ADR-048 TROOP_CAP_BASE=10000）**：本条 2026-07-22 记录的担忧已用 `strongholdRun.ts`/`strongholdCombatRun.ts` 实测确认——①②密度/blob 逐格比率量确认不受地图尺寸影响（仍 PASS），但**③持久 binding 稀释确实突破 15% 红线**（新数字见 §13-SLG-STRONGHOLD.2b），**§13-SLG-STRONGHOLD.5 的"新手开放门槛"结论也已失效**（新手初始 `TROOP_CAP_BASE=10000` > 险地/关隘守军，开局即 100% 胜率，见 .5 重跑记录）。**本节标题的"CLOSED"仅对①②/战力模拟的"能不能打下"结论成立；③持久稀释判据与.5 开放门槛判据均为 NEEDS ATTENTION，不要引用为已过线。**
>
...
### 13-SLG-STRONGHOLD.2b 重跑（100 种子，2026-07-27，ADR-049 地图 1500×1500）`[⚠️ ③ NEEDS ATTENTION — 突破 15% 红线]`

> 触发原因：ADR-049（2026-07-22）把地图从 500×500 放大到 1500×1500（面积 ×9），`strongholdRun.ts` 的 `SLG_MAP_W/H` 动态跟随自动扩大险地总量，但 `SLG_WORLD_CAPACITY_TARGET`（参与摊薄的人数）没有跟着涨——§13-SLG-STRONGHOLD 顶部 2026-07-22 记录的担忧本轮已用真实脚本证实成立。

**① 险地数量分布**（2,250,000 格地图）：
...
- **①b 分环带**（沿用 §13-SLG-STRONGHOLD.6 的按环带口径，本次一并测）：outer 命中率 0.1464%、resource 0.0641%（比值 2.28×，与旧版 2.46× 同量级）、**core 环带险地数仍恒为 0**——§13-SLG-STRONGHOLD.6 的根因（`strongholdMinDistRatio=0.25` 恒大于核心州半径 `PROVINCE_CORE_RADIUS_RATIO=0.11`）与地图尺寸无关，1500×1500 下依旧成立，**仍是 NEEDS ATTENTION（未在本轮修复范围内，见 .6）**。

**③ 持久 binding 龙头 vs A 轨稀疏（判据 15%）——突破红线**：
...
> 根因：险地总数按**地图面积**线性放大（9×，567→2,817 中位），而摊薄分母（`SLG_WORLD_CAPACITY_TARGET`=400 玩家、季常规刷量 504 binding/季）完全没变——险地数暴涨但人数没涨，人均可获得的持久 binding 龙头随之暴涨。旧版（500×500）在等级上限翻倍（5→10）后余量已从 2.8% 收窄到 12.6%（仅剩 2.4pp 安全边际），这次地图尺寸再扩 9× 把仅剩的边际直接击穿——即使最保守的「中位世界 + 仅 25% 占领率」也只剩 1pp 余量，p90/max 世界在 25% 占领率下已经 FAIL，50%/100% 占领率下全线大幅超标（27.9%–65.2%）。
> **[NEEDS ATTENTION，非本轮范围内可单方面调参解决]**——这是一处真实的经济红线突破，不是场景校准误差。可能的修复方向（供经济/SLG 玩法负责人拍板，本次审计不擅自改）：① 险地密度不再与地图面积线性绑定（如把 `strongholdThreshold` 或某个密度系数按地图面积反向缩放，让绝对数量不再随尺寸暴涨）；② `SLG_WORLD_CAPACITY_TARGET` 等比例上调以匹配更大地图（但这改变的是别的容量设计意图，非经济参数）；③ 下调 `STRONGHOLD_LOOT_MATERIAL_PER_LEVEL`（当前 4/级，40/满级）以压低单次掠夺的持久材料量。已 `spawn_task` 登记为独立后续项，跟踪标题「险地持久 binding 稀释在 1500×1500 地图下突破 15% 红线，需数值/密度重新拍板」。

### 13-SLG-STRONGHOLD.3 结论

- **判决（500×500+等级10 重跑，2026-07-05）**：①密度/方差 **✅ PASS**（中位 0.23%，CV 0.05，0% 零险地，504→636 跨度）；②blob 聚团 **✅ PASS**（平均 blob 1.0 格）；③持久龙头 **✅ PASS**（全种子 × 满占领率稀释 ≤ 12.6% ≪ 15%，但比旧版 2.8% 余量明显收窄——等级上限翻倍是主因，非地图尺寸）；④季资源/守军 **✅ sane**。
- **判决（1500×1500 重跑，2026-07-27，见 §13-SLG-STRONGHOLD.2b）**：①②仍 **✅ PASS**（逐格比率量不受地图尺寸影响）；**③持久龙头 ❌ NEEDS ATTENTION**——险地总量随地图面积 9× 线性放大而摊薄人数不变，稀释峰值从 12.6% 恶化到 65.2%（100% 占领率），**即便最保守场景（中位世界+25%占领率）也只剩 1pp 安全边际（14.0% vs 15% 红线）**；④仍 sane。**STRONGHOLD TRACK 的①②/战力子项仍 CLOSED，但③持久稀释判据整体降级为 NEEDS ATTENTION，不算过线**——需经济/SLG 玩法负责人对密度-容量联动关系重新拍板（见 .2b 建议方向）。
...
- **⚠️ 2026-07-27 更新（ADR-049 1500×1500 重跑，见 §13-SLG-STRONGHOLD.2b）**：地图面积 ×9 使险地总量线性暴涨（567→2,817 中位）而摊薄人数不变，稀释峰值恶化到 **65.2%（100% 占领率）**，即使最保守档也仅剩 1pp 余量——**当前常量组合下这条红线已不再安全，是待处理项，不要引用旧版 12.6% 作为当前有效结论**。
