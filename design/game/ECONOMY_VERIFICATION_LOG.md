...
## 13-SLG-STRONGHOLD. 险地生成 / 掠夺数值核验（§21.4 STRONGHOLD_*）`[全图口径已修复·CLOSED 2026-07-02，已用 500×500+等级10 重跑 2026-07-05，战力模拟补测 2026-07-16；分环带公平性 2026-07-22 发现 NEEDS ATTENTION，见 .6；1500×1500 重跑发现③破线，材料掉落再校准·已修复 CLOSED 2026-07-27，见 .2b]`
...
>
> **✅ ADR-049（2026-07-22）地图放大到 1500×1500 后已于 2026-07-27 重跑并修复**：①②仍 PASS（逐格比率量，放大后不变）；③持久 binding 稀释一度因险地绝对数随面积暴涨而突破 15% 红线，已通过下调 `STRONGHOLD_LOOT_MATERIAL_PER_LEVEL`（ADR-052）修复并复核通过——详见 **.2b**。
>
...
| `STRONGHOLD_LOOT_MATERIAL(_PER_LEVEL)` | `binding` / `0.8`（ADR-052，2026-07-27 从 `4` 下调，见 .2b；此前 500×500 时代为 `4`） | 每级持久材料掠夺 → level 10 = **8 binding**（进 SaveData，**持久**） |
...
### 13-SLG-STRONGHOLD.2b ADR-049（1500×1500）重跑：③破线 → 材料掉落再校准（ADR-052，2026-07-27）

> ADR-049（2026-07-22）把 `SLG_MAP_W/H` 从 500 扩到 1500，密度（p≈0.003 逐格 Bernoulli）刻意保持不变、只放大画布——这也意味着险地**绝对数量**随地图**面积**增长，而 `SLG_WORLD_CAPACITY_TARGET`（③稀释判据的摊薄分母，一个跟地图尺寸无关的服务器人口设计常量）没有跟着变。本节在新地图尺寸下用 `strongholdRun.ts` 实测重跑，确认 ADR-049 header 里当时标记的"待重跑"风险坐实，并给出修复。
...
- CV = 0.08（≤0.50 ✅）、0% 零险地世界（✅）——①②仍 PASS，逐格比率量不受地图尺寸影响，符合预期。
- 绝对数量从 500×500 时代的中位 567/max 636 涨到中位 2,817/max 3,286（约 **5×**，非 ADR-049 header 估算的 9×——环形布局下 outer/resource/core 三种州的排除区随面积的实际缩放比不是纯线性，见 .6 已知的分环带不均问题）。

**③ 修复前实测：持久 binding 稀释破线**（沿用 `STRONGHOLD_LOOT_MATERIAL_PER_LEVEL=4`、level10=40 binding/次 的旧值，套用新地图的险地数）：

| 世界 / 占领率 | 25% | 50% | 100% |
|---|---|---|---|
| 中位（2,817） | 14.0% ✅ | 27.9% ❌ | 55.9% ❌ |
| p90（3,116） | 15.5% ❌ | 30.9% ❌ | 61.8% ❌ |
| max（3,286） | 16.3% ❌ | 32.6% ❌ | **65.2% ❌** |

- 除"中位世界+25%占领率"这一最保守格外，**其余 8/9 格全部突破 15% 红线**——比之前预估的"仅最保守场景压线"更严重（p90/max 世界哪怕只 25% 占领率也已破线）。根因：险地数量随地图**面积**（1500²/500²=9×分母，实测约 5×）线性放大，而摊薄分母 `SLG_WORLD_CAPACITY_TARGET=400` 是跟地图尺寸无关的服务器人口设计常量（ADR-032/U4），两者从 ADR-049 起就不再匹配。

**修复方案取舍**（三个方向逐一评估，见任务背景列出的选项）：
...
全 9 格 PASS，max×100%占领的最坏情形余量 **13.0%**（vs 15% 红线，2 个百分点余量），与 500×500+等级10 时代 12.6% 的余量量级一致——③从 CONDITIONAL/破线恢复为 **PASS**，且余量水位对齐旧设计基线，不是勉强擦线过关。
- **决策记录**：见 [`DECISIONS.md`](../DECISIONS.md) ADR-052。`server/shared/test/siege.test.ts` 已同步更新期望值（level10=8）；`server/worldsvc/test/stronghold.e2e.test.ts` 因断言走 `strongholdMaterialLoot()` 动态求值，改常量后无需改动，复跑全绿（6/6）。

### 13-SLG-STRONGHOLD.3 结论

- **判决（500×500+等级10 重跑后，历史值，地图尺寸已于 ADR-049 变为 1500×1500，当前数字见 .2b）**：①密度/方差 **✅ PASS**（中位 0.23%，CV 0.05，0% 零险地，504→636 跨度）；②blob 聚团 **✅ PASS**（平均 blob 1.0 格）；③持久龙头 **✅ PASS**（全种子 × 满占领率稀释 ≤ 12.6% ≪ 15%，但比旧版 2.8% 余量明显收窄——等级上限翻倍是主因，非地图尺寸）；④季资源/守军 **✅ sane**。**STRONGHOLD TRACK 仍 CLOSED**，但 ③ 的红线余量值得记一笔，供未来再涨等级上限时参考。
...
- **2026-07-27 更新**：ADR-049 把地图放大到 1500×1500 后，①②在新尺寸下重跑仍 PASS，但③一度因险地绝对数随面积暴涨而突破 15% 红线（max×100%达 65.2%），已通过下调 `STRONGHOLD_LOOT_MATERIAL_PER_LEVEL`（4→0.8，ADR-052）修复，复跑③恢复 PASS（max×100%=13.0%，余量对齐本节历史基线）。**STRONGHOLD TRACK 保持 CLOSED**，本条历史判决数字（500×500 时代）不再代表当前地图尺寸下的真实状态，详见 §13-SLG-STRONGHOLD.2b。
...
- **2026-07-27 更新（ADR-049→ADR-052）**：地图放大到 1500×1500 后险地绝对数量随面积增长到中位 2,817/max 3,286（约 5×），而摊薄分母 `SLG_WORLD_CAPACITY_TARGET=400` 不变，binding 稀释峰值一度恶化到 **65.2%**（远超 15% 红线）。修复：`STRONGHOLD_LOOT_MATERIAL_PER_LEVEL` 4→0.8（level10 单次掠夺 40→8 binding），不改动地图密度（ADR-049 决策）或人口容量（ADR-032 决策）。修复后峰值回落到 **13.0%**，详见 §13-SLG-STRONGHOLD.2b。
