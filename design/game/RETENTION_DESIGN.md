...
**修复**（`client/src/scenes/DailyScene/panels/#renderCheckin` + `DailyScene.ts`）：把排序从"碰巧"变成"强制"，可签到格独占四条通道，其它格子一条都不许用：
...
1. **不是本页的**：横屏那张截图是 1861 宽的桌面窗口（设计→屏幕缩放 **0.792**），竖屏那张是 422 宽的小窗（**0.320**）。两边的格子**在设计空间里几乎一样大**（280×129 vs 165×132），差的全在缩放。同一台手机**横过来也一样糊**（844×390 → 0.361，日期数字 15.2 CSS px，和竖屏一模一样）。根因是竖屏设计宽度钉死 1080，属 `UI_DESIGN_LOG_2026-09.md` §49.1 记过、至今未做的那件大事。§49.1 那条可读性下限在这一页**一点忙都帮不上**：格内字号是 42/32/20 设计 px，全在下限（手机 20）之上，一个都没被抬；而且下限**只管文字不管图标**，奖励 glyph / 角标金币 / 绿勾全是按格高算的 sprite，没有任何保护。
...
**没做的**：竖屏设计宽度本身（§49.1）；横屏格子也有 ~16% 富余，但字号梯子吃掉大半收益（涨 16% 后日期数字仍吸附在 42 这档不动），暂不动一个报告者说「看着没什么」的版面。完整记录与同轮补上的图标门禁见 `UI_DESIGN_LOG_2026-09.md` §52。
