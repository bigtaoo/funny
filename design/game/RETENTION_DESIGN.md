...
### 10.15 修复：竖屏「每日任务」的领取/已领取按钮，文字压不住背景（2026-08-10）

**背景**：用户截图反馈——竖屏「每日任务」子 Tab，三项任务全部完成、当日奖励已领取后，灰色按钮上的「Claimed today」文字左右都超出按钮背景，看起来像按钮宽度画错了；横屏这个按钮一直是对的。跟 §10.14 是同一天报告模式的姊妹 bug：同一个 `renderDailyTasks`/`renderWeekly` 共用的"字号按 `cardH` 算、按钮宽度按 `cardW` 算"公式，在竖屏下 `cardH`（`PortraitLayout.designHeight` 随屏幕宽高比拉伸到远超横屏）撑大了字号，而 `cardW`（`PortraitLayout.designWidth` 固定 1080）没有跟着变，横屏因为设计高度矮很多，同一字号自然小得多，从未撞到按钮边缘。

**根因**：`renderDailyTasks` 的按钮宽度原来是纯固定比例 `btnW = cardW * 0.45`，跟这个按钮实际要显示的文字（「Claim +N coins」或「Claimed today」）完全无关；字号是按 `btnH`（从而 `cardH`）算的 `snapFont(btnH*0.36)`。横屏 `cardH` 相对 `cardW` 很矮，字号小，`cardW*0.45` 绰绰有余；竖屏 `cardH` 相对被拉得更高，字号变大，同一个 `0.45` 比例就装不下文字了。跟 §10.14 的差异：§10.14 是"文字侵入按钮"（换行没封顶），这次是"文字撑破按钮"（按钮宽度本身没跟着字号变化）——根因同源但表现在两处不同的几何量上。

**修复**：`client/src/scenes/DailyScene.ts#renderDailyTasks` 把按钮宽度从固定 `cardW*0.45` 改成 `Math.max(cardW*0.45, btnLabel.width + btnH*0.5)`——`0.45` 这个比例保留作下限（横屏这类"矮卡片"下继续保持原有宽度/观感，零变化），新增的 `btnLabel.width + padding` 作为"破限"上限，用文字自身实际测出的宽度反推按钮至少要多宽，不管是竖屏字号偏大、还是换成德语等更长的翻译串，都用同一条公式兜底，不用再为某个方向单独调比例。按钮/文字的定位逻辑不变（依然居中于按钮）。

**验证**：临时在 `client/src/entries/web.ts` 加一条 `?dailydebug` query 分支（跟 §10.14 同款手法：绕开登录/后端，直接 `new DailyScene(...)` 灌一份三项任务全完成、`rewardClaimed:true` 的假 `save`），用 Playwright 直连本地 `game-verify2`（9290）截图复现——修复前「Claimed today」左右溢出按钮背景（跟用户截图完全一致），修复后文字完整落在按钮内部；`git stash`/`stash pop` 来回切换 `DailyScene.ts` 各截一次前后对照。验证完删除了这条临时分支（不进正式代码）。`tsc --noEmit` 绿。

**回归测试**：新增 `client/test/ui/dailySceneTasksClaimButtonWidth.ui.ts`（5 例）。跟 §10.14 一样，headless `measureText` mock 按字符数固定给宽度、跟字号无关——"Claimed today"这类正常长度的字符串在 mock 下本来就测不出真实场景里的溢出（`cardW*0.45` 这个下限在 mock 里天然够用，新旧公式在竖屏/横屏下算出的结果一样，不会因为改了公式就变红）。①②竖屏/横屏「已领取」态：断言按钮背景宽度 ≥ label 测得宽度 + padding，且 label 水平居中于按钮（留 3px 容差给 `sketchPanel` 手绘边框的抖动噪声）；③④竖屏/横屏「可领取」态：同一不变量断言在有点击热区（`hits`）的分支上，并确认点击仍能触发 `onClaimDaily`；⑤**判别性用例**：把 `dailyCoinsReward` 换成一个 80 位数字字符串（绕过 TS 的 `number` 类型标注，`t()` 内部只是 `String(v)` 原样拼接，不会被数字的科学计数法折叠），撑出一条 93 字符的超长 label，让它在 mock 下也真正超过 `cardW*0.45` 这个下限，触发公式的"破限"分支——用这一例反证：临时把修复 revert 回旧代码复测，⑤会红（`hit.w` 卡在旧的固定 393px，label 要 783px+）而前 4 例仍然绿（证实前 4 例确实测不出旧 bug，属于"锁定几何不变量"而非"复现真实溢出"，跟 §10.14 的教训一致）；恢复修复后 5 例全部转绿。`npx vitest run --config vitest.ui.config.ts -t DailyScene`（36 例）、`tsc --noEmit` 全量复跑无回归。

### 10.16 调整：签到月历「今天可领的那一格」不是视觉焦点，眼睛先看到里程碑格（2026-09-05）

**背景**：用户截图反馈——打开签到月历，最先看到的是 7/14/21/30 这几个奖励最好的里程碑格，以及左上角已签到的那一片，真正要点的「今天可领」那一格反而要找。用户原话：「当前可以签到的格子应该是最显眼的，现在确实首先看到奖励最好的几个。」

**根因（可量化，不是观感问题）**：三类格子的视觉权重排反了，而且是一格一格长出来的、从没被整体看过一遍：

| 格子 | 填充 | 描边 | 额外通道 |
|---|---|---|---|
| 里程碑（未领） | `0xfaf0c8` 暖黄 | `0x8a7020` **1.8px**（`panelFrame` 烘焙权重吸附到 2.0） | 金币徽章 `+30/+40/…` |
| 已签到 | `0xd0ccc0` ← **全页最深的填充** | `C.line` 1.2 | 绿勾 `0.5em / alpha 0.85` |
| **可签到** | `0xb8e0c0` 淡薄荷 | `C.line` **1.2**（和普通格一样） | 无 |

可签到格只占**一个**通道（填充色），里程碑占**三个**，已签到那片深灰又是全页最重的色块——于是阅读顺序变成「里程碑 → 已签到 → 可签到」，正好倒过来。另外 `isMilestone && isClaimable` 会走一支金色填充 `0xffd88a`，也就是**签到走到第 7/14/21/30 天时，绿色焦点整格消失**（隐藏 bug，一直没人报，因为要连签七天才撞得到）。

**修复**（`client/src/scenes/DailyScene/panels.ts#renderCheckin` + `DailyScene.ts`）：把排序从"碰巧"变成"强制"，可签到格独占四条通道，其它格子一条都不许用：

1. **可签到**：填充 `0x86d29a`（原 `0xb8e0c0` 跟纸面明度太近）、描边 `0x2e7d32` **2.6px**（`panelFrame` 三档烘焙权重 1.2/2.0/2.6 的最粗一档）、**内缩再描一遍**的第二道框（笔记本上"圈重点"的手势，也是超过 2.6px 上限的唯一办法，`fillAlpha: 0`）、数字加粗且改深绿 `0x1b4d20`、格子**放大**并**最后绘制**（压住邻格边线）、**呼吸动效**（scale 1.03↔1.09，周期 1.4s）。里程碑同时是可签到日时也走这一套，金色分支删除——里程碑身份由徽章继续承担。
2. **里程碑（未领）**：降到一条通道。填充 `0xfaf0c8 → 0xf7f0dc`、描边 `0x8a7020@1.8 → 0xc9b47a@1.2`（跟普通格同宽），金币徽章保留但字色 `0x8a7020 → 0x9a8a55`、尺寸 `0.18 → 0.16em`。
3. **已签到**：退回纸面。填充 `0xd0ccc0 → 0xe6e2d8`、绿勾 `0.5em/0.85 → 0.42em/0.6`。
4. **当天已签完**（`nextCheckinDay` 返回 `null`）：原本焦点直接消失、页面又回到"里程碑最亮"。现在把下一格（`claimedDays.length + 1`）标成浅绿 `0xeaf3ec` + `0x8ab89a@2.0`，标题行右侧配一句「明天可领 · 第 N 天」。
5. **标题行 CTA**：可签到时右对齐一句「第 N 天 · 点击领取」（新增 i18n `daily.checkin.claimHint` / `daily.checkin.tomorrowHint`，三语）。30 个格子长得几乎一样，用文字点名比让眼睛去找快；它是冗余提示，跟标题挤到一起时**整条丢弃**（不换行、不压标题）。

**呼吸动效的接线**：`renderCheckin` 把可签到格单独装进一个 `PIXI.Container`（`pivot === position`，子节点保持原有绝对坐标，缩放绕格子中心），循环结束后才 `addChild`，并通过新增的 `DailyPanelCtx.setPulseTarget()` 交给场景；`DailyScene.update(dt)` 只改这一个节点的 `scale`，**不触发 re-render**（每帧重画 30 格的 `PIXI.Text` 正是 `tearDownChildren` 文档里记的 iPad 显存耗尽路径）。`render()` 开头把 target 清成 `null`——那个节点刚被 `tearDownChildren` 销毁，留着悬空引用下一帧就炸。用 scale 不用 alpha：`render/BoardView/bases.ts` 的 idle breathe 在 2026-07-25 已经踩过"alpha 呼吸被读成闪烁/渲染故障"。缩放会让烘好的文字位图糊，所以可签到格的文字改用 `scaledTxt(CHECKIN_PULSE.max)` 按峰值缩放出图，实际显示时只会等比或缩小。热区按 `CHECKIN_PULSE.min` 放大，否则格子边缘会有一圈"看着是绿的、点不动"的死区。

**验证**：worktree 里起 `web-e2e` 靶子（9190）+ 真 Chrome，`__nwE2E.views.showDaily()` 灌四种状态实拍：①第 4 天可领（普通格）②第 7 天可领（里程碑格，确认走绿色而非金色）③当天已签完（确认"明天"提示）④第 6/30 天可领（网格右边缘/下边缘，确认放大后不裁切）。呼吸在真浏览器里实测在 1.03–1.09 之间往返。竖屏因为本机窗口改不了尺寸，改用 headless 数值核对：三语 × 横竖屏共 6 组，打印标题与提示的 `x/width`，确认提示始终右对齐且不与标题相交。

**回归测试**：新增 `client/test/ui/dailySceneCheckinFocus.ui.ts`（8 例）。锁的是**排序关系**而不是具体色值（可签到描边宽度 > 里程碑 > 等于普通格；里程碑填充到普通格的通道距离 < 可签到到普通格的距离；可签到格是最后一个子节点、是唯一被缩放的节点、`pivot === position`），所以调色自由、把层级调回去才会红。唯一的绝对断言是「里程碑同时是可签到日时，填充必须等于普通可签到日的填充」——这条正是上面那个金色隐藏 bug。另有：`update(dt)` 只在区间内动且不换节点、切 Tab 后 `pulseTarget` 归 `null` 且 `update` 不抛（顺带断言那个旧节点确实已被销毁到碰 `.scale` 就抛，说明清空不是可选项）、当天已签完时无焦点格且下一格拿到提示描边、提示不压标题（横竖屏各一次）。`tsc --noEmit -p tsconfig.test.json` 绿，`vitest --config vitest.ui.config.ts test/ui/dailyScene` 42 例全绿。

**美术增补（同日，第二轮）**：在上面五条纯代码通道之上又加了两张手绘图——一支指进格子的弯箭头 + 一圈画在格子背后的放射星芒，都用可签到格那个绿 `0x2e7d32` 烤进 PNG（打包管线新加 `checkinCue` 一档墨色）。位置有三处是实拍之后才定下来、跟原设想相反的：箭头**一律从下方来**（从上方来时箭尾正好压在上一行里程碑格右上角的金币徽章上，第 24 天可领时最明显）、星芒**顶边加 mask**（第一行的向上光线会穿过"签到月历"小标题）、星芒 alpha 压到 `0.4`（`0.5` 时向下的光线把下一行的数字盖糊）。同轮修掉打包脚本一个静默 bug：`pack_tab_icons.cjs` 按"亮度→alpha"取墨，遇到**自带 alpha 通道的源图**（箭头这张就是）会把笔画外的透明像素当成满墨，打出一整块纯色方块；已改成先 `flatten` 到白底。图的 prompt、量化数据和三处位置修正的完整记录：[`../product/checkin-focus-cue-art.md`](../product/checkin-focus-cue-art.md)。新增 `client/test/render/checkinCueArt.test.ts`（4 例）：只烤 `checkinCue` 一档墨、packer 的 `INKS` 与 `panels.ts` 的 `INK_CLAIMABLE` 必须是同一个绿（两处各存一份、编译器和渲染器都不会比对）、两张图保持近正方形、以及星芒中心必须是空的（画成实心日轮就会从格子背后漏出一个绿方块——这条自己解 PNG 调色板 + 逐行反滤波来量）。

**测试补齐（2026-09-05，第三轮）**：前两轮的测试各自只盯住了自己那一半——`dailySceneCheckinFocus.ui.ts` 写在美术之前（只有五条代码通道），`checkinCueArt.test.ts` 量的是两张 PNG 本身（烤进去的绿、方正度、星芒中空），中间那层「`panels.ts` 把它们摆在哪」没有任何断言，而位置恰恰是算术所在。补 4 例进 `dailySceneCheckinFocus.ui.ts`（该文件 8 → 12 例）：

- **箭头的镜像与位置补偿**，左半边（第 3 天）/右半边（第 6 天）各一例。源图指向右下，另一个方向靠翻转 Container 得到，而镜像是绕自身原点缩放的，每次翻转都要补一整个 sprite 宽；写反了箭头就整体差一格宽落到格子另一侧，而且**只在右半边的日期上错**——挑错日子截图什么都看不出来。断言写在箭尾角上（`|scale| === 1` 时局部 `(0,0)` 恰好映射到 `(arrow.x, arrow.y)`），因为 UI harness 把所有 `.png` 换成 1×1 data URI、图永远不解码，节点有 transform 没有 bounds。
- **星芒的顶边 mask**：mask 上沿必须落在「签到月历」小标题下沿之下，**并且**不加 mask 时星芒确实会穿过那条线——少了后半句，把 mask 整个删掉这条也照样绿。
- **两个 cue 都挂在可签到格自己那个容器上**（跟着呼吸、跟着 teardown 一起消失），不是共享的网格容器。
- **当天已签完时一个 cue 都不画**（星芒是网格里唯一带 mask 的节点，所以「零个 mask」就是它的缺席证明——stub 掉的美术让所有 raster 节点长得一模一样，没法按贴图认）。
...
### 10.17 调整：竖屏月历格子被宽度钉死，格内一切跟着缩（2026-09-14）

**背景**：用户截图反馈——「这个页面，横屏时看着没什么，竖屏时就感觉格子里的东西看不清了」，并要求先分析原因，因为别的页面可能同病。

**归因（分两层，只有第二层是本页的）**：

1. **不是本页的**：横屏那张截图是 1861 宽的桌面窗口（设计→屏幕缩放 **0.792**），竖屏那张是 422 宽的小窗（**0.320**）。两边的格子**在设计空间里几乎一样大**（280×129 vs 165×132），差的全在缩放。同一台手机**横过来也一样糊**（844×390 → 0.361，日期数字 15.2 CSS px，和竖屏一模一样）。根因是竖屏设计宽度钉死 1080，属 `UI_DESIGN_LOG_2026-08.md` §49.1 记过、至今未做的那件大事。§49.1 那条可读性下限在这一页**一点忙都帮不上**：格内字号是 42/32/20 设计 px，全在下限（手机 20）之上，一个都没被抬；而且下限**只管文字不管图标**，奖励 glyph / 角标金币 / 绿勾全是按格高算的 sprite，没有任何保护。
2. **是本页的**：`cellH = Math.min(areaH*0.78/ROWS, cellW*0.8)` 里竖屏**一直是右边那项赢**（143 vs 184），格子被宽度钉死；§10.10 加的 spread 逻辑把富余**全摊成行距**——390×844 上每行 159 设计 px 间距对 143 的格子，**网格纵向 48% 是空白**。而格内一切都是 `ch` 的比例（数字 0.32、奖励 0.26/0.24、角标 0.16、绿勾 0.42），**格高是唯一能够到它们的旋钮**，这部分等于白扔。

**修复**（`client/src/scenes/DailyScene/panels/checkin.ts`）：

- **先算高度，纵横比只当封顶**。`gridTop`/`gridAvailH` 提到 `cellH` 之前算，竖屏 `cellH = min((gridAvailH − rowGap×(ROWS−1))/ROWS, cellW × PORTRAIT_MAX_ASPECT)`，剩下的才摊成行距。横屏分支（`min(areaH*0.78/ROWS, cellW*0.8)` + 固定 `h*0.006`）**逐字节不动**。
- `PORTRAIT_MAX_ASPECT = 1.25` 有依据：字号梯子顶在 `FS.display`（60），日期数字是 `ch×0.32`，`ch` 过 ~190 数字就不再长而「+N」还在长，`snapFont` 会把两者都吸到 60，**格内层级当场倒过来**。1.25 让 `ch` 落 206，卡在那之下。
- 效果（390×844，CSS px）：格高 132 → **206 设计 px（+56%）**，行距占比 48% → 19%；日期数字 15.2 → **21.7**、「+N」11.6 → **15.2**、奖励图标 12.3 → **19.1**、里程碑角标 7.2 → **11.6**。
- **taller cell 立刻撞出一处重叠**：里程碑格的两位数日期压在自己的 `+40` 上（14/21/30，重叠 33%）。角标钉右上、按 `ch` 算；数字居中、也按 `ch` 算；**决定它们碰不碰的格宽在两边公式里都不出现**——横屏 2.2:1 有富余所以从没碰过，竖屏新格子 0.8:1 当场就碰。改法是竖屏给角标留一条顶部（`NUM_TOP_FRAC = { landscape: 0.06, portrait: 0.22 }`），**30 格一律留**而不只给带角标的 4 格：30 个数字同基线才还读得出是个网格，taller cell 正好付得起（数字底边 ~0.61 ch，奖励图标顶边 0.66 ch）。这条重叠是**真浏览器巡检报出来的，不是肉眼看出来的**。

**验证**：本机真 Chrome（500×615，缩放 0.320，与报告截图同）改前/改后对比实拍；巡检 `client/portrait-report/phone-390x844/daily.png`（dpr 2）改前/改后各读一张；7 个视口新旧公式数值核对。`tsc` + `lint` + `build:web` 全过。新增 `client/test/ui/dailySceneCheckinCellHeight.ui.ts`（3 例），既有 `dailySceneCheckinRowSpread`(5)/`dailySceneCheckinFocus`(12) 无回归。

**没做的**：竖屏设计宽度本身（§49.1）；横屏格子也有 ~16% 富余，但字号梯子吃掉大半收益（涨 16% 后日期数字仍吸附在 42 这档不动），暂不动一个报告者说「看着没什么」的版面。完整记录与同轮补上的图标门禁见 `UI_DESIGN_LOG_2026-08.md` §52。
