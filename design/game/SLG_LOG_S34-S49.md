...
**未改动**：`WorldMapNet` 的 `loadMapViewport()`/`refreshMarches()`/`refreshWorldChat()`/`refreshMe()` 四个方法本身保留不动——它们在平移换视口、点击后局部刷新等场景下仍被独立调用；只有 `loadData()`（进图那一次性入口）改为调用新的 `enterWorld()` 单次聚合请求。

> **订正（2026-09-09）**：上一段原本写作「……5s 行军轮询（P1-2 已删的是*进图时*那次，其余轮询点未受影响）」，这句话对 `refreshWorldChat()` 是**错的**，且方向反了——P1-2 删的正是那个 5s `setInterval`（唯一的「其余轮询点」），而 `refreshWorldChat()` **只有**那一个调用方。本条写下时它就已经是死代码了，只是当时没核。2026-09-09 已删除该方法与 `net/loaders.ts` 里的同名 loader，详见 [`SLG_LOG_2026-08.md` 2026-09-09 一节](SLG_LOG_2026-08.md)。另外三个方法的表述属实（`loadMapViewport()` 由平移/缩放调用，`refreshMarches()` 由 `applyMarchUpdate`/`applySiegeResult` 推送调用，`refreshMe()` 由 City 叠加层返回等路径调用）。
