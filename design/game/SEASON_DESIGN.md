...
本文 2026-08-17 按 500 行约定拆分。**小节编号一律未变**，源码/文档里既有的 `SEASON_DESIGN.md §N` 引用照旧有效——按下表找所在分册。

| 内容 | 文件 |
|---|---|
| 开头 ~ 天梯赛季 + 战令 + 排行榜设计 — Ranked Season / Battle Pass / Leaderboard | **本文** |
| §13A 赛季时钟+排行榜（SE-1~SE-6）、§13B 战令（SE-7~SE-9）、§14 待定、§15 实现记录 | [`SEASON_DESIGN_IMPL_SPEC.md`](SEASON_DESIGN_IMPL_SPEC.md) |
...
**接下页** → [`SEASON_DESIGN_IMPL_SPEC.md`](SEASON_DESIGN_IMPL_SPEC.md)：§13A 赛季时钟+排行榜（SE-1~SE-6）、§13B 战令（SE-7~SE-9）、§14 待定、§15 实现记录。
