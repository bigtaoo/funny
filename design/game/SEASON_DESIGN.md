...
| **SE-5** | meta：`GET /leaderboard`（Top100 + 我的名次实算 + 称号 join）+ 复合索引；⚠️ 60s 进程缓存仍 TODO（当前每次实查）（2026-07-03：`getLeaderboard` 补 `me` 实算=`countDocuments(elo>myElo & 本季)+1`，未打本季则省略 → 契约 openapi + routes.gen + 客户端 openapi.ts 已同步） | SE-2 | P0 |
