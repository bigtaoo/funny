...
- **链路**：
  ...
  - ✅（2026-07-18）**队列复用给 PvE reject 审核**：`AntiCheatReviewDoc` 加 `kind?:'pvp_overclaim'|'pve_reject'`（缺省即 pvp_overclaim，旧记录不变）+ PvE 专属字段（`levelId`/`claimedStars`/`judgedStars`/`rejectCountAfter`/`severity`）+ 通用的 `resolvedBy`/`resolvedAt`/`resolution`。`pveVerify` 每次 `rejected` 写一条 `kind:'pve_reject'` 记录（详见 PVE_INTEGRITY_PLAN.md §8.6）。新增 `POST /internal/anticheat/reviews/:id/resolve`（meta，只改状态，不封号）→ admin `resolveAntiCheatReview`（`anticheat.action`：`resolution:'banned'` 时先调既有 `suspiciousPve.banAccount` 走 `/internal/accounts/:id/ban`，再标记 review 已处理，全库仅此一条封号执行路径）→ `POST /admin/anticheat/reviews/:id/resolve` → `tools/ops pageSuspicions` 每行加 Dismiss/Ban 按钮（`anticheat.action` 可见）。
