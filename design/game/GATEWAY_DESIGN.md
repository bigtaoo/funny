...
- WS 握手 `?token=<jwt>`（复用 meta 的 JWT，解出 accountId 绑定连接）。
  - **握手失败（4401）不是「socket 没开过」（2026-09-12 修复）**：握手校验发生在 **WS upgrade 完成之后**——gateway 先接受 upgrade，再在 `onConnection()` 里验 token，失败才 `ws.close(4401)`。所以客户端看到的是一次**成功的 open 紧跟一次 close**，`NetClient` 原先在 `onOpen` 里无条件把重连退避计数 `attempt` 清零，退避阶梯永远爬不上去：token 一旦过期，客户端就以约 1 秒一次的频率无限重握手。生产日志 2026-09-10 一次爆发 **265 条 `WS handshake rejected: invalid token ... jwt expired`**（连续间隔 <2s 的有 264 个）。修法两条，都在客户端：①`NetClient` 只有在 socket 存活满 `STABLE_OPEN_MS`（10s）后才把 `attempt` 清零，开了就被拒的连接继续爬阶梯；②`tokenProvider` 新增 `FatalTokenError`——`NetSession.freshToken()` 在「密码登录会话的 JWT 过期、且没有刷新端点」这条不可恢复分支上改为**抛出**它而不是把死 token 原样返回，`NetClient` 收到后直接进 `disconnected` 不再重连（配合已有的 `notifySessionExpired()` → forceLogout）。
  - **服务端只限日志，不限连接（2026-09-12）**：同一源的握手拒绝每 60 秒只详记前 **3** 条，其余折叠成一条汇总（`WS handshake rejections suppressed`，带 `total`/`suppressed`/`reasons`），窗口关闭时由心跳 sweep 刷出——否则一个卡住的客户端就能把 gateway 的日志流淡成白开水（265 条逐字相同的告警与第一条携带的信息量一样多）。**拒绝本身照旧执行**，每一次仍然 4401：一次拒绝的成本只是一次 JWT 验签，不值得防；而鉴权之前手上唯一的 key 是源 IP，拿它封连接会把运营商 NAT 后面的整批真玩家一起打掉。回归：`server/gateway/test/connRegistry-unit.test.ts`。
