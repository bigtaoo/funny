...
```
meta → gateway:  POST /gw/judge { seed, mode, endFrame, frames[], exclude[] }  → { ok, stateHash?, winnerSide?, judgeAccountId? }
                 // frames[].cmds[].commands 为 base64（game.proto opaque bytes）；exclude = 参赛双方 accountId
gateway → judge: ServerMsg.judge_request { request_id, seed, mode, end_frame, frames[] }   // 推给挑中的 canJudge 在线 socket
