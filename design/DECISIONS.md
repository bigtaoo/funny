...
- **决策**：
  ...
  4. **新增玩家主动发起的"出售给系统"入口**：`POST /skins/sell`（`idempotencyKey` 幂等），售价复用已有的 `DUPE_REFUND_COINS[目录稀有度]`（legendary 1500 / epic 400 / rare 50 / common 10，与 GACHA_DESIGN §4.3 早先写的数字一致，不另编新值），走 `commercial.grant` 入账。**明确不做**"抽到重复自动转币"——那是旧设计里从未真正实现的路径，现在的方向是"先给你一个真实道具，卖不卖你自己决定"。
  ...
  7. **客户端**：`SaveData.skinCounts?: Record<string,number>`（GET /save 自动 join；additive-only 字段，`migrate.ts` 的 `fillDefaults` 自动补 `{}`，未升 `SAVE_VERSION`）；`AuctionScene` picker 的 `listableSkins()` 从"未装备"放宽为"未装备，或有多余份数"，卡片新增出售分区（底部热区拆成"拍卖 ›"/"出售 ›"各一半，走 `client.sellSkin()` → `/skins/sell`，`sellBusy` 防双击）。
...
- **决策**：
  ...
  2. **明确排除 bridge/plankway（玩家已拥有的关口）**：`settleOccupation` 结算时把 `type` 硬编码成 `'territory'`，关口走这条路会丢失通行功能；关口易手本就低频/低风险，不值得为此教 `settleOccupation` 认第二种落地类型，故关口攻打保持即时易手不变。主城攻打（打不下来，只触发 `passiveRelocate`）、`target.structure` 血量未耗尽的磨血分支同样不受影响。
  3. **客户端弹窗改为服务器权威判断**：`WorldMapNet.applySiegeResult` 原先只要 `marchKind==='attack'` 就弹"Siege won!"+回放校验 modal；现在攻方胜利时先 `await` 地图刷新，再看目标格的 `contestedByMe`（既有字段，`core/map.ts` `o.contestedBy===accountId` 计算，本次未新增契约字段）——为真（本次胜利只是开了个倒计时，涵盖新的 PvP 占地与既有的驱逐胜利两种场景）则改成跟占中立地一样的轻量 toast（新增 `world.siegeWinHold`）；为假（主城/关口/结构磨血未破防/PvE 据点或关口）维持原 modal。回放入口不受影响，仍可从"Battle replays"列表打开。
...
- **影响**：`server/worldsvc/src/combatSiege/arrival.ts`（新增 `OCCUPY_HOLD_SEC`/`OccupationDoc` 依赖）；`client/src/scenes/worldmap/{WorldMapNet.ts,tileGraphics.ts,WorldMapInput.ts}`；`client/src/scenes/WorldMapScene.ts`（`applySiegeResult` 改为 `void` 调用异步方法）；`client/src/i18n/locales/{zh,en,de}.ts`（新增 `world.siegeWinHold`）；`client/test/ui/worldMapSiegeResultToast.ui.ts`（改为 async/await，新增 hold-toast 覆盖）；`server/worldsvc/test/` 下受影响的 e2e 用例；`design/game/SLG_DESIGN.md` §5.1/§5.3/§5.4.4。
- **补充（同一会话内测试联调发现，用户已确认接受）**：`landSiege` 的新占领倒计时分支给 `OccupationDoc` 写入了出征队伍的 `teamId`（沿用 `applyOccupy`/`startOccupationHold` 同一套字段），带来一个连带效应——带队出征攻打玩家领地的那支队伍，在 5 分钟倒计时期间会被 `combatMarch/command.ts` 的 TEAM_BUSY 门禁（`cols.occupations.findOne({...,teamId})`）判定为忙碌、无法重新派遣；倒计时结算后默认原地驻留（`mode:'idle'`）而非立刻恢复自由，除非该队伍开了 `autoReturn`。这与改动前"赢了攻城队伍立刻恢复自由"的旧行为不同，但与"和占领中立地完全一致"的决策方向吻合，用户确认接受、不做特殊区分。
- **验证**：`server/worldsvc` 全量 vitest **53 文件 / 453 例**全绿（含 `siege.e2e.test.ts`/`teams.e2e.test.ts`/`nation-bonus.e2e.test.ts`/`field-structure-attack.e2e.test.ts` 里改写的占领倒计时断言）+ `tsc --noEmit` 全绿；client `tsc --noEmit` 全绿、`npm run build:web` 全绿、`vitest --config vitest.ui.config.ts` 的 `worldMap*` 套件 **25 文件 / 239 例**全绿（含 `worldMapSiegeResultToast.ui.ts` 新增的占领倒计时 toast 覆盖）。未起真实登录态 + 完整后端栈做浏览器截图（需要双账号真实攻城场景，与自动化 e2e/UI 覆盖的成本不成比例）。
