...
- **决策**（用户拍板）：
  ...
  - **金币来源**：看广告（**按广告收益÷充值比例折等值金币**，数额小、不冲击充值）+ 充值 + 称号/成就一次性（保留龙头）。
  - **皮肤**：common/rare/部分 epic 金币直购；高级 epic + legendary 仅抽卡；活动限定直购。
- **影响**：新增 [`game/ECONOMY_NUMBERS.md`](game/ECONOMY_NUMBERS.md) 为经济/养成数值权威；取代 ECONOMY_BALANCE §2.1 旧"固定 50 coins/广告"。护甲为新引擎机制（待实现）。
