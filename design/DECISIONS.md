...
- **合并后 CI 回归修复（2026-07-29，PR #75）**：`reconcile()` 改整段取云端后，暴露了两处此前被"本地优先合并"掩盖的问题，均在 `full-link E2E (live stack)` 里炸出来：
  ...
  2. **网关地址从未持久化**：`createAppCore.ts` 的 `applyGatewayUrl()` 只更新内存 `state.gatewayUrl`，从未写回 `nw_gateway_ws`；下次纯 token 静默重启（`auth.ts` `resolveEntry()`）在异步 `adoptSession()` 落地前就同步用 `getGatewayWsUrl(storage)` 的兜底推导值（从 API base 推 `/gw` 路径）先连一次——本地/CI 环境 meta 和 gateway 不同源（`:18080` vs `:8086/gw`），推导值是错的，首次连接必然打错端口/路径。旧的"本地优先合并"下这条竞态路径此前从未被这套 3 客户端（A 注册充值改名→B 同账号登录→A 快照重启）测试真正压中过；改整段取云端后暴露。修复：`net/config.ts` 新增 `persistGatewayWsUrl()`，`applyGatewayUrl()` 每次拿到服务器真值就顺手写回 `nw_gateway_ws`，下次静默重启的同步推导就能读到真实地址，不再需要等首次连接失败后再被动纠正。
