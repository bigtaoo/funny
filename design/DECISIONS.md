...
- **对外名落地**（2026-07-02 补）：HTML `<title>` 已改 `Nivara — Notebook Wars`（主名 Nivara + 副标题 Notebook Wars），四份入口 `client/public/{index,web/index,crazygames/index,wechat/index}.html` 同步。局内 UI 里的 `game.title` 走 i18n 抽 key 为后续任务（记忆 game-name）。
