...
- **验证**：`server/metaserver/test/skin.e2e.test.ts`（14 例，新增"装备中的多余份可挂拍/可出售，最后一份仍被拒绝"+`sellSkinToSystem` 全套：售价按稀有度、幂等、未拥有 404、装备保护 409）+ `economy.e2e.test.ts` 新增断言（重复皮肤真的铸造第二个实例、`skinCounts` 正确）；metaserver 全量 **67 文件 / 840 例**全绿、shared **36 文件 / 713 例**全绿、auctionsvc **9 文件 / 97 例**全绿（挂单契约不变，零改动即通过）；client `tsc --noEmit` 全绿；`npm run build:web` 全绿；client `vitest run`（**151 文件 / 1221 例**）+ `vitest run --config vitest.ui.config.ts`（**133 文件 / 1241 例**，含 `auctionPickerDedupe.ui.ts` 新增 6 例：放宽后的 `listableSkins`、"×N"标签、`onSell` 接线、双击防抖、真实渲染不崩溃）全绿。
