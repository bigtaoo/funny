...
**其余 24 个超限文件**未动，留作后续任务（完整清单以 `server/scripts/file-length-baseline.json` 为准，这里的行数是入基线时的快照，之后可能有几行漂移）：`metaserver/src/{service/pve.ts(838),service/liveops.ts(742),service/economy.ts(674),economy.ts(682),internal/matchReport.ts(601),cards.ts(574),accounts.ts(528)}`、`worldsvc/src/{db.ts(916),combatSiege/arrival.ts(909),city.ts(682),season.ts(638),combatSiege/occupation.ts(576),combatMarch/command.ts(508)}`、`admin/src/httpApi.ts(712)`、`socialsvc/src/{httpApi.ts(708),familyService.ts(666),friendService.ts(541)}`、`engine/src/{types.ts(622),systems/AISystem.ts(573),systems/CombatSystem.ts(553),campaign/levelSchema.ts(527)}`、`commercial/src/{service/base.ts(571),iap.ts(517)}`、`metaserver/src/paddle.ts(492)`、`metaserver/src/service/auth.ts(495)`。按本次试点的判断标准分类：`db.ts`/`types.ts` 是纯 interface 声明集合，按业务域拆分风险最低（`shared/src/mongo.ts` 已按此思路拆完，见下）；`admin.ts`/`socialsvc httpApi.ts` 是同 `worldsvc/httpApi.ts` 一样的 if-chain 路由，可复用同一套 `RouteCtx` 链式调度写法；`pve.ts`/`liveops.ts`/`combatSiege/arrival.ts`/`occupation.ts` 已经是 mixin 链的一环但自身又长胖了——不因为"已经在链里"就默认续链，按上面的新优先级先评估这层能否拆成独立类组合，只有交叉调用清单列不完时才继续加子链（`combatMarch.ts`/`AnalyticsService` 的"二次拆分"先例仅在这个前提下沿用）。
