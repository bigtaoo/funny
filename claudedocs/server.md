...
- **并发瓶颈整改（2026-09-05，`WORLDSVC_CONCURRENCY_AUDIT_2026-09-05.md`）**：起因是「一个人连发 5 个队伍打地块，第 5 条明显卡」。**不是锁竞争——worldsvc 全仓没有任何锁**；是 A\* 寻路同步跑在唯一的事件循环上，一次**不可达**目标实测冻结整个进程 **2~6 秒**（1500×1500 地图被河流/山脉切成 20 个连通分量，只有 75 个渡口格连通，而渡口只对占领方开放，所以「不可达」是常态而非例外）。连发 5 条 = 每条排在前一条的寻路后面，跟并发量无关，一个人就能触发。
  ...
  - **scheduler**：5 个任务原来共用一个 `setInterval` + 一个 `running` 标志——最慢的任务决定所有任务的节奏。改成各自 interval + 各自守卫 + 各自计时，超出自身 interval 就告警；季节结算单独放到 30s。到期扫描上限改 `NW_SLG_ARRIVAL_SCAN_LIMIT` 且触顶告警（此前静默截断 → 行军默默晚到）。
