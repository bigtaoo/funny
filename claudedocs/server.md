...
- **contract 片段里的 description 别写进 `{...}` 内联映射（2026-09-07 踩到）**：YAML flow mapping 里的**逗号会结束这个值**，后半句变成未知 schema keyword，Fastify strict mode 于是**拒绝构建整张路由表**（现象：metaserver e2e 全红，`unknown keyword: "..."`），真上线就是全服 REST 起不来。含逗号/冒号的描述用块式写法。⚠️ 这类错只在**跑完整 metaserver 测试**时暴露——改完 contract 片段要先 `npm run gen:api:contracts` 重新打包 `openapi.yml`（`pretest` 会做）再跑。（2026-09-09 前这里还得先 `tsc -b`，因为 e2e 读的是 `dist/app.js`；现在 e2e 一律读 `src/`，`tsc -b` 只为 `@nw/shared` 的 dist。）
- **钱包权威**：`SaveData.wallet.coins` 是只读镜像；商业操作经 commercial → meta 编排 → 回推
- **库级凭据隔离（2026-09-12，COMMERCIAL_DESIGN §3.1）**：7 个连库进程各有**自己的 Mongo 用户**，只授 `readWrite` 自己那个库——`nw_world` 开 `notebook_wars_commercial` 直接拿授权错误。单一真相 `server/scripts/mongoDbMap.mjs`（服务→库→用户→环境变量），开号 `server/scripts/provisionMongoUsers.mjs`（`--atlas` 打印 Atlas CLI 命令；自托管直连建号），门禁 `npm run check:dbisolation`（变异测试 `commercial/test/check-db-isolation.test.ts`）。本地栈 mongo 已开 `--auth --keyFile`，`docker/local-up.ps1` 自动建号（先无认证建 root——localhost 例外只放行 `createUser`，探测 `getUser` 会失败——再以 root 建 7 个服务用户，两段必须分两次会话）。建完用 `--verify --env-file=…` 验（连上每个用户，断言读得到自己的库、被其余 6 个拒绝）。**线上已落地并关闭兜底（2026-09-12，ADR-090 补记二）**：7 个 Atlas 用户已建（走 Admin API `--atlas-api`——Atlas 拒绝驱动侧 `createUser`/`CMD_NOT_ALLOWED`，且其用户一律建在 `admin`，`authSource=admin` 而非服务自己的库），容器内 `--verify` 实测 7/7；各服务 `config.ts` 的 `?? NW_MONGO_URI` 已删，缺省退到 `DEV_MONGO_URI`（localhost 主机，借不到任何人的授权），`docker-compose.cloud.yml` 改成硬性 `${自己的:?…}`。⚠ **排错第一条**：服务报 `AtlasError 8000: user is not allowed to do action [createIndex] on [自己的库]` 时，**不是角色配窄了，是容器拿到了别人的连接串**——先 `docker compose -f docker-compose.cloud.yml config | grep MONGO_URI` 看实际值（2026-09-12 就是 VPS 还跑着 ADR-090 之前的 compose）。`readWrite` 单角色足够建索引，别加 `dbAdmin`。控制面凭证在 `D:\secrets` 的 `secrets/infra/atlas.yaml`（admin-only），**不要放 `funny/prod.yaml`**——`push-env.py` 会把整个文件推成 VPS 的 `.env`。
...
- **ESLint（新增 2026-08-26）**：`cd server && npm run lint`（= `eslint .`）。**在这之前 server/ 从来没有过 linter** —— 全仓库只有 `client/` 有，而且那一份也已经坏了很久没人发现（同日复活，见 [`client-testing.md`](client-testing.md)）。意味着规则一直只管着 UI 层，钱、匹配、模拟这三块一直没人看。
  ...
  - **首跑 623 个文件、58 个问题**，没有一个是「代码烂」，分布很说明问题：
    ...
    - **11 个 `any`**：9 个是 engine AI 测试里 `(cmd as any).col` 这种读联合类型字段（改成 `Extract<PlayerCommand, { type: 'play_card' }>`，順手把字段名拼错也变成编译错误）；2 个是 gateway/worldsvc `const mod: any = await import(spec)`（当时改成各自局部的 `IoRedisCtor` 结构类型；**这两个局部别名已于同日被下一条的共享 `RedisCtor` 取代并删除**——当时 gateway 那份注释自己就写了「narrowing `RedisLike` 是另一件事」，那件事就是下一条）。
...
- **Redis 客户端结构化类型（2026-08-26）**：`@nw/shared` 原先是 `export type RedisLike = any`，而这一个别名被 matchsvc / metaserver / commercial / gateway 约 20 个文件当成「Redis 客户端」的类型在用——等于**全服端没有一处 Redis 调用被类型检查**：命令名打错、参数顺序颠倒（`set(key, val, ttl, 'EX')`）统统编译通过，只在运行时炸；而这些调用绝大多数包在 best-effort 的 `catch` 里，连"炸"都是静默的。现改成 `shared/src/redisClient.ts` 里手写的结构化接口：`RedisLike`（**只声明实际被调用的命令、且用实际的调用签名**：get/set/mget/del/keys/expire/pexpire/hget/hset/hdel/hincrby/zadd/zrem/zrange/eval/publish/multi/pipeline）、`RedisMulti`/`RedisPipeline`、`RedisConnection`（= `RedisLike` + on/once/off/subscribe/duplicate/quit/disconnect，给持有连接的 `connect*` 用）、`RedisCtor` + `loadIoRedisCtor()`。风格沿用 worldsvc `WorldRedis` 的「只声明我们调的东西」。
  - **为什么不直接 `import type Redis from 'ioredis'`**：gateway/matchsvc/metaserver/worldsvc 四个包确实在 `package.json` 里声明了 ioredis，但 `@nw/shared` 没有（也不该有——它会被链进完全不碰 Redis 的进程），静态 import 会让没装 ioredis 的地方 `tsc -b` 直接挂。所以「变量 specifier 的动态 import」这条约束**仍然真实**，只是现在收敛成 `loadIoRedisCtor()` 里**唯一一处** cast，它下游全部结构化。
  - **gateway/worldsvc 的 `redis.ts` 保留各自本地的动态 import**（只把 `const mod: any` 换成 `as { default?: RedisCtor } & RedisCtor`），**不要**改成调 `loadIoRedisCtor()`：这两个包的 `redis-unit.test.ts` / `redis.test.ts` 靠 `vi.mock('ioredis')` 注入内存假客户端，而 `@nw/shared` 在 vitest 里是 externalized 的，从它内部发起的 `import('ioredis')` 走 Node 真实 loader，mock 拦不住。
  - **测试假客户端**：几处 in-memory fake（matchsvc `persist.test.ts`/`rehydrate.test.ts`、shared `activeMatch.test.ts`/`dailyCounter.test.ts`）只实现被调到的那几个命令，统一在工厂函数出口处 `as typeof fake & RedisLike` 一次放宽（保留 `vi.fn()` 的 Mock 类型和 `strings`/`zsets`/`fail` 等断言用字段），**而不是给接口成员统一加 `?`**——加 `?` 会让生产侧每个 `redis.xxx()` 都变成"可能 undefined"，把类型收益反手抵消掉。对比 worldsvc `WorldRedis` 的 `del?`/`hmergeJsonField?`：那两个是生产代码里真的会 `if (redis.del)` 分支的可选能力，性质不同。
  - **顺手暴露并修掉的既有错误**：metaserver 4 个 internal 路由测试把 `InternalCtx.redis` 赋成了 `() => { throw ... }`——一个**函数**塞进 `RedisLike | null` 字段，`any` 时代照收；现改为 `null`（这些 suite 都不注册唯一会读它的 matchReport 路由，行为等价）。matchsvc `matchStarter.test.ts` 的 `redis as never` 同理换成 `as unknown as RedisLike`。
  - **与上一条 ESLint 的关系**：`no-explicit-any` 现在是 error，而 `RedisLike = any` 是 server 侧**最大的一处**——它不在某个文件里孤立存在，而是经 `@nw/shared` 的 barrel 扩散成约 20 个文件的 `redis` 字段类型。上一条首跑只报了 gateway/worldsvc 那 2 个 `const mod: any`（那是**局部变量**，lint 看得见），`export type RedisLike = any` 这个**类型别名**本身不触发 `no-explicit-any` 的常见配置、也没被算进那 58 个问题里——所以「lint 绿」当时并不代表 Redis 调用被检查了。这一条补的就是这个洞。
...
- **并发瓶颈整改（2026-09-05，`WORLDSVC_CONCURRENCY_AUDIT_2026-09-05.md`）**：起因是「一个人连发 5 个队伍打地块，第 5 条明显卡」。**不是锁竞争——worldsvc 全仓没有任何锁**；是 A\* 寻路同步跑在唯一的事件循环上，一次**不可达**目标实测冻结整个进程 **2~6 秒**（1500×1500 地图被河流/山脉切成 20 个连通分量，只有 75 个渡口格连通，而渡口只对占领方开放，所以「不可达」是常态而非例外）。连发 5 条 = 每条排在前一条的寻路后面，跟并发量无关，一个人就能触发。
  - **地形/连通性预计算**（`shared/src/slg/mapTerrainIndex.ts`）：全图 flood-fill 出 `Uint8Array` 地形分类 + `Uint16Array` 连通分量 + 渡口邻接。`reachableThroughGates` 是**保守的必要条件**——只看地形与渡口，不看敌方主城/阻挡建筑（那些只会让可达性更差），所以「不可达」是可以直接短路的证明，「可能可达」再跑 A\*。实测预过滤命中率 100%、零误判。同一份数据顺带把 A\* 里每邻居一次的 `proceduralTile()`（约 1.14µs）换成数组下标读。
  - **⚠️ 写这类「保守预判」时最容易错的三条**（每条都是被 `mapTerrainIndex.test.ts` 的等价性测试当场逼出来的，不是想出来的）：① **起点格 A\* 根本不检查是否可走**——主城 footprint 或再派遣的野战队完全可能站在被判为障碍/渡口的格子上，A\* 照样从那里走出去；② 目的地豁免闸门规则（可以打上去，不能穿过去）；③ **渡口会连成链**（两格桥彼此相邻、不挨着任何分量），只做「分量→闸门→分量」的模型永远走不到对岸。最终模型是**两类节点（分量 + 开放渡口格）的 BFS**。
  - **计算池泛化 + 阶段 4 口子**：`siegeWorkerPool.ts`/`siegeWorker.ts` → `compute/{pool,worker}.ts`，消息体改判别联合；新增 `compute/types.ts` 的 `ComputeBackend` 接口（`runSiege`/`findPath`/`warmWorld`/`close`，全部 JSON 形状）、`compute/pathRunner.ts`（计算本体，无 worker/DB 依赖）、`compute/remote.ts`（独立计算服务的实现位 + 端点契约，未实现、选中即报错，**绝不静默退回本地池**）。`NW_COMPUTE_BACKEND=worker|remote` 选择。用户拍板：等玩家多了单独做计算服务，本轮只留口子。
  - **`compute-worker-module-graph.test.ts`（原 `siege-worker-module-graph.test.ts`）的 `ENTRY` 跟着改名**，并加断言要求走到 `compute/pathRunner.ts`——worker 多一个租户就得多一条，否则那条 Linux-only 的无扩展名相对导入陷阱会重新出现（见下文 2026-08-24 那条）。
  - **可观测性（这轮的前置条件）**：`shared/src/perfMetrics.ts` 的 `startEventLoopMonitor`（`perf_hooks.monitorEventLoopDelay`，≥250ms 告警，边沿触发）+ `RouteTimings`（定长 ring buffer，按标签存 p50/p90/p99/max），HTTP 路由与 scheduler 任务共用一张表，随 heartbeat 输出。**注意**：直方图的采样器在 `enable()` 之后的下一个 loop turn 才起来，所以跟 `startEventLoopMonitor` 处在同一个同步块里的卡顿测不到（要测特定同步段用 `RouteTimings.time`）。
  - **scheduler**：5 个任务原来共用一个 `setInterval` + 一个 `running` 标志——最慢的任务决定所有任务的节奏。改成各自 interval + 各自守卫 + 各自计时，超出自身 interval 就告警；季节结算单独放到 30s。到期扫描上限改 `NW_SLG_ARRIVAL_SCAN_LIMIT` 且触顶告警（此前静默截断 → 行军默默晚到）。**2026-09-09 起是 6 个任务**：到达 tick 拆成 `sched:arrivals`（走格子，2s）与 `sched:arrivalSettle`（结算，500ms + 150ms 时间片），见下方同名条目。
  - **socialsvc 家族/宗门成员缓存**（`socialsvcClient.ts`，10s TTL + worldsvc 自己写时立即失效）：`getFamiliesBySect`/`getFamiliesByIds` 在每次行军和每次地图轮询（×3）上各是一次跨服务 HTTP。**关键陷阱：失败的调用绝不能进缓存**——socialsvc 挂一下返回的 `[]` 被缓存 10s，等于宗门全员 ADR-039 连地判定失效。
  - **同一份 playerWorld 被重复读**：`getMap` 里 `computeVisionSources`/`familyMemberIds`/`sectMateMemberIds`/`allySectMemberIds` 四个各自 `findOne` 同一份文档、且串行——改成读一次传下去 + `Promise.all`。`startMarch` 同理（`computeMarchPath`、`isConnectedToSectTerritory` 都在重读 `pw`），并把 `meta.getSaveFields` 并进队伍忙碌检查那个 `Promise.all`。
  - **CI 门禁 + 200 bot 负载测试（当日追加）**：`worldsvc/test/march-dispatch-concurrency.e2e.test.ts` 是普通 e2e，落在 CI 已有的 worldsvc 分片里（**不用改 `ci.yml`**），守的是**性质**：源码里派发路径必须经 `getComputeBackend()`、不可达目标与并发长距离寻路时事件循环停顿 < 400ms。**变异验证两轮，两轮都改出真问题**——第一轮只有源码门禁红了，因为 `setInterval` 探针在同步块结束后**先排空微任务再跑定时器**，`stop()` 抢在那个本该记录多秒间隔的 tick 前面清掉了它，**探针恰好在循环被冻住时报告「很空闲」**；`stop()` 补记「距上次 tick 的时间」后，变异下报 2.7~11.5 秒。**教训：时序/并发门禁必须做变异验证，测不出回归的断言和通过的断言长得一模一样。**
  - **负载测试**（`worldsvc/test/load/slgOrders.load.ts` + `vitest.load.config.ts` + `npm run test:load -w @nw/worldsvc`，**不进 CI**，要活集群）：200 bot 实测 **75.6 指令/秒、服务端 p99 15.2ms、`loopLagMs.max` 28.9ms**（目标 80/秒达成，事件循环全程无停顿）。它读 worldsvc 自己的新端点 `GET /admin/world/metrics`——从客户端看「循环被堵死」和「服务器只是忙」长得一样，`loopLagMs.max` 是唯一能分开的数字。
  - **2026-09-26 负载测试改成「每轮一个专用新世界」**（ADR-092 的 3000 人阶梯，审计文档 §12.8）：赛季号 ≥900 的世界只给压测用，跑完只关不删；跑前等 Mongo ops/秒降到 30 以下，避免上一轮残留抢线程。`NW_LOAD_MODEL=paced` 是真人节奏模型（节奏是假设），`NW_LOAD_REPORT_FILE` 把每轮结果追加成一行 JSON。要模拟线上单 worker，起本地栈时加 `NW_COMPUTE_POOL_SIZE=1`。worldsvc 的 `/admin/world/metrics` 新增 `mongo.ops`、计算池排队/执行时间、超时与 worker 重启计数、推送计数、主线程利用率 `elu`。**别只看 `loopLagMs`：线程满载但不卡的时候，它看起来一切正常。**
  - **2026-09-26 worldsvc 多进程正确性前提**（审计文档 §12.10）：`advanceMarch` 开头的重读就是认领，写的是 `MarchDoc.stepLeaseUntil`（60 秒）；**新增任何「逐格推进行军」的路径都要先拿这个租约**，批量路径则要在过滤条件里带 `stepIndex` 和「未被持有」。socialsvc 成员缓存的失效走 Redis 频道 `nw:worldsvc:social-invalidate`。行军/攻城 ID 计数器从随机起点开始（`instance.ts`），指标带 `instance`。调度器仍没有按世界的租约（阶段 3），起多副本前要先做。
  - **2026-09-26 同一世界内并发结算**（审计文档 §12.11，`combatMarch/settlePlan.ts`）：结算这一趟先为每条到期行军预测写集合（`a:<账号>` / `t:<格子>`），**只有和本趟其他任何一条都不相交的才并发**（上限 `NW_SLG_SETTLE_CONCURRENCY`，默认 8，设 1 即退回纯串行），其余按原顺序作为屏障单独跑。**新增行军种类、或让某种结算多写一个别人的文档，都必须同步改 `settlementKeys`**——未知种类默认独占，但「已知种类多写了一个键」不会被任何东西拦住。
  - **2026-09-26 按世界的调度租约**（审计文档 §12.12，`worldLease.ts`）：`NW_SLG_WORLD_LEASE=1` 时各进程在 `schedulerLeases` 集合里登记存活并按会合哈希分世界，调度器的每个任务只扫自己持有的世界（`worldScope(worldIds)`）。**默认关，单进程不需要**；租约只是省重复扫描，正确性仍靠阶段 1 的认领。**新增定时任务必须接 `worldIds` 参数并在扫描里加 `worldScope`**，否则多副本时它会被每个进程各扫一遍（仍正确，但白干）。
  - **2026-09-26 主线程两刀**（§12.9）：`getMap` 的无主格走 `cachedProceduralTile`（按 32×32 分块懒填充，返回共享的冻结对象，只能拷贝）；`shared/src/jwt.ts` 按 secret 缓存 KeyObject。
  - **负载测试暴露的三件事**：① **`sched:arrivals` 成为新的第一瓶颈**（p50 1761ms / p90 6705ms，远超自己 2s 的 interval）——下推的深度批处理现在有实测依据了；② **本地 nginx 才是客户端秒级延迟的来源**：同批 bot 经 nginx p99 7870ms、直连 worldsvc p99 94ms，而 worldsvc 自己两轮都是 p99 约 16ms，病因是 `client/nginx.conf` 有 `proxy_http_version 1.1` 却**没有 `upstream { keepalive }`**（生产用 Caddy，不受影响，故未改 nginx，只让负载测试默认直连）；③ **计算池 `cpus-1` 的默认值现在有真实内存代价**——寻路让每个 worker 各存一份地形索引，22 核实测 21 workers → RSS 586MB vs 4 workers → 239MB，已加上限 `min(cpus-1, 8)`。
  - **没做的三件事及理由**（详见审计文档「实施记录」）：`MAX_NODES` 默认值**没下调**（那会改变哪些行军被判 `PATH_BLOCKED`，是玩法改动）；scheduler 当轮**没做深度批处理**（理由见下条）；路径缓存没做（预过滤后收益不再显著，而缓存键必须含闸门指纹才正确）。
  - **`sched:arrivals` 深度批处理（当日第三轮，审计文档第六节）**：负载测试把它变成实测第一瓶颈后补的一刀。**仍然不并发**——野战遭遇会写防守方的 `cardState`/驻防文档，跨玩家并发是真竞态，这条理由一个字没改。拿到收益的是另一个观察：`MARCH_SPEED_SEC_PER_TILE=6` 而 tick 是 2s，**绝大多数到期行军这一 tick 只是在空地上挪一格**，却付了全套约 7 次串行往返。于是新增 `combatMarch/arrivalBatch.ts`：先纯函数规划这一 tick 每条行军进入/离开哪些格（`planMarchSteps`，无 I/O），再按四条规则分流——**这一 tick 不到终点、进入的格 occ 为空、cover 为空、且没有第二条行军碰它任何一格**（进入的和离开的都算，legacy/return 的落点也算）——满足的走批处理，其余走**一行没改**的 `advanceMarch`/`applyArrival`。
    - **能批的全部原因是 occ/cover 各是「每世界一个 hash、field=tileId」**：整 tick 的格子 = 一次 `HMGET`。整批成本压到 `find` 1 + 投影过的 `playerWorld.find` 1 + `bulkWrite` 1 + `HMGET` 2 + 批量清 1 + 批量写 1，与行军条数无关。
    - **四个刻意取舍**：① playerWorld **只投影 `familyId`**（fast 路径只要这个；整份带着 `cardState`，几百份就是把往返问题换成 payload 问题，serial 路径照旧读整份）；② 中间格的 occ 写**折叠掉只写落点**（合法性来自撞格子规则 + occ 索引只有 `advanceMarch` 一个读者）；③ 批量清 occ 走 Lua **服务端逐 field 校验 `.id`**（裸 `HDEL` 会清掉窗口内接手该格的单位）；④ **Mongo 先写、Redis 后写**，游标 `bulkWrite` 带原来的守卫，只有匹配上的行军才拿 occ 条目——否则给一条已召回的行军写 occ 就是永久泄漏（没人会去清一个文档已不存在的 occ id）。
    - **友军占位也不批**：原路径遇到友军会 `skipOwnOcc` 保留对方条目，批处理会盖掉，语义不同就不批。
    - **撞格子规则故意做得粗**（有交集就两条都降级）：「谁踩到谁」正是遭遇的形状，聪明的规则要推理 tick 内先后顺序，那正是这段代码历史上被咬的方式；几百条行军撞在 1500×1500 上是罕见事件，粗规则几乎不花钱。
    - **`metrics.ts` 加累计计数器 `bumpCounter`**（`arrivals.batched`/`arrivals.serial`，随 `/admin/world/metrics` peek）：**把 600 条行军全部悄悄降级回逐条路径的回归，从外面看只是「有点慢」**，而这系统里所有毛病看起来都是「有点慢」。
    - **测试**：`arrival-batch-split.test.ts`（纯函数 16 例）钉规则；`arrival-batch-roundtrips.e2e.test.ts`（真 Mongo 8 例）**断言成本而非行为**——统计调用次数的集合 Proxy + 计数假 Redis，钉死每类往返的确切次数，并覆盖旧版 Redis（只有单 field 命令）的回退路径。**变异 4 轮全被抓**（永不批处理 / 去掉到达豁免 / 去掉撞格子规则 / 去掉召回守卫）。
    - **端到端复测（同日，重建容器后重跑 200 bot）**：`sched:arrivals` **p50 1761ms → 2.3/6.6ms（三个数量级）**，超 interval 告警 30 次 → 3/6 次，吞吐 73.8 指令/秒、`POST /world/march` p50 7.5ms/p99 70ms、`loopLagMs.max` ≤102ms。**但 p90 两轮 61ms vs 4091ms，差 67 倍——它量的根本不是同一件事。** 新的分因计数器当场说清了：32s 里 `batched 794 | serial 1797（arriving 1071，blocked 726）`，**尾巴是「到达结算」不是「走格子」**——一条 arriving 的 occupy 要跑一场真攻占战（计算池）+ 一次 metaserver 往返，1071 次挤进约 20 个 tick。**批处理压不下去它，那正是「不并发」时保护的那段代码。** 这也修正了原来的归因：p50 是走格子（已解决），p90/max 是到达结算（仍开放，**不能批，只能摊**）。
    - **所以负载测试断言打 p50 < 200ms**（批处理前 1761ms，逐条循环放回来立刻炸）+「至少一条走了批处理路径」；p90/max/分因只打印不断言——它们由结算突发主导，对它们设预算等于对这个测试无法归因的工作设预算。**刻意不把门槛挪到能通过的位置。**
    - **`blocked` 726 里很大一部分是「两条行军共享一个*离开*的格子」**（同一玩家从主城连发几支队伍，`path[0]` 都是主城）。互相离开同一格其实不冲突（清 occ 按自己 id 匹配守卫，最多一条命中），规则可细化成「只有*进入*的格子算冲突」——但估算收益仅约 50ms/tick，**远不是尾巴**，记着不改。
    - **⚠ 负载测试对同一个世界不可重复**：每轮换新 fleet id 但**加入同一个赛季世界**，状态一轮轮累积（三轮后：playerWorld 1601 份、已占领 17488 格、待结算占领 1117）。同一份代码三轮漂成 `POST /world/march` p50 7.5ms → 122.8ms、客户端派发 p99 99ms → 7471ms、`sched:occupations` p99 82ms → 4381ms。**世界越挤派发越贵，上一轮的待结算还在抢同一个线程。** 所以三轮里能站住的只有一个数：`sched:arrivals` p50 三轮都在 2~7ms 而批处理前 1761ms。**跟进项：负载测试该每轮开专用新世界（或跑前重置，破坏性、要用户拍板），否则它测的是「一个被跑脏的世界」。**
  - **到达结算拆成自己的任务 + 时间片（2026-09-09，审计文档第七节 / §6.7 第 1 条）**：上一条量出来的尾巴（`arriving 1071`，每条一场真攻占战 + 一次 metaserver 往返）。它**不能批，也正因同一个理由不能并发**，所以这一刀**不改善吞吐、也没有改善吞吐的办法**——它只是不让一阵结算独占那唯一的线程。按 `arriveAt` 一刀两断成两个**查询互不相交**的任务：`sched:arrivals`（2s，`nextStepAt ≤ t` **且** `arriveAt > t`，只走格子，已批处理 p50 ~2ms）与 `sched:arrivalSettle`（500ms，`arriveAt ≤ t`，结算，按 `arriveAt` 升序，受 `NW_SLG_ARRIVAL_SETTLE_SLICE_MS` 默认 **150ms** 的墙钟时间片约束，在两条结算**之间**检查、一次至少放行一条）。
    - **为什么是墙钟不是条数**：单条结算的成本随撞上什么差一个数量级，条数预算在空地和满编攻占战两种世界里意味着完全不同的两件事。150ms/500ms ≈ 风暴下结算最多占 30% 线程，平时一次都碰不到片。
    - **被推迟的不带任何状态**：不写标记、不占租约、不改时间——只是「仍然到期」，下一趟同一个查询照样找得到，所以两趟之间崩了也不丢。顺序按 `arriveAt` 升序（已有索引）：**扫描上限和时间片都会截断队列，没有顺序就是抽签，一条 march 可以反复输**。
    - **没有为拆分新增任何锁**：两半只在「步进扫描之后、结算扫描之前恰好到点」那一瞬可能相交，而既有守卫本来就管这个（`applyFastSteps` 的 `matchedCount` 确认查询、`advanceMarch` 每次重读 `live`）。`arrivals.arriving` 在走格子那半现在结构上恒为 0，仍照常上报——它变成了「两个查询有没有开始重叠」的探针。新增 `arrivals.settled` / `arrivals.deferred`，后者持续增长就是「结算吞吐才是天花板」这句话的数字形式。
    - **测试**：`arrival-settle-slice.e2e.test.ts` 7 例（片靠**传 `sliceMs` 参数**驱动而不是假时钟——结算是真异步 I/O，微小的片确定性只放行一条，于是不必在真 Mongo 旁边动 fake timers），四轮变异全部验红。
    - **⚠ 一条变异第一次不肯变红**：去掉 `.sort({arriveAt:1})` 后所有行为断言照样绿——因为查询就是按 `arriveAt` 过滤的，Mongo 自然走那个索引、本来就有序。**这不代表 sort 是装饰**：它是「保证」和「计划器巧合」的差别。于是那条用例改成两段——顺序仍按行为断言，**「顺序是被请求的」用一个记录 `.sort()` 参数的游标 Proxy 断言**。
    - **⚠ 本机测不出这一刀的收益**：负载测试对同一个世界不可重复（下一条），要横向比就得每轮开干净世界或重置，而重置是破坏性操作、要用户拍板。所以这一节**没有端到端数字**，只有单元/成本级别的保证；真上量时该看 `loopLagMs.max` 与 `POST /world/march` p99，而不是 `sched:arrivals` 自己的耗时。
    - **⚠ 本地栈坑**：`docker compose up -d --build worldsvc` 会顺带重启共享 `nw-server:local` 镜像的 metaserver/gateway/socialsvc/commercial，而 **nginx 只在启动时解析一次上游 DNS** → 所有 `/api/*` 立刻 502，负载测试在 ramp 阶段 200/200 全挂且报「HTML 不是合法 JSON」，看不出是 DNS。**修法 `docker restart nw-local-nginx`。** 与 keepalive 那条是同一个本地栈的两张脸。
    - **门禁失手 + 补拆（当日追加，审计文档 §7.6）**：上面这一刀把 `combatMarch/arrival.ts` 撑到 **592 行**，而那次**没跑 `npm run check:filelength`**——超了 500 门禁又不在基线里，当日分支一开 PR 就会红在 `ci.yml` 的 `server-checks`，等于让下一个人替自己发现。**没走基线豁免，按「拆分形态的优先级」形态①（独立函数模块）真拆**：`arrival.ts` 只留队列那一端（两个扫描 + `ARRIVAL_SCAN_LIMIT`/`SETTLE_SLICE_MS`/`warnIfCapped` + 计数器，214 行），走格子那半进 `arrivalWalk.ts`（`advanceMarch`，210 行），落地那半进 `arrivalSettle.ts`（`applyArrival` + 文件私有的 `applyMove`/`tryParkTeam`，214 行），5 个用到的 `SiegeService` 方法收窄成 `arrivalCtx.ts` 的 `ArrivalSiegeCtx`（照 `combatSiege/ctx.ts` 的惯例）。**边界不用重新判断**：§7.2 已经论证过这两半「查询互不相交、不共享状态」，那就是它们能各自成文件的现成理由；两半之间只有一条边（`advanceMarch` 走到末格调 `applyArrival`），所以 ctx 是 5 个方法的并集而不是两个接口。**零行为改动是机械核对出来的**：把新文件反向变换（还原缩进 + `core.`→`this.core.` + 签名换回方法）后跟 `git show HEAD:` 的对应行段 `diff`，两个新文件逐字节相同，`arrival.ts` 自己只有 3 处调用点改成传 `(this.core, this.siege, …)`——四条变异验证过的性质一条没碰，三个到达测试文件一行没改就是绿的。顺带清掉一处反射：`review-fixes-2026-08-03.e2e.test.ts` 原来靠 `(svc as any).…arrival.advanceMarch` 戳私有方法，现在 `advanceMarch` 是真导出的函数，直接 `import` 调。**补了一条结构门禁** `arrival-split-edges.test.ts`（3 例，纯静态读源码，4ms）钉那条边**单向**——`arrivalSettle.ts` 不许 import 走格子那半，两半都不许 import 回 `arrival.ts`；反向 import 不会编译失败，只会变成加载期 ESM 环，症状是某个 tick 里 `applyArrival is not a function`（读起来像到达逻辑的运行时 bug，不像一次 import 失误），跟 `compute-worker-module-graph.test.ts` 同一个理由。三轮变异全部验红。
- **`startMarch` 串行波次 13 → 6（2026-09-17，审计文档 §11.4）**：这一轮的判据是 §11.2 量出来的模型——共享层 Atlas 上约每 150 次操作撞一次 ~0.7 秒的停顿，所以**一条路由的 p90 ≈ 它串行发出的往返次数 × 那个命中率**，跟查询本身快不快无关（`POST /world/march` p50 137ms / p90 776ms 与 `getMe` 两波次只在 p99 露头，两个数都由同一个乘法解释）。四刀：① 开场三波（队伍解析 / 两个端点格 / 宗门加成）合成一批，端点格一条 `_id:{$in}` 拿两格；② `computeMarchPath` 四波合一——三个障碍扫描本来独立，第四个之所以卡在中间只是因为它把目标格的答案写进了 `$nin` 查询条件，改成在返回行上过滤即可解链，`destTile` 顺带由调用方传入；③ 扣兵与扣体力合成一条写（**并且更正确**：体力原来写在扣兵守卫可能已落空之后，而回滚不退体力）；④ 反向视野扇出移出响应路径（它是给别人的推送，派发已落库）。
  - **⚠️ 三个和这次改动本身一样值得记的**：**投机读要靠一条已有的不对称才安全**——`fromTile` 按调用方请求的坐标读，之所以成立是因为「唯一会改写起点的分支」恰好也是「唯一跳过起点归属检查的分支」，并且加了条内部断言防止以后出现第二种改写；**一个只建模了 collection 某一个方法的 test double 是给下一个人挖的坑**——`combatMarch-command-branch-gaps.test.ts` 的 `tiles.find` 一直无条件返回空游标，production 改用 `find` 的那一刻 17 个用例集体 TILE_NOT_OWNED；**「全局峰值并发度」不能当并发门禁**——第一版把开场批改回串行照旧全绿，因为 `computeMarchPath` 自己那一批把峰值撑住了，必须**按名字断言两两重叠**（并且用事件计数器而不是时钟）。
  - **仍开着的一波**（刻意没做）：`validateMarchTarget` 的 attack 分支里 `friendlyAccountIds` 与 `isConnectedToSectTerritory` 互不依赖但仍串行。它们夹在一串有序的 throw 之间，提前发第二个读就得接住它的 rejection，否则前面的 throw 会留下未处理的 promise——收益一波，风险是一类很难看见的 bug。
- **`getMe` 的 `territoryCount` 镜像（2026-09-15，审计文档 §十）**：`getMe` 是最高频的 SLG 往返，原来每次读都跑一次 `tiles.countDocuments({worldId,ownerId})`。镜像成 `PlayerWorldDoc.territoryCount` 后 3 → 2 次往返（锚点格那次留着，理由见 §10.3）。**镜像不新增维护点**：`core/yield.ts` 的单一出口本来就把该账号所有格子捞出来算 yield，格子数就是那次扫描的 `owned.length`，8 个 `$set { yieldRate }` 写点顺手多写一个字段——任何改归属的路径不重算 yield 本来就是 bug，所以两者不可能各自漂移。
  - **刻意改名 `recomputeYield` → `recomputeYieldAndCount`**：它在 7 个测试文件里有 test double，全是 `as unknown as WorldCore`，**tsc 看不见**。只改返回形状的话，陈旧的 double 会安静地返回旧形状、生产代码把 `undefined` 写进 Mongo、测试照样绿；改名让它们变成「不是函数」当场炸。**改一个被 `as unknown as` 打过桩的东西时，先问「陈旧的桩会红还是会绿」。**
  - 老文档回落到实时 count 并**写回一次**（条件 `{ territoryCount: { $exists: false } }`），所以代价是每个老账号一次、不是每次读一次。门禁 `getme-territory-mirror.e2e.test.ts` 第一条断言的是**`countDocuments` 被调 0 次**——只断言「数字对不对」对旧代码也是绿的。
...
- **拍卖行「我的竞拍」（2026-08-27）**：新集合 `auctionBids`（`_id=${auctionId}|${bidderId}`，每次出价 upsert，`amount`=我自己的最高价、TTL 锚在挂单 `expireAt`+30d）+ `GET /auction/myBids`（`AuctionServiceListing.getMyBids`，出价行 → 批量取挂单 → 推 `outcome: leading/outbid/won/lost`）。存在理由：`topBid` 只记当前领跑者，被反超后挂单上不留痕迹，「我的竞拍」不可能从市场档过滤出来。写入点在 `placeBid` 的 topBid CAS **之后**（CAS 输掉、escrow 已退的出价不留记录），失败只记日志不抛。机制权威 `design/game/AUCTION_DESIGN.md` §5.1/§5.2
...
- **跨集合结算账本（U13 收口，2026-08-24 第四轮）**：前三轮扫的是 worldsvc 的单文档并发写；拍卖成交是**唯一一条跨集合**的多步原子性问题，前三轮那套 rev CAS / 聚合管道完全帮不上忙。
  ...
  - **行为侧回归** `auctionsvc/test/journal-atomicity.e2e.test.ts`（真 Mongo，22 例 → **2026-08-27 起 23 例**：那次复核发现「同一竞拍者两次同额出价」这一例自己违反了本文件头注释的「绝不用顺序调两次代替并发」，`toHaveLength(1)` 是对某一种交错的断言、约二十跑一挂并被 `retry: 1` 糊住；拆成两例、两种交错各自注入后 `--retry=0` 连跑 5 次全绿。**产品代码无改动**——红的是断言。详见 [`server-testing-tooling.md`](server-testing-tooling.md) 确定性规则 3 下的补注）。两点刻意与既有 `auction.e2e.test.ts` 不同：①**下游 fake 是忠实的**——`commercial.spend` 复刻了真实的 insert-first orderId 槽位**和**跨账号归属校验、带余额、只记真实发生的扣款；邮件 fake 按 dispatchKey 去重且可指定某个交付失败 N 次。共用键在「只往数组里 push」的 fake 上看起来就是两次正常调用，**这轮的键位 bug 对那种 fake 是不可见的**。②**并发一律在读写窗口内注入**，绝不用「顺序调两次」代替——后者在修复前的代码上照样通过。三条关键用例已实测在修复前的代码上失败：去掉购买键里的 buyerId → 报 key 归属冲突而非 `AUCTION_CLOSED`；去掉结拍的 `rev` → `expected 'sold' to be 'open'`；去掉重复提交去重 → `expected 10 to be +0`（10 金币凭空退款）。另外把 `repairUnsettled` 改成 no-op 也验证了修复用例会挂。
