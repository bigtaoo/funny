...
**未处理（更大改动或更低优先级，非本轮范围）**：~~matchsvc 赛前状态纯内存无持久化~~（同日以独立分支解决，见下一节）；worldsvc `getMarches`/`getStationed` 全服拉取 + `computeMarchPath` 缺索引扫描（结构性，需要重新设计查询模式）；`siegeEngine` 势均力敌攻城同步阻塞事件循环（需 worker 线程）；gateway 控制消息无 per-connection 限流；admin 四眼审批例外（策略决策）；`dailyCounter.LocalBackend` 长期 Redis 故障下无界增长（概率低）。
