...
已合并的 5 个继承链先例（`WorldCore`/`combatMarch`/`AnalyticsService`/`MetaService`/`AuctionService`）不因为这条新顺序回头重写。下面"其余 27 个"里不管是还没上链的单类（`Gateway.ts`/`Matchsvc.ts`）还是已经身处某条链、自己又长胖的层（`pve.ts`/`liveops.ts`/`arrival.ts`/`occupation.ts`）——一视同仁，都先按 1→2→3 走一遍判断，不因为"已经在链里"就默认继续加链。
...
**其余 18 个超限文件**未动，留作后续任务（完整清单以 `server/scripts/file-length-baseline.json` 为准，这里的行数是入基线时的快照，之后可能有几行漂移）：`metaserver/src/{service/pve.ts(838),service/liveops.ts(742),service/economy.ts(689),economy.ts(693),internal/matchReport.ts(601),accounts.ts(528)}`、`worldsvc/src/{combatSiege/arrival.ts(909),city.ts(682),season.ts(638),combatSiege/occupation.ts(576),combatMarch/command.ts(508)}`、`socialsvc/src/friendService.ts(541)`、`engine/src/{systems/AISystem.ts(573),systems/CombatSystem.ts(553),campaign/levelSchema.ts(527)}`、`commercial/src/{service/base.ts(571),iap.ts(517)}`、`metaserver/src/paddle.ts(492)`、`metaserver/src/service/auth.ts(495)`。按本次试点的判断标准分类：`pve.ts`/`liveops.ts`/`combatSiege/arrival.ts`/`occupation.ts` 已经是 mixin 链的一环但自身又长胖了——不因为"已经在链里"就默认续链，按上面的新优先级先评估这层能否拆成独立类组合，只有交叉调用清单列不完时才继续加子链（`combatMarch.ts`/`AnalyticsService` 的"二次拆分"先例仅在这个前提下沿用）。
