...
> 拍卖行/装备 + 成就系统 worktree 均已于 2026-06-21 合并 main 后删除；**成就系统 S9 全部落地**（S9-7/S9-8 在 `nice-wu-5c3478` 收口，见索引表）；**装备 E2 faucet + E6 洗练已于 2026-06-22 在 `gifted-feistel-42c356` 合并 main**；**装备 E7 抽卡/保护道具已于 2026-06-22 在 `kind-williams-181b80` 合并 main（164093ee）**；**装备 E8 SLG 接入已于 2026-06-22 在 `busy-williams-a2b51a` 直接落 main（8a639173/fd4da3ac）**；后续 byUnit loadout·暴击/proc 框架续做时重新 `git worktree add`。
...
| 装备 E8 SLG 接入 | busy-williams-a2b51a | ~~`claude/busy-williams-a2b51a`~~（已删） | ~~`.claude\worktrees\busy-williams-a2b51a`~~（已删） | ✅ **已合 main 并清理**（直接落 main，2026-06-22，8a639173/fd4da3ac）：engine `EngineEquipmentInput` 导出 + metaserver `GET /internal/save-fields` + worldsvc `metaClient.getSaveFields` + `SiegeBattleInput` 扩展养成字段 + `applySiege/applyStrongholdSiege` 围攻前取存档注入引擎。装备系统 E0–E8 全部落地。失败降级不阻断行军；`SiegeDoc` 暂不存装备快照（replay 视觉精度后续可补）。`auctionService` 清过期 TODO 注释 |
| S11 天梯赛季+战令+排行榜 | ladder-season | ~~`worktree-ladder-season`~~（已删） | ~~`.claude\worktrees\ladder-season`~~（已删） | ✅ **已合 main 并清理**（merge 6ca25555，2026-06-22）：**SE-1~4 赛季时钟+软重置**——shared `season.ts`（`LadderSeasonDoc`/`softReset`/`computeFirstReachGrant`/`FIRST_REACH_COINS`/`SEASON_PEAK_COINS`/`makePvpSeasonDefaults`）+ mongo `ladderSeasons` 集合 + 复合索引 `pvp_season_elo`；**SE-5 排行榜服务端**——`GET /leaderboard` Top-100（当前赛季 ELO 降序+账号 join）+ `GET /internal/leaderboard`；**SE-3 admin 换赛季**——`POST /admin/ladder/season/roll`（CAS state=settling 并发守卫）；**SE-7~8 战令服务端**——shared `battlepass.ts`（`BATTLEPASS_DEFS` 30 级双轨奖励表/`claimBpReward`/`pendingBpRewards`/`xpToLevel`）+ meta `POST /battlepass/buy`（commercial.spend 600 + mutateSave 原子写）+ `POST /battlepass/claim`（orderId 幂等 + commercial.grant）；**SE-2 惰性迁移**——`metaserver/ladderSeason.ts` `migrateIfStale`（getSave/applyPvp 时自动结算上赛季+软重置+重置战令）+ `writeMigratedSave`（rev 守卫 3 重试）；**SE-8 BP XP 累加**——`applyPvp` 胜 +120/败 +60 XP + `xpToLevel` 派生；**段位首达金币**——`computeFirstReachGrant`+`reachedRanks`；**赛季峰值奖励**——`seasonPeakCoins`+邮件发放（idempotent `dispatchKey`）；client SaveData 镜像扩展 pvp 季字段+battlePass；openapi 三端点+`BattlePassData` schema；`openapi.ts` 重生；ApiClient 三方法。server tsc -b 全绿。SE-6/SE-9/SE-10 客户端见下行 |
| S11 天梯赛季客户端 SE-6/SE-9/SE-10 | season-client | ~~`feat/season-client`~~（已删） | ~~`.claude\worktrees\season-client`~~（已删） | ✅ **已合 main 并清理**（merge 5a032a59，2026-06-22）：**SE-6 排行榜+结算弹层**——`LeaderboardScene`（Top100 分行滑动+点击看资料弹层）+ `StatsScene` 赛季横幅行+排行榜/战令入口行 + `LobbyScene.showSeasonSettlement` 跨季首登弹层 + `AppViews/app.ts showLeaderboard/showBattlePass/showSeasonSettlement` + `createAppCore goLeaderboard/goBattlePass` + LAST_SEEN_SEASON_KEY 切季检测；**SE-9 战令面板**——`BattlePassScene`（双轨 30 级/四态：已领/可领/未达/付费锁/购 Pass）+ `battlepassDefs.ts` 客户端镜像；**SE-10 数值校准**——`BP_XP_PER_RANKED_LOSS` 60→40；i18n `season.*`/`leaderboard.*`/`battlepass.*` zh+en+de 全补。HeadlessAppViews showLeaderboard/showBattlePass/showTeams/showAchievements stubs 补全。**S11 天梯赛季+战令+排行榜全链路已 100% 落地** |
