...
### 11.5 bake 缓存跨场景累积到底涨多少（2026-09-10 实测，答掉 ADR-073 的未答项）

ADR-073 「明确不做」的第 ① 条把结论押后了：*「等字节口径先把真实累积量测出来再谈」*。这一节就是那次测量。

**怎么测的。** `bakeStats()` 在正常走屏时**没有任何可读面**——`MemoryMonitor.dump()` 只在某个预算已经炸了才吐它。所以先给它一个读口：`entries/web-e2e.ts` 的 `__nwE2E.bake`（永不发货的 e2e 入口，和既有的 `app`/`__nwAudio` 同一性质）。然后真实 Chrome + 本机 docker 栈（`localhost:8088`，`web-e2e` 目标带 `NW_*_BASE` 起在 9296），注册真账号、退出教学关进大厅，之后每一跳都调**真** nav 回调（`__nwE2E.state.<screen>Cb`），不是自造 stub。窗口 1280×631 / dpr 1.5 → 设计矩形 2191×1080、`gameLayer.scale` 0.585、`pageBakeResolution()` = 0.9375，于是**一张整页纹理 = 2054×1013×4 = 7.94 MiB**（下面所有数字都在这个视口下）。

**结论 1：重访不涨，一字节都不涨。** 大厅→花名册→SLG 地图→城池（地图上的 overlay）→设置，来回三轮：

| 轮次 | 每一跳读到的 `count` / `bytes` |
|---|---|
| 第 1 轮（首访） | 15 条 / 65.4 MiB → 24 条 / **112.5 MiB** |
| 第 2 轮 | 8 跳**全部** 24 条 / 112.5 MiB |
| 第 3 轮 | 8 跳**全部** 24 条 / 112.5 MiB |

key 本来就都是有界的（尺寸四舍五入进 key、`orientation`、avatar 的 seed 量化成 8 个变体、`hdr:` 折进的是场景标题而非玩家数据），量出来的正是这件事：**没有内容派生的 key，所以没有无界增长**。

**结论 2：首访是线性的，每个新屏 +1 张整页纹理。** 从大厅连着走 11 个屏（战役/统计/日常/活动/商店/社交/排行/成就/充值/邮件/拍卖/房间），每一屏都恰好 `+1` 条 `+7.94 MiB`，一次不差。把所有 `show*` 都走一遍、外加一场真教学关（棋盘纸 + 两条侧边涂鸦带）之后：**45 条 / 279 MiB**。其中 **33 条是 2054×1013 的整页纹理，合 262 MiB —— 全部的 94%**。

**结论 3：ADR-073 决策 3 的字节预算看不见这块。** `texBytes()` 扫的是 `PIXI.utils.BaseTextureCache`，而 `bake()` 用的 `PIXI.RenderTexture.create()` **从不注册进那个缓存**（单测直接验过：建一张 RenderTexture 前后 `BaseTextureCache` 的 key 数不变）。所以 `nw_tex_budget_mb`(256) 这道闸门对 bake 缓存**完全无效**——它守的是 URL 资源纹理那一路；bake 缓存只有 `dump()` 里的 `bakeStats()` 能看到，而 `dump()` 只在**别的**预算先炸时才跑。ADR 正文那句「由决策 3 的字节口径负责把它变可见」只兑现了一半：口径有了，闸门没接上。

**结论 4（这轮最值钱的一条）：那 33 张整页纹理的主体是同一张图的 30 份拷贝。** `buildPaperBackground` 的绘制只依赖 `(w, h, marginLine, railX)`——pen 的种子是常量 `0x5bd1c7`，`tag` 一个字都不进绘制，**只进 cache key**。真浏览器里用 `renderer.extract.pixels()` 逐字节比过：

| 比对 | 结果 |
|---|---|
| `settingsbg` vs `campbg`（都有红边线、`railX` 默认 9%） | 8,322,808 字节**逐字节相同** |
| `cardbg` vs `statsbg`（都有红边线、`railX`=216） | **逐字节相同** |
| `settingsbg` vs `cardbg`（`railX` 不同） | 15,479 字节不同 |
| `settingsbg` vs `worldmap`（`marginLine` 不同） | 7,721 字节不同 |

即这 30 份纸背景在这个视口下只是 **3 张不同的图**：(有红边线, railX 默认 9%)、(有红边线, railX=216)、(无红边线)。剩下 3 张整页是真的不同（`lobbybg`/`decorc`/`wear`），其余都是 KB~MB 级的小件（`panelFrameAtlas` 3.1 MiB、3 张 `hdr:` 各 2.45 MiB、3 帧 `boil:lobby-title` 各 0.16 MiB、1 张 `avatarrim` 0.05 MiB）。

**判定：不加淘汰器。** 两条理由，一条是它解决不了问题，一条是它自带代价：

- 主体不是「缓存太久」，是**同一张图存了 30 份**。淘汰器只会让这 30 份轮流被踢掉再重烘——而 `bake` 是一次 `SketchPen` 全路径重画（整页 ~28 条 jitter 线 + 边线），于是内存问题换成切屏帧问题，换错了方向。
- 决策 1 末条写过的危险窗口仍然成立：活着的场景持着 Sprite 时销毁纹理会画出破图，`ScalingManager.resize` 先跑、场景重建在 180 ms 之后。为一个 key 去重就能消掉的问题引入这个风险不值。

**正解是去重 cache key**：`buildPaperBackground` 按 `(w, h, marginLine, railX)` 入 key，而不是按 `tag`。279 MiB → 约 **65 MiB**（省掉 27 张重复整页 = 214 MiB），**画质零变化**（上表的逐字节相同就是证明）。这条**单独立项**，不在本次测量里顺手做——它要动约 30 个调用点共用的那个 key，得配自己的回归（「两个不同 tag 同尺寸只烘一张」+ 「`marginLine`/`railX` 不同必须分开烘」，后者正是上表第 3、4 行防的东西）。

**手机侧（推导，未实测）**，同一套公式 `designW×designH×res²×4`：

| 设备 | 设计矩形 / scale / res | 每张整页 | ×33 张整页 |
|---|---|---|---|
| 崩溃那台内嵌 WebView（vp 750×270、dpr 3） | 2592×1080 / 0.25 / 0.75 | 6.0 MiB | ≈ 198 MiB |
| iPhone 13 全屏横屏（vp 844×390、dpr 3） | 2337×1080 / 0.361 / 1.125 | 12.2 MiB | ≈ 402 MiB |
...
~~**仍未做**：§11.5 结论 3 那一条——bake 缓存至今没有字节闸门~~ **已做（2026-09-14，见 §11.8）**。

**给后人的教训**：

- **`tag` 是命名空间，不是内容哈希。** 一个只用来「区分同尺寸的不同画法」的 key 字段，在画法其实相同的时候就是个乘法器。写 cache key 时问一句「这个字段真的进绘制了吗」——本例里 30 份拷贝的成因就是这一句没问。
- **「不淘汰」本身不是问题，「有界」才是判据。** 这个缓存 key 全是有界的，重访三轮零增长；真正的账在「首访 × 屏数」上，而那笔账的解法是去重不是淘汰。
- **预算和口径是两件事。** 加了 `bakeStats()` 不等于 bake 缓存被守住了：`texBytes()` 扫不到 RenderTexture，所以字节闸门至今没盖住这块。**新增一个字节口径时，要顺手确认它进的是哪条预算的输入。**

### 11.8 第四道闸门：bake 缓存按「屏数」计（2026-09-14）

§11.5 结论 3 开的那条口子补上了：`MemoryMonitor` 现在有**第四道闸门**，守的就是 `texBytes()` structurally 看不见的那张表。同时把这块的量级第一次**量准**了（去重之后从没人重测过）。

#### 量了什么：`test/browser/bakeBudget.spec.ts`

真 Chromium + 本机 docker 栈，注册真账号 + 灌种子数据，然后走 `src/testing/layoutStops.ts` 的**全部 40 站**（跟竖屏巡检同一张表、同一套走法——走法本身这次抽进了 `test/browser/lib/walk.ts`，两个 spec 共用一份）。`NW_BAKE=1` 才跑，平时 skip。
...
| 几何 | 条数 | 总量 | 一张后台缓冲 | **屏数** |
|---|---|---|---|---|
| 1280×631 / dpr 1.5（桌面） | 26 | **75.7 MiB** | 6.9 MiB | **10.9** |
| 390×844 / dpr 3（iPhone 13 竖屏） | 25 | **59.8 MiB** | 5.0 MiB | **11.9** |

**同一几何下与 §11.5 直接可比**：1280×631 那行 2026-09-10 是 **45 条 / 279 MiB**，现在 **26 条 / 75.7 MiB**——§11.6 那次去重兑现了 **−73%**，比当时估的「约 65 MiB」略高（估算没算上 `result-margin`/`board`/`decor` 这几张只有打完一局才烘的整页）。

**顺带订正一条 ADR-073 的推导**：那里按 dpr 3 推 iPhone 13「每张整页 12.2 MiB」。实测是 **5.4 MiB**——`renderPolicy.MAX_RENDER_RESOLUTION = 2` 先把 backbuffer 截到 2，`pageBakeResolution()` 才乘 `designScale`（2 × 0.361 → 进位 0.75）。**推导链里漏了一个上游的 cap，结果高估了一倍多。**

#### 闸门为什么按「屏数」而不是 MB

整页 bake 是按它实际覆盖的设备像素烘的，于是**同样健康的行为**在手机是 5.4 MiB/张、在 1280 桌面是 7.9 MiB/张、在 2592 宽的高 DPI 桌面约 24 MiB/张——四倍散布。一个绝对 MB 数跨不过去：按手机定，每台 retina 桌面都要报；按桌面定，手机涨到三倍健康值都不吭声。换算成「几个后台缓冲」（`bake.ts` 的 `screenBytes()`），上面两台读数是 10.9 和 11.9，**同一个数量级、同一句话**。

**为什么不能用「几张整页」当单位**：那会把 2026-08-25 那个 bug 自己除掉——整页被过采样 16 倍时，单张的成本也涨 16 倍，比值纹丝不动。后台缓冲是**屏幕真正需要的**那个量，所以一张过采样的整页就是 16 个。

#### 闸门本身

`nw_bake_budget_screens`，默认 **24**（实测天花板 ~12 的两倍）。形状跟既有两道一致：**超预算且仍在涨**才报——这张缓存只在「玩家打开了一个还没付过钱的新屏」时涨，涨停之后就不再报，所以一个停在高位的客户端只会在每次再往前走一步时报一条，而不是每 30 秒一条。报文带 `largest.key`（`paper:2191x1080:197@0.9375` 这种），**直接点名去看哪块美术**。`anr` 上下文也加了 `bakeMB`：卡死时手上攥着 200 MB 整页而 `texMB` 只有 8 MB，以前读起来像「纯计算卡顿」。

预算常量搬到了 `src/cache/memoryBudgets.ts`（四个数一处，不 import PIXI）——这样那份 Playwright spec 能 import 到**真正发货的那个数**去断言，而不是断言一个抄过来的副本。

#### 给后人的教训

- **「加了口径」和「接上闸门」差了一整步**，§11.5 已经写过一次；这次补的就是那一步。新增口径时顺手问：它进的是哪条预算的 `if`？
- **预算的单位要跟它守的东西同构。** 这块的成本随设备像素变，所以按设备像素归一化；而归一化的分母必须是**外部事实**（屏幕大小），不能是被守的量自己的一个函数，否则 bug 会把自己除掉。
- **推导链里的 cap 要逐个核。** 12.2 → 5.4 的差别不在公式，在公式上游还有一道 `MAX_RENDER_RESOLUTION`。
