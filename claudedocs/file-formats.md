...
- **icons_atlas 重打包：三张合并页最后一张转正，顺带清掉 8 帧死重量**（2026-08-27，详见 [`design/game/ASSET_PACKAGING_LOG.md`](../design/game/ASSET_PACKAGING_LOG.md) §16）：
  - **量化损失比 decor 那页大得多**：把 equipment/material/factions 三个源子图集重新无损生成后逐像素比对，线上那张 `icons_atlas.png` 有 **97.8% 的像素**与真源不同（其中 alpha>0 的 119,859 个），最坏单通道偏 254、最坏 **alpha 偏 63/255**。修完的验收判据不是"看起来一样"，而是**合并页里 17 帧全部与其无损中间产物逐字节相等**（已确认 17/17）。
  - **8 帧死重量**：页里第四组是 `avatarAtlas.ts` 读的 8 张白线图，`presetAvatarArt.ts` 换成独立胸像 PNG、loader 已删，但帧还在这张**阻塞启动**的页里躺了几周。发现不了的原因值得记：这张图集**没有任何消费者枚举帧名**（`frameNames()` 全仓库只有 decor 那页调用），三个活消费者都是显式 key 查找——**不可达的帧不报错、不渲染，只花启动字节和纹理内存**，连"退化成程序化 glyph"这种可观测症状都没有。所以新守卫断言的是**缺席**+**帧集合恰好相等**，而不是只断言活帧存在。
  - **`--max-width` 新参数 + 一条反直觉结论**：`patchMergedAtlas.js` 的 reflow 原来钉死 2048（抄 `world` 组）。shelf packer 只在放不下时换行，**上限远宽于内容就会摊成一长条**：17 帧在 2048 下是 `1946×388`、利用率 49.9%；扫到 520 是 `520×778`、93.1%。净效果 **PNG 字节 166→294 KB（涨），解码后纹理 6.00→1.54 MB（降 74%）**。这一页是 §11.2 阻塞层里唯一的 atlas，而 ADR-073 之后纹理字节才是被预算管的量，所以测试钉的是**面积预算**而非字节预算——钉字节会把这次改动本身钉红。**上限不是目标值，reflow 一页时要扫一遍。**
  - **连带修根因**：`mergeAtlasPages.js` 那行 `png({ compressionLevel: 9, effort: 10 })` **一直没改**（三张页都是靠 `patchMergedAtlas.js` 事后逐页转正的）。已改成只留 `compressionLevel`，否则复活 `mergeAssetAtlases.js` 或新加一组会再造第四张调色板页。
  - **`pack_faction_atlas.js` 的「trivially safe」注释是错的**：它写着"全是白线只有 alpha 变化，量化无害"。实测量化 vs 无损差 **92.8% 像素、alpha 偏到 8/255、单通道偏到 255**（量化器完全可以给一个近透明的白挑黑色调色板项）。**「能装进 256 项」≠「sharp 的量化器会挑那 256 项」。** 这份产物 072131d8 后不再发布，量化只会把损失烧进真正上线的合并页，故改为无损。
  - **`art/ui/{equipment,material}/build-atlas.js` 一直跑不起来**：`requireSharp()` 的候选路径写的是 `dist/index.cjs`，而本仓库装的 sharp **0.32.6** 入口是 `lib/index.js`；两个候选全落空后会 fall through 到一个没有 `package.json` 的 `ROOT_DIR` 上 `npm install`。改成 require 包根让 node 读 `main`。**顺带订正上一条里的版本号**：本条之前写"当前钉住的 sharp 0.35.3"，实测 `client/node_modules/sharp` 是 **0.32.6**（`client/package.json` 里根本没有 sharp，是传递依赖）；`paletteBitDepth` 优于 `isPalette` 的结论在 0.32.6 上重新验证过仍然成立。
  - **一个被实测否掉的顺手改动**：两个 builder 都用 sharp `composite()` 拼图，而 08-19 的教训说"永远别用 composite 摆帧"。量了一下：equipment 图集 composite vs memcpy 差 16,590 像素（8.44%），但**其中 alpha>0 的只有 590 个、最大偏差 1/255、alpha 偏差 0**，其余全是 alpha=0 的透明像素（composite 归零其 RGB，PIXI 走 premultiplied 不参与显示）。**结论：这里不值得改。** 通用规则也要按位置定量——同一条警告在"往已有内容的页上盖帧"时是硬伤，在"往空画布摆互不重叠的块"时只剩 ±1。
  - **回归测试**：`client/test/ui/iconsAtlasEncoding.ui.ts`（7 例）。变异验证：`png({effort:10})` 原地重量化 → 「非调色板」变红；往 JSON 塞回 `coin` 帧 → 「帧集合」变红。
