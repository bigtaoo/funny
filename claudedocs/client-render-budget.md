...
> 拍板记录：[ADR-083](../design/DECISIONS_ADR-070-085.md#adr-083-渲染循环三级节流dpr-上限-2maxfps-60菜单场景按需重绘含派生式变更检测--重绘地板--accepted--2026-09-08)（三级节流）、ADR-085（世界地图也改按需重绘 + 把 8.25 ms 那个数更正成 1.3 ms）。
...
## 9.5 卡顿告警的天花板取样修正，以及一条被我误判成假阳性的真卡顿（2026-09-12）

§1 末尾那句「曝光量不够，别当结论」到期了：`58aca9e` 在 2026-09-11 出了 **10 条 `cpu`**（`fps 19–25 / thresholdFps 25`）。

**我第一遍把它们判成了假阳性，这个判断是错的。** 记在这里，因为推翻它的证据比结论本身更有用。

### 错在哪里

假设是：`effectiveFpsWarn()` 在**窗口结束时**读 `ticker.maxFPS`，而 `renderPolicy.applyIdleThrottles()` 是**一 tick 之内**在 `IDLE_FPS`(20)/`TARGET_FPS`(60) 之间翻的，所以一个被压到 20 、收尾被顶回 60 的窗口，均值 ~22 会被拿去和 25 比。

这个机制**确实存在**，两路证据：
- 本机真 Chrome 量过（登录屏，每 4 秒碰一下，100 ms 采样重建 2 秒窗口）：**20 个窗口里有 5 个真的跨在天花板变化上**（窗口内最低天花板 15–20、收尾 55–60）。
- 线上 `render_profile` 直接拍到现行：2026-09-12 09:53 `AuctionScene` 是 `maxFps:60, fpsP50:20, fpsMin:20, fpsMax:59, tickPerSec:35` —— 一半的窗口在 20，写报告时天花板已经是 60。

所以修法本身是对的，也留着：天花板改成**窗口内见过的最小值**（`windowMinCap`，每 tick 采样）。

**但它解释不了 09-11 那 10 条。** 同一会话的 `render_profile` 说得很清楚（dpr 2 / 画布 2048×1308）：

| 时间 (UTC) | scene | maxFps | fpsP50 | fpsMin | fpsMax | tick/s | skip% |
|---|---|---|---|---|---|---|---|
| 07:55 | RL | 60 | 30 | 26 | 30 | 29 | 0 |
| 08:01 | LobbyScene | 60 | 26 | 16 | 30 | 26 | 66 |
| 08:07 | CityScene | 60 | 28 | 21 | 30 | 27 | 52 |
| 08:12–08:37 | GameScene ×6 | 60 | 30 | 20–27 | 30–31 | 29–30 | 2–14 |

`maxFps` 全程 **60**、`fpsMax` 卡在 **30** —— 这台设备的**显示刷新本身就在 30 Hz 一线**，循环一直在要 60、一直没拿到，而且还会掭到 16–23。这种窗口的 `windowMinCap` 就是 60，阈值照旧算 25，修完之后**它们照报** —— 而且该报：30 Hz 的屏幕连续 10 秒掉到 19–23是看得见的卡。

`DEFAULT_FPS_WARN` 那 5 帧余量的注释写的是「避免 30Hz 锁定设备的假阳性」，25 < 30 本来就该盖住；盖不住是因为它真的掉到 25 以下了。

### 读 `cpu` 告警的心法（修正后）

1. `thresholdFps` 是**推导值**不是常量，和 `fps` 要一起看。
2. **光看 anomaly 分不出真假**：`{fps, thresholdFps, sustainedMs}` 三个字段里没有任何东西能告诉你天花板在窗口**中间**是多少。必须去 `analyticsvc` 拍 `render_profile`，比 **`maxFps` vs `fpsMax`**：两者都高 = 真掉帧；`fpsMax` 卡在一个硬天花板（30/20）= 设备或节流的上限。
3. 这是同族假阳性的**第四次**（隐藏标签页 → 空闲帧率帽 → 本节的天花板采样点）。§上文那条教训仍然成立：**任何「主动让它变慢」的旋钮都要去改所有把「慢」当异常的探测器**；再加一条：**改探测器之前，先拿另一路遥测把“这条告警到底是不是假的”坐实，别用机制可能性反推。**

### 留下的真问题

一台 dpr 2 / 2048×1308 的设备，GameScene 跑在 ~30 并掭到 16–23（`skipPct` 只有 2–14%，所以**不是按需重绘能省的**，就是真在画）。这是目前唯一有实测支撑的客户端性能开放项，下一步是拿同等 dpr/画布的环境量 GameScene 的每帧成本。→ **2026-09-13 接着做了，见 §9.6：仪器先补上了，因为光靠已有字段问不出「这 33 ms 花在哪」。**

## 9.6 一帧的钱花在哪：`render_profile` 加上 `updP50` / `rndP50`（2026-09-13）

§9.5 把问题停在「下一步量每帧成本」，但**那一步在当时是问不出来的**：`render_profile` 能说这台设备慢（`fpsP50`）、能说它是不是被允许快（`maxFps` vs `fpsMax`）、能说它有没有少画（`skipPct`），**唯独不能说那一帧的时间去了哪**。而这正是决定「该改客户端哪一半」的分岔：是我们的 JS 太慢（场景 `update` / 提交绘制），还是时间**根本不在 JS 里**（显示刷新上限、GPU 填充率、合成器）。09-12 那两次十分钟内被推翻的判断，栽的都是这个分岔。

所以先修仪器，再谈优化 —— 和 `genTop` 是同一条路子。

### 加了什么

两个调用点**本来就在计时**：`SceneManager.tickScene()` 量每次 `scene.update()`，`app.ts` 包住 `renderer.render()`。它们只把 **≥200 ms 的离群值**留给 ANR 报告（`LONG_FRAME_MS`），一个稳定跑 20–30 fps、单次调用从不破 200 ms 的设备在那里是完全不可见的。现在同一批样本另外**累加**一份（`anrContext.ts` 的 `frameCost` / `takeFrameCost()`，两次加法一次比较，不留样本、不分配），由 `PerfMonitor` 在它本来就有的 2 秒窗口里取走：

| 字段 | 含义 |
|---|---|
| `updP50` | 每 **tick** 的 `scene.update()` 毫秒数（窗口均值的中位数） |
| `rndP50` | 每 **tick** 的 `renderer.render()` 毫秒数 |
| `updMax` / `rndMax` | 本次报告跨度内**单次调用**的最坏值 |

两个口径上的坑，都在代码注释里钉住了：

- **除以 tick 数，不是除以调用数。** 一个 tick 可能记两次 `update`（场景 + overlay），按调用数平均会把有 overlay 的屏幕报成真实成本的一半。
- **`rnd` 也除以 tick 数**，而不是除以实际绘制的帧数 —— 被跳过的帧天然贡献 0，所以这个数**已经含了 `skipPct` 的折扣**，可以直接和帧周期 `1000/fpsP50` 相比。按「每次绘制」报会让一个因为按需重绘而变便宜的场景看起来贵了一倍。
- 窗口被判为 hidden 时**照样要 drain**（`takeFrameCost()` 在 hidden 分支**之前**调用）：累加器是读即清零，丢掉的窗口不清就会漏进下一个窗口，而被节流的后台标签页正是那个最不该留下的数。

### 怎么读

拿 `updP50 + rndP50` 和帧周期 `1000 / fpsP50` 比：

- **接近帧周期** → 主线程是瓶颈，活儿是我们自己的，可以砍。
- **远小于帧周期** → 时间不在我们的 JS 里。显示刷新被锁、GPU 填充率、或者合成器 —— **再怎么优化 JS 也不会让 `fpsP50` 动**。

### 本机实测（真 Chrome，Intel Arc Pro，`localhost` 全栈）

GameScene，真打一局 AI，画布 1920×947 @ res 1.5：

```
fpsP50 59   maxFps 60   skipPct 8   updP50 0.4   rndP50 0.8   updMax 9.9   rndMax 41.7
```

16.9 ms 的帧里我们的 JS 占 **1.2 ms（7%）**。同一局把 `renderer.resolution` 从 1 扫到 3（CSS 尺寸不变，只变后备缓冲密度）：

| resolution | 画布 | Mpx | ms/paint |
|---|---|---|---|
| 1 | 1280×575 | 0.74 | 0.308 |
| 1.5 | 1920×863 | 1.66 | 0.328 |
| 2 | 2560×1151 | 2.95 | 0.445 |
| 2.5 | 3200×1438 | 4.60 | 0.803 |
| 3 | 3840×1726 | 6.63 | 0.967 |

线性拟合 ≈ **0.22 ms 固定 + 0.11 ms/Mpx**。也就是说这块桌面 GPU 上，出事那台设备的 2.68 Mpx 只值 ~0.5 ms/帧。**这不能拿去给那台设备结案**（移动 GPU 的填充率可以差一到两个数量级），它确定的是**成本曲线的形状**：一个不大的固定项加一个温和的像素项，而不是某个随像素爆炸的东西。

### 量的时候栽的两跤，都写进 spec 里了

`client/test/browser/frameCost.spec.ts`（`NW_FRAMECOST=1` 才跑，需要本地 docker 栈）是这套测量的常驻夹具，两条教训焊在里面：

1. **`gl?.finish()` 会静默地什么都不做。** 第一版用可选链调 `finish()`，`renderer.gl` 缺席时（canvas 回退）整句蒸发，量到的只是**命令提交**。当时报出来的是「4 倍像素只贵 19%」—— 在硬件上是胡说，在软件光栅上是平常，而输出里**没有任何字段能分辨这两者**。改成 1×1 的 `readPixels`：它不能被跳过、不能被重排。换掉之后同一环境的数字从 0.253/0.300 ms 变成 **57.9/268.5 ms**。
2. **所以必须把 `UNMASKED_RENDERER_WEBGL` 打进输出。** Playwright 的 Chromium 在本机跑的是 `SwiftShader`（软件光栅），它的 4 倍像素→5.7 倍成本是真的，但那是 CPU 在画。现在每次输出第一行就是 GPU 名字，下一个人不会再把软件数当硬件数。
