...
- **⚠️ 高频 `render()` 场景的文本纹理销毁（2026-06-27 事故）**：画布场景（`LoginScene` 等）用「`container.removeChildren()` + 全量重画」模式重渲。`removeChildren()` **不销毁**子对象——而 `sketchUi.txt()` 每次 `new PIXI.Text`，**每个 Text 自带一张 GPU 纹理**，孤儿纹理要等 PIXI 纹理 GC（默认 ~60s）才回收。`LoginScene` 尤其致命：**每次按键** + 光标闪烁 **每 0.5s** 都全量 render，连打字时纹理瞬间堆满 → iPad Safari WebGL 内存预算极小 → 上下文丢失 → Safari 刷新页面（用户反馈「注册输昵称一直崩溃」）。**修复契约**：重渲前遍历 `removeChildren()` 返回值，`PIXI.Text` 用 `destroy({texture:true,baseTexture:true})` 释放自有纹理，其余（Graphics/包 `bake()` 共享纹理的 Sprite）用 `destroy({children:true})`（默认 `texture:false`，**不碰共享烘焙纹理**）。其他逐键/逐帧 render 的输入场景（`ChatScene`/`ShopScene` 等）同理待排查。
