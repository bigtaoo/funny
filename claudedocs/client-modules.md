...
- **⚠️ 场景淡入淡出切换 + 输入冻结契约（2026-07-11 事故：大厅「Store」进商城→点「Shop」页却跳去「Career」）**：`SceneManager.goto()` 现走全屏黑幕 cross-fade（fade-out 120ms → swap → fade-in 160ms，冷启动/resize 重建传 `{instant:true}` 跳过）。淡入期间**新场景在 `new XxxScene()` 时即已构造并订阅输入**，旧场景要等 fade-out 结束的 swap 才 `destroy()`——故约 280ms 内新旧两套 hit-rect **同时在线**。关键坑：**输入不走 PIXI**（`WebAdapter` 直接把 DOM `pointer*` 喂进 `InputManager`），所以黑幕的 `eventMode='static'` 挡不住点击——淡入期一次点击会落到旧场景仍在线的 hit-rect（大厅底栏 Store 隔壁就是 Career）→ 误导航。
  ...
  - **首击中止 fade**（避免 280ms 死区）：`suppress` 期间的 pointer-**down** 触发 `onSuppressedInput` 钩子 → `SceneManager.skipTransition()` 立即跳到目标场景并解冻；该 down 被吞不下发，且 `swallowNextUp()` 一次性吃掉配对的 pointer-**up**，使这次「中止点击」不会误点到刚挂载的新场景。
  - **回归测试**：`inputManager.test.ts`（suppress 丢弃三通道 / suppressed-down 触发钩子且不下发 / `swallowNextUp` 一次性 / suppress 期间 arm 的 swallow 跨解冻仍生效）+ `sceneManager.ui.ts`（fade 延迟 swap、`{instant}` 同帧、fade-out 中途 retarget、冻结跨 fade 后解冻、首击中止 + 真 `InputManager` 集成：中止后配对 up 被吞、下一击正常送达）。**旧的 5 个生命周期用例已改为步进假 ticker 跨过 fade**（加 fade 后原「同帧 swap」断言全会失败，勿退回）。
