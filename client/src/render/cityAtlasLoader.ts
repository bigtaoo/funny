 ...
 *
 * Hand-drawn city images at 256 px each. Source atlas regenerated by
 * art/ui/slg-building/pack_city_atlas.js into `assets/slg/city_atlas.{png,json}`,
 * then folded into the shared `assets/slg/world_atlas.{png,json}` by
 * art/scripts/mergeAssetAtlases.js (see worldAtlas.ts):
...
export function getCityTexture(tier: 1 | 2 | 3 | 4): PIXI.Texture | null {
  return atlas.getTexture(`city_lv${tier}`);
}
...
export function getCityTextureForLevel(level: number): PIXI.Texture | null {
  ...
  return atlas.getTexture(`city_l${lv}`) ?? atlas.getTexture(`city_lv${cityTier(lv)}`);
}
