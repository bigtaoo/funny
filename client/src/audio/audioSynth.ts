...
const VOICES: Record<AudioCue, (ctx: AudioContext, bus: GainNode) => void> = {
  ...
  // 平局：**同一个音重复两次，音高不动**。胜是三音上行、负是二音下行，方向就是那两个 stinger 携带
  // 的全部信息；平局要说的恰恰是「没有方向」，所以它既不该上也不该下。第二声比第一声长而略轻，
  // 于是它读起来是「停在这里了」而不是一次卡住的重复。中音 C（523，与胜利的第一个音同音）是刻意的：
  // 三个 stinger 由此共享同一个起点，玩家分辨结果靠的是**之后往哪走**，而不是记住三段旋律。
  // ⚠️ 这两个 gain 比胜/负的 0.12–0.13 高，**不是**混音决策，是补偿一个几何差异（实测出来的，
  // §0.2 (C) 那条叠加律的另一面）：胜利三个音的 delay 0/0.09/0.18 配 dur 0.16 是**互相重叠**的，
  // 同相叠加把交付峰值抬到 0.132；失败两个音重叠 0.06 s，抬到 0.118。平局这两个音**刻意不重叠**
  // （第一个 0.16 s 结束、第二个 0.20 s 才起——那个空隙就是「停住了」的听感），于是交付峰值等于
  // 单音峰值。照 0.12 写下去实测只有 0.0943，比两个同档 stinger 轻 25%，与 catalogue 里
  // 「三个 stinger 同权重」的意图相反。抬到 0.15 后实测 0.118，落回它们的区间。
  //
  // **不要用「加一点重叠」去代替抬增益**：那个空隙承载的是这个 cue 的全部语义。
  'sfx.result.draw': (c, b) => {
    tone(c, b, { freq: 523, type: 'triangle', dur: 0.16, gain: 0.15 });
    tone(c, b, { freq: 523, type: 'triangle', dur: 0.30, gain: 0.13, delay: 0.20 });
  },
