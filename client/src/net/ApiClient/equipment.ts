...
import type { SaveData, LeanSaveResponse, EquipmentInstance, EquipSlot, CardInstance } from '../../game/meta/SaveData';
import { type Constructor, type ApiClientBaseCtor } from './base';

export interface EquipmentApi {
  craftEquipment(defId: string, idempotencyKey: string): Promise<{ save: LeanSaveResponse; instance: EquipmentInstance }>;
  enhanceEquipment(
    instanceId: string,
  ...
  equipEquipment(slot: EquipSlot, instanceId: string | null, cardInstanceId: string): Promise<{ save: LeanSaveResponse }>;
  fuseCards(
    targetCardId: string,
...
export function EquipmentMixin<TBase extends ApiClientBaseCtor>(Base: TBase): TBase & Constructor<EquipmentApi> {
  return class extends Base {
    ...
    async craftEquipment(
      ...
      return this.post<{ save: LeanSaveResponse; instance: EquipmentInstance }>('/equipment/craft', {
        defId,
        idempotencyKey,
      });
    }
    ...
    async enhanceEquipment(
      ...
      return this.post<{ success: boolean; instance: EquipmentInstance; save: LeanSaveResponse }>(
        '/equipment/enhance',
    ...
    async salvageEquipment(
      ...
      return this.post<{ refunded: Record<string, number>; save: LeanSaveResponse }>('/equipment/salvage', {
        instanceIds,
        idempotencyKey,
      });
    }
    ...
    async equipEquipment(
      ...
      return this.post<{ save: LeanSaveResponse }>('/equipment/equip', {
        slot,
        instanceId,
        cardInstanceId,
      });
