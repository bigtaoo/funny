...
/** Gacha/monetized-card-product domain (see ../ApiClient.ts assembly + ./core.ts for the shared transport). */
export class GachaService implements GachaApi {
  ...
  async getGachaPools(): Promise<GachaPool[]> {
    const data = await this.core.request<{ pools: GachaPool[] }>('GET', '/gacha/pools');
    return data.pools;
  }
  ...
  async redeemFate(itemId: string): Promise<{ save: SaveData; granted: string }> {
    return this.core.post<{ save: SaveData; granted: string }>('/fate/redeem', { itemId });
  }
  ...
  async monthlyCardBuy(platform: string, receipt: string): Promise<{ save: SaveData }> {
    return this.core.post<{ save: SaveData }>('/monthly-card/buy', { platform, receipt });
  }
  ...
  async yearCardBuy(platform: string, receipt: string): Promise<{ save: SaveData }> {
    return this.core.post<{ save: SaveData }>('/year-card/buy', { platform, receipt });
  }
  ...
  async monthlyCardClaim(): Promise<{ save: SaveData; claimed: number }> {
    return this.core.post<{ save: SaveData; claimed: number }>('/monthly-card/claim', {});
  }
  ...
  async starterBuy(
    ...
    return this.core.post<{ save: SaveData; results: GachaResultEntry[] }>('/starter/buy', {
      productId,
      platform,
      receipt,
    });
  ...
  async claimRechargeMilestone(
    tierId: number
  ): Promise<{ save: SaveData; rewards: RechargeReward[] }> {
    return this.core.post<{ save: SaveData; rewards: RechargeReward[] }>('/recharge/claim', {
      tierId,
    });
  }
}
