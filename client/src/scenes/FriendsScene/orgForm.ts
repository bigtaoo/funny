...
export class OrgFormPanel {
  ...
  private drawFamilyCreateForm(px: number, panelW: number, startY: number): void {
    ...
    const nameVal = txt(caretDisplay(core.familyCreateName, core.familyActiveInput === 'name' && core.caretOn, ' '), snapFont(Math.round(fH * 0.4)), C.dark);
    ...
    const tagVal = txt(caretDisplay(core.familyCreateTag, core.familyActiveInput === 'tag' && core.caretOn, ' '), snapFont(Math.round(fH * 0.4)), C.dark);
  ...
  private drawFamilyJoinForm(px: number, panelW: number, startY: number): void {
    ...
    const searchVal = txt(
      core.familyBrowseQuery || searchActive
        ? caretDisplay(core.familyBrowseQuery, searchActive && core.caretOn, ' ')
        : t('social.family.searchPlaceholder'),
      snapFont(Math.round(fH * 0.4)), core.familyBrowseQuery || searchActive ? C.dark : C.mid,
    );
  ...
  private drawSectCreateForm(px: number, panelW: number, startY: number): void {
    ...
    nameBg.x = px; nameBg.y = cy;
    core.container.addChild(nameBg);
    const nameVal = txt(caretDisplay(core.sectCreateName, core.sectActiveInput === 'name' && core.caretOn, ' '), snapFont(Math.round(fH * 0.4)), C.dark);
    nameVal.anchor.set(0, 0.5); nameVal.x = px + Math.round(panelW * 0.04); nameVal.y = cy + fH / 2;
    core.container.addChild(nameVal);
    core.hits.push({ rect: { x: px, y: cy, w: panelW, h: fH }, fn: () => {
    ...
    tagBg.x = px; tagBg.y = cy;
    core.container.addChild(tagBg);
    const tagVal = txt(caretDisplay(core.sectCreateTag, core.sectActiveInput === 'tag' && core.caretOn, ' '), snapFont(Math.round(fH * 0.4)), C.dark);
    tagVal.anchor.set(0, 0.5); tagVal.x = px + Math.round(panelW * 0.04); tagVal.y = cy + fH / 2;
    core.container.addChild(tagVal);
    core.hits.push({ rect: { x: px, y: cy, w: panelW, h: fH }, fn: () => {
  ...
  private drawSectJoinForm(px: number, panelW: number, startY: number): void {
    ...
    const idVal = txt(caretDisplay(core.sectJoinId, core.sectActiveInput === 'id' && core.caretOn, ' '), snapFont(Math.round(fH * 0.4)), C.dark);
