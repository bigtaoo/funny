...
// Converted from BidMixin(Base) to composition (2026-08-11) — see core.ts's file-header comment.
...
import { auctionLabelText } from './itemLabels';
...
export class BidPanel {
  ...
  openBidForm(auc: AuctionView): void {
    ...
    const fresh = core.bidAuction?.auctionId !== auc.auctionId;
    const minBid = this.minBidFor(auc);
    core.bidAuction = auc;
    if (fresh) core.bidAmount = minBid;
    core.bidAmount = Math.max(core.bidAmount, minBid);
    ...
    let cy = my + 26;
    const titleLbl = txt(auctionLabelText(auc), FS.bodyLg, C.dark, true);
    ...
    addNumInput(core, ml, mx + 12, cy, t('auction.bid') + ':', core.bidAmount, (v) => { core.bidAmount = Math.max(minBid, v); this.openBidForm(auc); }, 1.5);
    cy += 50;
