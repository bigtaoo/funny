...
//
// ShopPanel depends on ActionsPanel (via the narrow ActionHandlers interface — the buy/redeem/deal
// runners it calls) but ActionsPanel has no dependency back on ShopPanel: a one-way dependency, so a
// plain independent class over `core` + `actions` (2026-08-11: converted from the former
...
// ActionsPanel.onBuyBulk. Materials aren't included here: their per-item `qty` already bundles units,
...
export class ShopPanel {
  ...
  buildShopCards(): CardSpec[] {
    ...
    if (core.cb.buyMonthlyCard) {
      ...
      if (core.cb.claimMonthlyCard) {
        ...
        buttons.push({
          ...
          primary: false,
          fn: () => void this.actions.runDeal(() => core.cb.claimMonthlyCard!(), 'shop.monthlyClaimed'),
        });
      }
      specs.push({
    ...
    // Each kind sorts ahead of the next regardless of their order in core.items.
    if (core.items && core.items.length > 0) {
      const owned = new Set(core.cb.getOwnedSkins());
      for (const item of core.items) {
        if (item.kind !== 'item') continue;
        // Consumables aren't "owned" — always re-buyable while affordable.
        const canBuy = !busy && core.cb.getCoins() >= item.cost;
        const canBuy10 = !busy && core.cb.getCoins() >= item.cost * BULK_BUY_QTY;
      ...
      for (const item of core.items) {
        ...
        const canBuy = !busy && !capped && core.cb.getCoins() >= item.cost;
        ...
        specs.push({
          ...
          buttons: [{
            ...
            enabled: canBuy, primary: true, fn: () => void this.actions.onBuy(item.id, matTitle),
          }],
        });
      }
      for (const item of core.items) {
        ...
        const canBuy = !isOwned && !busy && core.cb.getCoins() >= item.cost;
        ...
        specs.push({
          ...
          buttons: [{
            ...
            fn: () => void this.actions.onBuy(item.id, skinTitle),
          }],
        });
      }
    }
