...
export class ShopSceneBase {
  ...
  protected drawCard(body: PIXI.Container, spec: CardSpec, x: number, y: number, cw: number, ch: number): void {
    ...
    let yuanRow: { price: PIXI.Text; strike?: PIXI.Text; h: number } | undefined;
    if (spec.yuanPrice !== undefined) {
      const price = txt(`¥${spec.yuanPrice}`, snapFont(Math.round(ch * 0.11)), C.gold, true);
      const strike = spec.yuanStrike !== undefined
        ? txt(`¥${spec.yuanStrike}`, snapFont(Math.round(ch * 0.07)), C.mid, false)
        : undefined;
      yuanRow = { price, strike, h: price.height };
    }
    const priceReserve = (coinRow ? coinRow.h + rowGap : 0) + (yuanRow ? yuanRow.h + rowGap : 0);
