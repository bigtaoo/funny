// Incremental repaints for SectScene — everything that used to be an unconditional full render().
//
// SectScene renders immediate-mode: render() tears down bodyLayer and rebuilds every Text/Graphics
// (see core.beginRender + render/sketchUi.ts's tearDownChildren). That is the repo-wide convention
// for these hand-drawn pages and it is the right thing for an actual state change — but three
// things were driving it far more often than the content actually changed (sect-incremental-repaint,
// 2026-08-25), each of which moves exactly one thing:
//
//   - a drag/wheel scroll, once per frame for the whole gesture (a full rebuild of the family
//     roster AND the chat column, hand-drawn sketchPanel borders and all);
//   - the focused field's caret, twice a second;
//   - a keystroke in the create-form / channel input.
//
// So this class holds the handles needed to move just that thing, and core.update() / input.ts route
// those three through here instead of through render(). Every path falls back to a full render() if
// the object it wants is missing or dead, so a missed registration degrades to the old behaviour
// rather than to a stale screen.
//
// Same shape as FriendsScene/repaint.ts (social-tab-switch-cost, 2026-08-20), with one difference
// that shows up everywhere below: this scene has TWO independently-scrolling columns in the
// landscape split view (family roster + sect channel), so the scroll state is per-column ("band")
// instead of a single layer, and the caller says which column it is moving.
//
 ...
 * string instead of the whole tree (see SectRepaint above). Every field on this scene goes through
 ...
 * FriendsScene/chrome.ts's caretText.
 */
export function caretText(core: SectSceneCore, opts: {
  active: boolean;
  value: string;
  size: number;
