...
describe('unit progression — hard wall preserved', () => {
  it('buildPvpBlueprints() ignores card levels entirely (signature has no card param)', () => {
    void cardsAtLevel(UNIT_MAX_LEVEL);
...
describe('applyUnitLevels — stat scaling', () => {
  ...
  it('clamps level above max and below 1', () => {
    ...
    const over = buildCampaignBlueprints([card(UnitType.Infantry, 999)]);
    const atMax = buildCampaignBlueprints([card(UnitType.Infantry, UNIT_MAX_LEVEL)]);
...
describe('applyUnitLevels — trait breakpoints T3 / T6 / T9', () => {
  ...
  it('+1 spawn (T9) only at max level', () => {
    const below = buildCampaignBlueprints([card(UnitType.Infantry, TRAIT_BREAKPOINTS.bonusSpawn.level - 1)]);
    expect(below[UnitType.Infantry].spawnCount).toBe(UNIT_BLUEPRINTS[UnitType.Infantry].spawnCount);
    const at = buildCampaignBlueprints([card(UnitType.Infantry, TRAIT_BREAKPOINTS.bonusSpawn.level)]);
