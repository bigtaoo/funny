...
// Each fake below declares the parameter list of the real function it stands in for. That is not
// decoration: vitest infers `mock.calls` from this signature, so a zero-arg `vi.fn(() => …)` types
// the recorded args as the empty tuple `[]` and every `calls[0][0]` assertion in this file becomes
// a TS2493 compile error. The tests still pass either way (esbuild strips types), so the only thing
// that catches it is `npm run typecheck` — which is how this file shipped red on 2026-08-15 and
// stayed red until 2026-08-16. Keep the parameters typed when adding a new fake here.
const buildIcon = vi.fn((_kind: string, _size: number, _color: number) => ({ kind: 'drawn' }));
const buildCoinIcon = vi.fn((_kind: string, _size: number, _color: number) => ({ kind: 'coin' }));
const buildMaterialIcon = vi.fn((_kind: string, _size: number, _color: number) => ({ kind: 'material' }));
...
vi.mock('../../src/render/icons', () => ({
  buildIcon: (kind: string, size: number, color: number) => buildIcon(kind, size, color),
  preloadTabIconTextures: () => preloadTabIconTextures(),
}));
vi.mock('../../src/render/atlas/coinIconAtlas', () => ({
  buildCoinIcon: (kind: string, size: number, color: number) => buildCoinIcon(kind, size, color),
  loadCoinIconAtlas: () => loadCoinIconAtlas(),
}));
vi.mock('../../src/render/atlas/materialAtlas', () => ({
  buildMaterialIcon: (kind: string, size: number, color: number) => buildMaterialIcon(kind, size, color),
