...
//
//   1. `helpers.teamOrder()` — the four-source predicate ("is this team away from home?") that
//      mirrors worldsvc's TEAM_BUSY gate (siege holds joined marches/occupations/stations on
//      2026-09-12, 围攻驻留). Asserted directly here: a rendered-label test can only show which
//      branch won, not that the ranking and the ownership filters hold across every combination.
//   2. The load fan-out itself — that all six slices are actually requested, and that /world/teams
...
const siege = (over: Partial<SiegeHoldView> = {}): SiegeHoldView =>
  ({ siegeId: 's1', tile: '7:8', x: 7, y: 8, teamId: 't1', dueAt: 3_000, damage: 120, isBase: true, ...over }) as unknown as SiegeHoldView;
...
describe('CityScene teamOrder — the four-source away-from-home predicate (§8.9)', () => {
  it('returns null when the team appears in none of the four lists', () => {
    ...
    expect(teamOrder(
      [march({ teamId: 't2' })], [occ({ teamId: 't3' })], [siege({ teamId: 't5' })], [station({ teamId: 't4' })], 't1',
    )).toBeNull();
  });

  it('ranks march > occupation/siege > station (several can name the same slot mid-transition)', () => {
    const all = teamOrder([march()], [occ()], [siege()], [station()], 't1');
    ...
    // 围攻驻留 (2026-09-12): so does a pending base/city durability hit — the team is standing on the
    // target until it lands, which is exactly the state this predicate exists to report.
    const besieging = teamOrder([], [], [siege()], [station()], 't1');
    expect(besieging && 'siege' in besieging).toBe(true);
  });

  it('returns the siege hold itself — countdown intact — when it is the only source', () => {
    const r = teamOrder([], [], [siege()], [], 't1');
    expect(r).toEqual({ siege: siege() });
    // The row reads dueAt off it for the countdown, so it must arrive unwrapped, not summarised.
    expect(r && 'siege' in r && r.siege.dueAt).toBe(3_000);
  });
