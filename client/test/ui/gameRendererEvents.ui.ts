...
describe('GameRenderer EventMixin — projectile pool', () => {
  it('fired → hit removes the sprite and recycles it into the pool for the next fire', () => {
    ...
    r.events.handleEvent(
      { type: 'projectile_fired', projectileId: 1, attackerId: 1, from: { col: 2, y_fp: toFp(3) }, kind: 'arrow' },
      engine.state,
    );
    expect(r.events.projectileSprites.has(1)).toBe(true);
    const firstSprite = r.events.projectileSprites.get(1);
    expect(r.events.projectileLayer.children).toContain(firstSprite);

    r.events.handleEvent({ type: 'projectile_hit', projectileId: 1 }, engine.state);
    expect(r.events.projectileSprites.has(1)).toBe(false);
    expect(r.events.projectilePool).toContain(firstSprite);
    ...
    expect(r.events.projectileSprites.get(2)).toBe(firstSprite);
    expect(r.events.projectilePool).not.toContain(firstSprite);
    renderer.destroy();
  });
  ...
  it('an expired (fizzled) projectile is also removed and recycled', () => {
    ...
    r.events.handleEvent({ type: 'projectile_expired', projectileId: 9 }, engine.state);
    expect(r.events.projectileSprites.has(9)).toBe(false);
    expect(r.events.projectilePool.length).toBe(1);
    renderer.destroy();
  });
  ...
  it('projectile_moved updates the tracked sprite position to the authoritative coords', () => {
    ...
    r.events.handleEvent({ type: 'projectile_moved', projectileId: 3, col_fp: toFp(2), y_fp: toFp(4) }, engine.state);

    const sprite = r.events.projectileSprites.get(3);
...
describe('GameRenderer EventMixin — escort sprite lifecycle', () => {
  it('spawned → moved tracks position; died and arrived both remove the sprite immediately', () => {
    ...
    expect(r.events.escortSprites.has('e1')).toBe(true);
    const sprite = r.events.escortSprites.get('e1');
    expect(r.events.escortLayer.children).toContain(sprite);
    const spawnPos = layout.gridToScreen(2, 3);
    expect(sprite.x).toBeCloseTo(spawnPos.x);
    expect(sprite.y).toBeCloseTo(spawnPos.y);

    r.events.handleEvent({ type: 'escort_moved', escortId: 'e1', col_fp: toFp(4), row_fp: toFp(5) }, engine.state);
    ...
    r.events.handleEvent({ type: 'escort_hp_changed', escortId: 'e1', hp: 40, maxHp: 100 }, engine.state);
    expect(r.events.escortSprites.has('e1')).toBe(true);

    r.events.handleEvent({ type: 'escort_died', escortId: 'e1' }, engine.state);
    ...
    expect(r.events.escortSprites.has('e1')).toBe(false);
    renderer.destroy();
  });
  ...
  it('escort_arrived also removes the sprite from tracking', () => {
    ...
    expect(r.events.escortSprites.has('e2')).toBe(true);
    r.events.handleEvent({ type: 'escort_arrived', escortId: 'e2' }, engine.state);
    expect(r.events.escortSprites.has('e2')).toBe(false);
    renderer.destroy();
  });
  ...
  it('an hp/moved/died event for an unknown escortId is a no-op, not a throw', () => {
    ...
    const r = renderer as any;
    expect(() => {
      r.events.handleEvent({ type: 'escort_moved', escortId: 'ghost', col_fp: toFp(0), row_fp: toFp(0) }, engine.state);
      r.events.handleEvent({ type: 'escort_hp_changed', escortId: 'ghost', hp: 1, maxHp: 1 }, engine.state);
      r.events.handleEvent({ type: 'escort_died', escortId: 'ghost' }, engine.state);
      r.events.handleEvent({ type: 'escort_arrived', escortId: 'ghost' }, engine.state);
    }).not.toThrow();
    renderer.destroy();
  });
});
...
describe('GameRenderer EventMixin — base-damage vignette', () => {
  it('base_hp_changed for the LOCAL owner flashes the vignette to full and it decays over time', () => {
    const { engine, renderer } = buildRenderer();
    const r = renderer as any;
    expect(r.events.vignetteAlpha).toBe(0);

    r.events.handleEvent({ type: 'base_hp_changed', owner: 0, hp: 50, maxHp: 100 }, engine.state); // localOwner defaults to 0 (Bottom)
    expect(r.events.vignetteAlpha).toBe(1);

    renderer.update(1 / 30);
    expect(r.events.vignetteAlpha).toBeLessThan(1);
    expect(r.events.vignetteAlpha).toBeGreaterThan(0);
    renderer.destroy();
  });
  ...
  it('base_hp_changed for the OPPONENT does not flash the local vignette', () => {
    ...
    r.events.handleEvent({ type: 'base_hp_changed', owner: 1, hp: 50, maxHp: 100 }, engine.state);
    expect(r.events.vignetteAlpha).toBe(0);
    renderer.destroy();
  });
});
...
describe('GameRenderer EventMixin — spell VFX color follows faction, not the caster', () => {
  it('a local-owner Meteor cast plays the VFX in the friend color', () => {
    const { engine, renderer } = buildRenderer();
    const r = renderer as any;
    const play = vi.spyOn(r.core.vfxSystem, 'play');
  ...
  it('a Rockslide cast is routed to the board sweep, not the generic one-shot VFX', () => {
    ...
    const r = renderer as any;
    const play  = vi.spyOn(r.core.vfxSystem, 'play');
    const sweep = vi.spyOn(r.core.boardView, 'playRockslideEffect');
...
describe('GameRenderer EventMixin — settlement gate (render-side)', () => {
  it('game_over fires onGameEnd exactly once even if a second game_over event arrives', () => {
    ...
    try {
      ...
      r.events.handleEvent({ type: 'game_stats', stats, summary }, engine.state);
      r.events.handleEvent({ type: 'game_over', winner: 0 }, engine.state);
